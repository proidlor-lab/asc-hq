...
class tmapgenerator : public tdialogbox {
  public:
   ...
   void init(void);
   void showmap(void);
   void generatefinishmap(
      void);  // Bodensch�tze werden nicht draufmontiert, damit keine Farbverf�lschung
  ...
  protected:
   Uint8 flip, showland, showdesert, showforest, showmaterial, showfuel, correctvalues,
      calculatevalues;
...
void tmapgenerator::init(void) {
   ...
   if (maxbarsize >= 1)
      addbutton("1", 345, 395, 360, 410, 0, 1, 13, true);
   addkey(13, ct_1);
   if (maxbarsize >= 2)
      addbutton("2", 365, 395, 380, 410, 0, 1, 14, true);
   addkey(14, ct_2);
   if (maxbarsize >= 3)
      addbutton("3", 385, 395, 400, 410, 0, 1, 15, true);
   addkey(15, ct_3);
   if (maxbarsize >= 4)
      addbutton("4", 405, 395, 420, 410, 0, 1, 16, true);
   addkey(16, ct_4);
   if (maxbarsize >= 5)
      addbutton("5", 425, 395, 440, 410, 0, 1, 17, true);
   addkey(17, ct_5);
      ...
      for (i = 0; i < plasma.blockcount; i++) {
         ...
         do {
            if (mblocks[layer]->mempointer[i] <= mblocks[layer]->tileval[j]) {
               if (mblocks[layer]->color[j] != ctransparent)
         ...
         if ((j == mblocks[layer]->tilevalcount) &&
             (mblocks[layer]->color[int(mblocks[layer]->tilevalcount)] != ctransparent))
...
void tmapgenerator::showmontlayer(int sx, int sy, int barsize) {
   ...
   /*byte *constlayer2 = new ( byte[plasma.maxx * plasma.maxy + 4]); // =blockcount + 2 * word f�r
   x/y-size (word *) constlayer2[0] = plasma.maxx; (word *) constlayer2[1] = plasma.maxy;
   ...
   if (flip == true) {
      for (i = plasma.blockcount - 1; i >= 0; i--) {
         if (barsize <= 1)
            putpixel(x, y, constructionlayer[i]);
         else
            bar(x, y, x + barsize - 1, y + barsize - 1, constructionlayer[i]);
         x += barsize;
         if (x >= sx + (plasma.maxx) * barsize) {
            x = sx;
            y += barsize;
...
void tmapgenerator::generatefinishmap(void) {  // layers eintragen, die terrain beeinflussen !
   overwritecolorcount = 0;
   if ((showland == true) && (initialized[0] == true))
      montlayer(0);
   else
      memset(constructionlayer, lightgray, plasma.blockcount);

   overwritecolorcount = 2;
   overwritecolor[0] = mblocks[clland]->color[4];
   overwritecolor[1] = lightgray;
   if ((showforest == true) && (initialized[1] == true))
      montlayer(1);

   overwritecolorcount = 2;
   overwritecolor[0] = mblocks[clland]->color[4];
   overwritecolor[1] = lightgray;
   if ((showdesert == true) && (initialized[2] == true))
      montlayer(2);
}
...
void tmapgenerator::showmap(void) {  // layers, die den bodentyp nicht beeinflussen
   generatefinishmap();

   overwritecolorcount = 0;
   if ((showmaterial == true) && (initialized[3] == true))
      montlayer(3);

   overwritecolorcount = 0;
   if ((showfuel == true) && (initialized[4] == true))
      montlayer(4);
   ...
   for (int y = 0; y < actmap->ysize; y++) {
      for (int x = 0; x < actmap->xsize; x++) {
         ...
         if (checkland(x, y) == false) {
            if ((x > 1) && (y > 1) && (y < actmap->ysize - 2) &&
                (x < actmap->xsize - 2)) {  // nicht am Rand
               if (checkland(x + 1, y))
                  whereland |= 4;  // 2
               if (checkland(x - 1, y))
                  whereland |= 64;  // 6
...
void tmapgenerator::setpfield(int number) {
   switch (constructionlayer[number]) {
      ...
      case cland:
         ...
         break;
      case cmount:
         pf->typ = btyp[2]->weather[0];
         break;
      case cforest:
         pf->typ = btyp[3]->weather[0];
         break;
      case cdesert:
         pf->typ = btyp[4]->weather[0];
         break;
      case cshallowwater:
         pf->typ = btyp[5]->weather[0];
         break;
      case cdeepwater:
         pf->typ = btyp[6]->weather[0];
         break;
      case cverydeepwater:
         pf->typ = btyp[7]->weather[0];
         break;
      default:
         pf->typ = btyp[1]->weather[0];
         break;
...
void tmapgenerator::setmaterial(int number) {
   for (int i = mblocks[clmaterial]->tilevalcount - 1; i >= 0; i--) {
      if (mblocks[clmaterial]->mempointer[number] > mblocks[clmaterial]->tileval[i]) {
         int test = (((mblocks[clmaterial]->mempointer[number] - mblocks[clmaterial]->tileval[i]) *
                      matdiv[i] / enhance) +
                     (i * 80)) *
                    14 / 10;
         if (test > 255)
...
void tmapgenerator::setfuel(int number) {
   for (int i = mblocks[clfuel]->tilevalcount - 1; i >= 0; i--) {
      if (mblocks[clfuel]->mempointer[number] > mblocks[clfuel]->tileval[i]) {
         int test = (((mblocks[clfuel]->mempointer[number] - mblocks[clfuel]->tileval[i]) *
                      fueldiv[i] / enhance) +
                     (i * 80)) *
                    14 / 10;
         if (test > 255)
...
void tmapgenerator::setmap(void) {
   ...
   if (initialized[clmaterial] == true) {
      matdiv[0] =
         81 * enhance / (mblocks[clmaterial]->tileval[1] - mblocks[clmaterial]->tileval[0] + 1);
      matdiv[1] =
         80 * enhance / (mblocks[clmaterial]->tileval[2] - mblocks[clmaterial]->tileval[1] + 1);
      matdiv[2] =
         95 * enhance / (mblocks[clmaterial]->actpres - mblocks[clmaterial]->tileval[1] + 1);
      ...
      if (flip == true) {
         for (i = 0; i < (plasma.maxx / 2); i++) {
            for (j = 0; j < plasma.maxy; j++) {
               pf = &actmap->field[(plasma.maxx / 2 - i - 1) +
                                   ((plasma.maxy - j - 1) * plasma.maxx / 2)];
               int plasmalayernr = (i * 2 + (j & 1)) + (j * plasma.maxx);
               setpfield(plasmalayernr);
               pf->setparams();
               if (initialized[clmaterial] == true)
                  setmaterial(plasmalayernr);
               if (initialized[clfuel] == true)
                  setfuel(plasmalayernr);
               if (pf->vehicle)
...
void tmapgenerator::buttonpressed(int id) {
   switch (id) {
         ...
         if (initialized[layer] == false) {
            ...
            switch (layer) {
               case clland: {
                  ...
                  plasma.memblock->tilevalcount = 5;  // {anzahl Values -1 ( immer h�chster Wert in
                                                      // der [-Klammer) Montlayer �ndern !!!}
               ...
               case clforest: {
                  ...
                  showforest = true;
                  addbutton("~F~orest", 60, 397, 120, 405, 3, 1, 22,
                            true);  // maximaler x-wert 340 -> barsize-buttons
         ...
         if (initialized[layer] == true) {
            ...
            if (correctvalues == true) {
               for (i = 0; i < plasma.memblock->acttile; i++) {
                  if (plasma.memblock->tileval[i] >
                      plasma.memblock->tileval[plasma.memblock->acttile])
                     plasma.memblock->tileval[i] =
                        plasma.memblock->tileval[plasma.memblock->acttile];
               } /* endfor */
            ...
            if ((calculatevalues == true) &&
                (plasma.memblock->acttile < plasma.memblock->tilevalcount)) {
               int div =
                  plasma.memblock->maxset - plasma.memblock->tileval[plasma.memblock->acttile];
      ...
      case 11:
      case 12:
         if ((initialized[layer] == true) && (plasma.memblock->tilevalcount > 1)) {
            if (id == 11) {
               if (plasma.memblock->acttile > 0)
                  plasma.memblock->acttile--;
               else
                  plasma.memblock->acttile = plasma.memblock->tilevalcount - 1;
            } else {
               if (plasma.memblock->acttile < plasma.memblock->tilevalcount - 1)
                  plasma.memblock->acttile++;
               else
                  plasma.memblock->acttile = 0;
            } /* endif */
            activefontsettings.font = schriften.smallarial;
            activefontsettings.color = red;
            activefontsettings.background = lightgray;
   ...
   if (flip == true) {
      for (i=blockcount-1;i>0 ;i-- ) {
         if ( barsize <= 1 ) {
            ...
            if (memblock->mempointer[i] > memblock->tilevalue1 ) {
               if (memblock->mempointer[i] <= memblock->tilevalue2 )
bar(x,y,x+barsize,y+barsize,memblock->col2); else bar(x,y,x+barsize,y+barsize,memblock->col3);
            }
      ...
      for (i=0;i<blockcount-1;i++ ) {
         if ( barsize <= 1 ) {
            ...
            if (memblock->mempointer[i] > memblock->tilevalue1 ) {
               if (memblock->mempointer[i] <= memblock->tilevalue2 )
bar(x,y,x+barsize-1,y+barsize-1,memblock->col2); else
bar(x,y,x+barsize-1,y+barsize-1,memblock->col3);
            }
...
void tplasma::generateplasma(Uint8 resettile) {
   ...
   if (memblock->startblocksize / 2 >= maxx)
      memblock->startblocksize = maxx / 2;
   ...
   if (resettile == true) {
      ...
      for (int i = 0; i < memblock->tilevalcount; i++) {
         if (memblock->tileval[i] > memblock->maxset)
            memblock->tileval[i] = memblock->maxset;
      } /* endfor */
   } /* endif */
      ...
      for (int y = 0; y < (maxy_local / blocksize) + 1; y++) {
         for (int x = 0; x < (maxx / blocksize) + 1; x++) {
            ...
            if (blocksize == sblocksize) {
               ...
               colour =
                  getmem(x * blocksize >= maxx ? 0 : x * blocksize + blocksize, y * blocksize);
               colour +=
                  getmem(x * blocksize,
                         y * blocksize < 2 * blocksize ? maxy_local : y * blocksize - blocksize);
               colour += getmem(x * blocksize < 2 * blocksize ? maxx : x * blocksize - blocksize,
                                y * blocksize);
               colour +=
                  getmem(x * blocksize,
                         y * blocksize > maxy_local - blocksize ? 0 : y * blocksize + blocksize);

               // colour = memblock->mempointer[ (x * blocksize >= maxx ? 0 : x * blocksize +
               // blocksize) + ( ( y * blocksize ) * maxx ) ]; colour += memblock->mempointer[ (x *
               // blocksize ) + ((y * blocksize < 2 * blocksize ? maxy_local : y * blocksize -
               // blocksize)  * maxx ) ]; colour += memblock->mempointer[ (x * blocksize < 2 *
               // blocksize ? maxx : x * blocksize - blocksize) + ((y * blocksize) * maxx ) ];
               // colour += memblock->mempointer[ (x * blocksize) + ((y * blocksize < 2 * blocksize
               // ? maxy_local : y * blocksize - blocksize) * maxx ) ];
               colour /= 4;
            }
            if (blocksize != 1)
               membar(x * blocksize, y * blocksize, x * blocksize + blocksize - 1,
                      y * blocksize + blocksize - 1, colour);
            else
               setmemory(x, y, colour);
