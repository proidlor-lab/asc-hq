...
const char* execactionnames[execactionscount] = {"End MapEdit",
                                                 "Help",
                                                 "Goto EditMode",
                                                 "Select terrain",
                                                 "Select terrainAll",
...
ASCString getbipath(void) {
   ...
   while (!exist(filename)) {
      ...
      if (!exist(filename) && cnt == 1)
         displaymessage(
            "The 'mis' and 'ger' / 'eng' directories must be lower case to import files from them "
            "!",
            1);
#endif
   }
  ...
  public:
   PlayerColorPanel() : PG_Widget(NULL, PG_Rect(20, 20, 320, 40)) {
      ...
      for (int i = 0; i < max(actmap->getPlayerCount(), 9); ++i) {
         PG_Widget* bar = new ColoredBar(actmap->getPlayer(i).getColor(), this,
                                         PG_Rect(gap + i * (width + gap), gap, width, width));
         PG_Label* lab =
            new PG_Label(bar, PG_Rect(5, 5, width - 10, width - 10), ASCString::toString(i));
         lab->SetFontSize(15);
      }

      PG_Application::GetApp()->sigAppIdle.connect(
         sigc::hide(sigc::mem_fun(*this, &PlayerColorPanel::idler)));
   }
...
int countPlayersBinary(const ContainerBase* cont) {
   ...
   for (ContainerBase::Cargo::const_iterator i = cont->getCargo().begin();
        i != cont->getCargo().end(); ++i)
      if (*i)
...
void execaction(int code) {
   switch (code) {
      ...
      case act_deletething: {
         ...
         if (pf2 != NULL) {
            if (!removeCurrentItem()) {
               if (pf2->vehicle != NULL)
                  delete pf2->vehicle;
               else if (pf2->building != NULL)
                  delete pf2->building;
               else if (!pf2->mines.empty())
                  pf2->removemine(-1);
               else
      ...
      case act_resizemap:
         resizemap();
         break;
      case act_movebuilding:
         movebuilding();
         break;
      case act_setactweatherglobal:
         setweatherall(selection.getWeather());
         displaymap();
         break;
      case act_terraininfo:
         viewterraininfo(actmap, actmap->getCursor(), true);
         break;
      case act_setunitfilter:
         selectunitsetfilter();
         filtersChangedSignal();
         break;
      // case act_setzoom : choosezoomlevel();
      //    break;
      case act_unitsettransformation:
         unitsettransformation();
         break;
      case act_unitSetInformation:
         viewUnitSetinfo();
         break;
      case act_primaryAction:
      ...
      case act_switchmaps:
         ...
         updateFieldInfo();
         break;
      case act_editArchivalInformation:
         editArchivalInformation();
         break;
      case act_displayResourceComparison:
         resourceComparison();
         break;
         /*
      ...
      case act_cutToClipboard:
         ...
         break;
      case act_saveClipboard:
         saveClipboard();
         break;

      case act_readClipBoard:
         readClipboard();
         break;
      case act_setTurnNumber:
         actmap->time.set(getid("Turn", actmap->time.turn(), 0, maxint), 0);
         break;
      case act_showPipeNet:
         mainScreenWidget->getMapDisplay()->toggleMapLayer("pipes");
         repaintMap();
         break;
      case act_editResearch:
         editResearch();
         break;
      case act_editResearchPoints:
         editResearchPoints();
         break;
      case act_generateTechTree:
         generateTechTree();
         break;
      case act_editTechAdapter:
         editTechAdapter();
         break;
      case act_playerStrengthSummary:
         pbpplayerstatistics(actmap);
         break;
      case act_unitGuideDialog:
         unitGuideWindow(2);
         break;
   }
}
...
void execaction_pg(int code) {
   switch (code) {
      ...
      case act_selbodentypAll:
         mainScreenWidget->selectTerrainList();
         break;
      case act_selunit:
         mainScreenWidget->selectVehicle();
         break;
      case act_selbuilding:
         mainScreenWidget->selectBuilding();
         break;
      case act_selobject:
         mainScreenWidget->selectObject();
         break;
      case act_selobjectAll:
         mainScreenWidget->selectObjectList();
         /* if ( mapSwitcher.getDefaultAction() == MapSwitcher::select )
                                execaction ( act_setactivefieldvals );
                             execaction( act_switchmaps);
                             */
         break;
      case act_selmine:
         mainScreenWidget->selectMine();
         break;
      case act_unitinfo:
         unitInfoDialog();
         break;
      case act_setmapparameters:
      ...
      case act_setupplayers:
         setupPlayers(actmap, true);
         break;
      case act_loadmap: {
         if (mapsaved == false)
      ...
      case act_end: {
         if (mapSwitcher.getDefaultAction() == MapSwitcher::select)
            execaction(act_switchmaps);
         else if (choice_dlg("Do you really want to quit ?", "~y~es", "~n~o") == 1) {
      ...
      case act_changecargo:
         ...
         break;
      case act_createresources2:
         resourcePlacementDialog();
         displaymap();
         break;
      case act_changeproduction:
         if (getactfield() && getactfield()->getContainer()) {
            if (getactfield()->getContainer()->baseType->hasFunction(
         ...
         break;
      case act_selectgraphicset:
         selectgraphicset();
         // showallchoices();
         break;
      case act_transformMap:
         transformMap();
         break;
      ...
      case act_events:
         eventEditor(actmap);
         break;
      case act_resetPlayerData:
         adminGame(actmap);
         break;
      case act_changemapvals:
         editMap(actmap);
         break;
      case act_newmap:
         newmap();
      ...
      case act_help:
         help(1000);
         break;
      case act_pbpstatistics:
         pbpplayerstatistics(actmap);
         break;
      case act_exchangeGraphics:
         exchangeGraphics();
         break;
      case act_openControlPanel:
         showPlayerPanel(true);
         break;
      case act_releaseControlPanel:
         showPlayerPanel(false);
         break;
      case act_dumpAllVehicleDefinitions: {
         StatusMessageWindowHolder smw =
            MessagingHub::Instance().infoMessageWindow("dumping all units");
         for (int i = 0; i < vehicleTypeRepository.getNum(); ++i) {
            VehicleType* veh = vehicleTypeRepository.getObject_byPos(i);
            tn_file_buf_stream stream("Vehicle" + ASCString::toString(i) + ".dump",
                                      tnstream::writing);
            PropertyWritingContainer pc("VehicleDump", stream);
            veh->runTextIO(pc);
         }
      ...
      case act_dumpAllBuildings: {
         StatusMessageWindowHolder smw =
            MessagingHub::Instance().infoMessageWindow("dumping all buildings");
         for (int i = 0; i < buildingTypeRepository.getNum(); ++i) {
            BuildingType* bld = buildingTypeRepository.getObject_byPos(i);
            tn_file_buf_stream stream("Building" + ASCString::toString(i) + ".dump",
                                      tnstream::writing);
            PropertyWritingContainer pc("BuildingDump", stream);
            bld->runTextIO(pc);
         }
      ...
      case act_dumpAllTerrain: {
         StatusMessageWindowHolder smw =
            MessagingHub::Instance().infoMessageWindow("dumping all terrain");
         for (int i = 0; i < terrainTypeRepository.getNum(); ++i) {
            TerrainType* bld = terrainTypeRepository.getObject_byPos(i);
            tn_file_buf_stream stream("terrain" + ASCString::toString(i) + ".dump",
                                      tnstream::writing);
      ...
      case act_dumpAllObjects: {
         ...
         for (int i = 0; i < objectTypeRepository.getNum(); ++i) {
            ...
            tn_file_buf_stream stream("object" + ASCString::toString(i) + ".dump",
                                      tnstream::writing);
            PropertyWritingContainer pc("objectDump", stream);
            bld->runTextIO(pc);
            smw.SetText(text + ": " +
                        ASCString::toString(int(i * 100 / objectTypeRepository.getNum())) + "%");
         }
      }; break;

      case act_clearSelection:
         selection.clear();
         break;
      case act_locateItemByID:
         locateItemByID();
         break;
      case act_mirrorMap:
         mirrorMap();
