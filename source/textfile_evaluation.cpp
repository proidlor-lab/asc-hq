...
class PropertyTemplate : public PropertyContainer::Property {
  ...
  public:
   ...
   PropertyTemplate(T& property_, const T& defaultValue_)
      : PropertyContainer::Property(true), property(property_), defaultValue(defaultValue_){};
};
...
class IntegerArrayProperty : public PTIA {
  ...
  public:
   IntegerArrayProperty(vector<int>& property_, bool _required)
      : PTIA(property_), required(_required){};
};
...
class IntRangeArrayProperty : public PTIRA {
  ...
  public:
   IntRangeArrayProperty(vector<IntRange>& property_, bool _required)
      : PTIRA(property_), required(_required){};
};
...
class TagArrayProperty : public PTTA {
  ...
  public:
   TagArrayProperty(BitSet& property_, int tagNum_, const char** tags_, bool inverted_)
      : PTTA(property_), tagNum(tagNum_), tags(tags_), inverted(inverted_){};
};
...
class TagIntProperty : public PTTI {
  ...
  public:
   TagIntProperty(int& property_, int tagNum_, const char** tags_, bool inverted_)
      : PTTI(property_), tagNum(tagNum_), tags(tags_), inverted(inverted_){};
   TagIntProperty(int& property_, int tagNum_, const char** tags_, int defaultValue_,
                  bool inverted_)
      : PTTI(property_, defaultValue_), tagNum(tagNum_), tags(tags_), inverted(inverted_){};
};
...
class NamedIntProperty : public PTNI {
  ...
  public:
   NamedIntProperty(int& property_, int tagNum_, const char** tags_)
      : PTNI(property_), tagNum(tagNum_), tags(tags_){};
   NamedIntProperty(int& property_, int tagNum_, const char** tags_, int defaultValue_)
      : PTNI(property_, defaultValue_), tagNum(tagNum_), tags(tags_){};
};
...
/*
         ...
         class ImageProperty : public PTIMG {
            ...
            protected:
               ...
               ASCString toString ( ) const;
            public:
               ImageProperty ( void* &property_, const ASCString& fileName_ ) : PTIMG ( property_ ),
   fileName ( fileName_ ) {};
         };
*/
...
class ASCImageProperty : public PTIMG2 {
  ...
  public:
   ASCImageProperty(Surface& property_, ASCString& fileName_, bool applyFieldMask)
      : PTIMG2(property_), fileName(fileName_), fieldMask(applyFieldMask){};
};
/*
         ...
         class ImageArrayProperty : public PTIMGA {
            ...
            protected:
               ...
               ASCString toString ( ) const;
            public:
               ImageArrayProperty ( PropertyType &property_, const ASCString& fileName_ ) : PTIMGA (
   property_ ), fileName ( fileName_ ) {};
         };
*/
...
class ASCImageArrayProperty : public PTIMGA2 {
  ...
  public:
   ASCImageArrayProperty(PropertyType& property_, ASCString& fileName_)
      : PTIMGA2(property_), fileName(fileName_){};
};
...
void PropertyContainer::addIntRangeArray(const ASCString& name, vector<IntRange>& property,
                                         bool required) {
   IntRangeArrayProperty* ip = new IntRangeArrayProperty(property, required);
   setup(ip, name);
}

void PropertyContainer::addTagArray(const ASCString& name, BitSet& property, int tagNum,
                                    const char** tags, bool inverted) {
   TagArrayProperty* ip = new TagArrayProperty(property, tagNum, tags, inverted);
   setup(ip, name);
}

void PropertyContainer::addTagInteger(const ASCString& name, int& property, int tagNum,
                                      const char** tags, bool inverted) {
...
void PropertyContainer::addTagInteger(const ASCString& name, int& property, int tagNum,
                                      const char** tags, int defaultValue, bool inverted) {
   TagIntProperty* ip = new TagIntProperty(property, tagNum, tags, defaultValue, inverted);
   setup(ip, name);
}

void PropertyContainer::addNamedInteger(const ASCString& name, int& property, int tagNum,
                                        const char** tags) {
...
void PropertyContainer::addNamedInteger(const ASCString& name, int& property, int tagNum,
                                        const char** tags, int defaultValue) {
...
void PropertyContainer::addImageArray(const ASCString& name, vector<Surface>& property,
                                      ASCString& filename) {
   ASCImageArrayProperty* ip = new ASCImageArrayProperty(property, filename);
   setup(ip, name);
}

void PropertyContainer::addImage(const ASCString& name, Surface& property, ASCString& filename,
                                 bool applyFieldMask) {
...
PropertyReadingContainer ::PropertyReadingContainer(const ASCString& baseName,
   ...
   : PropertyContainer(baseName, tpg, true) {
   openBracket(baseName);
}

PropertyReadingContainer ::~PropertyReadingContainer() {}

PropertyWritingContainer ::PropertyWritingContainer(const ASCString& baseName, tnstream& stream)
   : PropertyContainer(baseName, NULL, false), stream(stream) {
...
T PropertyTemplate<T>::parse(const TextPropertyGroup::Entry& entry) const {
   ...
   if (!entry.parent)
      propertyContainer->error(
         ASCString("PropertyContainer::PropertyTemplate::parse - no parent for operator ") +
         TextFormatParser::operations[entry.op] + " at entry " + entry.propertyName + " !");

   switch (entry.op) {
      case TextPropertyGroup::Entry::mult_eq:
         return operation_mult(entry);
      case TextPropertyGroup::Entry::add_eq:
         return operation_add(entry);
      case TextPropertyGroup::Entry::sub_eq:
         return operation_sub(entry);
      default:;
   }
...
void PropertyTemplate<T>::operation_not_defined(const TextPropertyGroup::Entry& entry) const {
   propertyContainer->error(ASCString("operator ") + TextFormatParser::operations[entry.op] +
...
bool BoolProperty::operation_eq(const TextPropertyGroup::Entry& entry) const {
   ...
   if (s.compare_ci("true") == 0 || s.compare_ci("1") == 0)
      return true;
   else if (s.compare_ci("false") == 0 || s.compare_ci("0") == 0)
      return false;
   else {
...
StringArrayProperty::PropertyType
StringArrayProperty::operation_eq(const TextPropertyGroup::Entry& entry) const {
   PropertyType sa;
...
template <class T>
typename ValArrayProperty<T>::PropertyType
ValArrayProperty<T>::operation_add(const TextPropertyGroup::Entry& entry) const {
...
template <class T>
typename ValArrayProperty<T>::PropertyType
ValArrayProperty<T>::operation_sub(const TextPropertyGroup::Entry& entry) const {
   PropertyType child = this->operation_eq(entry);
   PropertyType parent = this->parse(*entry.parent);
...
template <class T>
typename ValArrayProperty<T>::PropertyType
ValArrayProperty<T>::operation_mult(const TextPropertyGroup::Entry& entry) const {
   PropertyType child = this->operation_eq(entry);
   PropertyType parent = this->parse(*entry.parent);
...
IntegerArrayProperty::PropertyType
IntegerArrayProperty::operation_eq(const TextPropertyGroup::Entry& entry) const {
   PropertyType ia;
   StringTokenizer st(entry.value, true);
   ASCString s = st.getNextToken();
   while (!s.empty()) {
...
DoubleArrayProperty::PropertyType
DoubleArrayProperty::operation_eq(const TextPropertyGroup::Entry& entry) const {
   PropertyType ia;
   StringTokenizer st(entry.value, true);
   ASCString s = st.getNextToken();
   while (!s.empty()) {
...
IntRangeArrayProperty::PropertyType
IntRangeArrayProperty::operation_eq(const TextPropertyGroup::Entry& entry) const {
...
ASCImageProperty::PropertyType
ASCImageProperty::operation_eq(const TextPropertyGroup::Entry& entry) const {
   try {
...
ASCString ASCImageProperty::toString() const {
   ASCString valueToWrite =
      constructFileName(0, "", extractFileName_withoutSuffix(fileName) + ".png");
   writePNG(valueToWrite, property);
   return valueToWrite;
}

ASCImageArrayProperty::PropertyType
ASCImageArrayProperty::operation_eq(const TextPropertyGroup::Entry& entry) const {
   try {
      boost::smatch what;
...
ASCString ASCImageArrayProperty::toString() const {
   ...
   Surface s = Surface::createSurface(1100, 100 * (num / 10 + 1),
                                      property.front().GetPixelFormat().BitsPerPixel(), 0);

   int cnt = 0;
   for (PropertyType::iterator i = property.begin(); i != property.end(); i++) {
      if (i->valid())
         megaBlitter<ColorTransform_None, ColorMerger_AlphaOverwrite, SourcePixelSelector_Plain,
                     TargetPixelSelector_All>(*i, s, SPoint((cnt % 10) * 100, (cnt / 10) * 100),
