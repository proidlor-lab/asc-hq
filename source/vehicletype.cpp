...
const char* cwaffentypen[weaponTypeNum] = {
   "cruise missile",  "mine",   "bomb",    "large missile",     "small missile", "torpedo",
   "machine gun",     "cannon", "service", "ammunition refuel", "laser",         "shootable",
   "object placement"};
...
const bool weaponAmmo[weaponTypeNum] = {true, true,  true,  true,  true,  true, true,
      ...
      for (j = 0; j < 16; j++) {
         ...
         if ( weapons.weapon[j].getScalarWeaponType() == cwminen )
            if ( weapons.weapon[j].mindistance < 8 )
               warningMessage ( ASCString("unit with id ") + strrr ( id ) + " has invalid mine
         weapon range ");
         */
...
bool SingleWeapon::equals(const SingleWeapon* otherWeapon) const {
   if (otherWeapon->targ == this->targ && otherWeapon->sourceheight == this->sourceheight &&
       otherWeapon->maxdistance == this->maxdistance &&
       otherWeapon->mindistance == this->mindistance && otherWeapon->count == this->count &&
       otherWeapon->maxstrength == this->maxstrength &&
       otherWeapon->minstrength == this->minstrength && otherWeapon->gettype() == this->gettype()) {
      bool equal = true;
      for (int i = 0; i < 13; i++) {
...
void VehicleType::runTextIO(PropertyContainer& pc) {
   ...
   pc.addNamedInteger("AIJobOverride", job, AiParameter::jobNum, AIjobs,
                      AiParameter::job_undefined);
   ...
   pc.addNamedInteger("CalculationMethod", costCalcMethod, productionCostCalculationMethodNum,
                      productionCostCalculationMethod, 0);
   if (pc.isReading()) {
   ...
   if (costCalcMethod != 0) {
      displayLogMessage(4, "unit %s id %d has a production cost of %d E; %d M;  %d F \n",
                        name.c_str(), id, productionCost.energy, productionCost.material,
                        productionCost.fuel);
   }
}
...
void VehicleType::paint(Surface& s, SPoint pos, const PlayerColor& player, int direction) const {
   megaBlitter<ColorTransform_PlayerCol, ColorMerger_AlphaOverwrite, SourcePixelSelector_Plain,
               TargetPixelSelector_All>(getImage(), s, pos, player, nullParam, nullParam,
                                        nullParam);
}
...
void VehicleType::paint(Surface& s, SPoint pos) const {
   megaBlitter<ColorTransform_None, ColorMerger_AlphaOverwrite, SourcePixelSelector_Plain,
               TargetPixelSelector_All>(getImage(), s, pos, nullParam, nullParam, nullParam,
                                        nullParam);
}
...
Resources VehicleType ::calcProductionCost() {
   ...
   if (unitCostCalculatorName.toLower() == "pbp")
      ...
      throw ASCmsgException("Invalid cost calculator for unit " + ASCString::toString(id));
}
