...
const char* widgetTypes[widgetTypeNum] = {
   "image",        "area",     "statictext",    "textoutput", "bargraph", "specialDisplay",
   "specialInput", "dummy",    "multilinetext", "scrollarea", "button",   "radiobutton",
   "checkbox",     "lineedit", "slider",        "plain",      "listbox"};
...
const char* imageModes[imageModeNum] = {"tile", "stretch", "tile3h", "tile3v", "tile9"};

const int textAlignNum = 3;
const char* textAlignment[textAlignNum] = {"left", "center", "right"};

const int barDirectionNum = 4;
const char* barDirections[barDirectionNum] = {"left2right", "right2left", "top2buttom",
                                              "buttom2top"};

const int sliderDirectionNum = 2;
const char* sliderDirections[sliderDirectionNum] = {"vertical", "horizontal"};

ASCGUI_Window::WidgetParameters::WidgetParameters()
   : backgroundMode(PG_Draw::TILE),
     textAlign(PG_Label::LEFT),
     textAlign_defined(false),
     fontColor(0xffffff),
     fontColor_defined(false),
     fontAlpha(255),
     fontAlpha_defined(false),
     fontSize(8),
     fontSize_defined(false),
     backgroundColor_defined(false),
     transparency(0),
     transparency_defined(false),
     bordersize(1),
     bordersize_defined(false),
     hidden(false) {}
...
void ASCGUI_Window::WidgetParameters::assign(PG_ThemeWidget* widget) {
   ...
   if (!backgroundImage.empty()) {
      widget->SetBackground(IconRepository::getIcon(backgroundImage).getBaseSurface(),
                            backgroundMode);
...
ASCGUI_Window::ASCGUI_Window(PG_Widget* parent, const PG_Rect& r, const ASCString& panelName_,
                             const ASCString& baseStyle, bool loadTheme)
   : PG_Window(parent, ASC_PG_Dialog::centerRectangle(r), "", DEFAULT, baseStyle, 9),
     panelName(panelName_),
     textPropertyGroup(NULL) {
...
void ASCGUI_Window::parsePanelASCTXT(PropertyReadingContainer& pc, PG_Widget* parent,
      ...
      if (type == Image) {
         ...
         if (!filename.empty()) {
            try {
               ...
               PG_Image* img = new PG_Image(parent, PG_Point(r.x, r.y), surf.getBaseSurface(),
                                            false, PG_Draw::BkMode(imgMode));
               newWidget = img;
            ...
            PG_Image* img =
               new PG_Image(parent, PG_Point(r.x, r.y), NULL, false, PG_Draw::BkMode(imgMode));
            newWidget = img;
            widgetParams.assign(img);
            parsePanelASCTXT(pc, img, widgetParams);
         }
         ...
         */
         ...
         int direction = 0;
         pc.addNamedInteger("Orientation", direction, sliderDirectionNum, sliderDirections,
                            direction);

         PG_ScrollBar* sb =
            new PG_Slider(parent, r, PG_ScrollBar::ScrollDirection(direction), -1, style);
...
void ASCGUI_Window::rename(const ASCString& widgetName, const ASCString& newname,
                           PG_Widget* parent) {
   if (!parent)
      parent = this;
...
void ASCGUI_Window::setLabelText(const ASCString& widgetName, const ASCString& text,
                                 PG_Widget* parent) {
   if (!parent)
      parent = this;
...
void ASCGUI_Window::setImage(const ASCString& widgetName, const ASCString& filename,
                             PG_Widget* parent) {
...
void ASCGUI_Window::setWidgetTransparency(const ASCString& widgetName, int transparency,
                                          PG_Widget* parent) {
   if (!parent)
      parent = this;
...
LayoutablePanel ::LayoutablePanel(PG_Widget* parent, const PG_Rect& r, const ASCString& panelName_,
                                  bool loadTheme)
...
ConfigurableWindow ::ConfigurableWindow(PG_Widget* parent, const PG_Rect& r,
                                        const ASCString& panelName_, bool loadTheme)
