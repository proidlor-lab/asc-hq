...
Menu::Menu(PG_Widget* parent, const PG_Rect& rect)
   : PG_MenuBar(parent, rect, "MenuBar"), currentMenu(NULL) {
...
class AutoTextContextAction : public ContextAction {
  public:
   ASCString getText(const MapCoordinate& pos) { return execactionnames[getActionID()]; };
};

class ContainerProperties : public ContextAction {
  public:
   bool available(const MapCoordinate& pos) { return actmap->getField(pos)->getContainer(); };
   ...
   int getActionID() { return act_changeunitvals; }
};
  ...
  public:
   bool available(const MapCoordinate& pos) {
      return actmap->getField(pos)->getContainer() &&
             actmap->getField(pos)->getContainer()->baseType->maxLoadableUnits > 0;
   };
   ...
   int getActionID() { return act_changecargo; }
};

class DeleteVehicle : public ContextAction {
  public:
   bool available(const MapCoordinate& pos) { return actmap->getField(pos)->vehicle; };

   ASCString getText(const MapCoordinate& pos) { return "delete unit"; };

   int getActionID() { return act_deleteunit; }
};

class DeleteBuilding : public ContextAction {
  public:
   bool available(const MapCoordinate& pos) { return actmap->getField(pos)->building; };

   ASCString getText(const MapCoordinate& pos) { return "delete building"; };

   int getActionID() { return act_deletebuilding; }
};
...
class ContainerProduction : public AutoTextContextAction {
  public:
   bool available(const MapCoordinate& pos) {
      return actmap->getField(pos)->getContainer() &&
             actmap->getField(pos)->getContainer()->baseType->hasFunction(
                ContainerBaseType::InternalVehicleProduction);
   };

   int getActionID() { return act_changeproduction; }
};

class DeleteMine : public AutoTextContextAction {
  public:
   bool available(const MapCoordinate& pos) { return !actmap->getField(pos)->mines.empty(); };

   int getActionID() { return act_deletemine; }
};
...
class ChangeMineStrength : public AutoTextContextAction {
  public:
   ...
   int getActionID() { return act_changeminestrength; }
};

class FieldResources : public AutoTextContextAction {
  public:
   bool available(const MapCoordinate& pos) { return true; };

   int getActionID() { return act_changeresources; }
};

class DeleteTopObject : public AutoTextContextAction {
  public:
   bool available(const MapCoordinate& pos) { return !actmap->getField(pos)->objects.empty(); };

   int getActionID() { return act_deletetopmostobject; }
};
...
class DeleteAllObjects : public AutoTextContextAction {
  public:
   ...
   int getActionID() { return act_deleteallobjects; }
};
...
Maped_MainScreenWidget::Maped_MainScreenWidget(PG_Application& application)
   ...
   mapDisplay->mouseButtonOnField.connect(
      sigc::mem_fun(*this, &Maped_MainScreenWidget::clickOnMap));
   ...
   PG_Application::GetApp()->sigKeyDown.connect(
      sigc::mem_fun(*this, &Maped_MainScreenWidget::eventKeyDown));
   ...
   buttonC->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &Maped_MainScreenWidget::clearSelection)));
   ypos += 25;

   PG_Button* button = new PG_Button(this, PG_Rect(xpos, ypos, w, 20), "Select Vehicle");
   button->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &Maped_MainScreenWidget::selectVehicle)));
   ypos += 25;

   PG_Button* button2 = new PG_Button(this, PG_Rect(xpos, ypos, w, 20), "Select Building");
   button2->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &Maped_MainScreenWidget::selectBuilding)));
   ypos += 25;

   PG_Button* button3 = new PG_Button(this, PG_Rect(xpos, ypos, w - 50, 20), "Sel. Object");
   button3->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &Maped_MainScreenWidget::selectObject)));

   PG_Button* button3b = new PG_Button(this, PG_Rect(xpos + w - 45, ypos, 45, 20), "List");
   button3b->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &Maped_MainScreenWidget::selectObjectList)));
   ypos += 25;

   PG_Button* button4 = new PG_Button(this, PG_Rect(xpos, ypos, w - 50, 20), "Sel. Terrain");
   button4->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &Maped_MainScreenWidget::selectTerrain)));

   PG_Button* button4b = new PG_Button(this, PG_Rect(xpos + w - 45, ypos, 45, 20), "List");
   button4b->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &Maped_MainScreenWidget::selectTerrainList)));
   ypos += 25;
   ...
   button6->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &Maped_MainScreenWidget::selectLuaBrush)));
   ypos += 25;
   ...
   brushSelector->selectionSignal.connect(
      sigc::mem_fun(*this, &Maped_MainScreenWidget::brushChanged));
   ypos += 25;
   ...
   //   weatherSelector->selectionSignal.connect( sigc::mem_fun( *currentSelectionWidget,
   //   &SelectionItemWidget::Update ));
   ...
   currentSelectionWidget =
      new SelectionItemWidget(this, PG_Rect(Width() - BuildingItem::Width() - 10, ypos,
                                            BuildingItem::Width(), BuildingItem::Height()));
   ...
   selection.selectionChanged.connect(
      sigc::mem_fun(*currentSelectionWidget, &SelectionItemWidget::set));
   selection.selectionChanged.connect(
      sigc::mem_fun(*this, &Maped_MainScreenWidget::selectionChanged));
...
bool Maped_MainScreenWidget::clickOnMap(const MapCoordinate& field, const SPoint& pos, bool changed,
                                        int button, int prio) {
   if (prio > 1)
      return false;
   ...
   if (button == 3) {
      ...
      int counter = 0;
      for (deallocating_vector<ContextAction*>::iterator i = contextActions.begin();
           i != contextActions.end(); ++i)
      ...
      contextMenu->sigSelectMenuItem.connect(
         sigc::mem_fun(*this, &Maped_MainScreenWidget::runContextAction));
...
bool Maped_MainScreenWidget::eventKeyDown(const PG_MessageObject* o, const SDL_KeyboardEvent* key) {
   ...
   if (!mod) {
      switch (key->keysym.sym) {
         ...
         case SDLK_F1:
            execaction_ev(act_help);
            return true;
         case SDLK_F2:
            execaction_ev(act_selunit);
            return true;
         case SDLK_F3:
            execaction_ev(act_selbodentyp);
            return true;
         case SDLK_F4:
            execaction_ev(act_selobject);
            return true;
         case SDLK_F5:
            execaction_ev(act_selbuilding);
            return true;
         case SDLK_F6:
            execaction_ev(act_selmine);
            return true;
         ...
         case SDLK_F11:
            ...
            return true;
         case SDLK_a:
            execaction_ev(act_movebuilding);
            return true;
         case SDLK_b:
            execaction_ev(act_changeresources);
            return true;
         case SDLK_c:
            execaction_ev(act_changecargo);
            return true;
         case SDLK_d:
            execaction_ev(act_changeterraindir);
            return true;
         case SDLK_e:
            execaction_ev(act_events);
            return true;
         case SDLK_f:
            execaction_ev(act_changeproduction);
            return true;
         case SDLK_g:
            execaction_ev(act_mapgenerator);
            return true;
         case SDLK_h:
            execaction_ev(act_setactivefieldvals);
            return true;
         case SDLK_DELETE:
            execaction_ev(act_deletething);
            return true;
         case SDLK_l:
            execaction_ev(act_showpalette);
            return true;
         case SDLK_m:
            execaction_ev(act_changeminestrength);
            return true;
         case SDLK_o:
            execaction_ev(act_changeplayers);
            return true;

         case SDLK_p:
            execaction_ev(act_changeunitvals);
            return true;

         case SDLK_r:
            execaction_ev(act_resizemap);
            return true;
         ...
         case SDLK_v:
            execaction_ev(act_viewmap);
            return true;

         case SDLK_x:
            execaction_ev(act_mirrorcursorx);
            return true;

         case SDLK_y:
            execaction_ev(act_mirrorcursory);
            return true;

         case SDLK_z:
            execaction_ev(act_setzoom);
            return true;

         case SDLK_7:
            execaction_ev(act_terraininfo);
            return true;

         case SDLK_8:
            execaction_ev(act_placemine);
            return true;

         case SDLK_9:
            execaction_ev(act_showPipeNet);
            return true;
         ...
         case SDLK_ESCAPE:
            execaction_ev(act_clearSelection);
            return true;
         case SDLK_PLUS:
         case SDLK_KP_PLUS:
            execaction_ev(act_increase_zoom);
            return true;

         case SDLK_MINUS:
         case SDLK_KP_MINUS:
            execaction_ev(act_decrease_zoom);
            return true;
   ...
   if (mod & KMOD_CTRL) {
      switch (key->keysym.sym) {
         case SDLK_F3:
            execaction_ev(act_selbodentypAll);
            return true;

         case SDLK_F4:
            execaction_ev(act_selobjectAll);
            return true;
   ...
   if (mod & KMOD_SHIFT) {
      switch (key->keysym.sym) {
         case SDLK_d:
            execaction_ev(act_changeunitdir);
            return true;
         case SDLK_F3:
            execaction_ev(act_selbodentypAll);
            return true;
   ...
   if (mod & KMOD_ALT) {
      switch (key->keysym.sym) {
         ...
         *c = 1;
            ...
            */
            {
               for (Player::VehicleList::iterator i = actmap->player[1].vehicleList.begin();
                    i != actmap->player[1].vehicleList.end(); ++i)
                  (*i)->direction = 3;
            }
            return true;
...
class MapItemSelectionWindow : public ItemSelectorWindow {
  public:
   MapItemSelectionWindow(PG_Widget* parent, const PG_Rect& r, const ASCString& title,
...
void showSelectionWindow(PG_Widget* parent, PG_WindowPointer& selectionWindow,
                         const ItemRepository<ItemType>& itemRepository) {
   if (!selectionWindow) {
      ItemSelectorWindow* sw = new MapItemSelectionWindow(
         parent, PG_Rect(parent->Width() - 300, 100, 280, parent->Height() - 150), "select item",
         new MapItemTypeWidgetFactory<MapItemTypeWidget<ItemType>>(itemRepository));
