...
void PackageRepository ::readTextFiles(PropertyReadingContainer& prc, const ASCString& fileName,
...
void PackageRepository ::postChecks() {
   ...
   for (PackageRepository::iterator i = packageRepository.begin(); i != packageRepository.end();
        ++i) {
...
void PackageRepository::checkPackageDependency(const Package* pack,
   ...
   for (Package::Dependencies::const_iterator dep = pack->dependencies.begin();
        dep != pack->dependencies.end(); ++dep) {
      const Package* p = getPackage(dep->name);
      if (p) {
         if (p->version < dep->version) {
            ASCString s = "The package " + dep->name + " (" + p->location +
                          ") on your system is outdated.\n" + "Package " + pack->name + " (" +
                          pack->location + ") requires at least " + dep->name + " " +
         ...
         if (packageData) {
            for (PackageData::Packages::const_iterator ref = packageData->packages.begin();
                 ref != packageData->packages.end(); ++ref)
...
Package* PackageRepository::_getPackage(const ASCString& name) {
   for (PackageRepository::const_iterator p = packageRepository.begin();
        p != packageRepository.end(); ++p)
