         ...
         if (fzt->weapons.weapon[i].offensive())
            for (int j = 0; j < 8; j++)
               if (fzt->weapons.weapon[i].targ & (1 << j)) {
                  ...
                  for (int e = (fzt->weapons.weapon[i].mindistance + maxmalq - 1) / maxmalq;
                       e <= fzt->weapons.weapon[i].maxdistance / maxmalq;
                       e++) {  // the distance between two fields is maxmalq
                     d++;
                     int n =
                        int(WeapDist::getWeaponStrength(&fzt->weapons.weapon[i], 0, e * maxmalq) *
                            fzt->weapons.weapon[i].maxstrength * af.strength_damage(getdamage()) *
                            (1 + af.strength_experience(getexpirience())));
...
AiParameter::JobList AI::chooseJob(const VehicleType* typ) {
   ...
   if ((typ->hasFunction(ContainerBaseType::ExternalRepair) || service) && maxmove >= minmalq &&
       maxstrength < 45)
   ...
   if ((maxstrength * 1.5 < typ->view ||
        (maxstrength * 1.5 < typ->jamming &&
         !typ->hasFunction(ContainerBaseType::JamsOnlyOwnField))) &&
       maxmove > minmalq)
...
void AI ::calculateThreat(Building* bld, int player) {
   ...
   int value = (bld->plus.energy / 10) + (bld->plus.fuel / 10) + (bld->plus.material / 10) +
               (bld->actstorage.energy / 20) + (bld->actstorage.fuel / 20) +
               (bld->actstorage.material / 20);

   for (ContainerBase::Cargo::const_iterator i = bld->getCargo().begin();
        i != bld->getCargo().end(); ++i)
      if (*i) {
         if (!(*i)->aiparam[player])
            calculateThreat(*i);
      ...
      for (weap = 0; weap < veh->typ->weapons.count; weap++)
         if (veh->height & veh->typ->weapons.weapon[weap].sourceheight)
            if ((1 << height) & veh->typ->weapons.weapon[weap].targ)
               if (veh->typ->weapons.weapon[weap].shootable() &&
                   veh->typ->weapons.weapon[weap].offensive()) {
                  initsearch(MapCoordinate(x, y),
                             veh->typ->weapons.weapon[weap].maxdistance / maxmalq,
                             veh->typ->weapons.weapon[weap].mindistance / maxmalq);
                  startsearch();
               }
}
...
void AI ::WeaponThreatRange ::testfield(const MapCoordinate& mc) {
   if (dist * maxmalq <= veh->typ->weapons.weapon[weap].maxdistance)
      if (dist * maxmalq >= veh->typ->weapons.weapon[weap].mindistance) {
         AttackFormula af(ai->getMap());
         int strength =
            int(WeapDist::getWeaponStrength(&veh->typ->weapons.weapon[weap],
                                            ai->getMap()->getField(mc)->getWeather(),
                                            dist * maxmalq, veh->height, 1 << height) *
                veh->typ->weapons.weapon[weap].maxstrength *
                (1 + af.strength_experience(veh->getExperience_offensive()) +
                 af.strength_attackbonus(gamemap->getField(startPos)->getattackbonus())) *
   ...
   for (int y = 0; y < activemap->ysize; y++) {
      ...
      for (int x = 0; x < activemap->xsize; x++) {
            ...
            if (fld->vehicle && getPlayer().diplomacy.isHostile(fld->vehicle->getOwner())) {
               ...
               if (!fld->vehicle->typ->wait) {
                  ...
                  if (MoveUnitCommand::avail(fld->vehicle)) {
                     ...
                     for (set<MapCoordinate3D>::const_iterator i = fields.begin();
                          i != fields.end(); ++i)
                        wr.run(fld->vehicle, i->x, i->y, singleUnitThreat);

                     const set<MapCoordinate3D>& ifields = muc.getReachableFieldsIndirect();
                     for (set<MapCoordinate3D>::const_iterator i = ifields.begin();
                          i != ifields.end(); ++i)
                        wr.run(fld->vehicle, i->x, i->y, singleUnitThreat);
                  }
                  tus.restore();
         ...
         for (int i = 0; i < sidenum; i++) {
            MapField* f =
               getMap()->getField(getNeighbouringFieldCoordinate(MapCoordinate(x, y), i));
...
void AI ::calculateAllThreats(void) {
      ...
      for (int v = 0; v < vehicleTypeRepository.getNum(); v++) {
         ...
         if (fzt)
            if (fzt->hasFunction(ContainerBaseType::ConquerBuildings))
               if (fzt->movement[getFirstBit(chfahrend)] >
                   maxTrooperMove)  // buildings can only be conquered on ground level, or by moving
                                    // to adjecent field which is less
   ...
   for (int height = 0; height < 8; height++)
      if (maxWeapDist[height] < 0) {
         maxWeapDist[height] =
            0;  // It may be possible that there is no weapon to shoot to a specific height
            ...
            if (fzt)
               for (int w = 0; w < fzt->weapons.count; w++)
                  if (fzt->weapons.weapon[w].maxdistance > maxWeapDist[height])
                     if (fzt->weapons.weapon[w].targ &
                         (1
                          << height))  // targ is a bitmap, each bit standing for a level of height
   ...
   for (int v = 0; v < 9; v++)
      if (activemap->player[v].exist() || v == 8) {
         ...
         for (Player::VehicleList::iterator vi = getPlayer(v).vehicleList.begin();
              vi != getPlayer(v).vehicleList.end(); vi++) {
            Vehicle* veh = *vi;
         ...
         for (Player::BuildingList::iterator bi = getPlayer(v).buildingList.begin();
              bi != getPlayer(v).buildingList.end(); bi++)
            calculateThreat(*bi);
      }
}
...
void AI ::Section ::init(int _x, int _y, int xsize, int ysize, int _xp, int _yp) {
   ...
   if (y1 < 0)
      y1 = 0;
   if (x2 >= ai->activemap->xsize)
      x2 = ai->activemap->xsize - 1;
   if (y2 >= ai->activemap->ysize)
      y2 = ai->activemap->ysize - 1;
   ...
   if (!section) {
      ...
      for (int x = 0; x < numX; x++)
         for (int y = 0; y < numY; y++)
            section[x + numX * y].init(ai, x * ai->activemap->xsize / numX,
                                       y * ai->activemap->ysize / numY, sizeX, sizeY, x, y);
   }
}
...
*/
MapCoordinate AI ::Sections ::getAlternativeField(const MapCoordinate& pos,
                                                  map<MapCoordinate, int>* destinationCounter,
                                                  int height, AStar3D* ast) {
...
AI::Section* AI ::Sections ::getBest(int pass, Vehicle* veh, MapCoordinate3D* dest,
                                     bool allowRefuellOrder, bool secondRun,
                                     map<MapCoordinate, int>* destinationCounter) {
   /*
         ...
         for ( int y = 0; y < numY; y++ ) {
            for ( int x = 0; x < numX; x++ ) {
                ...
                if ( sec.avgUnitThreat.threat[ veh->getValueType(h) ] ) {
                   int relThreat = int( 4*maxSectionThread /
   sec.avgUnitThreat.threat[veh->getValueType(h)] + 1); f /= relThreat;
                }

                int dist = beeline ( veh->xpos, veh->ypos, sec.centerx, sec.centery ) + 3 *
   veh->maxMovement(); if ( dist ) f /= log(double(dist));
   ...
   */
      ...
      if (veh->typ->height & h)
         for (int y = 0; y < numY; y++)
            for (int x = 0; x < numX; x++) {
               ...
               if (sec.avgUnitThreat.threat[veh->getValueType(h)]) {
                  int relThreat =
                     int(4 * maxSectionThread / sec.avgUnitThreat.threat[veh->getValueType(h)] + 1);
                  f /= relThreat;
               }
               ...
               */
               ...
               int dist =
                  beeline(veh->xpos, veh->ypos, sec.centerx, sec.centery) + 3 * veh->maxMovement();
               if (dist)
                  f /= log(double(dist));
               ...
               if (f > d) {
                     ...
                     for (int xp = sec.x1; xp <= sec.x2; xp++) {
                        if (ast->getFieldAccess(xp, yp) & h) {
                           ...
                           if (mandist < mindist) {
                              ...
                              if (destinationCounter && destinationCounter->find(MapCoordinate(
                                                           xp, yp)) != destinationCounter->end()) {
                                 /* we are checking if there are too many units heading for this
                                    field already check how many are for neighbouringfields */

                                 MapCoordinate alt = getAlternativeField(
                                    MapCoordinate(xp, yp), destinationCounter, h, ast);
                                 xtogoSec = alt.x;
                                 ytogoSec = alt.y;
                              } else {
                                 xtogoSec = xp;
                  ...
                  if (xtogoSec >= 0 && ytogoSec >= 0) {
                     if (!rfc ||
                         rfc->returnFromPositionPossible(MapCoordinate3D(xtogoSec, ytogoSec, h))) {
                        int notAccessible = 100 * nac / (nac + ac);
                        if (notAccessible < 85 &&
                            targets) {  // less than 85% of fields not accessible
                           float nf =
                              f * (100 - notAccessible) / 100;  // *  ( 100 - notAccessible );
                           if (nf > d) {
                              d = nf;
                              // nd = nf;
                              frst = &getForPos(x, y);
                     ...
                     } else if (allowRefuellOrder) {
                        if (rfc &&
                            rfc->returnFromPositionPossible(MapCoordinate3D(xtogoSec, ytogoSec, h),
                                                            veh->getStorageCapacity().fuel))
   ...
   if (!frst) {
      ...
      else if (RefuelConstraint::necessary(veh, *ai) && !secondRun)
