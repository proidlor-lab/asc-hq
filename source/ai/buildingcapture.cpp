...
float AI ::getCaptureValue(const Building* bld, Vehicle* veh) {
   ...
   if (ast.visited.back().gval >= 0)
      ...
      return getCaptureValue(bld, (int) (ast.visited.back().gval / veh->getMovement())) -
             0.0001 * veh->aiparam[getPlayerNum()]->getValue();
   else
      return -1;
...
class SearchReconquerBuilding : public SearchFields {
  ...
  public:
   ...
   SearchReconquerBuilding(AI& _ai, Building* bld)
      : SearchFields(_ai.getMap()), ai(_ai), buildingToCapture(bld), mode(3){};
};
...
void SearchReconquerBuilding ::unitfound(Vehicle* eht) {
   ...
   buildingToCapture->aiparam[ai.getPlayerNum()]->setAdditionalValue(
      buildingToCapture->aiparam[ai.getPlayerNum()]->getValue());
}
                  ...
                  for ( int w = 0; w <= 31; w++)
                     if ( bld->loading[w] )
                        if ( canUnitCapture ( bld->loading[w] ))
                           if ( bld->loading[w]->typ->movement[getFirstBit(bld->loading[w]->height)]
      <= beeline(xp,yp,startx,starty)) unitfound ( bld->loading[w] );
...
bool AI ::checkReConquer(Building* bld, Vehicle* veh) {
   ...
   if (enemyNear && veh) {
      ...
      //! if the units inside the building are more worth than the own unit, capture the building
      //! regardless of whether it can be recaptured
...
void AI ::checkConquer() {
   ...
   for (BuildingCaptureContainer::iterator bi = buildingCapture.begin();
        bi != buildingCapture.end();) {
      ...
      if (!getPlayer(bld->getOwner()).diplomacy.isHostile(getPlayerNum()) ||
         ...
         buildingCapture.erase(bi);
      } else if (veh && veh->color != getPlayerNum() * 8)
         buildingCapture.erase(bi);
   ...
   for (BuildingCaptureContainer::iterator bi = buildingCapture.begin();
        bi != buildingCapture.end(); bi++)
      if (bi->second.state != BuildingCapture::conq_unreachable)
         num_reachable_buildings++;
   ...
   for (int c = 0; c <= 8; c++) {
      if (c < 8) {
         if (!getPlayer(c).exist())
            continue;
         if (!getPlayer().diplomacy.isHostile(c))
            continue;
      }
      for (Player::BuildingList::iterator bi = getPlayer(c).buildingList.begin();
           bi != getPlayer(c).buildingList.end(); bi++) {
         Building* bld = *bi;
         int reachable = 0;
         ...
         for (Player::VehicleList::iterator vi = getPlayer().vehicleList.begin();
            ...
            if (fieldAccessible(bld->getEntryField(), veh) != 2)
               continue;
            if (c != 8 && !(veh->typ->hasFunction(ContainerBaseType::ConquerBuildings)))
               continue;
            if (veh->aiparam[getPlayerNum()]->hasJob(AiParameter::job_conquer) &&
                veh->aiparam[getPlayerNum()]->getTask() != AiParameter::tsk_nothing)
               continue;
            ...
            if (val > 0 && enemyNear)
               val -= 0.1 * veh->aiparam[getPlayerNum()]->getValue();
