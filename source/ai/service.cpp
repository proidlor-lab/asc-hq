...
AI ::ServiceOrder ::ServiceOrder(AI* _ai, AI::ServiceOrder::Service _requiredService, int UnitID,
                                 int _pos) {
   active = false;
   failure = 0;
   ai = _ai;
...
int AI::ServiceOrder::possible(Vehicle* supplier) {
   ...
   TransferHandler transferHandler(supplier, getTargetUnit(),
                                   TransferHandler::ignoreHeight + TransferHandler::ignoreDistance);

   int serviceAmount = 0;
   for (TransferHandler::Transfers::const_iterator i = transferHandler.getTransfers().begin();
        i != transferHandler.getTransfers().end(); ++i) {
      if ((*i)->getID() == getServiceID()) {
         int now = (*i)->getAmount(getTargetUnit());
         int poss =
            (*i)->getMax(getTargetUnit(), true);         // the amound that the servicer can provide
...
bool AI::ServiceOrder::execute1st(Vehicle* supplier) {
   ...
   for (int h = 0; h < 8; h++) {
      if (supplier->typ->height & (1 << h)) {
         for (int i = 0; i < sidenum; i++) {
            ...
            if (fld && fieldAccessible(fld, supplier, 1 << h) == 2 && !fld->building &&
                !fld->vehicle) {
               bool result = false;
               ...
               for (TransferHandler::Transfers::const_iterator i =
                       transferHandler.getTransfers().begin();
                    i != transferHandler.getTransfers().end(); ++i)
                  if ((*i)->getID() == getServiceID())
                     result = true;
   ...
   if (path.size()) {
      ...
      if (!targ->maxMovement() ||
          beeline(supplier, targ) / supplySpeed <
             nextServiceBuildingDistance / targ->maxMovement() ||
          nextServiceBuildingDistance < 0) {
...
bool AI::ServiceOrder::serviceUnitExists() {
   for (Player::VehicleList::iterator vi = ai->getPlayer().vehicleList.begin();
        vi != ai->getPlayer().vehicleList.end(); vi++)
      if (possible(*vi))
         return true;
   return false;
}
...
bool AI::ServiceOrder::valid() const {
   switch (requiredService) {
      case srv_repair:
         return getTargetUnit()->damage > 0;
      case srv_ammo:
         return getTargetUnit()->ammo[position] <
                getTargetUnit()->typ->weapons.weapon[position].count;
      case srv_resource:
         return getTargetUnit()->getResource(
                   getTargetUnit()->getStorageCapacity().resource(position), position, 1) <
                getTargetUnit()->getStorageCapacity().resource(position);
      default:
         return false;
   }
}
...
bool AI::ServiceOrder::operator==(const ServiceOrder& so) const {
   return targetUnitID == so.targetUnitID && requiredService == so.requiredService && ai == so.ai &&
...
AI::ServiceOrder& AI ::issueService(AI::ServiceOrder::Service requiredService, int UnitID,
                                    int pos) {
      ...
      if (getMap()->getField(veh->getPosition())->unitHere(veh)) {
         ...
         for (int i = 0; i < resourceTypeNum; i++)
            if (veh->maxMovement())  // stationary units are ignored
               if (veh->getResource(maxint, i, 1) <
                   veh->getStorageCapacity().resource(i) * config.resourceLimit.resource(i) / 100)
      ...
      for (Player::BuildingList::iterator bi = getPlayer().buildingList.begin();
         ...
         if (astar.getFieldAccess(bld->getEntry())) {  // ####TRANS
            ...
            /*
               ...
               buildingPos.z = 1 << getFirstBit ( astar.getFieldAccess(bld->getEntry()) &
               bld->typ->loadcapability );
               */
            ...
            if (loadable) {
               ...
               switch (so.requiredService) {
                  case ServiceOrder::srv_repair:
                     if (bld->canRepair(veh)) {
                  ...
                  case ServiceOrder::srv_resource: {
                     Resources needed = veh->getStorageCapacity() -
                                        veh->getResource(Resources(maxint, maxint, maxint), true);
                  ...
                  case ServiceOrder::srv_ammo: {
                     ...
                     for (int i = 0; i < veh->typ->weapons.count; i++)
                        if (veh->typ->weapons.weapon[i].requiresAmmo())
                           ammoNeeded[veh->typ->weapons.weapon[i].getScalarWeaponType()] +=
                              veh->typ->weapons.weapon[i].count - veh->ammo[i];
   ...
   for (ServiceOrderContainer::iterator i = serviceOrders.begin(); i != serviceOrders.end(); i++) {
      if (!i->getServiceUnit() && i->getTargetUnit()) {
         ...
         if (poss && beeline(i->getTargetUnit(), supplyUnit)) {
            float f = i->getTargetUnit()->aiparam[getPlayerNum()]->getValue() * poss / 100 /
                      beeline(i->getTargetUnit(), supplyUnit);
...
AI::AiResult AI ::executeServices() {
   ...
   serviceOrders.erase(remove_if(serviceOrders.begin(), serviceOrders.end(), ServiceOrder::invalid),
                       serviceOrders.end());
         ...
         if (i->getServiceUnit()) {
            ...
            if ((veh->height & (chtieffliegend | chfliegend | chhochfliegend)) &&
               ...
               if (dest.valid() && veh->canMove()) {
                  ...
                  // displaymessage("warning: no service building found found for unit %s - %d!",1,
                  // veh->typ->description, veh->typ->id);
               }
            }
         }
      }
               ...
               if (fld->building) {
                  ...
                  if (veh->damage) {
                     if (RepairUnitCommand::avail(fld->building)) {
                        std::unique_ptr<RepairUnitCommand> ruc(
                           new RepairUnitCommand(fld->building));
   ...
   /*
   for ( ServiceOrderContainer::iterator i = serviceOrders.begin(); i != serviceOrders.end(); i++ )
   { if ( !i->timeOut() && i->requiredService == VehicleService::srv_repair ) { Vehicle* veh =
   i->getTargetUnit(); veh->aiparam[getPlayerNum()]->getTask() = AiParameter::tsk_serviceRetreat;
          Building* bld = findServiceBuilding( *i );
          if ( bld )
             veh->aiparam[ getPlayerNum() ]->dest = bld->getEntry ( );
          else {
