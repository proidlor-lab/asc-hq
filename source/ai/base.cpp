      ...
      for ( i = 0; i <= 8; i++) {
         ...
         if (i == actmap->actplayer) {
            ...
            while (building != NULL) {
               ...
               ccrcb.init(3);
               ccrcb.initsuche(building->xpos,building->ypos,(maxfusstruppenmove + maxtransportmove)
      / 8 + 1,0); ccrcb.startsuche(); int j; ccrcb.returnresult( &j ); ccrcb.done(); building =
      building->next;
            }
         }
      }
...
class AI_KeyboardWatcher : public sigc::trackable {
  ...
  public:
   AI_KeyboardWatcher(CloseScreenCallback callback)
      : lock(NULL),
        w(NULL),
        menuLocker(mainScreenWidget, MainScreenWidget::LockOptions::Menu +
                                        MainScreenWidget::LockOptions::MapActions +
                                        MainScreenWidget::LockOptions::MapControl) {
      ...
      if (PG_Application::GetApp())
         PG_Application::GetApp()->sigKeyDown.connect(
            sigc::mem_fun(*this, &AI_KeyboardWatcher::keyPressed));
   };
   ...
   */
   ...
   if (benchMark)  // (CGameOptions::Instance()->runAI == 2 && duration > 100*60) ||
      displaymessage(
         ...
         " tactics: %d \n"
         " strategy: %d \n",
         3, float(duration) / 100, setupTime / 100, serviceTime / 100, conquerTime / 100,
         containerTime / 100, tacticsTime / 100, strategyTime / 100);
      ...
      if (i != getPlayerNum()) {
         ...
         if (getPlayer().diplomacy.getProposal(i, &proposal)) {
            ...
            if (proposal > getPlayer().diplomacy.getState(i))
               new Message("Your diplomatic proposal is declined", getMap(), 1 << i,
                           1 << getPlayerNum());
            else {
...
void AI ::showFieldInformation(int x, int y) {
   ...
   const char* fieldinfo =
      "#font02#Field Information (%d,%d)#font01##aeinzug20##eeinzug10##crtp10#"
      "threat orbit: %d\n"
   ...
   sprintf(text, fieldinfo, x, y, threat.threat[7], threat.threat[6], threat.threat[5],
           threat.threat[4], threat.threat[3], threat.threat[2], threat.threat[1], threat.threat[0],
           getFieldInformation(x, y).control);

   MapField* fld = getMap()->getField(x, y);
   if (fld->vehicle && fieldvisiblenow(fld) && fld->vehicle->aiparam[getPlayerNum()]) {
      char text2[1000];
      sprintf(text2, "\nunit nwid: %d ; typeid: %d", fld->vehicle->networkid,
              fld->vehicle->typ->id);
      strcat(text, text2);
      ...
      if (fld->vehicle->aiparam) {
         sprintf(text2, "\nunit value: %d; xtogo: %d, ytogo: %d; ztogo: %d;\njob %s ; task %s \n",
                 aip.getValue(), aip.dest.x, aip.dest.y, aip.dest.getBitmappedHeight(),
                 AIjobs[aip.getJob()], AItasks[aip.getTask()]);
         strcat(text, text2);
      }
   ...
   s += strrr(sec.yp);
   s += "\n";
   const char* threattypes[4] = {"absUnitThreat", "avgUnitThreat", "absFieldThreat",
                                 "avgFieldThreat"};
   ...
   for (int i = 0; i < 4; i++) {
      for (int h = 0; h < 8; h++) {
         ...
         switch (i) {
            case 0:
               s += strrr(sec.absUnitThreat.threat[h]);
               break;
            case 1:
               s += strrr(sec.avgUnitThreat.threat[h]);
               break;
            case 2:
               s += strrr(sec.absFieldThreat.threat[h]);
               break;
            case 3:
               s += strrr(sec.avgFieldThreat.threat[h]);
               break;
         }
         s += "\n";
      }
...
void AI ::write(tnstream& stream) const {
   ...
   for (ServiceOrderContainer::const_iterator i = serviceOrders.begin(); i != serviceOrders.end();
        i++) {
      stream.writeInt(1);
      i->write(stream);
   }
   ...
   for (map<MapCoordinate, BuildingCapture>::const_iterator i = buildingCapture.begin();
        i != buildingCapture.end(); i++) {
      stream.writeInt(1);
   ...
   for (UnitTypeSuccess::const_iterator i = unitTypeSuccess.begin(); i != unitTypeSuccess.end();
        ++i) {
