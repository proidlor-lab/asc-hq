...
class AI : public BaseAI {
   ...
   class ServiceOrder {
     ...
     public:
      ...
      static bool targetDestroyed(const ServiceOrder& so) { return !so.getTargetUnit(); };
   ...
   //! this stores units that have been exhaustively used in tactics and should be skipped from
   //! strategy section
   ...
   //! issues a single service. If the same service-order already exists, it will not be issued a
   //! second time
   ...
   class RefuelConstraint {
     ...
     public:
      RefuelConstraint(AI& ai_, Vehicle* veh_, int maxMove_ = -1)
         : ai(ai_), veh(veh_), ast(NULL), positionsCalculated(false), maxMove(maxMove_){};
      ...
      //! checks whether the unit can crash do to lack of fuel; this is usually true for airplanes.
      //! A unit that does not crash does not need to care about landing positions.
   ...
   class BuildingValueComp : public binary_function<Building*, Building*, bool> {
     ...
     public:
      ...
      bool operator()(const Building*& a, const Building*& b) const {
         return ai->buildingCapture[a->getEntry()].captureValue >
                ai->buildingCapture[b->getEntry()].captureValue;
      };
   };
  ...
  private:
   ...
   bool moveUnit(Vehicle* veh, const MapCoordinate3D& destination, bool intoBuildings = true,
                 bool intoTransports = true);

   void getAttacks(AStar3D& vm, Vehicle* veh, TargetVector& tv, int hemmingBonus,
                   bool justOne = false, bool executeService = true);
   void searchTargets(Vehicle* veh, const MapCoordinate3D& pos, TargetVector& tl, int moveDist,
   ...
   float getAttackValue(const tfight& battle, Vehicle* attackingUnit, Vehicle* attackedUnit,
                        float factor = 1);
   ...
   */
   ...
   void tactics_findBestAttackOrder(Vehicle** units, int* attackOrder, Vehicle* enemy, int depth,
                                    int damage, int& finalDamage, int* finalOrder,
                                    int& finalAttackNum);
   void tactics_findBestAttackUnits(const MoveVariantContainer& mvc,
   ...
   class UnitDistribution {
     public:
      ...
      UnitDistribution() : calculated(false) {
   ...
   class Sections {
      AI* ai;
      Section* section;
      MapCoordinate getAlternativeField(const MapCoordinate& pos,
                                        map<MapCoordinate, int>* destinationCounter, int height,
                                        AStar3D* ast);
     ...
     public:
      ...
      int numY;
      void calculate(void);
      Section& getForCoordinate(int xc,
                                int yc);   //!< returns the section whose center is nearest to x,y
      ...
      Section* getBest(int pass, Vehicle* veh, MapCoordinate3D* dest = NULL,
                       bool allowRefuellOrder = false, bool secondRun = false,
                       map<MapCoordinate, int>* destinationCounter = NULL);
      Sections(AI* _ai);
      void reset();
      ~Sections();
   } sections;
  ...
  public:
   ...
   //! starts the Ai. If benchmark is true, the AI might take longer since it is not time limited,
   //! it won't display any graphics and will output the time it needed to run completely
