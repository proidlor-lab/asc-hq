...
bool AI ::runUnitTask(Vehicle* veh) {
   if (veh->aiparam[getPlayerNum()]->getTask() == AiParameter::tsk_move ||
...
int AI ::getBestHeight(Vehicle* veh) {
   ...
   for (int k = 0; k < 8; k++)
      if (veh->typ->height & (1 << k)) {
         float t = sections.getForCoordinate(veh->xpos, veh->ypos)
                      .avgFieldThreat.threat[veh->getValueType(1 << k)];
...
MapCoordinate3D AI::RefuelConstraint::getNearestRefuellingPosition(bool buildingRequired,
                                                                   bool refuel, bool repair) {
   findPath();
   ...
   */
   ...
   for (AStar3D::VisitedContainer::iterator i = ast->visited.begin(); i != ast->visited.end();
        i++) {
      AStar3D::Node n = *i;
      int dist = int(n.gval);
      ...
      if ((veh->height > chfahrend) && (n.h.getNumericalHeight() == chfahrend)) {
         ...
         if (fi.control == -1 ||
             !ai.getMap()->player[fi.control].diplomacy.isHostile(ai.getPlayerNum()))
         ...
         for ( int h = 0; h < 8; h++ )
            if ( veh->typ->height & ( 1 << h)) {
                ...
                if ( node ) {
                   ...
                   // let's check for landing
                   if ((veh->height > chfahrend) && (fld->a.temp & chfahrend) && ( 1 << h) ==
   chfahrend ) {
                      // we don't want to land in hostile territory
                      FieldInformation& fi = ai.getFieldInformation ( x, y );
                      if ( fi.control == -1 || getdiplomaticstatus2 ( fi.control * 8,
   ai.getPlayerNum()*8 ) == capeace ) landingPositions[dist] = MapCoordinate3D( x,y,chfahrend);
                   }
                }
            }
            */
   ...
   if (buildingRequired) {
      for (ReachableBuildings::iterator rb = reachableBuildings.begin();
           rb != reachableBuildings.end(); rb++) {
...
bool AI::RefuelConstraint::returnFromPositionPossible(const MapCoordinate3D& pos,
                                                      int theoreticalFuel) {
   if (!veh->typ->fuelConsumption)
      return true;
   ...
   if (theoreticalFuel - (dist + dist2) / maxmalq * veh->typ->fuelConsumption >
       0.2 * veh->getStorageCapacity().fuel)
      return true;
   else
      return false;
}
...
AI::VehicleTypeEfficiencyCalculator::VehicleTypeEfficiencyCalculator(AI& _ai, Vehicle* _attacker,
                                                                     Vehicle* _target)
...
AI::AiResult AI ::container(ContainerBase* cb) {
   ...
   for (vector<Vehicle*>::const_iterator j = cb->getCargo().begin(); j != cb->getCargo().end();
        ++j) {
      Vehicle* veh = *j;
      if (veh)
...
AI::AiResult AI::buildings(int process) {
   ...
   for (Player::BuildingList::iterator bi = getPlayer().buildingList.begin();
        bi != getPlayer().buildingList.end(); bi++) {
      buildingCounter++;
      displaymessage2("processing building %d", buildingCounter);

      int unitCounter = 0;
      for (vector<Vehicle*>::const_iterator j = (*bi)->getCargo().begin();
           j != (*bi)->getCargo().end(); j++) {
         Vehicle* veh = *j;
         if (veh) {
            ++unitCounter;
...
bool AI ::moveUnit(Vehicle* veh, const MapCoordinate3D& destination, bool intoBuildings,
                   bool intoTransports) {
   ...
   else if (beeline(veh->getPosition(), destination) > veh->maxMovement())
   ...
   if (res.successful()) {
      ...
      displaymessage("AI :: moveUnit (path) \n error in movement step 3 with unit %d", 1,
                     veh->networkid);
      return false;
   }
...
AI::CheckFieldRecon ::CheckFieldRecon(AI* _ai)
   : SearchFields(_ai->getMap()), player(_ai->getPlayerNum()), ai(_ai) {
   for (int i = 0; i < 3; i++) {
      ownFields[i] = 0;
      enemyFields[i] = 0;
   }
...
void AI ::calcReconPositions() {
   ...
   for (int y = 0; y < getMap()->ysize; y++)
      for (int x = 0; x < getMap()->xsize; x++) {
         ...
         if (fi.control == getPlayerNum() && !fld->building &&
             (!fld->vehicle ||
              fld->vehicle->aiparam[getPlayerNum()]->getJob() == AiParameter::job_recon)) {
...
void AI ::runReconUnits() {
   ...
   for (Player::VehicleList::iterator vi = getPlayer().vehicleList.begin();
      ...
      if (veh->aiparam[getPlayerNum()] &&
          veh->aiparam[getPlayerNum()]->getJob() == AiParameter::job_recon)
         ...
         if (veh->aiparam[getPlayerNum()] &&
            ...
            if (reconPositions.find(veh->getPosition()) == reconPositions.end()) {
               ...
               if (!reconPositions.empty()) {
                  ...
                  ast.findAllAccessibleFields();
                  for (ReconPositions::iterator i = reconPositions.begin();
                       i != reconPositions.end(); i++) {
...
AI::UnitDistribution::Group AI::getUnitDistributionGroup(VehicleType* vt) {
   switch (chooseJob(vt).front()) {
      case AiParameter::job_supply:
         return UnitDistribution::service;
      case AiParameter::job_recon:
         return UnitDistribution::recon;
      case AiParameter::job_conquer:
         return UnitDistribution::conquer;
...
AI::UnitDistribution AI::calcUnitDistribution() {
   ...
   if (unitCount) {
      ...
      for (Player::VehicleList::iterator i = getPlayer().vehicleList.begin();
           i != getPlayer().vehicleList.end(); i++)
   ...
   */
   ...
   for (int p = 0; p < 8; p++)
      if (getMap()->player[p].diplomacy.isHostile(getPlayerNum()))
         for (Player::VehicleList::iterator vli = getMap()->player[p].vehicleList.begin();
              vli != getMap()->player[p].vehicleList.end(); vli++) {
            if ((*vli)->aiparam[getPlayerNum()]) {
               enemyThreat += (*vli)->aiparam[getPlayerNum()]->threat;
               enemyValue[(*vli)->aiparam[getPlayerNum()]->valueType] +=
                  (*vli)->aiparam[getPlayerNum()]->getValue();
            }
         }
   ...
   for (Player::BuildingList::iterator bli = getPlayer().buildingList.begin();
        bli != getPlayer().buildingList.end(); bli++) {
      Building* bld = *bli;
      for (int i = 0; i < bld->getProduction().size(); i++)
         if (bld->getProduction()[i] && bld->vehicleUnloadable(bld->getProduction()[i])) {
            ...
            if (typ->aiparam[getPlayerNum()]->getValue()) {
               if (typ->aiparam[getPlayerNum()]->getValue() <
                   enemyThreat.threat[typ->aiparam[getPlayerNum()]->valueType])
                  danger = enemyThreat.threat[typ->aiparam[getPlayerNum()]->valueType] /
                           typ->aiparam[getPlayerNum()]->getValue();
            }
   ...
   if (!produceable.empty()) {
      ...
      do {
         ...
         //              {
         for (Produceable::reverse_iterator p = produceable.rbegin(); p != produceable.rend();
              p++) {
            //                    if ( getUnitDistributionGroup ( p->second.vt) == i ) {
            ProductionRating& pr = p->second;

            if (find(lockedBuildings.begin(), lockedBuildings.end(), pr.bld) ==
                lockedBuildings.end()) {
               ...
               if (!lack && pr.bld->vehicleUnloadSystem(pr.vt, 255)) {
                  ...
                  if (!(lack & (1 << 10))) {
                     if (pr.bld->getResource(pr.vt->productionCost, 1) +
                            pr.bld->netResourcePlus() * config.waitForResourcePlus >=
                         pr.vt->productionCost)
