...
void AI ::searchTargets(Vehicle* veh, const MapCoordinate3D& pos, TargetVector& tl, int moveDist,
                        AStar3D& vm, int hemmingBonus) {
   npush(veh->xpos);
   npush(veh->ypos);
   npush(veh->height);
   ...
   if (unitRadarActivated)
      veh->removeview();
   // int fieldsWithChangedVisibility = evaluateviewcalculation ( getMap(), veh->xpos, veh->ypos,
   // veh->typ->view, 0xff );
   veh->xpos = pos.x;
   veh->ypos = pos.y;
   veh->height = pos.getBitmappedHeight();
   veh->addview();
   int fieldsWithChangedVisibility =
      evaluateviewcalculation(getMap(), veh->getPosition(), veh->view, 0xff);

   if (AttackCommand::avail(veh)) {
      AttackCommand va(veh);
      va.searchTargets();
      for (AttackCommand::FieldList::const_iterator i = va.getAttackableUnits().begin();
           i != va.getAttackableUnits().end(); ++i) {
         ...
         for (int nf = 0; nf < sidenum; nf++) {
            MapCoordinate mc =
               getNeighbouringFieldCoordinate(MapCoordinate(mv->attackx, mv->attacky), nf);
            MapField* fld = getMap()->getField(mc);
            if (fld && !veh->typ->wait)
               mv->neighbouringFieldsReachable[nf] =
                  (vm.visited.find(MapCoordinate3D(mc.x, mc.y, pos.getBitmappedHeight())) ||
                   (veh->xpos == mc.x && veh->ypos == mc.y)) &&
                  !fld->building && (!fld->vehicle || fld->unitHere(veh));
            else
         ...
         for (int nf = 0; nf < sidenum - 1 && nf < hemmingBonus; nf++) {
            ...
            MapCoordinate mc = getNeighbouringFieldCoordinate(
               MapCoordinate(mv->attackx, mv->attacky), checkDir % sidenum);
            MapField* fld = getMap()->getField(mc);
...
bool AI::MoveVariant::operator>(const AI::MoveVariant& mv2) const {
   return (result > mv2.result || (result == mv2.result && positionThreat < mv2.positionThreat) ||
           (result == mv2.result && positionThreat == mv2.positionThreat &&
            moveDist < mv2.moveDist));  // mv1.moveDist < mv2.moveDist
}
...
bool operator> ( const AI::MoveVariant& mv1, const AI::MoveVariant& mv2 )
{
   return (     mv1.result > mv2.result
            ...
            || (mv1.result == mv2.result && mv1.positionThreat == mv2.positionThreat && mv1.moveDist
< mv2.moveDist) );  // mv1.moveDist < mv2.moveDist
}
...
bool AI::moveVariantComp(const AI::MoveVariant* mv1, const AI::MoveVariant* mv2) {
   return *mv1 < *mv2;
   // return ( mv1->result < mv2->result || (mv1->result == mv2->result && mv1->moveDist >
   // mv2->moveDist ));
}
...
void AI::getAttacks(AStar3D& vm, Vehicle* veh, TargetVector& tv, int hemmingBonus, bool justOne,
   ...
   if (getMap()
          ->getField(veh->xpos, veh->ypos)
          ->unitHere(veh))  // unit not inside a building or transport
   ...
   if (!veh->typ->wait) {
      ...
      int fuelLacking = 0;
      for (AStar3D::VisitedContainer::iterator ff = vm.visited.begin(); ff != vm.visited.end();
           ++ff) {
         ...
         if (!node.hasAttacked) {
            ...
            if (!fld->vehicle && !fld->building) {
               if (!apl || apl->returnFromPositionPossible(node.h)) {
                  searchTargets(veh, node.h, tv, beeline(node.h.x, node.h.y, orgxpos, orgypos), vm,
                                hemmingBonus);
                  if (tv.size() && justOne)
                     return;
               } else
...
AI::AiResult AI::executeMoveAttack(Vehicle* veh, TargetVector& tv) {
   ...
   if (!res.successful())
      displaymessage("AI :: executeMoveAttack \n error in attack step 2 with unit %d", 1,
                     veh->networkid);
   ...
   if (!res.successful())
      displaymessage("AI :: executeMoveAttack \n error in attack step 3 with unit %d", 1,
                     veh->networkid);
   else
      va.release();
...
AI::AiResult AI::moveToSavePlace(Vehicle* veh, int preferredHeight) {
   ...
   if (getMap()
          ->getField(veh->xpos, veh->ypos)
          ->unitHere(veh)) {  // vehicle not in building / transport
      threat =
         int(getFieldThreat(veh->xpos, veh->ypos).threat[veh->aiparam[getPlayerNum()]->valueType] *
                1.5 +
             1);
      // multiplying with 1.5 to make this field a bit unattractive, to allow other units (for
      // example buggies) to attack from this field to, since it is probably quite a good position
      // (else it would not have been chosen)
   }
   ...
   for (set<MapCoordinate3D>::const_iterator i = fields.begin(); i != fields.end(); ++i) {
      ...
      if (!fld->vehicle && !fld->building) {
         ...
         // make fields far away a bit unattractive; we don't want to move the whole distance back
         // again next turn
         int t = int(ait.threat[veh->aiparam[getPlayerNum()]->valueType] * log(double(_dist)) /
                     log(double(10)));
   ...
   if (veh->xpos != xtogo || veh->ypos != ytogo) {
      ...
      if (res.successful())
         ...
         displaymessage("AI :: moveToSavePlace \n error in movement3 step 3 with unit %d", 1,
                        veh->networkid);
...
AI::AiResult AI::tactics(void) {
   const int tsk_num = 3;
   AiParameter::Task tasks[tsk_num] = {AiParameter::tsk_nothing, AiParameter::tsk_tactics,
   ...
   for (Player::VehicleList::iterator vi = getPlayer().vehicleList.begin();
      ...
      if (veh->aiparam[getPlayerNum()]->getJob() == AiParameter::job_fight ||
          ...
          (veh->aiparam[getPlayerNum()]->getJob() == AiParameter::job_guard &&
           veh->aiparam[getPlayerNum()]->dest_nwid == -1 &&
           veh->aiparam[getPlayerNum()]->dest.x == -1))
      ...
      if (getMap()->getField(veh->getPosition())->vehicle != veh) {
         ...
         if (transport) {
            const ContainerBaseType::TransportationIO* unloadSystem =
               transport->vehicleUnloadSystem(veh->typ, -1);
      ...
      if (maxWeapDist > 0) {
         ...
         for (int x = veh->xpos - xdist; x <= veh->xpos + xdist; x++)
            for (int y = veh->ypos - ydist; y <= veh->ypos + ydist; y++) {
               ...
               if (fld) {
                  if (fld->vehicle &&
                      getPlayer(veh->getOwner()).diplomacy.isHostile(fld->vehicle->getOwner()))
                     enemiesNear = true;
                  if (fld->building &&
                      getPlayer(veh->getOwner()).diplomacy.isHostile(fld->building->getOwner()))
                     enemiesNear = true;
               }
            }
   ...
   while (!tactVehicles.empty()) {
      ...
      do {
            ...
            if (veh) {
               ...
               if (stat == -1) {  // couldn't change height due to blocked way or something similar
                  ...
                  if (tv.size()) {
                     ...
                     if (beeline(mv->movePos.x, mv->movePos.y, mv->attackx, mv->attacky) >
                            maxmalq ||
                         veh->height >= chtieffliegend ||
                         (mv->enemy && mv->enemy->height >= chtieffliegend))
                        directAttack = true;

                     int freeNeighbouringFields = 0;
                     for (int j = 0; j < sidenum; j++) {
                        MapField* fld = getMap()->getField(getNeighbouringFieldCoordinate(
                           MapCoordinate(mv->attackx, mv->attacky), j));
                        if (fld)
                     ...
                     if (directAttack) {
                        /* to avoid recalculating the vision with every variant we are going to make
                           it easy... Since all units now only attack the neighbouring fields, which
                           is generally visible, the AI does not cheat here.
                        */
                        VisibilityStates org_vision = getVision();
         ...
         // if there were direct attacks, some fields may now be blocked by units that did a
         // directAttack, so all other units plans must be recalculated
         ...
         */
         do {  // while currentTarget != targets.end()
            ...
            if (enemy) {
               ...
               tactics_findBestAttackUnits(attacker, mvci, positions, 0, finalPositions, finalValue,
                                           0, 0, ticker);
               ...
               if (finalValue > 0) {
                  for (int i = 0; i < sidenum; i++)
                     if (finalPositions[i]) {
                        ...
                        setVision(org_vision);
                        MapCoordinate affected =
                           MapCoordinate(finalPositions[i]->xpos, finalPositions[i]->ypos);
                        MapCoordinate3D dst = getNeighbouringFieldCoordinate(
                           MapCoordinate3D(enemy->xpos, enemy->ypos, finalPositions[i]->height), i);
                        ...
                        if (!getMap()->getUnit(nwid)) {
                           TactVehicles::iterator att =
                              find(tactVehicles.begin(), tactVehicles.end(), nwid);
                  ...
                  if (unitcount) {
                     ...
                     tactics_findBestAttackOrder(finalPositions, attackOrder, enemy, 0,
                                                 enemy->damage, finalDamage, finalOrder,
                                                 finalAttackNum);

                     vector<Vehicle*> unitsToMoveAwayAfterAttacking;

                     MapField* enemyField = getMap()->getField(enemy->xpos, enemy->ypos);
                     for (int i = 0; i < finalAttackNum && enemyField->vehicle == enemy &&
                                     finalAttackNum < maxint;
                          i++) {
                        checkKeys();
                        if (finalOrder[i] < 0)
                           warningMessage("!!!");
                        ...
                        if (i < finalAttackNum && finalPositions[finalOrder[i]]) {
                           ...
                           VehicleTypeEfficiencyCalculator vtec(
                              *this, finalPositions[finalOrder[i]], enemy);
                           ...
                           if (getMap()->getUnit(nwid) &&
                               a->typ->hasFunction(ContainerBaseType::MoveAfterAttack))
                              unitsToMoveAwayAfterAttacking.push_back(a);
                        }
                     }

                     for (vector<Vehicle*>::iterator i = unitsToMoveAwayAfterAttacking.begin();
                          i != unitsToMoveAwayAfterAttacking.end(); ++i)
                        moveToSavePlace(*i);
            ...
            do {
               ...
               if (currentTarget != targets.end()) {
                  if (hemmingBonus == currentTarget->second.size() - 1) {
                     for (AffectedFields::iterator i = affectedFields.begin();
                          i != affectedFields.end(); i++)
                        for (MoveVariantContainer::iterator j = currentTarget->second.begin();
                             j != currentTarget->second.end(); j++) {
                           Vehicle* veh = j->attacker;

                           // here are only vehicles that attack neighbouring fields; ranged attacks
                           // are handled as "directAttacks" earlier
...
void AI ::tactics_findBestAttackOrder(Vehicle** units, int* attackOrder, Vehicle* enemy, int depth,
                                      int damage, int& finalDamage, int* finalOrder,
                                      int& finalAttackNum) {
   ...
   for (int i = 0; i < sidenum; i++) {
      ...
      if (units[i] && !found) {
         ...
         npush(units[i]->ypos);
         MapCoordinate mc =
            getNeighbouringFieldCoordinate(MapCoordinate(enemy->xpos, enemy->ypos), i);
         units[i]->xpos = mc.x;
         units[i]->ypos = mc.y;
         ...
         tactics_findBestAttackOrder(units, attackOrder, enemy, depth + 1, damage + newdamage,
                                     finalDamage, finalOrder, finalAttackNum);
...
float AI ::getAttackValue(const tfight& battle, Vehicle* attackingUnit, Vehicle* attackedUnit,
                          float factor) {
   calculateThreat(attackedUnit);
   float result = (battle.dv.damage - attackedUnit->damage) * getValue(attackedUnit) * factor -
                  1 / config.aggressiveness * (battle.av.damage - attackingUnit->damage) *
                     getValue(attackingUnit);
...
class UnitAttacksUnit_FakeHemming : public tunitattacksunit {
   ...
   bool checkHemming(Vehicle* d_eht, int direc) { return neighbours[direc]; };

  public:
   UnitAttacksUnit_FakeHemming(AI* ai, Vehicle* attacker, Vehicle* defender, Vehicle** _neighbours)
      : tunitattacksunit(attacker, defender) {
      for (int i = 0; i < sidenum; i++) {
         MapField* fld =
            ai->getMap()->getField(getNeighbouringFieldCoordinate(attacker->getPosition(), i));
...
void AI ::tactics_findBestAttackUnits(const MoveVariantContainer& mvc,
                                      MoveVariantContainer::iterator& m, Vehicle** positions,
                                      float value, Vehicle** finalPosition, float& finalValue,
                                      int unitsPositioned, int recursionDepth, int startTime) {
   if (m == mvc.end() || unitsPositioned >= 6 || recursionDepth >= 8 ||
       (startTime + config.maxTactTime < ticker && !benchMark)) {
      float value = 0;
      ...
      for (int i = 0; i < sidenum; i++) {
         if (m->neighbouringFieldsReachable[i] && !positions[i]) {
            ...
            if (m->result > 0)
               ...
               value += 1;
            m++;
            tactics_findBestAttackUnits(mvc, m, positions, value, finalPosition, finalValue,
                                        unitsPositioned + 1, recursionDepth + 1, startTime);
            m--;
            value -= m->result;
            positions[i] = NULL;
         } else {
            m++;
            tactics_findBestAttackUnits(mvc, m, positions, value, finalPosition, finalValue,
                                        unitsPositioned, recursionDepth + 1, startTime);
            m--;
         }
      }
   }
