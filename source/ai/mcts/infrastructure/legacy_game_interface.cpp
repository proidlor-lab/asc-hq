...
LegacyGameInterface::LegacyGameInterface(GameMap* gameMap) : gameMap(gameMap) {
   if (!gameMap) {
      throw std::invalid_argument("LegacyGameInterface: gameMap cannot be null");
   }
}

bool LegacyGameInterface::executeMove(UnitID unitID, ::MapCoordinate destination,
...
bool LegacyGameInterface::executeAttack(UnitID attackerID, UnitID targetID,
                                        MapDisplayInterface* display) {
   // LEGACY CODE INTEGRATION:
...
UnitID LegacyGameInterface::getUnitAt(::MapCoordinate position) const {
   ...
   if (position.x < 0 || position.x >= gameMap->xsize || position.y < 0 ||
       position.y >= gameMap->ysize) {
