...
void applyKeyValue(const std::string& key, const std::string& value, ParsedProfile& profile) {
    ...
    if (k == "maxiterations") cfg.maxIterations = parseNumber<int>(value, cfg.maxIterations);
    else if (k == "maxtimems") cfg.maxTimeMs = parseNumber<int>(value, cfg.maxTimeMs);
    else if (k == "rolloutdepthlimit") cfg.rolloutDepthLimit = parseNumber<int>(value, cfg.rolloutDepthLimit);
    else if (k == "explorationconstant") cfg.explorationConstant = parseNumber<double>(value, cfg.explorationConstant);
    else if (k == "earlyterminationthreshold") cfg.earlyTerminationThreshold = parseNumber<double>(value, cfg.earlyTerminationThreshold);
    else if (k == "materialweight") cfg.materialWeight = parseNumber<double>(value, cfg.materialWeight);
    else if (k == "positionweight") cfg.positionWeight = parseNumber<double>(value, cfg.positionWeight);
    else if (k == "healthweight") cfg.healthWeight = parseNumber<double>(value, cfg.healthWeight);
    else if (k == "threatweight") cfg.threatWeight = parseNumber<double>(value, cfg.threatWeight);
    else if (k == "agentprofile") cfg.agentProfile = value;
    else if (k == "enablelogging") cfg.enableLogging = parseBool(value, cfg.enableLogging);
    else if (k == "enabledebugoutput") cfg.enableDebugOutput = parseBool(value, cfg.enableDebugOutput);
}
...
MCTSConfigLoader::ProfileConfig MCTSConfigLoader::loadProfile(const std::string& name) {
    ...
    if (key == "aggressive" || key == "mcts_aggressive") base = builtinAggressive();
    else if (key == "defensive" || key == "mcts_defensive") base = builtinDefensive();
...
std::map<std::string, MCTSConfigLoader::ConfigEntry> MCTSConfigLoader::loadConfigFile(const std::string&) {
    return {};
}

void MCTSConfigLoader::registerConfig(const std::string&, const MCTSConfig&, const std::string&) {}
std::vector<std::string> MCTSConfigLoader::getAvailableConfigs() { return {}; }
MCTSConfigLoader::ConfigEntry MCTSConfigLoader::getConfigEntry(const std::string&) { return {}; }
bool MCTSConfigLoader::hasConfig(const std::string&) { return false; }
...
std::string MCTSConfigLoader::trim(const std::string& s) { return s; }
std::pair<std::string, std::string> MCTSConfigLoader::parseLine(const std::string& line) { return {line, {}}; }
