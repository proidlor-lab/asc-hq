...
class MCTSSearch {
  public:
   ...
   MCTSSearch(std::unique_ptr<ITacticalEvaluator> evaluator,
      ...
      : evaluator_(std::move(evaluator)), config_(config), root_(nullptr) {
    ...
    */
   MCTSResult search(const IGameState& initialState, PlayerID perspective);
   ...
   const MCTSConfig& getConfig() const noexcept { return config_; }
    ...
    */
   const MCTSNode* getRootNode() const { return root_.get(); }
  ...
  private:
    ...
    */
   std::unique_ptr<MCTSNode> createChildNode(MCTSNode* parent, const Action& action);
