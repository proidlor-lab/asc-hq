...
class MCTSNode {
  public:
    ...
    */
   MCTSNode(std::unique_ptr<IGameState> state, MCTSNode* parent, const Action& action,
   ...
   const IGameState& getState() const noexcept { return *state_; }

   IGameState& getStateMutable() noexcept { return *state_; }

   PlayerID getPerspective() const noexcept { return perspective_; }

   // ========== Tree Navigation ==========

   MCTSNode* getParent() const noexcept { return parent_; }

   const std::vector<std::unique_ptr<MCTSNode>>& getChildren() const noexcept { return children_; }
   ...
   bool hasAction() const noexcept { return action_.has_value(); }

   const Action& getAction() const { return action_.value(); }

   // ========== Statistics ==========

   int getVisits() const noexcept { return visits_; }

   double getTotalValue() const noexcept { return totalValue_; }
   ...
   double getUCB1Value(double explorationConstant = 1.414213562373095) const noexcept {
      ...
      double exploration =
         explorationConstant *
         std::sqrt(std::log(static_cast<double>(parent_->visits_)) / static_cast<double>(visits_));
    ...
    */
   void markFullyExpanded() noexcept { isFullyExpanded_ = true; }

   /**
    * Check if node is fully expanded
    */
   bool isFullyExpanded() const noexcept { return isFullyExpanded_; }
   ...
   Stats getStats() const noexcept {
      return Stats{visits_, getAverageValue(), static_cast<int>(children_.size()), getDepth(),
                   isFullyExpanded_};
   }
