...
struct ActionResult {
   ...
   constexpr ActionResult() noexcept : code(ActionResultCode::Success) {}

   explicit constexpr ActionResult(ActionResultCode c) noexcept : code(c) {}
...
[[nodiscard]] inline std::string getActionTypeName(const Action& action) {
   return std::visit(overloaded{[](const MoveAction&) { return std::string("Move"); },
...
[[nodiscard]] inline UnitID getActionUnitID(const Action& action) {
   return std::visit(overloaded{[](const MoveAction& m) { return m.unitID; },
...
[[nodiscard]] inline std::string actionToString(const Action& action) {
   return std::visit(overloaded{[](const MoveAction& m) { return m.toString(); },
