...
int CombatCalculator::calculateDamage(const UnitSnapshot& attacker, const UnitSnapshot& defender,
                                      int weaponIndex, int distance, int terrainDefenseBonus,
                                      float hemmingFactor) {
   // Validate inputs
   ...
   if (defender.type != nullptr) {
      auto targetCheck =
         canWeaponTarget(weapon, attacker.height, defender.height, defender.type->movemalustyp);
   ...
   float absStrength = static_cast<float>(baseStrength) * (1.0f + expOffensiveBonus + attackBonus) *
                       damageStateFactor * hemmingFactor;
   ...
   float absDefense =
      (static_cast<float>(armor) / armorDivisor) * (1.0f + defenseBonus + expDefensiveBonus);
   ...
   int newDamage =
      static_cast<int>(std::ceil(static_cast<float>(defender.damage) + damageIncrease));
...
WeaponTargetCheck CombatCalculator::canWeaponTarget(const ::SingleWeapon& weapon,
                                                    int attackerHeight, int defenderHeight,
...
int CombatCalculator::getWeaponStrength(const ::SingleWeapon& weapon, int distance) {
   ...
   if (distance <= minRange) {
      ...
      if (range > 0) {
         ...
         return static_cast<int>(weapon.maxstrength -
                                 ratio * (weapon.maxstrength - weapon.minstrength));
      } else {
         return weapon.maxstrength;
      }
   }
...
float CombatCalculator::calculateExperienceBonus(int experience, float maxBonus,
