...
class SimpleCombatEvaluator : public ITacticalEvaluator {
  public:
   ...
   EvaluationResult evaluate(const IGameState& snapshot,
                             const EvaluationContext& context) const override;

   bool isTerminalState(const IGameState& snapshot, PlayerID player) const override;

   std::unique_ptr<ITacticalEvaluator> clone() const override {
      return std::make_unique<SimpleCombatEvaluator>();
   }

   const char* getName() const noexcept override { return "SimpleCombatEvaluator"; }
  ...
  private:
    ...
    */
   float evaluateMaterial(const IGameState& snapshot, PlayerID player) const;
    ...
    * @return Score (-1.0 to +1.0)
    */
   float evaluatePosition(const IGameState& snapshot, PlayerID player) const;
    ...
    * @return Score (-1.0 to +1.0)
    */
   float evaluateHealth(const IGameState& snapshot, PlayerID player) const;
    ...
    */
   float evaluateThreat(const IGameState& snapshot, PlayerID player) const;
    ...
    */
   static bool isInReactionFireZone(const UnitSnapshot& unit, const IGameState& snapshot);
    ...
    */
   static float getHeightAdvantage(const UnitSnapshot& unit, const IGameState& snapshot);
    ...
    */
   static float getTerrainBonus(const UnitSnapshot& unit, const IGameState& snapshot);
    ...
    */
   static int countNearbyEnemies(const UnitSnapshot& unit, const IGameState& snapshot, int range);
    ...
    */
   static std::vector<const UnitSnapshot*> getEnemyUnits(const IGameState& snapshot,
                                                         PlayerID player);
