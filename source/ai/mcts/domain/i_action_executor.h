...
class IActionExecutor {
  public:
    ...
    */
   [[nodiscard]] virtual ActionResult
   execute(const Action& action, const ExecutionContext& context = ExecutionContext()) = 0;
    ...
    */
   [[nodiscard]] virtual std::vector<Action>
   generateLegalActions(UnitID unitID, bool includeWait = true) const = 0;
...
class ActionExecutorFactory {
  public:
    ...
    */
   [[nodiscard]] static std::unique_ptr<IActionExecutor>
   createSimulationExecutor(std::unique_ptr<IGameState> snapshot, GameMap* legacyMap = nullptr);
    ...
    * @return Unique pointer to executor
    */
   [[nodiscard]] static std::unique_ptr<IActionExecutor> createRealGameExecutor(GameMap* gameMap);
};
