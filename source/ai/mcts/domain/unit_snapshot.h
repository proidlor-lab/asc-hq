...
struct UnitSnapshot {
   ...
   UnitSnapshot()
      : networkID(0),
        ...
        attacked(false),
        ammoMask(0xFFFF),
        experienceOffensive(0),
        experienceDefensive(0),
        direction(0),
        _padding(0) {}
    ...
    */
   constexpr MapCoordinate getPosition() const noexcept { return MapCoordinate(x, y); }

   /**
    * Calculate approximate HP percentage (0-100, C++23: constexpr)
    */
   constexpr uint8_t getHPPercent() const noexcept { return static_cast<uint8_t>(100 - damage); }

   /**
    * Check if unit is destroyed (C++23: constexpr)
    */
   constexpr bool isDestroyed() const noexcept { return damage >= 100; }

   /**
    * Check if unit can move (C++23: constexpr)
    */
   constexpr bool canMove() const noexcept { return movement > 0 && fuel > 0 && !isDestroyed(); }
