...
class RealGameActionExecutor : public IActionExecutor {
  ...
  private:
   ...
   [[nodiscard]] ActionResult executeMoveReal(const MoveAction& action,
                                              const ExecutionContext& context);

   [[nodiscard]] ActionResult executeAttackReal(const AttackAction& action,
                                                const ExecutionContext& context);

   [[nodiscard]] ActionResult executeWaitReal(const WaitAction& action,
                                              const ExecutionContext& context);

   // ========== Helper Methods ==========

   void invalidateSnapshot() const { snapshotDirty_ = true; }
