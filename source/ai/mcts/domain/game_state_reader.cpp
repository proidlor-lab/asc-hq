...
GameStateReader::GameStateReader(const GameMap* gameMap) 
    : map(gameMap) 
{
    if (!map) {
...
std::vector<const Vehicle*> GameStateReader::getUnitsInRange(
...
ResourceSnapshot GameStateReader::getPlayerResources(PlayerID player) const {
    ...
    return ResourceSnapshot(
        static_cast<int16_t>(res.energy),
        static_cast<int16_t>(res.material),
...
std::unique_ptr<GameStateSnapshot> GameStateReader::createFullSnapshot(
...
std::unique_ptr<GameStateSnapshot> GameStateReader::createTacticalSnapshot(
    const std::vector<UnitID>& unitIDs,
    const MapCoordinate& center,
    int radius
) const {
    auto snapshot = std::make_unique<GameStateSnapshot>();
...
bool GameStateReader::isValidCoordinate(const MapCoordinate& pos) const {
    return pos.x >= 0 && pos.x < map->xsize && 
           pos.y >= 0 && pos.y < map->ysize;
}
...
bool GameStateReader::isInRange(
...
void GameStateReader::addUnitToSnapshot(
...
void GameStateReader::addTerrainToSnapshot(
    GameStateSnapshot* snapshot, 
    const MapCoordinate& pos
) const {
    if (!isValidCoordinate(pos)) return;
