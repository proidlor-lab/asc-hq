...
RealGameActionExecutor::RealGameActionExecutor(GameMap* gameMap) : gameMap_(gameMap) {
   if (!gameMap_) {
...
ActionResult RealGameActionExecutor::execute(const Action& action,
   ...
   return std::visit(
      overloaded{[this, &context](const MoveAction& m) { return executeMoveReal(m, context); },
...
ActionResult RealGameActionExecutor::isLegal(const Action& action) const {
   ...
   return std::visit(overloaded{[](const MoveAction&) { return ActionResult::success(); },
                                [](const AttackAction&) { return ActionResult::success(); },
                                [](const WaitAction&) { return ActionResult::success(); }},
                     action);
}

std::vector<Action> RealGameActionExecutor::generateLegalActions(UnitID unitID,
...
ActionResult RealGameActionExecutor::executeMoveReal(const MoveAction& action,
                                                     const ExecutionContext& context) {
   // Phase 0.2 STUB
...
ActionResult RealGameActionExecutor::executeAttackReal(const AttackAction& action,
                                                       const ExecutionContext& context) {
   // Phase 0.2 STUB
...
ActionResult RealGameActionExecutor::executeWaitReal(const WaitAction& action,
                                                     const ExecutionContext& context) {
   // Phase 0.2 STUB
