...
bool CombatAbility::isAvailable(const IGameState& state, const UnitSnapshot& unit) const {
...
std::vector<Action> CombatAbility::generateActions(const IGameState& state,
   ...
   for (const auto* target : targets) {
      actions.emplace_back(AttackAction{
         unit.networkID, target->getPosition(),
...
std::vector<const UnitSnapshot*> CombatAbility::findTargetsInRange(const IGameState& state,
                                                                   const UnitSnapshot& unit) const {
