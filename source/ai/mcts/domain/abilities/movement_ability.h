...
private:
     ...
     */
    [[nodiscard]] std::vector<MoveAction> generateReachableMoves(
        const IGameState& state,
     ...
     */
    [[nodiscard]] std::vector<MoveAction> generateAdjacentMoves(
        const IGameState& state,
        const UnitSnapshot& unit) const;
     ...
     */
    [[nodiscard]] std::vector<MapCoordinate> getAdjacentHexes(
