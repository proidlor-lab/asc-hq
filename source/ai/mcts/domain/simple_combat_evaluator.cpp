...
EvaluationResult SimpleCombatEvaluator::evaluate(const IGameState& snapshot,
...
bool SimpleCombatEvaluator::isTerminalState(const IGameState& snapshot, PlayerID player) const {
...
float SimpleCombatEvaluator::evaluateMaterial(const IGameState& snapshot, PlayerID player) const {
...
float SimpleCombatEvaluator::evaluatePosition(const IGameState& snapshot, PlayerID player) const {
   auto playerUnits = snapshot.getPlayerUnits(player);
   if (playerUnits.empty()) {
      return -1.0f;
   }
...
float SimpleCombatEvaluator::evaluateHealth(const IGameState& snapshot, PlayerID player) const {
   auto playerUnits = snapshot.getPlayerUnits(player);
   if (playerUnits.empty()) {
      return -1.0f;
   }
...
float SimpleCombatEvaluator::evaluateThreat(const IGameState& snapshot, PlayerID player) const {
   auto playerUnits = snapshot.getPlayerUnits(player);
   if (playerUnits.empty()) {
      return -1.0f;
   }
...
bool SimpleCombatEvaluator::isInReactionFireZone(const UnitSnapshot& unit,
   ...
   for (const auto& enemy : snapshot.getUnits()) {
      ...
      for (int i = 0; i < enemy.type->weapons.count && i < 16; ++i) {
         ...
         if (distance >= minRange && distance <= maxRange && (enemy.ammoMask & (1 << i))) {
            return true;
         }
      }
   }
...
float SimpleCombatEvaluator::getHeightAdvantage(const UnitSnapshot& unit,
                                                const IGameState& snapshot) {
...
float SimpleCombatEvaluator::getTerrainBonus(const UnitSnapshot& unit, const IGameState& snapshot) {
...
int SimpleCombatEvaluator::countNearbyEnemies(const UnitSnapshot& unit, const IGameState& snapshot,
                                              int range) {
   int count = 0;
...
std::vector<const UnitSnapshot*> SimpleCombatEvaluator::getEnemyUnits(const IGameState& snapshot,
                                                                      PlayerID player) {
