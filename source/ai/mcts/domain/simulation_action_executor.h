...
class SimulationActionExecutor : public IActionExecutor {
  public:
   ...
   execute(const Action& action, const ExecutionContext& context = ExecutionContext()) override;

   [[nodiscard]] ActionResult isLegal(const Action& action) const override;

   [[nodiscard]] std::vector<Action> generateLegalActions(UnitID unitID,
                                                          bool includeWait = true) const override;

   bool undo() override;

   [[nodiscard]] const IGameState& getState() const override { return *state_; }
    ...
    */
   [[nodiscard]] GameStateSnapshot& getStateMutable() { return *state_; }
    ...
    */
   [[nodiscard]] const std::vector<Action>& getActionHistory() const { return actionHistory_; }
  ...
  private:
   ...
   [[nodiscard]] ActionResult executeMove(const MoveAction& action,
                                          const ExecutionContext& context);

   [[nodiscard]] ActionResult executeAttack(const AttackAction& action,
                                            const ExecutionContext& context);

   [[nodiscard]] ActionResult executeWait(const WaitAction& action,
                                          const ExecutionContext& context);
    ...
    */
   [[nodiscard]] int calculateMovementCost(const UnitSnapshot& unit, const MapCoordinate& from,
                                           const MapCoordinate& to) const;

   /**
    * Check if position is reachable by unit
    */
   [[nodiscard]] bool isReachable(const UnitSnapshot& unit, const MapCoordinate& dest) const;
    ...
    */
   [[nodiscard]] int calculateDamage(const UnitSnapshot& attacker, const UnitSnapshot& defender,
    ...
    */
   [[nodiscard]] int canHitTarget(const UnitSnapshot& attacker, const MapCoordinate& target,
                                  int weaponIndex) const;
    ...
    */
   [[nodiscard]] bool simulateReactionFire(UnitSnapshot& movingUnit,
    ...
    */
   [[nodiscard]] static constexpr int hexDistance(const MapCoordinate& a,
                                                  const MapCoordinate& b) noexcept {
    ...
    */
   [[nodiscard]] std::vector<MapCoordinate> getNeighbors(const MapCoordinate& pos) const;
