...
float CombatCalculatorAdapter::calculateExpectedDamage(const UnitSnapshot& attacker,
                                                       const UnitSnapshot& target,
                                                       const MapCoordinate& attackFrom) const {
...
int CombatCalculatorAdapter::hexDistance(const MapCoordinate& a, const MapCoordinate& b) noexcept {
...
float CombatCalculatorAdapter::computeDamageForWeapon(const UnitSnapshot& attacker,
   ...
   return static_cast<float>(CombatCalculator::calculateDamage(
      attacker, target, weaponIndex, distance, terrainDefenseBonus, hemmingFactor));
}
