...
void ReactionFireDetector::initialize(const GameStateSnapshot* state,
...
const std::vector<ReactionFireThreat>& ReactionFireDetector::getThreatsAt(
    const MapCoordinate& coordinate) const {
...
int ReactionFireDetector::hexDistance(const MapCoordinate& a,
...
bool ReactionFireDetector::isStaticUnit(const UnitSnapshot& unit) const {
    ...
    const auto maxMove = std::max_element(
        unit.type->movement.begin(),
        unit.type->movement.end());
...
void ReactionFireDetector::addThreatArea(const UnitSnapshot& unit) {
    ...
    for (int dx = -maxRange; dx <= maxRange; ++dx) {
        for (int dy = -maxRange; dy <= maxRange; ++dy) {
            MapCoordinate candidate(
                static_cast<int16_t>(origin.x + dx),
                static_cast<int16_t>(origin.y + dy));

            if (candidate.x < 0 || candidate.y < 0 ||
                candidate.x >= state_->mapWidth ||
