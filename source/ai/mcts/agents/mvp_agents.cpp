...
LegalMoveAgent::LegalMoveAgent()
    : name_("LegalMove"), weight_(1.0f) {}

AgentScore LegalMoveAgent::evaluate(const Action& action,
    ...
    std::visit(overloaded {
        ...
        [&](const AttackAction& attack) {
            if (!context.actingUnit || attack.attackerID != context.actingUnit->networkID) {
                score.isVeto = true;
...
ReactionFireAgent::ReactionFireAgent()
    : name_("ReactionFire"), weight_(3.0f) {}

AgentScore ReactionFireAgent::evaluate(const Action& action,
                                       const AgentContext& context) const {
    AgentScore score{};
    ...
    std::visit(overloaded {
        [&](const MoveAction& move) {
            const auto& threats = context.rfDetector->getThreatsAt(move.destination);
            if (threats.empty()) {
                score.utility = 0.0f;
                return;
            }
            ...
            for (const auto& threat : threats) {
                totalDamage += computeExpectedDamage(threat, context, move.destination);
            }
...
float ReactionFireAgent::computeExpectedDamage(const ReactionFireThreat& threat,
    ...
    return context.combatCalculator->calculateExpectedDamage(
        *threat.attacker,
        defenderCopy,
...
AggressivenessAgent::AggressivenessAgent()
    : name_("Aggressiveness"), weight_(1.5f) {}

AgentScore AggressivenessAgent::evaluate(const Action& action,
                                         const AgentContext& context) const {
    AgentScore score{};
    if (!context.actingUnit) {
        return score;
...
TargetPriorityAgent::TargetPriorityAgent()
    : name_("TargetPriority"), weight_(1.2f) {}

AgentScore TargetPriorityAgent::evaluate(const Action& action,
                                         const AgentContext& context) const {
    AgentScore score{};
    ...
    std::visit(overloaded {
        [&](const AttackAction& attack) {
            ...
            if (targetUnit->type) {
                valueFactor = std::min(1.0f, static_cast<float>(targetUnit->type->armor) / 200.0f);
            }
...
ServiceUtilityAgent::ServiceUtilityAgent()
    : name_("ServiceUtility"), weight_(2.5f) {}

AgentScore ServiceUtilityAgent::evaluate(const Action& action,
                                         const AgentContext& context) const {
    AgentScore score{};
    if (!context.actingUnit) {
        return score;
