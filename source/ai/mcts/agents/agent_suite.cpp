...
const UnitSnapshot* findUnitForAction(const GameStateSnapshot& state, const Action& action) {
   UnitID id = std::visit(overloaded{[](const MoveAction& move) { return move.unitID; },
...
AgentSuite::AgentSuite()
   : activeProfile_("AgentWeights.Balanced"), aggregator_(AgentAggregationConfig{}) {
...
std::vector<ScoredAction> AgentSuite::scoreActions(const GameStateSnapshot& state, PlayerID player,
                                                   const std::vector<Action>& actions,
                                                   const ICombatCalculator* calculator,
                                                   const AgentAggregationConfig& config,
                                                   int searchDepth, bool isRollout) const {
   ...
   for (const auto& action : actions) {
      ...
      AgentContext agentContext{state,      unit,      player,      nullptr, &rfDetector,
                                calculator, isRollout, searchDepth, config};
   ...
   std::sort(scored.begin(), scored.end(), [](const ScoredAction& lhs, const ScoredAction& rhs) {
...
std::vector<std::filesystem::path> AgentSuite::defaultWeightPaths() {
   return {std::filesystem::path("mcts_agents.ini"),
