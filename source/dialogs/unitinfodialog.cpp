...
void assignWeaponInfo(Panel* panel, PG_Widget* widget, const SingleWeapon& weapon) {
   ...
   panel->setImage(
      "weapon_symbol1",
      IconRepository::getIcon(SingleWeapon::getIconFileName(scalarType) + "-small.png"), widget);
...
const int paneNum = 5;
static const char* paneName[paneNum] = {"information", "movement", "weapons", "transport",
                                        "description"};
...
class UnitInfoDialog : public Panel {
   ...
   bool onEntranceClick(PG_MessageObject* obj, const SDL_MouseButtonEvent* event, int entranceNum) {
      if (vt) {
         ...
         if (swi) {
            ...
            setWidgetTransparency("pad_transport_in",
                                  tio.mode & ContainerBaseType::TransportationIO::In ? 0 : 255);
            setWidgetTransparency("pad_transport_out",
                                  tio.mode & ContainerBaseType::TransportationIO::Out ? 0 : 255);
            setWidgetTransparency("pad_transport_docking",
                                  tio.mode & ContainerBaseType::TransportationIO::Docking ? 0
                                                                                          : 255);

            for (int i = 0; i < cmovemalitypenum; ++i)
               if ((vt->vehicleCategoriesStorable & (1 << i)) &&
                   (tio.vehicleCategoriesLoadable & (1 << i)))
                  setImage(ASCString("unitpad_transport_") + unitCategoryTags[i],
                           IconRepository::getIcon("pad_symbol_ok.png"));
               else
                  setImage(ASCString("unitpad_transport_") + unitCategoryTags[i],
                           IconRepository::getIcon("pad_symbol_no.png"));
   ...
   SPoint getWeaponGraphCoords(int maxdist, int maxstrength, int dist, int strength) {
      ...
      p.y = weaponGraph->Height() - border -
            (weaponGraph->Height() - 2 * border) * strength / maxstrength;
   ...
   void painter(const PG_Rect& src, const ASCString& name, const PG_Rect& dst) {
         ...
         if (vt) {
            if (veh)
               vt->paint(screen, SPoint(dst.x, dst.y),
                         veh->getMap()->getPlayer(veh).getPlayerColor());
            else
               vt->paint(screen, SPoint(dst.x, dst.y));
         }
         ...
         if (vt && weaponGraph) {
            ...
            if (maxdist > 0 && maxstrength > 0)
               for (int i = 0; i < vt->weapons.count; ++i) {
                  ...
                  if (mind == maxd) {
                     SPoint p = getWeaponGraphCoords(maxdist / maxmalq, maxstrength, mind,
                                                     vt->weapons.weapon[i].maxstrength);
                     PG_Draw::DrawLine(PG_Application::GetScreen(), p.x, p.y - 2, p.x, p.y + 2,
                                       linecolor, linewidth);
                     PG_Draw::DrawLine(PG_Application::GetScreen(), p.x - 2, p.y, p.x + 2, p.y,
                                       linecolor, linewidth);
                  } else {
                     SPoint p = getWeaponGraphCoords(maxdist / maxmalq, maxstrength, mind,
                                                     vt->weapons.weapon[i].maxstrength);
                     SPoint p2 = getWeaponGraphCoords(maxdist / maxmalq, maxstrength, maxd,
                                                      vt->weapons.weapon[i].minstrength);
                     PG_Draw::DrawLine(PG_Application::GetScreen(), p.x, p.y, p2.x, p2.y, linecolor,
                                       linewidth);
                  }
               }
         }
      ...
      if (name == "unitpad_transport_leveldisplay") {
         int xoffs = 0;
         for (EntranceHeights::iterator i = entranceHeights.begin(); i != entranceHeights.end();
              ++i) {
            for (int j = 0; j < 2; ++j) {
               Surface& icon = IconRepository::getIcon(
                  ASCString("height-a") + ASCString::toString(j == 0 ? i->second : i->first) +
                  ".png");
               int y;
               if (j == 0)
                  y = 27;
  ...
  public:
   UnitInfoDialog(PG_Widget* parent, const Vehicle* vehicle, const VehicleType* vehicleType)
      : Panel(parent, PG_Rect::null, "UnitInfoDialog", false),
        veh(vehicle),
        vt(vehicleType),
        weaponGraph(NULL),
        currentWeapon(-1) {
      ...
      if (vt) {
         ...
         for (int i = 0; i < ContainerBaseType::functionNum; ++i)
            if (vt->hasFunction(ContainerBaseType::ContainerFunctions(i)))
               abilities +=
                  ContainerBaseType::getFunctionName(ContainerBaseType::ContainerFunctions(i)) +
                  ASCString("\n");
         if (vt->wait)
   ...
   void userHandler(const ASCString& label, PropertyReadingContainer& pc, PG_Widget* parent,
         ...
         for (int i = 0; i < vt->heightChangeMethodNum; ++i) {
            ...
            for (int j = 0; j < 8; ++j)
               if (vt->height & vt->heightChangeMethod[i].startHeight & (1 << j)) {
                  ...
                  new PG_Image(sw, PG_Point(xoffs, yoffs),
                               IconRepository::getIcon(filename).getBaseSurface(), false);
                  ++counter;
               }
            ...
            setLabelText("unitpad_move_changeheight_movepoints", vt->heightChangeMethod[i].moveCost,
                         sw);
      ...
      if (label == "unitpad_terrainaccess" && vt) {
         ...
         for (int i = 0; i < terrainPropertyNum; ++i) {
            if (vt->terrainaccess.terrain.test(i) || vt->terrainaccess.terrainkill.test(i) ||
                vt->terrainaccess.terrainnot.test(i) || vt->terrainaccess.terrainreq.test(i)) {
               pc.openBracket("LineWidget");
               widgetParams.runTextIO(pc);
               PG_Rect r = parseRect(pc, parent);
               r.y += yoffset;
               ...
               if (w) {
                  int xoffs = 0;
                  static const char* iconName[] = {"pad_symbol_ok.png", "pad_symbol_warn.png",
                                                   "pad_symbol_no.png", "pad_symbol_kill.png"};
                  const TerrainBits* bits[] = {
                     &vt->terrainaccess.terrain, &vt->terrainaccess.terrainreq,
                     &vt->terrainaccess.terrainnot, &vt->terrainaccess.terrainkill};
                  for (int icon = 0; icon < 4; ++icon) {
                     bool set = bits[icon]->test(i);

                     if (set) {
                        PG_Image* img = new PG_Image(
                           w, PG_Point(xoffs, 0),
                           IconRepository::getIcon(iconName[icon]).getBaseSurface(), false);
                        xoffs += img->Width();
                     }
                  }
               }
      ...
      if (label == "unitpad_weaponlist" && vt) {
         ...
         for (int i = 0; i < vt->weapons.count; ++i) {
            ...
            sw->sigMouseButtonDown.connect(
               sigc::bind(sigc::mem_fun(*this, &UnitInfoDialog::onWeaponClick), i));
         }
      }
      if (label == "entrancesystems" && vt) {
         ...
         for (int i = 0; i < vt->entranceSystems.size(); ++i) {
            ...
            // assignWeaponInfo( this, sw, vt->weapons.weapon[i] );
            sw->sigMouseButtonDown.connect(
               sigc::bind(sigc::mem_fun(*this, &UnitInfoDialog::onEntranceClick), i));
         }
      }
   };
};
