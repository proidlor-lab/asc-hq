...
class AdminGameWindow : public ASC_PG_Dialog {
  private:
   ...
   void resetView(int player) { gamemap->getPlayer(player).resetView(); }
   void resetResearch(int player) { gamemap->getPlayer(player).resetResearch(); }

   void resetTribute(int player) { gamemap->getPlayer(player).resetTribute(); }

   void resetPassword(int player) { gamemap->getPlayer(player).resetPassword(); }
   ...
   void newEmail(int player) {
      gamemap->player[player].email = editString2(
         "Mail Address for " + gamemap->player[player].getName(), gamemap->player[player].email);
   }
   ...
   void deleteUnits(int player) {
      while (gamemap->player[player].vehicleList.begin() !=
             gamemap->player[player].vehicleList.end())
   ...
   void deleteBuildings(int player) {
      while (gamemap->player[player].buildingList.begin() !=
             gamemap->player[player].buildingList.end())
   ...
   void deleteProduction(int player) {
      for (Player::BuildingList::iterator i = gamemap->player[player].buildingList.begin();
           i != gamemap->player[player].buildingList.end(); ++i)
   ...
   bool saveAsMap() {
      ...
      if (playerSetup->Apply()) {
         ...
         if (gamemap->_resourcemode == 1)
            warningMessage(
               "This map uses BI resource mode. Saving as map will delete all global resources of "
               "all players");

         ASCString name = selectFile(mapextension, false);
         if (!name.empty()) {
            StatusMessageWindowHolder smw =
               MessagingHub::Instance().infoMessageWindow("saving " + name);
  ...
  public:
   AdminGameWindow(GameMap* actmap, PG_Widget* parent, TurnSkipper* turnSkipper)
      : ASC_PG_Dialog(parent, PG_Rect(-1, -1, 600, min(1000, PG_Application::GetScreenHeight())),
                      "Admin Game"),
        gamemap(actmap),
        successfullyClosed(false) {
      this->turnSkipper = turnSkipper;

      scrollwidget = new PG_ScrollWidget(
         this, PG_Rect(1, GetTitlebarHeight(), Width() - 2, Height() - GetTitlebarHeight() - 2));
      ...
      int ypos = gap;
      playerlistbox =
         (new MultiListBox(scrollwidget, PG_Rect(gap, ypos, selectorWidth, selectorHeight)))
            ->getListBox();
      ...
      actionlistbox = (new MultiListBox(scrollwidget, PG_Rect(2 * gap + selectorWidth, gap,
                                                              selectorWidth, selectorHeight)))
                         ->getListBox();
      new ActionItem(actionlistbox, 20, "reset view",
                     PlayerActionFunctor(this, &AdminGameWindow::resetView));
      new ActionItem(actionlistbox, 20, "reset research",
                     PlayerActionFunctor(this, &AdminGameWindow::resetResearch));
      new ActionItem(actionlistbox, 20, "reset tribute",
                     PlayerActionFunctor(this, &AdminGameWindow::resetTribute));
      new ActionItem(actionlistbox, 20, "reset diplomacy (->war)",
                     PlayerActionFunctor(this, &AdminGameWindow::resetDiplomacy));
      new ActionItem(actionlistbox, 20, "reset password",
                     PlayerActionFunctor(this, &AdminGameWindow::resetPassword));
      new ActionItem(actionlistbox, 20, "delete units",
                     PlayerActionFunctor(this, &AdminGameWindow::deleteUnits));
      new ActionItem(actionlistbox, 20, "delete buildings",
                     PlayerActionFunctor(this, &AdminGameWindow::deleteBuildings));
      new ActionItem(actionlistbox, 20, "delete production",
                     PlayerActionFunctor(this, &AdminGameWindow::deleteProduction));
      new ActionItem(actionlistbox, 20, "delete resources + ammo",
                     PlayerActionFunctor(this, &AdminGameWindow::deleteResources));
      new ActionItem(actionlistbox, 20, "delete mines",
                     PlayerActionFunctor(this, &AdminGameWindow::deleteMines));
      new ActionItem(actionlistbox, 20, "make buildings neutral",
                     PlayerActionFunctor(this, &AdminGameWindow::neutralBuildings));
      new ActionItem(actionlistbox, 20, "enter new password",
                     PlayerActionFunctor(this, &AdminGameWindow::newPassword));
      new ActionItem(actionlistbox, 20, "edit email",
                     PlayerActionFunctor(this, &AdminGameWindow::newEmail));
      new ActionItem(actionlistbox, 20, "export technology",
                     PlayerActionFunctor(this, &AdminGameWindow::exportTechs));
      new ActionItem(actionlistbox, 20, "import technology",
                     PlayerActionFunctor(this, &AdminGameWindow::importTechs));

      (new PG_Button(scrollwidget, PG_Rect(3 * gap + 2 * selectorWidth, ypos, 50, selectorHeight),
                     "Apply"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &AdminGameWindow::apply)));
      ...
      if (actmap->network) {
         ypos += 30;
         (new PG_Button(scrollwidget, PG_Rect(20, ypos, 200, 20), "Setup data transfer"))
            ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &AdminGameWindow::netSetup)));
      }

      ypos += 30;
      playerSetup = new PlayerSetupWidget(
         gamemap, PlayerSetupWidget::AllEditable, scrollwidget,
         PG_Rect(gap, ypos, Width() - 3 * gap, PlayerSetupWidget::guessHeight(gamemap)));

      ypos += PlayerSetupWidget::guessHeight(gamemap) + gap;

      (new PG_Button(scrollwidget, PG_Rect(gap, ypos, Width() - 3 * gap, 30), "OK"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &AdminGameWindow::ok)));
      ypos += 40;

      if (actmap->getgameparameter(cgp_superVisorCanSaveMap)) {
         (new PG_Button(scrollwidget, PG_Rect(gap, ypos, Width() - 3 * gap, 30), "Save as Map"))
            ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &AdminGameWindow::saveAsMap)));
         ypos += 40;
      }
   ...
   bool GetStatus() { return successfullyClosed; }
};
