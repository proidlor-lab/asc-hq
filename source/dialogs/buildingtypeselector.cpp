...
BuildingTypeBaseWidget ::BuildingTypeBaseWidget(PG_Widget* parent, const PG_Point& pos, int width,
                                                const BuildingType* buildingType,
                                                const Player& player)
   : SelectionWidget(parent, PG_Rect(pos.x, pos.y, width, getBuildingHeight(buildingType) + 10)),
     vt(buildingType),
     actplayer(player) {
...
void BuildingTypeBaseWidget::display(SDL_Surface* surface, const PG_Rect& src, const PG_Rect& dst) {
   ...
   for (int y = 0; y < 6; ++y)
      for (int x = 0; x < 4; ++x)
         if (vt->fieldExists(BuildingType::LocalCoordinate(x, y)))
            vt->paintSingleField(getClippingSurface(), SPoint(5 - xoffs, 5 - yoffs),
                                 BuildingType::LocalCoordinate(x, y), actplayer.getPlayerColor());
...
BuildingTypeResourceWidget::BuildingTypeResourceWidget(PG_Widget* parent, const PG_Point& pos,
                                                       int width, const BuildingType* BuildingType,
...
BuildingTypeCountWidget::BuildingTypeCountWidget(PG_Widget* parent, const PG_Point& pos, int width,
                                                 const BuildingType* BuildingType,
...
BuildingTypeSelectionItemFactory ::BuildingTypeSelectionItemFactory(Resources plantResources,
                                                                    ...
                                                                    const Player& player)
   : actplayer(player), original_items(types) {
   restart();
...
SelectionWidget* BuildingTypeSelectionItemFactory::spawnNextItem(PG_Widget* parent,
   ...
   if (it != items.end()) {
      ...
      return new BuildingTypeResourceWidget(parent, pos, parent->Width() - 15, v, lackingResources,
                                            cost, actplayer);
   } else
      return NULL;
};
