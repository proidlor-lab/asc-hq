...
void GameParameterEditorWidget ::runTextIO(PropertyContainer& pc) {
   for (int i = 0; i < gameparameternum; ++i) {
      if (!gameParameterSettings[i].legacy) {
         ...
         if (pc.isReading()) {
            if (!(values[i] >= gameParameterSettings[i].minValue &&
                  values[i] <= gameParameterSettings[i].maxValue))
...
GameParameterEditorWidget ::GameParameterEditorWidget(GameMap* gamemap, PG_Widget* parent,
                                                      const PG_Rect& rect)
   : PG_Widget(parent, rect), actmap(gamemap) {
   SetTransparency(255);

   propertyEditor = new PG_PropertyEditor(this, PG_Rect(0, 0, rect.Width() - 110, rect.Height()),
                                          "PropertyEditor", 70);

   PG_Button* load = new PG_Button(this, PG_Rect(rect.Width() - 100, 0, 100, 30), "Load");
   load->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &GameParameterEditorWidget::LoadParameter)));

   PG_Button* save = new PG_Button(this, PG_Rect(rect.Width() - 100, 40, 100, 30), "Save");
   save->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &GameParameterEditorWidget::SaveParameter)));

   PG_Button* def = new PG_Button(this, PG_Rect(rect.Width() - 100, 80, 100, 30), "Default");
   def->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &GameParameterEditorWidget::ResetParameter)));
   ...
   for (int i = 0; i < gameparameternum; ++i) {
      ...
      if (!gameParameterSettings[i].legacy) {
         if (gameParameterSettings[i].minValue == 0 && gameParameterSettings[i].maxValue == 1) {
            new PG_PropertyField_Checkbox<int>(propertyEditor, gameParameterSettings[i].longName,
                                               &values[i]);
         } else {
            PG_PropertyField_Integer<int>* ip = new PG_PropertyField_Integer<int>(
               propertyEditor, gameParameterSettings[i].longName, &values[i]);
...
class EditMapParameters : public ASC_PG_Dialog {
  ...
  public:
   EditMapParameters(GameMap* actmap, PG_Widget* parent)
      : ASC_PG_Dialog(parent, PG_Rect(-1, -1, 750, 500), "Edit Map Parameters") {
      gpew = new GameParameterEditorWidget(
         actmap, this,
         PG_Rect(10, GetTitlebarHeight(), Width() - 20, Height() - GetTitlebarHeight() - 40));
