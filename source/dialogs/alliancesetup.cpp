...
class ListBoxImageItem : public PG_ListBoxBaseItem {
  ...
  public:
   ListBoxImageItem(PG_ListBox* parent, PG_Point pos, SelectionType s)
      : PG_ListBoxBaseItem(parent, diplomaticStateIconSize + 2 * diplomaticStateIconSpace),
        state(s) {
      new PG_Image(this, PG_Point(diplomaticStateIconSpace, diplomaticStateIconSpace),
                   IconRepository::getIcon(getDiplomaticStateImage(s)).getBaseSurface(), false);
      new PG_Label(this,
                   PG_Rect(diplomaticStateIconSize + 5, diplomaticStateIconSpace, 200,
                           diplomaticStateIconSize),
                   getStateName<SelectionType>(s));
   }
...
class DiplomaticModeChooser : public PG_Widget {
  ...
  protected:
   void selectMode() {
      PG_ListBox listBox(NULL, PG_Rect(x, y + Height(), 250,
                                       getItemNum<SelectionType>() * (diplomaticStateIconSpace * 2 +
                                                                      diplomaticStateIconSize) +
                                          4));
      for (int i = 0; i < getItemNum<SelectionType>(); ++i) {
         ListBoxImageItem<SelectionType>* item =
            new ListBoxImageItem<SelectionType>(NULL, PG_Point(0, 0), SelectionType(i));
  ...
  public:
   DiplomaticModeChooser(PG_Widget* parent, const PG_Rect& pos, SelectionType& dm, bool writeable)
      : PG_Widget(parent, pos, true), mode(dm), writePossible(writeable){};
...
AllianceSetupWidget::AllianceSetupWidget(GameMap* gamemap, ApplyStrategy* applyStrategy,
                                         bool allEditable, PG_Widget* parent, const PG_Rect& r,
...

{
   ...
   for (int i = 0; i < actmap->getPlayerCount(); ++i) {
      ...
      for (int j = 0; j < actmap->getPlayerCount(); ++j) {
         DiplomaticStateVector::QueuedStateChanges::const_iterator change =
            diplo.queuedStateChanges.find(j);
...
#define calcy(counter) (barOverhang + counter * (lineHeight + spacing))
   ...
   for (int i = 0; i < actmap->getPlayerCount(); ++i)
      if (actmap->player[i].exist()) {
         ...
         ColoredBar* verticalBar =
            new ColoredBar(actmap->player[i].getColor(), this, PG_Rect(x, 0, colWidth, colHeight));
   ...
   for (int i = 0; i < actmap->getPlayerCount(); ++i)  // rows
      if (actmap->player[i].exist()) {
         ...
         ColoredBar* horizontalBar = new ColoredBar(actmap->player[i].getColor(), this,
                                                    PG_Rect(0, y, lineLength, lineHeight));
         horizontalBar->SetTransparency(128);

         pw.name = new PG_LineEdit(
            horizontalBar, PG_Rect(sqaureWidth, barSpace, nameLength, lineHeight - 2 * barSpace));
         pw.name->SetText(actmap->player[i].getName());
         pw.name->SetEditable(false);

         new PG_ToolTipHelp(pw.name, "Position: " + ASCString::toString(i));

         PG_ThemeWidget* col = new PG_ThemeWidget(
            horizontalBar,
            PG_Rect(barSpace, barSpace, sqaureWidth - 2 * barSpace, lineHeight - 2 * barSpace));
         ...
         for (int j = 0; j < actmap->getPlayerCount(); ++j)  // columns
            if (actmap->player[j].exist()) {
               if (i != j) {
                  ...
                  PG_Rect rect(x + (colWidth - diplomaticStateIconSize) / 2,
                               (lineHeight - diplomaticStateIconSize) / 2, diplomaticStateIconSize,
                               diplomaticStateIconSize);
                  if (allEditable) {
                     DiplomaticModeChooser<DiplomaticStates>* dmc =
                        new DiplomaticModeChooser<DiplomaticStates>(horizontalBar, rect,
                                                                    getState(i, j), true);
                     dmc->sigStateChange.connect(
                        sigc::bind(sigc::mem_fun(*this, &AllianceSetupWidget::setState), j, i));
                     diplomaticWidgets[linearize(i, j)] = dmc;
                  } else {
                     DiplomaticTransitions& s = stateChanges[i][j];
                     DiplomaticModeChooser<DiplomaticTransitions>* dmc =
                        new DiplomaticModeChooser<DiplomaticTransitions>(horizontalBar, rect, s,
                                                                         i == gamemap->actplayer);
                     // dmc->sigStateChange.connect( sigc::bind( sigc::mem_fun( *this,
                     // &AllianceSetupWidget::setState ), j, i));
                     diplomaticWidgets[linearize(i, j)] = dmc;
                  }
               }
               ++cnt2;
         ...
         if (allEditable) {
            ...
            if (acting == actmap->actplayer) {
               if (stateChanges[acting][second] == SNEAK_ATTACK) {
                  ...
                  DiplomaticStates t;
                  DiplomaticStateVector::QueuedStateChanges::iterator q =
                     actmap->player[acting].diplomacy.queuedStateChanges.find(second);
...
class AllianceSetupWindow : public ASC_PG_Dialog {
  ...
  public:
   AllianceSetupWindow(GameMap* actmap, AllianceSetupWidget::ApplyStrategy* strategy,
                       bool allEditable, PG_Widget* parent, const PG_Rect& r)
      : ASC_PG_Dialog(parent, r, "Diplomacy"), changed(false) {
      asw = new AllianceSetupWidget(actmap, strategy, allEditable, this,
...
bool setupalliances(GameMap* actmap, AllianceSetupWidget::ApplyStrategy* strategy,
                    bool supervisor) {
...
void DirectAllianceSetupStrategy::setState(GameMap* map, int actingPlayer, int towardsPlayer,
