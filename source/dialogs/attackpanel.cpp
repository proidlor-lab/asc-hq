...
class GetAttackerImage : public FightVisitor {
  private:
   ...
   void paint(UnitAttacksSomething& battle) {
      battle.getAttackingUnit()->typ->paint(
         surf, SPoint(dst.x, dst.y), battle.getAttackingUnit()->getOwningPlayer().getPlayerColor());
   }

  public:
   GetAttackerImage(tfight& battle, Surface& surface, const SDL_Rect& dest)
      : dst(dest), surf(surface) {
      battle.visit(*this);
   };
...
class GetTargetImage : public FightVisitor {
  ...
  public:
   ...
   void visit(tmineattacksunit& battle) {
      battle.getTarget()->typ->paint(surf, SPoint(dst.x, dst.y),
                                     battle.getTarget()->getOwningPlayer().getPlayerColor());
   }

   void visit(tunitattacksbuilding& battle) {
      battle.getTarget()->paintSingleField(surf, SPoint(dst.x, dst.y),
                                           battle.getTarget()->typ->entry);
   };
...
class AttackPanel : public Panel {
  ...
  public:
   ...
   void setBarGraphValue(const ASCString& widgetName, float fraction) {
      Panel::setBargraphValue(widgetName, fraction);
   };
   ...
   void setLabelText(const ASCString& widgetName, const ASCString& i) {
      Panel::setLabelText(widgetName, i);
   };
   ...
   void setBarGraphColor(const ASCString& widgetName, PG_Color color) {
      Panel::setBarGraphColor(widgetName, color);
   };
...
AttackPanel::AttackPanel(tfight& engine_)
   : Panel(PG_Application::GetWidgetById(ASC_PG_App::mainScreenID), PG_Rect(0, 0, 170, 200),
           "Attack"),
     engine(engine_) {
...
void AttackPanel::painter(const PG_Rect& src, const ASCString& name, const PG_Rect& dst) {
   ...
   if (name == "attacker_unitexp") {
      s.Blit(IconRepository::getIcon(
                "experience" +
                ASCString::toString(engine.getIconIndex(attacker_exp_offensive, true)) + ".png"),
             SPoint(dst.x, dst.y));
      return;
   }
   if (name == "defender_unitexp") {
      s.Blit(IconRepository::getIcon(
                "experience" +
                ASCString::toString(engine.getIconIndex(defender_exp_defensive, false)) + ".png"),
             SPoint(dst.x, dst.y));
      return;
   }
   if (name == "attacker_level" && engine.av.height) {
      s.Blit(IconRepository::getIcon("height-a" +
                                     ASCString::toString(getFirstBit(engine.av.height)) + ".png"),
             SPoint(dst.x, dst.y));
      return;
   }
   if (name == "defender_level" && engine.dv.height) {
      s.Blit(IconRepository::getIcon("height-a" +
                                     ASCString::toString(getFirstBit(engine.dv.height)) + ".png"),
             SPoint(dst.x, dst.y));
      return;
   }

   if (name == "attacker_weaponsymbol" && engine.av.weapontype >= 0) {
      s.Blit(IconRepository::getIcon(SingleWeapon::getIconFileName(engine.av.weapontype) +
                                     "-small.png"),
             SPoint(dst.x, dst.y));
      return;
   }
   if (name == "defender_weaponsymbol" && engine.dv.weapontype >= 0) {
      s.Blit(IconRepository::getIcon(SingleWeapon::getIconFileName(engine.dv.weapontype) +
                                     "-small.png"),
             SPoint(dst.x, dst.y));
      return;
   }
...
class BattleSoundPlayer : public FightVisitor {
  private:
   ...
   void play(UnitAttacksSomething& battle) {
      SoundList::getInstance().playSound(
         SoundList::shooting,
         battle.getAttackingUnit()->getWeapon(battle.av.weapnum)->getScalarWeaponType(), false,
         battle.getAttackingUnit()->getWeapon(battle.av.weapnum)->soundLabel);
   };

  public:
   BattleSoundPlayer(tfight& battle) : first(true), the_battle(battle) { battle.visit(*this); };
   ...
   void visit(tunitattacksunit& battle) {
      if (first)
         ...
         if (battle.av.damage >= 100)
            SoundList::getInstance().playSound(SoundList::unitExplodes, 0, false,
                                               battle.getAttackingUnit()->typ->killSoundLabel);

         if (battle.dv.damage >= 100)
            SoundList::getInstance().playSound(SoundList::unitExplodes, 0, false,
                                               battle.getTarget()->typ->killSoundLabel);
      }
   };
...
void showAttackAnimation(tfight& battle, GameMap* actmap, int ad, int dd) {
   ...
   PG_Color attackingColor =
      lighten_Color(actmap->player[battle.getAttackingPlayer()].getColor(), 22);
   PG_Color defendingColor =
      lighten_Color(actmap->player[battle.getDefendingPlayer()].getColor(), 22);
   ...
   at->dispValue("attacker_attackbonus", battle.strength_attackbonus(battle.av.attackbonus),
                 maxattackshown, attackingColor);
   at->dispValue("defender_attackbonus", battle.strength_attackbonus(battle.dv.attackbonus),
                 maxattackshown, defendingColor);

   at->dispValue("attacker_defencebonus", battle.defense_defensebonus(battle.av.defensebonus),
                 maxattackshown, attackingColor);
   at->dispValue("defender_defencebonus", battle.defense_defensebonus(battle.dv.defensebonus),
                 maxattackshown, defendingColor);
