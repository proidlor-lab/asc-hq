...
class InternalAmmoTransferWidget : public PG_Widget {
  ...
  public:
   InternalAmmoTransferWidget(PG_Widget* parent, const PG_Rect& pos,
                              InternalAmmoTransferHandler* handler, int weaponID);
...
InternalAmmoTransferWidget::InternalAmmoTransferWidget(PG_Widget* parent, const PG_Rect& pos,
                                                       InternalAmmoTransferHandler* handler,
                                                       int weaponID)
   : PG_Widget(parent, pos), slider(NULL), _weaponID(weaponID) {
   _handler = handler;
...
InternalAmmoTransferWindow ::InternalAmmoTransferWindow(Vehicle* source, PG_Widget* parent)
   : ASC_PG_Dialog(NULL, PG_Rect(30, 30, 400, 400), "Transfer"),
     vehicle(source),
     handler(source)  //, second( destination ), handler( source, destination )
{
   int ypos = 30;
   int border = 10;
