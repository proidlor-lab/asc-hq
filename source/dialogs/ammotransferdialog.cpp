...
class TransferWidget : public PG_Widget {
  private:
   ...
   bool updateRange() {
      ...
      int max;
      if (trans->getMax(trans->getDstContainer(), true) * 3 <
          trans->getMax(trans->getDstContainer(), false))
  ...
  public:
   TransferWidget(PG_Widget* parent, const PG_Rect& pos, Transferrable* transferrable,
                  TransferHandler& handler)
      : PG_Widget(parent, pos), trans(transferrable), slider(NULL) {
...
AmmoTransferWindow ::AmmoTransferWindow(ContainerBase* source, ContainerBase* destination,
                                        PG_Widget* parent, ServiceCommand* command)
   ...
   (new PG_ThemeWidget(area, PG_Rect(5, 3, fieldsizex, fieldsizey)))
      ->SetBackground(img1.getBaseSurface(), PG_Draw::STRETCH);
   (new PG_ThemeWidget(area, PG_Rect(area->Width() - 5 - fieldsizex, 3, fieldsizex, fieldsizey)))
      ->SetBackground(img2.getBaseSurface(), PG_Draw::STRETCH);

   ypos = fieldsizex + 5;
   if (handler->ammoProductionPossible()) {
      PG_CheckButton* production =
         new PG_CheckButton(area, PG_Rect(border, ypos, area->w - 30, 20), "allow ammo production");
   ...
   for (TransferHandler::Transfers::iterator i = handler->getTransfers().begin();
        i != handler->getTransfers().end(); ++i) {
   ...
   PG_Button* b = new PG_Button(
      this, PG_Rect(w - buttonWidth - border, h - 30 - border, buttonWidth, 30), "OK");
