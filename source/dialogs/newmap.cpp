...
class NewMap : public ASC_PG_Dialog {
  ...
  public:
   NewMap(GameMap* map)
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 500, 400), "Map Properties"),
        terrainid(2998),
        success(false) {
      if (map) {
         create = false;
      ...
      properties =
         new ASC_PropertyEditor(this, PG_Rect(20, ypos, Width() - 20, Height() - ypos - 60));
      ...
      new PG_PropertyField_Checkbox<bool>(properties, "Direct Access",
                                          &gamemap->campaign.directaccess);
      (new PG_PropertyField_String<ASCString>(properties, "Password",
                                              &gamemap->codeWord));  //->SetPassHidden('*');
      ...
      (new PG_PropertyField_Integer<int>(properties, "Wind Speed", &gamemap->weather.windSpeed))
         ->SetRange(0, 255);

      static const char* directionNames[7] = {"South",     "SouthWest", "NorthWest", "North",
                                              "NorthEast", "SouthEast", NULL};

      new PG_PropertyField_IntDropDown<int>(properties, "Wind Direction",
                                            &gamemap->weather.windDirection, directionNames);
      new PG_PropertyField_String<ASCString>(properties, "Native Event Language",
                                             &gamemap->nativeMessageLanguage);
      ...
      AddStandardButton("Cancel")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &NewMap::cancel)));
