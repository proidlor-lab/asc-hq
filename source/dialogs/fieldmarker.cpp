...
class CoordinateItem : public PG_ListBoxItem {
  ...
  public:
   CoordinateItem(PG_Widget* parent, const MapCoordinate& pos)
      : PG_ListBoxItem(parent, 20, toString(pos)), p(pos){};
   MapCoordinate getPos() const { return p; };
};

SelectFromMap::SelectFromMap(CoordinateList& list, GameMap* map, bool justOne, bool readOnly)
   : ASC_PG_Dialog(NULL,
                   PG_Rect(PG_Application::GetScreenWidth() - 150,
                           PG_Application::GetScreenHeight() - 300, 150, 300),
                   "Select Fields"),
     listbox(NULL),
     actmap(map),
     coordinateList(list) {
...
bool SelectFromMap::markField2(const MapCoordinate& pos, const SPoint& mouse, bool cursorChanged,
                               int button, int prio) {
   if (prio <= 2)
      markField(pos);
   return true;
