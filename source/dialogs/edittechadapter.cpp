...
class TechAdapterWidget : public SelectionWidget {
  ...
  public:
   TechAdapterWidget(PG_Widget* parent, const PG_Point& pos, int width,
                     const ASCString& adapterName)
      : SelectionWidget(parent, PG_Rect(pos.x, pos.y, width, 10)), name(adapterName) {
...
void TechAdapterSelectionItemFactory::add(set<ASCString>& list,
                                          const TechAdapterDependency& adapter) {
   const TechAdapterDependency::RequiredTechAdapter& deps = adapter.listAdapters();
   for (TechAdapterDependency::RequiredTechAdapter::const_iterator i = deps.begin();
        i != deps.end(); ++i) {
      list.insert(*i);
   }
}
...
class EditTechAdapter : public ASC_PG_Dialog {
   ...
   bool containsAdapter(const ASCString& adapter, const TechAdapterDependency& dependency) {
      return find(dependency.listAdapters().begin(), dependency.listAdapters().end(), adapter) !=
             dependency.listAdapters().end();
   }
   ...
   void techAdapterSelected(const ASCString& adapter) {
      ...
      for (int i = 0; i < vehicleTypeRepository.getNum(); ++i)
         if (containsAdapter(adapter, vehicleTypeRepository.getObject_byPos(i)->techDependency))
            local +=
               "#vehicletype=" + ASCString::toString(vehicleTypeRepository.getObject_byPos(i)->id) +
               "# " + vehicleTypeRepository.getObject_byPos(i)->getName() + " \n";
  ...
  protected:
   bool handleButtonClick(PG_Button* button) { return false; };
  ...
  public:
   EditTechAdapter(Player& my_player)
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 770, 600), "Edit Tech Adapter"),
        allAdapterfactory(NULL),
        playerFactory(NULL),
        player(my_player) {
      playerAdapter.insert(player.research.getPredefinedTechAdapter().begin(),
                           player.research.getPredefinedTechAdapter().end());
      ...
      allAdapterfactory->techSelected.connect(
         sigc::mem_fun(*this, &EditTechAdapter::techAdapterSelected));
      allItemSelector = new ItemSelectorWidget(
         eleft, PG_Rect(10, 10, eleft->Width() - 20, eleft->Height() - 20), allAdapterfactory);

      playerFactory = new TechAdapterSelectionItemFactory(playerAdapter);
      playerFactory->techSelected.connect(
         sigc::mem_fun(*this, &EditTechAdapter::techAdapterSelected));
      playerItemSelector = new ItemSelectorWidget(
         eright, PG_Rect(10, 10, eright->Width() - 20, eright->Height() - 20), playerFactory);

      (new PG_Button(
          this,
          PG_Rect(left.x + left.Width() + 10, left.y, right.x - left.x - left.Width() - 20, 30),
          "Add >>"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &EditTechAdapter::add)));
      (new PG_Button(this,
                     PG_Rect(left.x + left.Width() + 10, left.y + 40,
                             right.x - left.x - left.Width() - 20, 30),
                     "Remove <<"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &EditTechAdapter::remove)));
      (new PG_Button(this,
                     PG_Rect(left.x + left.Width() + 10, left.y + 80,
                             right.x - left.x - left.Width() - 20, 30),
                     "Add Range"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &EditTechAdapter::addRange)));
      ...
      AddStandardButton("~O~K")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &EditTechAdapter::ok)));
      AddStandardButton("~C~ancel")
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &EditTechAdapter::cancel)));
   };
};
