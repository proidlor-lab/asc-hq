...
VehicleTypeBaseWidget ::VehicleTypeBaseWidget(PG_Widget* parent, const PG_Point& pos, int width,
                                              const VehicleType* vehicletype, const Player& player)
   : SelectionWidget(parent, PG_Rect(pos.x, pos.y, width, fieldsizey + 10)),
     vt(vehicletype),
     actplayer(player) {
   int col1 = 50;
   int lineheight = 20;
   ...
   PG_Label* lbl2 =
      new PG_Label(this, PG_Rect(col1, lineheight, 3 * sw, lineheight), vt->description);
   lbl2->SetFontSize(lbl2->GetFontSize() - 2);

   PG_Button* b = new PG_Button(this, PG_Rect(buttonXPos(width, 0), Height() / 2 - lineheight,
                                              2 * lineheight, 2 * lineheight));
...
VehicleBaseWidget ::VehicleBaseWidget(PG_Widget* parent, const PG_Point& pos, int width,
                                      const Vehicle* vehicle, const Player& player)
   : SelectionWidget(parent, PG_Rect(pos.x, pos.y, width, fieldsizey + 10)),
     v(vehicle),
     actplayer(player) {
   int col1 = 50;
   ...
   PG_Label* lbl2 =
      new PG_Label(this, PG_Rect(col1, lineheight, 3 * sw, lineheight), v->typ->description);
   lbl2->SetFontSize(lbl2->GetFontSize() - 3);

   if (v->getName() != v->typ->name) {
      PG_Label* lbl3 = new PG_Label(this, PG_Rect(col1, lineheight * 2, 3 * sw, lineheight),
                                    '>' + v->getName() + '<');
...
VehicleTypeResourceWidget::VehicleTypeResourceWidget(PG_Widget* parent, const PG_Point& pos,
                                                     int width, const VehicleType* vehicletype,
                                                     int lackingResources, const Resources& cost,
                                                     const Player& player)
   ...
   for (int i = 0; i < 3; ++i) {
      new PG_Image(this, PG_Point(col1 + 2 + 5 * sw, i * 12 + 5),
                   IconRepository::getIcon(filenames[i]).getBaseSurface(), false);
      PG_Label* lbl = new PG_Label(this, PG_Rect(col1 + 3 * sw, i * 12, sw * 2, lineheight),
                                   ASCString::toString(cost.resource(i)));
...
VehicleTypeCountWidget::VehicleTypeCountWidget(PG_Widget* parent, const PG_Point& pos, int width,
                                               const VehicleType* vehicletype, const Player& player,
                                               int number)
   : VehicleTypeBaseWidget(parent, pos, width, vehicletype, player) {
   int col1 = 50;
   ...
   PG_Label* lbl = new PG_Label(this, PG_Rect(col1 + 4 * sw, 0, sw * 2, lineheight * 2),
                                ASCString::toString(number));
   lbl->SetAlignment(PG_Label::RIGHT);
   lbl->SetFontSize(lbl->GetFontSize() + 5);
}

VehicleTypeCountLocateWidget::VehicleTypeCountLocateWidget(PG_Widget* parent, const PG_Point& pos,
                                                           int width,
                                                           const VehicleType* vehicletype,
                                                           const Player& player, int number)
   : VehicleTypeCountWidget(parent, pos, width, vehicletype, player, number) {
   int lineheight = 20;

   PG_Button* b = new PG_Button(this, PG_Rect(buttonXPos(width, 1), Height() / 2 - lineheight,
                                              2 * lineheight, 2 * lineheight));
...
SelectionWidget* VehicleTypeSelectionItemFactory::spawnNextItem(PG_Widget* parent,
   ...
   if (it != items.end()) {
      ...
      if (showResourcesForUnit) {
         ...
         return new VehicleTypeResourceWidget(parent, pos, parent->Width() - 15, v,
                                              lackingResources, cost, actplayer);
      } else
