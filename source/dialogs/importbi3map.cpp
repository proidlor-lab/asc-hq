...
class ImportBI3MapDialog : public ASC_PG_Dialog {
   ...
   bool ok() {
      ...
      if (!filename.empty()) {
         ...
         try {
            if (insert == false) {
               importbattleislemap(bi3Path, filename,
                                   terrainTypeRepository.getObject_byID(9999)->weather[0],
                                   getImportTable(), &errorOutput);
               if (!errorOutput.empty()) {
                  ViewFormattedText vft("Results of Import", errorOutput,
                                        PG_Rect(-1, -1, 400, 400));
                  vft.Show();
                  vft.RunModal();
               }
  ...
  public:
   ...
   ASCString getImportTable() {
      if (importTable->GetSelectedIndex() >= 0 &&
          importTable->GetSelectedIndex() < importTables.size())
...
ImportBI3MapDialog::ImportBI3MapDialog(GameMap* gamemap, bool insert)
   : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 450, 370), "Import BI Map"), actmap(gamemap) {
   this->insert = insert;
   AddStandardButton("OK")->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &ImportBI3MapDialog::ok)));
   AddStandardButton("Cancel")->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &ImportBI3MapDialog::cancel)));
   ...
   new PG_Label(this, PG_Rect(20, ypos, Width() - 40, 20),
                "BI3 item translation table (note that data is only read at startup of ASC):");
   ypos += 25;
