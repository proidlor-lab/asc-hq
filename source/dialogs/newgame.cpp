...
class StartMultiplayerGame : public ConfigurableWindow {
  private:
   ...
   void fileNameMarked(const ASCString& filename) { this->filename = filename; };
...
const char* StartMultiplayerGame::buttonLabels[8] = {
   "NewCampaign", "ContinueCampaign", "SinglePlayer", "Hotseat", "PBEM", "PBP", "PBEMServer", NULL};

StartMultiplayerGame::StartMultiplayerGame(PG_MessageObject* c)
   : ConfigurableWindow(NULL, PG_Rect::null, "newmultiplayergame", false),
     startButton(false),
     success(false),
     newMap(NULL),
     page(ModeSelection),
     ...
     mapParameterEditor(NULL),
     mapParameterEditorParent(NULL),
     allianceSetup(NULL),
     allianceSetupParent(NULL),
     playerSetup(NULL),
     playerSetupParent(NULL),
     emailSetup(NULL),
     emailSetupParent(NULL),
     pbemserver(NULL) {
   setup();
...
void StartMultiplayerGame::loadCampaigns() {
   ...
   list->sigSelectItem.connect(
      sigc::hide(sigc::mem_fun(*this, &StartMultiplayerGame::campaignSelected)));
      ...
      case FilenameSelection: {
         if (!filename.empty())
            if (exist(filename)) {
               delete newMap;
               newMap =
                  mapLoadingExceptionChecker(filename, MapLoadingFunction(tmaploaders::loadmap));
               if (newMap) {
...
GameMap* StartMultiplayerGame::searchForMap(const ASCString& password) {
   StatusMessageWindowHolder smw =
      MessagingHub::Instance().infoMessageWindow("please wait, searching for map...");
...
bool StartMultiplayerGame::nextPage() {
   ...
   if (oldpage != page) {
      if (page == PlayerSetup && playerSetupParent) {
         ...
         if (mode == Skirmish)
            playerSetup = new PlayerSetupWidget(
               newMap, PlayerSetupWidget::AllEditableSinglePlayer, playerSetupParent,
               PG_Rect(0, 0, playerSetupParent->Width(), playerSetupParent->Height()));
         else
            playerSetup = new PlayerSetupWidget(
               newMap, PlayerSetupWidget::AllEditable, playerSetupParent,
               PG_Rect(0, 0, playerSetupParent->Width(), playerSetupParent->Height()));
      }

      if (page == AllianceSetup && allianceSetupParent) {
         delete allianceSetup;
         allianceSetup = new AllianceSetupWidget(
            newMap, &directAllianceSetupStrategy, true, allianceSetupParent,
            PG_Rect(0, 0, allianceSetupParent->Width(), allianceSetupParent->Height()));
      }
      ...
      if (page == MapParameterEditor && mapParameterEditorParent) {
         ...
         mapParameterEditor = new GameParameterEditorWidget(
            newMap, mapParameterEditorParent,
            PG_Rect(0, 0, mapParameterEditorParent->Width(), mapParameterEditorParent->Height()));
      }

      if (page == EmailSetup && emailSetupParent) {
         delete emailSetup;
         emailSetup = new EmailSetupWidget(
            newMap, -1, emailSetupParent,
            PG_Rect(0, 0, emailSetupParent->Width(), emailSetupParent->Height()));
      }
...
void StartMultiplayerGame::userHandler(const ASCString& label, PropertyReadingContainer& pc,
   ...
   if (label == "FileList") {
      ...
      factory->filenameSelectedKeyb.connect(
         sigc::mem_fun(*this, &StartMultiplayerGame::fileNameSelected));
      factory->filenameSelectedMouse.connect(
         sigc::mem_fun(*this, &StartMultiplayerGame::fileNameMarked));
...
bool StartMultiplayerGame::checkPlayerStat() {
   ...
   if (mode == Skirmish) {
      ...
      for (int i = 0; i < newMap->getPlayerCount(); ++i)
         if (newMap->player[i].exist())
            if (newMap->player[i].stat == Player::human ||
                newMap->player[i].stat == Player::supervisor) {
               if (humanFound) {
   ...
   if (mode == PBEM || mode == Hotseat) {
      ...
      if (humanNum <= 1)
         for (int i = 0; i < newMap->getPlayerCount(); ++i)
            if (newMap->player[i].exist())
               if (newMap->player[i].stat == Player::computer ||
                   newMap->player[i].stat == Player::off) {
                  newMap->player[i].stat = Player::human;
...
void StartMultiplayerGame::setupNetwork() {
   if (mode == PBP || mode == PBEM) {
      if (newMap &&
          (!newMap->network || newMap->network->getMechanismID() != FileTransfer::mechanismID())) {
...
bool StartMultiplayerGame::start() {
   ...
   if ((mode == NewCampaign || mode == ContinueCampaign) &&
       CGameOptions::Instance()->recordCampaignMaps)
