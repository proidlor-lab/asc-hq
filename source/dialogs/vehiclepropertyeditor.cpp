...
static const char* cdirnames[7] = {"north",      "north-east", "south-east", "south",
                                   "south-west", "north-west", NULL};
...
class VehiclePropertyEditor : public ASC_PG_Dialog {
  ...
  public:
   VehiclePropertyEditor(PG_Widget* parent, Vehicle* veh)
      : ASC_PG_Dialog(parent, PG_Rect(50, 50, 500, 550), "Edit Unit Properties") {
      this->veh = veh;
      ...
      (new PG_PropertyField_Integer<int>(propertyEditor, "Experience offensive",
                                         &experienceOffensive))
         ->SetRange(0, 1000);

      experienceDefensive = veh->getExperience_defensive();
      (new PG_PropertyField_Integer<int>(propertyEditor, "Experience defensive",
                                         &experienceDefensive))
         ->SetRange(0, 1000);
      ...
      for (int i = 0; i < 8; ++i) {
         playerNames[i] =
            ASCString("(") + ASCString::toString(i) + ") " + veh->getMap()->player[i].getName();
      ...
      for (int w = 0; w < veh->typ->weapons.count; w++)
         (new PG_PropertyField_Integer<int>(propertyEditor,
                                            "Ammo Weapon " + ASCString::toString(w) + " " +
                                               veh->typ->weapons.weapon[w].getName(),
                                            &veh->ammo[w]))
      ...
      new PG_PropertyField_IntDropDown<int>(propertyEditor, "Level of height", &unitHeight,
                                            cheightNames);

      new PG_PropertyField_IntDropDown<Uint8>(propertyEditor, "Orientation", &veh->direction,
                                              cdirnames);
