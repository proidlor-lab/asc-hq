...
class TechWidget : public SelectionWidget {
  ...
  public:
   TechWidget(PG_Widget* parent, const PG_Point& pos, int width, const Technology* technology,
              int player = 0);
...
TechWidget ::TechWidget(PG_Widget* parent, const PG_Point& pos, int width,
                        const Technology* technology, int player)
   : SelectionWidget(parent, PG_Rect(pos.x, pos.y, width, widgetHeight)),
     tech(technology),
     actplayer(player) {
   int lineheight = 20;
   ...
   new PG_Label(this, PG_Rect(xoffs, 30, www, 25),
                ASCString::toString(tech->researchpoints) + " RP");

   if (tech->relatedUnitID > 0) {
      PG_Button* b =
         new PG_Button(this, PG_Rect(width - 2 * lineheight - 10, Height() / 2 - lineheight,
...
TechnologySelectionItemFactory ::TechnologySelectionItemFactory(Player& theplayer)
   : player(theplayer), allTechs(false) {
   restart();
};

bool techComp(const Technology* t1, const Technology* t2) {
   return (t1->relatedUnitID > 0 && t2->relatedUnitID < 0) || (t1->techlevel < t2->techlevel) ||
...
class ChooseTech : public ASC_PG_Dialog {
  ...
  public:
   ChooseTech(Player& my_player)
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 770, 600), "Choose Technology"),
        factory(NULL),
        player(my_player),
        goal(NULL),
        okPressed(false) {
      ...
      (allTechsCheckButton =
          new PG_CheckButton(this, PG_Rect(10, Height() - 40, 300, 25), "Show ~A~ll Technologies"))
         ->sigClick.connect(sigc::mem_fun(*this, &ChooseTech::changeTechView));
      ...
      availLabel = new PG_Label(this, PG_Rect(450, 280, 300, 25),
                                "Accumulated research points: " +
                                   ASCString::toString(player.research.progress));

      (new PG_Button(this, PG_Rect(450, 320, 300, 20), "List Prerequisites"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ChooseTech::showPrerequisites)));

      AddStandardButton("~C~ancel")
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ChooseTech::cancel)));
      AddStandardButton("~O~K")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &ChooseTech::ok)));
   };

   bool selectionPerformed() { return okPressed; }
};
