...
class ExchangeGraphics : public ASC_PG_Dialog {
   ...
   ASCString getIdentifier(const ObjectType* o) { return "O: " + o->filename; }
   ...
   void setnewimage(ObjectType* obj, const ASCString& filename) {
      obj->weatherPicture[0].images =
         loadASCFieldImageArray(filename, obj->weatherPicture[0].images.size());
      obj->displayMethod = 0;
      /*
      ...
      if (fld) {
         try {
            ...
            if (selectedType->GetSelectedItemIndex() == 1) {
               if (fld->objects.size())
                  setnewimage(objectTypeRepository.getObject_byID(fld->objects[0].typ->id),
                              filename->GetText());
            }
         } catch (...) {
         ...
         if (selectedType->GetSelectedItemIndex() == 0) {
            if (fld->typ->bi_pict >= 0) {
               if (orgTerrainGFX.find(fld->typ->bi_pict) != orgTerrainGFX.end()) {
                  GraphicSetManager::Instance().setPic(fld->typ->bi_pict,
                                                       orgTerrainGFX[fld->typ->bi_pict]);
   ...
   void newCursorPos() {
      ...
      if (terrain) {
         ...
         if (newGFX.find(fld->typ->bi_pict) != newGFX.end() ||
             newSurfaces.find(getIdentifier(fld->typ)) != newSurfaces.end())
            s += " (R) ";
      ...
      if (object) {
         if (fld->objects.size()) {
            ...
            if (fld->objects.front().typ->weatherPicture[0].bi3pic[0] >= 0)
               s += " GFX=" +
                    ASCString::toString(fld->objects.front().typ->weatherPicture[0].bi3pic[0]);
  ...
  public:
   ExchangeGraphics()
      : ASC_PG_Dialog(NULL,
                      PG_Rect(PG_Application::GetScreenWidth() - dlg_width, -1, dlg_width, 450),
                      "Exchange Graphics"),
        filename(NULL),
        imageNum(NULL),
        terrain(NULL),
      ...
      (new PG_Button(this, PG_Rect(10, 100, 180, 30), "Select Filename"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ExchangeGraphics::fileSelect)));

      terrain = new PG_Label(this, PG_Rect(10, 140, 180, 25));
      object = new PG_Label(this, PG_Rect(10, 170, 180, 25));

      (new PG_Button(this, PG_Rect(10, 200, 180, 30), "Apply"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ExchangeGraphics::apply)));
      (new PG_Button(this, PG_Rect(10, 240, 180, 30), "Restore Original"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ExchangeGraphics::restore)));
      (new PG_Button(this, PG_Rect(10, 280, 180, 30), "Replacement Summary"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ExchangeGraphics::summary)));
      (new PG_Button(this, PG_Rect(10, 320, 180, 30), "Load from file"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ExchangeGraphics::readFile)));
      (new PG_Button(this, PG_Rect(10, 360, 180, 30), "Close"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ExchangeGraphics::close)));
      (new PG_Button(this, PG_Rect(10, 400, 180, 30), "Snow"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ExchangeGraphics::snow)));
