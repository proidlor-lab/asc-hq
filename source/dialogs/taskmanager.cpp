...
class TaskWidget : public ActionWidget {
  ...
  public:
   TaskWidget(PG_Widget* parent, const PG_Point& pos, int width, Command* command, GameMap* map,
              TaskContainer* taskContainer)
      : ActionWidget(parent, pos, width, *command, map), gamemap(map) {
      this->taskContainer = taskContainer;
      this->command = command;

      (new PG_Button(this, PG_Rect(width - 70, 5, 60, 15), "Run"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &TaskWidget::run)));
      (new PG_Button(this, PG_Rect(width - 70, 22, 60, 15), "Cancel"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &TaskWidget::cancel)));
   }
...
class TaskFactory : public SelectionItemFactory {
  ...
  public:
   ...
   void restart() { a = tasks->pendingCommands.begin(); };
...
class TaskManager : public ASC_PG_Dialog {
  ...
  public:
   TaskManager(GameMap* map)
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 600, 550), "Manage Tasks"), gamemap(map) {
      StandardButtonDirection(Horizontal);
      AddStandardButton("Close")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &TaskManager::ok)));
      AddStandardButton("Run All")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &TaskManager::run)));
      selection = new ItemSelectorWidget(this, PG_Rect(5, 20, Width() - 10, Height() - 70),
                                         new TaskFactory(map));
   }
};
