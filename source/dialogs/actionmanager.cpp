...
class ActionSelectionWidget : public ActionWidget {
  ...
  public:
   ActionSelectionWidget(PG_Widget* parent, const PG_Point& pos, int width, const Command& action,
                         ActionContainer& actionContainer, GameMap* map)
...
class ActionFactory : public SelectionItemFactory {
  ...
  public:
   ...
   void restart() { a = actions.getActions().begin(); };
...
class ActionManager : public ASC_PG_Dialog {
  ...
  public:
   ActionManager(GameMap* map)
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 400, 550), "Manage Actions"), gamemap(map) {
      StandardButtonDirection(Horizontal);
      AddStandardButton("Close")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &ActionManager::ok)));
      AddStandardButton("Run")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &ActionManager::run)));
      selection = new ItemSelectorWidget(this, PG_Rect(5, 20, Width() - 10, Height() - 70),
                                         new ActionFactory(map));
   ...
   void findUnitAtCursor() {
      if (gamemap->getCursor().valid()) {
         selection->selectItem(Loki::Functor<bool, LOKI_TYPELIST_1(SelectionWidget*)>(
                                  this, &ActionManager::isAtCursor),
                               false);
      }
   }
};
