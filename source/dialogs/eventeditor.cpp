...
class EventEditor : public ASC_PG_Dialog {
  private:
   ...
   void actionSelected(int i) {
      ...
      else if (event->action->getActionID() != eai) {
         delete event->action;
         event->action = NULL;
         event->spawnAction(eai);
      }
  ...
  public:
   EventEditor(GameMap* actmap, Event* event)
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 700, 500), "Edit Event"),
        result(false),
        gamemap(actmap) {
      this->event = event;
      ...
      (new PG_Button(this, PG_Rect(labelWidth + 50 + 300, ypos, 100, 25), "setup"))
         ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &EventEditor::setupEvent)));
      ypos += 40;
      ...
      for (int e = 0; e < 4; ++e) {
         ...
         DropDownSelector* trigger =
            new DropDownSelector(this, PG_Rect(labelWidth + 30, ypos, 200, 25), triggerNames);

         (new PG_Button(this, PG_Rect(labelWidth + 50 + 200, ypos, 100, 25), "setup"))
            ->sigClick.connect(sigc::bind(sigc::mem_fun(*this, &EventEditor::setupTrigger), e));

         PG_CheckButton* inv =
            new PG_CheckButton(this, PG_Rect(labelWidth + 70 + 300, ypos, 100, 25), "invert");
         ...
         trigger->selectionSignal.connect(
            sigc::bind(sigc::mem_fun(*this, &EventEditor::triggerSelected), e));
         ypos += 40;
      }

      new PG_Label(this, PG_Rect(10, ypos, labelWidth, 25), "Players:");
      playerlistbox =
         (new MultiListBox(this, PG_Rect(labelWidth + 30, ypos, 150, 160)))->getListBox();
      for (int i = 0; i < event->getMap()->getPlayerCount(); ++i) {
         ...
         if (gamemap->player[i].exist())
            ...
            item =
               new PG_ListBoxItem(playerlistbox, 20, "inactive player " + ASCString::toString(i));
      ...
      static const char* connectionNames[2] = {"all trigger must be triggered",
                                               "only one trigger must be triggered"};
      triggerConnection =
         new DropDownSelector(this, PG_Rect(labelWidth + 30, ypos, 300, 25), 2, connectionNames);
      ...
      AddStandardButton("Cancel")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &EventEditor::QuitModal)));
...
class EventList : public ASC_PG_Dialog {
  ...
  public:
   EventList(GameMap* gameMap)
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 600, 400), "Edit Events"), gamemap(gameMap) {
      int w = 500;
