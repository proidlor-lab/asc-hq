...
ASCString showSurfaceInfo(Surface& surface) {
   ...
   s +=
      "BitsPerPixel: " + ASCString::toString(surface.getBaseSurface()->format->BitsPerPixel) + "\n";
...
void showCargoSummary(MapField* fld) {
   ...
   if (actmap->getCurrentPlayer().diplomacy.isAllied(fld->vehicle)) {
      ...
      s += "\n";
      s += "Strength index: " +
           ASCString::toString(StatisticsCalculator::strength(fld->vehicle, true));
...
class FontViewer : public ASC_PG_Dialog {
  ...
  public:
   FontViewer()
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 18 * spacing, 19 * spacing), "view character set") {
      for (int i = 32; i < 255; ++i) {
         ASCString s;
         s += char(i);
         new PG_Label(this,
                      PG_Rect((i % 16 + 1) * spacing, (i / 16 + 2) * spacing, spacing, spacing), s);
      }
   };
};
...
void resourceAnalysis() {
   ASCString s;
   Resources total;
   for (Player::BuildingList::iterator j = actmap->player[actmap->actplayer].buildingList.begin();
        j != actmap->player[actmap->actplayer].buildingList.end(); j++) {
      Resources res = (*j)->getResource(Resources(maxint, maxint, maxint), true, 0);
      MapCoordinate pos = (*j)->getPosition();
      s += (*j)->getName() + " #pos150#(" + ASCString::toString(pos.x) + "/" +
           ASCString::toString(pos.y) + pos.toString() + ") : #pos300#" +
           ASCString::toString(res.energy) + "#pos400#" + ASCString::toString(res.material) +
           "#pos500#" + ASCString::toString(res.fuel) + "\n";
      total += res;
   }
   s += "\nTotal:\n";
   ...
   s += "\nIncluding units:\n";
   for (Player::VehicleList::iterator j = actmap->player[actmap->actplayer].vehicleList.begin();
        j != actmap->player[actmap->actplayer].vehicleList.end(); j++)
...
void showUnitEndurance() {
   ...
   for (vector<VehicleType*>::iterator i = units.begin(); i != units.end(); ++i) {
      ...
      u.format("#vehicletype=%d# %s : %d fuel ; %s fields range ; %d turns endurance \n", (*i)->id,
               (*i)->getName().c_str(), (*i)->getStorageCapacity(0).fuel, range.c_str(),
               UnitHooveringLogic::getEndurance(*i));
      s += u;
   }
...
void viewMiningPower() {
   ...
   for (Player::BuildingList::iterator i = actmap->getCurrentPlayer().buildingList.begin();
        i != actmap->getCurrentPlayer().buildingList.end(); ++i)
      if ((*i)->baseType->hasFunction(ContainerBaseType::MiningStation)) {
         ...
         ASCString txt = ASCString::toString(output) + " (" + ASCString::toString(power) + "%) " +
                         (*i)->getPosition().toString() + " " + (*i)->baseType->name + "\n";
         info[(*i)->baseType] += txt;
      }
...
void createUnitCostList() {
   ...
   for (int i = 0; i < vehicleTypeRepository.getNum(); ++i) {
      ...
      cout << v->getName() << ";" << v->id << ";" << getUnitSetID(v) << ";" << newc.energy << ";"
           << newc.material << ";" << oldc.energy << ";" << oldc.material << "\n";
   }
}
