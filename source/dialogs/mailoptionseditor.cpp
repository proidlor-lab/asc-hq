  ...
  public:
   MailOptionsDialog() : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 600, 350), "Email Options") {
      ...
      if (clientSetups.size() > 0) {
         ...
         PG_Button* apply =
            new PG_Button(this, PG_Rect(Width() / 2 + 20, 30, Width() / 2 - 30, 20), "Apply");
         apply->sigClick.connect(
            sigc::hide(sigc::mem_fun(*this, &MailOptionsDialog::cannedConfigSelected)));

         // dds->selectionSignal.connect( sigc::mem_fun( *this,
         // &MailOptionsDialog::cannedConfigSelected));
      } else
         dds = NULL;
      ...
      AddStandardButton("OK")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &MailOptionsDialog::ok)));
      AddStandardButton("Cancel")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &MailOptionsDialog::cancel)));
   }
};
