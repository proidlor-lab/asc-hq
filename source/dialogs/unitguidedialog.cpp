...
class DataTab : public PG_Widget {
  ...
  private:
   ...
   PG_Widget *movementTab, *weapons1Tab, *weapons2Tab, *productionTab, *transportTab, *generalTab,
      *terrainAccessTab1, *terrainAccessTab2;
   PG_Label *nameGeneral, *iconGeneral;  //, *categoryIcon;
   PG_Label *id, *category, *armor, *view, *jamming, *maxWind, *tankEnery, *tankMat, *tankFuel,
      *fuelConsuption, *maxSpeed, *weight, *autoRepairRate, *productionCostEnergy,
      *productionCostMaterial, *productionCostFuel;
...
DataTab::DataTab(PG_Widget* parent, const PG_Rect& pos, bool showDescription)
   : PG_Widget(parent, pos) {
   currentPage = general;
   currentUnit = NULL;
   parentPos = pos;
   compareData = false;
   ...
   for (int i = 0; i < 30; i++) {
      terrainAccessLabel[i] =
         new PG_Label(terrainAccessTab1, PG_Rect(xPos, yPos, width, labelHeight));
      yPos += lineHeight;
   }
   ...
   for (int i = 30; i < terrainPropertyNum; i++) {
      terrainAccessLabel[i] =
         new PG_Label(terrainAccessTab2, PG_Rect(xPos, yPos, width, labelHeight));
      yPos += lineHeight;
   }
...
void DataTab::setData() {
   ...
   if (showDescriptionOnly) {
      ...
      id->SetText(buffer);
      // categoryIcon->SetIcon( IconRepository::getIcon( moveMaliTypeIcons[
      // currentUnit->movemalustyp ] ).getBaseSurface() );
      ...
      for (int i = 0; i < currentUnit->weapons.count; i++) {
         ...
         if (compareType != NULL) {
            costMaterial =
               costMaterial - compareType->weapons.weapon[i].laserRechargeCost.resource(1);
         }
...
UnitGuideWidget::UnitGuideWidget(PG_Widget* parent, const PG_Rect& pos, int tabs)
   : PG_Widget(parent, pos) {
   loadCategories();
   ...
   mainCategory->selectionSignal.connect(
      sigc::mem_fun(*this, &UnitGuideWidget::mainCategoryChanged));
   xPos += 220;
   ...
   */
   ...
   for (int i = 0; i < tabs; i++) {
      DataTab* dataTab =
         new DataTab(this, PG_Rect(xPos, yPos, 150, pos.Height() - yPos - 10), false);
...
bool UnitGuideWidget::displayUnit() {
   //	Hide();
   dataTabs[displayTab->GetSelectedItemIndex()]->setUnit(
      selectedUnits[unit->GetSelectedItemIndex()]);
...
void UnitGuideWidget::selectUnits() {
   ...
   int filter = mainCategories[mainCategory->GetSelectedItemIndex()]
                   .subCategories[subCategory->GetSelectedItemIndex()]
                   .category;
...
UnitGuideWindow::UnitGuideWindow(PG_Widget* parent, const PG_Rect& pos, int tabs)
   : ASC_PG_Dialog(parent, pos, "UnitGuide") {
