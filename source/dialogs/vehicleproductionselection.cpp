...
VehicleProduction_SelectionItemFactory::VehicleProduction_SelectionItemFactory(
   ...
   : VehicleTypeSelectionItemFactory(plantResources,
                                     convertAndCreateArrays(produceableUnits, &items),
                                     productionplant->getMap()->getCurrentPlayer()),
     fillResources(true),
     fillAmmo(true),
     plant(productionplant),
     produceables(produceableUnits) {}

const VehicleProduction_SelectionItemFactory::Container&
VehicleProduction_SelectionItemFactory::convertAndCreateArrays(
   const ConstructUnitCommand::Producables& from, Container** items) {
   *items = new Container();
   return convertArrays(from, **items);
}

const VehicleProduction_SelectionItemFactory::Container&
VehicleProduction_SelectionItemFactory::convertArrays(const ConstructUnitCommand::Producables& from,
                                                      Container& items) {
   items.clear();
   for (ConstructUnitCommand::Producables::const_iterator i = from.begin(); i != from.end(); ++i)
      if (!(i->prerequisites.getValue() &
            (ConstructUnitCommand::Lack::Research | ConstructUnitCommand::Lack::Unloadability)))
...
Resources VehicleProduction_SelectionItemFactory::getCost(const VehicleType* type) {
   for (ConstructUnitCommand::Producables::const_iterator i = produceables.begin();
        i != produceables.end(); ++i) {
      if (i->type == type) {
         Resources cost = i->cost;
         if (fillResources)
            cost += Resources(0, type->getStorageCapacity(plant->getMap()->_resourcemode).material,
                              type->getStorageCapacity(plant->getMap()->_resourcemode).fuel);

         if (fillAmmo)
            for (int w = 0; w < type->weapons.count; ++w)
               if (type->weapons.weapon[w].requiresAmmo()) {
                  int wt = type->weapons.weapon[w].getScalarWeaponType();
                  cost += Resources(ammoProductionCost[wt][0], ammoProductionCost[wt][1],
                                    ammoProductionCost[wt][2]) *
                          type->weapons.weapon[w].count;
               }
         return cost;
      }
   }
...
AddProductionLine_SelectionItemFactory::AddProductionLine_SelectionItemFactory(
   ContainerBase* my_plant, const Container& types)
   : VehicleTypeSelectionItemFactory(my_plant->getResource(Resources(maxint, maxint, maxint), true),
                                     types, my_plant->getMap()->getCurrentPlayer()),
     plant(my_plant){
...
VehicleProduction_SelectionWindow::VehicleProduction_SelectionWindow(
   PG_Widget* parent, const PG_Rect& r, ContainerBase* plant,
   const ConstructUnitCommand::Producables& produceableUnits, bool internally)
   : ASC_PG_Dialog(parent, r, "Choose Vehicle Type"),
     selected(NULL),
     finallySelected(NULL),
     isw(NULL),
     factory(NULL),
     my_plant(plant),
     produceables(produceableUnits) {
   factory = new VehicleProduction_SelectionItemFactory(
      plant->getResource(Resources(maxint, maxint, maxint), true), plant, produceableUnits);
   if (internally) {
   ...
   factory->sigVehicleTypeSelected.connect(
      sigc::mem_fun(*this, &VehicleProduction_SelectionWindow::vtSelected));
   factory->sigVehicleTypeMarked.connect(
      sigc::mem_fun(*this, &VehicleProduction_SelectionWindow::vtMarked));

   isw = new ItemSelectorWidget(
      this, PG_Rect(10, GetTitlebarHeight(), r.Width() - 20, r.Height() - GetTitlebarHeight() - 40),
      factory);
   isw->sigQuitModal.connect(
      sigc::mem_fun(*this, &VehicleProduction_SelectionWindow::quitSignalled));

   int y = GetTitlebarHeight() + isw->Height();
   if (internally) {
      PG_CheckButton* fillRes = new PG_CheckButton(this, PG_Rect(10, y + 2, r.Width() / 2 - 50, 20),
                                                   "Fill with Resources");
      if (factory->getResourceFilling())
         fillRes->SetPressed();
      fillRes->sigClick.connect(
         sigc::mem_fun(*factory, &VehicleProduction_SelectionItemFactory::setResourceFilling));
      if (plant->baseType->hasFunction(ContainerBaseType::AmmoProduction)) {
         PG_CheckButton* fillAmmo =
            new PG_CheckButton(this, PG_Rect(10, y + 20, r.Width() / 2 - 50, 20), "Fill with Ammo");
         if (factory->getAmmoFilling())
            fillAmmo->SetPressed();
         fillAmmo->sigClick.connect(
            sigc::mem_fun(*factory, &VehicleProduction_SelectionItemFactory::setAmmoFilling));
      } else
         factory->setAmmoFilling(false);
   ...
   factory->reloadAllItems.connect(
      sigc::mem_fun(*this, &VehicleProduction_SelectionWindow::reLoadAndUpdate));
   ...
   b->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &VehicleProduction_SelectionWindow::produce)));
   ...
   if (!plant->baseType->hasFunction(ContainerBaseType::NoProductionCustomization) && internally) {
      ...
      b2->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &VehicleProduction_SelectionWindow::addProductionLine)));
      new PG_ToolTipHelp(b2, "Add production line");

      PG_Button* b3 = new PG_Button(this, PG_Rect(rr.x - rr.h - 5, rr.y, rr.h, rr.h), "-");
      b3->sigClick.connect(sigc::hide(
         sigc::mem_fun(*this, &VehicleProduction_SelectionWindow::removeProductionLine)));
...
bool VehicleProduction_SelectionWindow::addProductionLine() {
   ...
   {
      ItemSelectorWindow isw(
         NULL, PG_Rect(100, 150, 400, 400), "choose production line",
         new AddProductionLine_SelectionItemFactory(my_plant, bplc.productionLinesBuyable()));
      isw.Show();
      isw.RunModal();
   }
   reLoadAndUpdate();
...
bool VehicleProduction_SelectionWindow::removeProductionLine() {
   if (selected &&
       choice_dlg("do you really want to remove this production line ?", "~y~es", "~n~o") == 1) {
