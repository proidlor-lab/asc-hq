...
class StoringPosition : public PG_Widget {
  ...
  public:
   ...
   static vector<StoringPosition*> setup(PG_Widget* parent, ContainerBase* container,
                                         HighLightingManager& highLightingManager,
                                         int& unitColumnCount);
   Vehicle* getUnit();
   StoringPosition(PG_Widget* parent, const PG_Point& pos, const PG_Point& unitPos,
                   HighLightingManager& highLightingManager,
                   const ContainerBase::Cargo& storageVector, int number, bool regularPosition,
