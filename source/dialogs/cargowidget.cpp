...
StoringPosition ::StoringPosition(PG_Widget* parent, const PG_Point& pos, const PG_Point& unitPos,
                                  HighLightingManager& highLightingManager,
                                  const ContainerBase::Cargo& storageVector, int number,
                                  bool regularPosition, CargoWidget* cargoWidget)
   : PG_Widget(parent, CalcSize(pos), true),
     highlight(highLightingManager),
     storage(storageVector),
     num(number),
     regular(regularPosition),
     unitPosition(unitPos),
     dragState(Off),
     dragTarget(NoDragging) {
...
void StoringPosition::setLabelText(const ASCString& widgetName, const ASCString& text,
                                   PG_Widget* parent) {
   if (!parent)
      parent = this;
...
void StoringPosition ::eventDraw(SDL_Surface* surface, const PG_Rect& src) {
   ...
   if (num < storage.size() && storage[num]) {
      ...
      if (storage[num]->getMovement() > 0)
         storage[num]->paint(clippingSurface, SPoint(xpos, ypos),
                             0);  // storage[num]->getOwner() );
      else
         storage[num]->paint(clippingSurface, SPoint(xpos, ypos), true,
                             0);  // storage[num]->getMap()->getNeutralPlayerNum() );

      setBargraphValue("DamageBar", float(100 - storage[num]->damage) / 100);

      if (storage[num]->getStorageCapacity().fuel) {
         float f = float(storage[num]->getResource(maxint, 2, true, 0)) /
                   storage[num]->getStorageCapacity().fuel;
         setBargraphValue("FuelBar", f);
      } else
         setBargraphValue("FuelBar", 0);

      if (storage[num]->getStorageCapacity().material) {
         float f = float(storage[num]->getResource(maxint, 1, true, 0)) /
                   storage[num]->getStorageCapacity().material;
   ...
   // PG_Draw::BlitSurface( clippingSurface.getBaseSurface(), src, PG_Application::GetScreen(),
   // dst);
...
bool StoringPosition::eventMouseButtonDown(const SDL_MouseButtonEvent* button) {
   ...
   */
   ...
   if (button->button == CGameOptions::Instance()->mouse.dragndropbutton && cargoWidget &&
       cargoWidget->dragNdropEnabled() && getUnit()) {
      SetCapture();
      dragState = Pressed;
   ...
   if (dragState != Off) {
      ...
      if (dragState == Pressed) {
         if (square(motion->x - dragPointStart.x) + square(motion->y - dragPointStart.y) > 9) {
            ...
            static Surface surf;
            if (!surf.valid()) {
               surf = Surface::createSurface(fieldsizex + 20, fieldsizey + 20,
                                             32);  // somewhat larger because of potential shadow
...
vector<StoringPosition*> StoringPosition ::setup(PG_Widget* parent, ContainerBase* container,
                                                 HighLightingManager& highLightingManager,
                                                 int& unitColumnCount) {
   ...
   if (parent) {
      ...
      for (int i = 0; i < posNum; ++i) {
         StoringPosition* sp = new StoringPosition(
            parent, PG_Point(x, y), PG_Point(-1, -1), highLightingManager, container->getCargo(), i,
            container->baseType->maxLoadableUnits >= container->getCargo().size());
...
CargoWidget ::CargoWidget(PG_Widget* parent, const PG_Rect& pos, ContainerBase* container,
                          bool setup)
   : PG_ScrollWidget(parent, pos), dragNdrop(true), unitColumnCount(0), draggedUnit(NULL) {
   this->container = container;
   SetTransparency(255);

   if (setup)
      registerStoringPositions(
         StoringPosition::setup(this, container, unitHighLight, unitColumnCount), unitColumnCount);

   if (my_objVerticalScrollbar)
      my_objVerticalScrollbar->sigScrollTrack.connect(
         sigc::mem_fun(*this, &CargoWidget::handleScrollTrack));

   if (my_objHorizontalScrollbar)
      my_objHorizontalScrollbar->sigScrollTrack.connect(
         sigc::mem_fun(*this, &CargoWidget::handleScrollTrack));
};
...
void CargoWidget ::checkStoringPosition(int oldpos, int newpos) {
   ...
   //  PG_ScrollWidget* unitScrollArea = dynamic_cast<PG_ScrollWidget*>(FindChild( "UnitScrollArea",
   //  true )); if ( unitScrollArea )
...
void CargoWidget ::startDrag(Vehicle* v) {
   for (StoringPositionVector::iterator i = storingPositionVector.begin();
        i != storingPositionVector.end(); ++i) {
