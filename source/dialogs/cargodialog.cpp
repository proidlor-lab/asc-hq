...
class SubWinButton : public PG_Button {
  ...
  public:
   ...
   SubWinButton(PG_Widget* parent, const SPoint& pos, SubWindow* subWindow)
      : PG_Button(parent, PG_Rect(pos.x, pos.y, calcWidth(), calcHeight()), "", -1,
                  "SubWinButton") {
      SetBackground(PRESSED, IconRepository::getIcon("cargo-buttonpressed.png").getBaseSurface());
      SetBackground(HIGHLITED,
                    IconRepository::getIcon("cargo-buttonhighlighted.png").getBaseSurface());
      SetBackground(UNPRESSED,
                    IconRepository::getIcon("cargo-buttonunpressed.png").getBaseSurface());
      SetBorderSize(0, 0, 0);
...
class CargoDialog : public Panel {
   ...
   void containerDestroyed() { shutdownImmediately = true; }
      ...
      if ((mod & KMOD_CTRL) && !(mod & KMOD_SHIFT)) {
         switch (key->keysym.unicode) {
            case 26:  // Z
               ...
               cargoChanged();
               for (Activesubwindows::iterator i = activesubwindows.begin();
                    i != activesubwindows.end(); ++i)
                  (*i)->update();
               return true;
         }
      }
   ...
   void onUnitClick(Vehicle* veh, SPoint pos, bool first) {
      ...
      if (mainScreenWidget && mainScreenWidget->getGuiHost() &&
          (!first || CGameOptions::Instance()->mouse.singleClickAction)) {
         SPoint iconPos = pos;
         if (veh) {
   ...
   void painter(const PG_Rect& src, const ASCString& name, const PG_Rect& dst) {
      ...
      if (name == "SelectedImage") {
         if (getMarkedUnit())
            getMarkedUnit()->typ->paint(screen, SPoint(dst.x, dst.y),
                                        getMarkedUnit()->getOwningPlayer().getPlayerColor());
      }
   };
   ...
   void updateLoadMeter() {
      if (container->baseType->maxLoadableWeight > 0)
         setBargraphValue("LoadingMeter",
                          float(container->cargoWeight()) / container->baseType->maxLoadableWeight);
   }
   ...
   int getResourceValue(int resourcetype, int y, int scope) {
      ...
      switch (y) {
         case 0: {  // avail
            ...
            return gr.getresource(container->getPosition().x, container->getPosition().y,
                                  resourcetype, maxint, 1, player, scope);
         }
         case 1: {  // tank
            GetResourceCapacity grc(container->getMap());
            return grc.getresource(container->getPosition().x, container->getPosition().y,
                                   resourcetype, maxint, 1, player, scope);
         }
         case 2: {  // plus
            GetResourcePlus grp(container->getMap());
            return grp.getresource(container->getPosition().x, container->getPosition().y,
                                   resourcetype, player, scope);
         }
         case 3: {  // usage
            GetResourceUsage gru(container->getMap());
            return gru.getresource(container->getPosition().x, container->getPosition().y,
                                   resourcetype, player, scope);
         }
      } /* endswitch */
      return -1;
  ...
  public:
   ...
   void addAvailableSubwin(SubWindow* w) { activesubwindows.push_back(w); };
   ...
   void cargoChanged() {
      ...
      if (cargoWidget) {
         ...
         if (mainScreenWidget && mainScreenWidget->getUnitInfoPanel())
            mainScreenWidget->getUnitInfoPanel()->showUnitData(cargoWidget->getMarkedUnit(), NULL,
                                                               NULL, true);
   ...
   bool ProcessEvent(const SDL_Event* event, bool bModal = false) {
      // the unit info panel must get the events first. If a unit is renamed and a character
      // entered, this key event shall go to the renaming - and not to the unit actions.
      if (mainScreenWidget && mainScreenWidget->getUnitInfoPanel())
   ...
   void userHandler(const ASCString& label, PropertyReadingContainer& pc, PG_Widget* parent,
   ...
   void setUnitInfoWidgets(const ContainerBase* container, const ASCString& prefix) {
      if (container) {
         ...
         int slots = 0;
         for (ContainerBase::Cargo::const_iterator i = container->getCargo().begin();
              i != container->getCargo().end(); ++i)
            if (*i)
               ++slots;

         if (container->baseType->maxLoadableUnits)
            setBargraphValue(prefix + "LoadingMeter3",
                             float(slots) / float(container->baseType->maxLoadableUnits));
         else
   ...
   void updateVariables() {
      ...
      // from solarpower
      {
         ...
         static const char* weathernames[] = {
            "terrain_weather_dry.png",       "terrain_weather_lightrain.png",
            "terrain_weather_heavyrain.png", "terrain_weather_lightsnow.png",
            "terrain_weather_heavysnow.png", "terrain_weather_ice.png"};
         ...
         setImage(
            "Weather",
            weathernames[container->getMap()->getField(container->getPosition())->getWeather()],
            widget);
      }
      ...
      // from windpower
      {
         ...
         if (container->getMap()->weatherSystem)
            setLabelText("WindSpeed", container->getMap()->weatherSystem->getCurrentWindSpeed(),
                         widget);
#else
      ...
      // from cargo
      {
         ...
         if (getMarkedUnit()) {
            ...
            if (getMarkedUnit()->baseType->infoImageSmallFilename.length() > 0) {
               setImage("Selected3DImageSmall", getMarkedUnit()->baseType->infoImageSmallFilename,
                        widget);
         ...
         if (container->baseType->maxLoadableWeight > 0)
            setBargraphValue(
               "LoadingMeter2",
               float(container->cargoWeight()) / container->baseType->maxLoadableWeight, widget);
         ...
         if (container->baseType->maxLoadableWeight >= 1000000) {
            ...
            setLabelText("FreeWeight",
                         container->baseType->maxLoadableWeight - container->cargoWeight(), widget);
            setLabelText("MaxWeight", container->baseType->maxLoadableWeight, widget);
         }

         setLabelText("FreeSlots",
                      container->baseType->maxLoadableUnits - container->vehiclesLoaded(), widget);
      ...
      // from buildingcontrol
      {
         ...
         if (bld) {
            if (RepairBuildingCommand::avail(bld)) {
               ...
               setLabelText("RepairCostLabel",
                            "Cost for repairing " + ASCString::toString(repairData.damageDelta) +
                               "%",
                            widget);
...
{ "ammotransfer", "ammoproduction", "info", "cargoinfo", "conventionalpower", "mining",
"netcontrol", "research", "resourceinfo", "solarpower", "windpower"
};
*/
...
class SolarPowerWindow : public SubWindow {
  public:
   ...
   ASCString getASCTXTname() { return "solarpower"; };

   ASCString getFullName() { return "Solar Power Generation"; };

   void update() { cargoDialog->updateVariables(); }
};
...
class WindPowerWindow : public SubWindow {
  public:
   ...
   ASCString getASCTXTname() { return "windpower"; };

   ASCString getFullName() { return "Wind Power Generation"; };
...
class NetControlWindow : public SubWindow {
  private:
   bool click(PG_Button* b, int x, int y) { return true; }
  ...
  public:
   ...
   ASCString getASCTXTname() { return "netcontrol"; };

   ASCString getFullName() { return "Resource Network Control"; };
      ...
      if (widget) {
         for (int x = 0; x < 3; ++x) {
            for (int y = 0; y < 4; ++y) {
               ...
               if (b) {
                  b->sigClick.connect(
                     sigc::bind(sigc::bind(sigc::mem_fun(*this, &NetControlWindow::click), x), y));
                  if (y >= 2)
                     b->SetToggle(true);
               }
            }
...
class CargoInfoWindow : public SubWindow {
  public:
   bool available(CargoDialog* cd) { return true; };

   ASCString getASCTXTname() { return "cargoinfo"; };

   ASCString getFullName() { return "Cargo Info"; };
   ...
   void registerChilds(CargoDialog* cd) { SubWindow::registerChilds(cd); }
...
class BuildingControlWindow : public SubWindow {
  public:
   ...
   ASCString getASCTXTname() { return "damagecontrol"; };

   ASCString getFullName() { return "Building Control"; };
...
class ResourceInfoWindow : public SubWindow {
  public:
   bool available(CargoDialog* cd) {
      if (!cd->getMap()->getCurrentPlayer().diplomacy.isAllied(cd->getContainer()) &&
          cd->getMap()->getCurrentPlayer().stat != Player::supervisor)
         return false;

      return dynamic_cast<Building*>(cd->getContainer()) != NULL;
   };

   ASCString getASCTXTname() { return "resourceinfo"; };

   ASCString getFullName() { return "Resource Network Information"; };
      ...
      for (int c = 0; c < 3; c++)
         for (int x = 0; x < 3; x++)
            for (int y = 0; y < 4; y++) {
               ...
               label += xx;
               label += yy;
               if ((y != 1 || value[c][x][y] < mx * 10 || value[c][x][y] < 1000000000) &&
                   (!activeContainer->getMap()->isResourceGlobal(x) || y != 0 ||
                    c == 2))  // don't show extremely high numbers
...
class GraphWidget : public PG_Widget {
  ...
  public:
   GraphWidget(PG_Widget* parent, const PG_Rect& rect)
      : PG_Widget(parent, rect), xrange(1), yrange(1){};
   ...
   void addVerticalLine(int x, int color) { verticalLines[x] = color; }

   void clearVerticalLines() { verticalLines.clear(); }
   ...
   void eventBlit(SDL_Surface* surface, const PG_Rect& src, const PG_Rect& dst) {
      ...
      for (Bars::iterator b = bars.begin(); b != bars.end(); ++b) {
         ...
         PG_Point pos = ClientToScreen(x, Height() - y);
         paintFilledRectangle<4>(s, SPoint(pos.x, pos.y), x2 - x, y,
                                 ColorMerger_ColoredOverwrite<4>(mapColor(b->second)));
...
class ResearchGraph : public GraphWidget {
  ...
  protected:
   ...
   void addSecondaryLab(ContainerBase* lab) {
      if (lab->baseType->nominalresearchpoints &&
          lab->baseType->hasFunction(ContainerBaseType::Research)) {
         int rel = lab->researchpoints * cont->baseType->nominalresearchpoints /
                   lab->baseType->nominalresearchpoints;
   ...
   void recalc() {
      ...
      for (Player::BuildingList::iterator i = player.buildingList.begin();
           i != player.buildingList.end(); ++i)
         addSecondaryLab(*i);

      for (Player::VehicleList::iterator i = player.vehicleList.begin();
           i != player.vehicleList.end(); ++i)
         addSecondaryLab(*i);
   ...
   bool setResearch(ContainerBase* lab, int x) {
      if (cont->baseType->nominalresearchpoints) {
         ...
         if (returnResourcenUseForResearch(lab, lab->maxresearchpoints) == Resources(0, 0, 0))
            ...
            res = x * xrange / Width() * lab->baseType->nominalresearchpoints /
                  cont->baseType->nominalresearchpoints;
   ...
   void click(int x, int button) { setResearch(x, button == 3); }
  ...
  public:
   ResearchGraph(PG_Widget* parent, const PG_Rect& rect, ContainerBase* container)
      : GraphWidget(parent, rect), cont(container) {
      setRange(cont->maxresearchpoints + 1,
               returnResourcenUseForResearch(cont, cont->maxresearchpoints).energy);
      addCurve(0x00ff00);
      recalc();
   }
...
class ResearchWindow : public SubWindow {
  public:
   ...
   ASCString getASCTXTname() { return "research"; };

   ASCString getFullName() { return "Research Lab"; };
   ...
   void update() {
      ...
      cargoDialog->setLabelText("ResPerTurnLocal",
                                activeContainer->researchpoints * player.research.getMultiplier(),
                                widget);
      ...
      for (int r = 0; r < 3; ++r)
         cargoDialog->setLabelText("CostGlobal" + ASCString::toString(r), globalCost.resource(r),
                                   widget);
...
class MatterAndMiningBaseWindow : public SubWindow {
  ...
  private:
   ...
   void setnewpower(ContainerBase* c, int power) {
      if (hasFunction(c)) {
         std::unique_ptr<SetResourceProcessingRateCommand> srprc(
            new SetResourceProcessingRateCommand(c, power));
   ...
   void setnewpower(int pwr) {
      ...
      if (allbuildings) {
         for (Player::BuildingList::iterator bi = player.buildingList.begin();
              bi != player.buildingList.end(); bi++)
            setnewpower(*bi, pwr);
         for (Player::VehicleList::iterator bi = player.vehicleList.begin();
              bi != player.vehicleList.end(); bi++)
            setnewpower(*bi, pwr);
      } else {
  ...
  public:
   ...
   void update() {
      ...
      if (first && slider) {
         ...
         slider->sigScrollTrack.connect(
            sigc::mem_fun(*this, &MatterAndMiningBaseWindow::scrollTrack));
      }

      for (int r = 0; r < 3; ++r)
         if (container()->maxplus.resource(r)) {
            slider->SetPosition(100 * container()->plus.resource(r) /
                                container()->maxplus.resource(r));
            break;
         }
...
class MatterConversionWindow : public MatterAndMiningBaseWindow {
  protected:
   ...
   Resources getOutput() { return container()->plus; }
  ...
  public:
   ASCString getASCTXTname() { return "conventionalpower"; };

   ASCString getFullName() { return "Power Generation"; };
};
  ...
  protected:
   int getBarHeight(int bar) {
      ...
      switch (bar % 4) {
         ...
         case 2:
         case 3:
            return 100 * mininginfo.avail[bar / 4].resource(r) /
                   mininginfo.max[bar / 4].resource(r);
      };
      return 0;
   };
...
class MiningWindow : public MatterAndMiningBaseWindow {
  ...
  public:
   ASCString getASCTXTname() { return "mining"; };

   ASCString getFullName() { return "Mining Facility"; };

   void update() { MatterAndMiningBaseWindow::update(); }
};
...
class DamageBarWidget : public PG_ThemeWidget {
  ...
  public:
   DamageBarWidget(PG_Widget* parent, const PG_Rect& rect, ContainerBase* container)
      : PG_ThemeWidget(parent, rect, false) {
   ...
   void eventBlit(SDL_Surface* surface, const PG_Rect& src, const PG_Rect& dst) {
      ...
      if (w2 > 0) {
         ...
         drawLine<4>(s, cm, SPoint(dst.x, dst.y + dst.h - h - 1),
                     SPoint(dst.x + w2, dst.y + dst.h - h - 1));
         drawLine<4>(s, cm, SPoint(dst.x, dst.y + dst.h - h),
                     SPoint(dst.x + w2, dst.y + dst.h - h));
      ...
      /*         Uint32 c = color.MapRGBA( PG_Application::GetScreen()->format,
         255-GetTransparency()); for ( Colors::iterator i = colors.begin(); i != colors.end(); ++i)
                  if ( fraction < i->first ) {
...
CargoDialog ::CargoDialog(PG_Widget* parent, ContainerBase* cb)
   : Panel(parent, PG_Rect::null, "cargodialog", false),
     container(cb),
     setupOK(false),
     cargoWidget(NULL),
     researchWindow(NULL),
...
void CargoDialog::userHandler(const ASCString& label, PropertyReadingContainer& pc,
                              PG_Widget* parent, WidgetParameters widgetParams) {
   ...
   if (label == "ResearchGraph") {
      ResearchGraph* graph =
         new ResearchGraph(parent, PG_Rect(0, 0, parent->Width(), parent->Height()), container);
   ...
   if (label == "MiningGraph") {
      ...
      new PG_ToolTipHelp(mg,
                         "Horizontal: distance from building ; vertical: amount of resources(M/F)");
   }

   if (label == "DamageBar") {
      DamageBarWidget* dbw =
         new DamageBarWidget(parent, PG_Rect(0, 0, parent->Width(), parent->Height()), container);
   ...
   if (label == "ScrollArea") {
      ...
      if (unitScrollArea) {
         cargoWidget = new CargoWidget(
            unitScrollArea,
            PG_Rect(1, 1, unitScrollArea->Width() - 2, unitScrollArea->Height() - 2), container,
            false);
         cargoWidget->enableDragNDrop(true);
         cargoWidget->sigDragDone.connect(
            sigc::mem_fun(*this, &CargoDialog::dragUnitToInnerContainer));
         cargoWidget->sigDragAvail.connect(
            sigc::mem_fun(*this, &CargoDialog::dragUnitToInnerContainerAvail));
         ...
         for (int i = 0; i < posNum; ++i) {
            ...
            StoringPosition* sp = new StoringPosition(
               cargoWidget, PG_Point(x, y), PG_Point(unitposx, unitposy),
               cargoWidget->getHighLightingManager(), container->getCargo(), i,
               container->baseType->maxLoadableUnits >= container->getCargo().size(), cargoWidget);
         ...
         if (ciw)
            cargoWidget->unitMarked.connect(
               sigc::hide(sigc::mem_fun(*ciw, &CargoInfoWindow::update)));

         if (mainScreenWidget && mainScreenWidget->getGuiHost())
            cargoWidget->unitMarked.connect(
               sigc::hide(sigc::mem_fun(*this, &CargoDialog::clearSmallIcons)));
   ...
   if (label == "UnitTypeList" || label == "UnitTypeListHorizontal") {
      ...
      for (int i = 0; i < cmovemalitypenum; ++i) {
         if (container->baseType->vehicleCategoriesStorable & (1 << i)) {
            PG_Image* img =
               new PG_Image(parent, PG_Point(x, y),
                            IconRepository::getIcon(moveMaliTypeIcons[i]).getBaseSurface(), false);
...
void Movement::execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   for (set<MapCoordinate3D>::const_iterator i = move->getReachableFields().begin();
        i != move->getReachableFields().end(); ++i)
      unit->getMap()->getField(*i)->setaTemp(1);

   for (set<MapCoordinate3D>::const_iterator i = move->getReachableFieldsIndirect().begin();
        i != move->getReachableFieldsIndirect().end(); ++i)
...
void UnitProduction::execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   std::unique_ptr<ConstructUnitCommand> production(
      new ConstructUnitCommand(parent.getContainer()));
   ...
   if (v) {
      for (ConstructUnitCommand::Producables::const_iterator i = producables.begin();
           i != producables.end(); ++i)
         if (i->type == v) {
            if (i->prerequisites.getValue() &
                (ConstructUnitCommand::Lack::Energy | ConstructUnitCommand::Lack::Material |
                 ConstructUnitCommand::Lack::Fuel)) {
   ...
   parent.cargoChanged();  // we need to update the resources because new production lines may have
                           // been build
...
bool MoveUnitIntoInnerContainer::available(const MapCoordinate& pos, ContainerBase* subject,
                                           int num) {
   if (!subject)
      return false;
...
Surface& MoveUnitIntoInnerContainer::getImage(const MapCoordinate& pos, ContainerBase* subject,
                                              int num) {
   return IconRepository::getIcon("container-in.png");
};

ASCString MoveUnitIntoInnerContainer::getName(const MapCoordinate& pos, ContainerBase* subject,
                                              int num) {
...
void MoveUnitIntoInnerContainer::execute(const MapCoordinate& pos, ContainerBase* subject,
                                         int num) {
   if (!subject)
      return;
   ...
   choiceDialog(
      "You can also use Drag'n'Drop to move units in the Cargo Dialog\nMouse button: " +
         CGameOptions::Mouse::getButtonName(CGameOptions::Instance()->mouse.dragndropbutton),
      "OK", "", "dragndropinfo");
...
bool RefuelUnitDialogCommand ::available(const MapCoordinate& pos, ContainerBase* subject,
                                         int num) {
   if (!subject)
      return false;
...
Surface& RefuelUnitDialogCommand ::getImage(const MapCoordinate& pos, ContainerBase* subject,
                                            int num) {
...
ASCString RefuelUnitDialogCommand ::getName(const MapCoordinate& pos, ContainerBase* subject,
                                            int num) {
...
bool RecycleUnitCommandButton ::available(const MapCoordinate& pos, ContainerBase* subject,
                                          int num) {
   if (!subject)
      return false;
...
void RecycleUnitCommandButton ::execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   ASCString msg =
      "do you really want to " + getCommandName(parent.getContainer()) + " this unit ?";
...
Surface& RecycleUnitCommandButton ::getImage(const MapCoordinate& pos, ContainerBase* subject,
                                             int num) {
...
ASCString RecycleUnitCommandButton ::getName(const MapCoordinate& pos, ContainerBase* subject,
                                             int num) {
   if (!subject)
      return "";
