...
ResourcePlacement::ResourcePlacement(GameMap& m, double fr, double mr, unsigned short maxFOffset,
                                     unsigned short maxMOffset, int addFreeFieldsPercFuel,
                                     int addFreeFieldsPercMaterial)
   : map(m),
     fuelRoughness(fr),
     materialRoughness(fr),
     maxFuelOffset(maxFOffset),
     maxMaterialOffset(maxMOffset),
     additionalResourceFreeFieldsPercentageFuel(addFreeFieldsPercFuel),
     additionalResourceFreeFieldsPercentageMaterial(addFreeFieldsPercMaterial),
     stepCount(0) {
...
void ResourcePlacement::runDS() {
   ...
   if (placeFuel) {
      a->setTemp3(
         createRandomValue(MAXFUELVALUE * (1 + additionalResourceFreeFieldsPercentageFuel / 100)));
      b->setTemp3(
         createRandomValue(MAXFUELVALUE * (1 + additionalResourceFreeFieldsPercentageFuel / 100)));
      c->setTemp3(
         createRandomValue(MAXFUELVALUE * (1 + additionalResourceFreeFieldsPercentageFuel / 100)));
   ...
   if (placeMaterial) {
      c->setTemp4(createRandomValue(MAXMATERIALVALUE *
                                    (1 + additionalResourceFreeFieldsPercentageMaterial / 100)));
      d->setTemp4(createRandomValue(MAXMATERIALVALUE *
                                    (1 + additionalResourceFreeFieldsPercentageMaterial / 100)));
      a->setTemp4(createRandomValue(MAXMATERIALVALUE *
                                    (1 + additionalResourceFreeFieldsPercentageMaterial / 100)));
      c->setTemp4(createRandomValue(MAXMATERIALVALUE *
                                    (1 + additionalResourceFreeFieldsPercentageMaterial / 100)));
...
MapField* ResourcePlacement::calculateCornerPoint(MapField* a, MapField* b,
                                                  MapField* diamondPoint) {
   int x = 0;
   int y = 0;
   if ((a->getx() == b->getx())) {
      x = a->getx();
...
void ResourcePlacement::step(Rect r) {
   ...
   if (!((f->getx() - r.a->getx() < 2) && (diamondPoint->gety() - f->gety() < 2))) {
      // cout << "r.a: " << r.a->getx() << "," << r.a->gety() << " f: " << f->getx() << "," <<
      // f->gety() << " diamondPoint: " << diamondPoint->getx() << "," << diamondPoint->gety() <<
      // "i: " << i->getx() << "," << i->gety() << endl;
      step(r1);
   }
   if (!((r.b->getx() - f->getx() < 2) && (diamondPoint->gety() - r.b->gety() < 2))) {
      // cout << " f: " << f << "r.b: " << r.b << "g: " << g  <<" diamondPoint: " << diamondPoint <<
      // endl;
      step(r2);
   }
   if (!((g->getx() - diamondPoint->getx() < 2) && (r.c->gety() - g->gety() < 2))) {
      // cout << "g: " << g->getx() << "," << g->gety()<< " diamond: " << diamondPoint->getx()<< ","
      // << diamondPoint->gety() << " c: " << r.c->getx()<< "," << r.c->gety() << " h: " <<
      // h->getx() << "," << h->gety() << endl;
      step(r3);
   }
...
MapField* ResourcePlacement::calculateDiamondPoint(MapField* a, MapField* b, MapField* c,
                                                   MapField* d) {
   int x = 0;
   int y = 0;
...
int ResourcePlacement::calculateCornerValueFuel(MapField* a, MapField* b, MapField* c) {
   int value =
      (a->getTemp3() + b->getTemp3() + c->getTemp3()) / 3 +
      createRandomValue(static_cast<int>(calculateCurrentOffset(maxFuelOffset) * fuelRoughness)) *
         createAlgebraicSign();
...
int ResourcePlacement::calculateDiamondValueFuel(MapField* a, MapField* b, MapField* c,
                                                 MapField* d) {
   int value = static_cast<int>(
      (a->getTemp3() + b->getTemp3() + c->getTemp3() + d->getTemp3()) / 4 +
      createRandomValue(static_cast<int>(calculateCurrentOffset(maxFuelOffset) * fuelRoughness)) *
         createAlgebraicSign());
   if (value > MAXFUELVALUE * (1 + additionalResourceFreeFieldsPercentageFuel / 100)) {
      value = MAXFUELVALUE * (1 + additionalResourceFreeFieldsPercentageFuel / 100);
...
int ResourcePlacement::calculateCornerValueMaterial(MapField* a, MapField* b, MapField* c) {
   int value = (a->getTemp4() + b->getTemp4() + c->getTemp4()) / 3 +
               createRandomValue(
                  static_cast<int>(calculateCurrentOffset(maxMaterialOffset) * materialRoughness) *
                  createAlgebraicSign());
...
int ResourcePlacement::calculateDiamondValueMaterial(MapField* a, MapField* b, MapField* c,
                                                     MapField* d) {
   int value = (a->getTemp4() + b->getTemp4() + c->getTemp4() + d->getTemp4()) / 4 +
               createRandomValue(
                  static_cast<int>(calculateCurrentOffset(maxMaterialOffset) * materialRoughness)) *
                  createAlgebraicSign();
   if (value > MAXMATERIALVALUE * (1 + additionalResourceFreeFieldsPercentageMaterial / 100)) {
...
void ResourcePlacement::setFieldValueMaterial(MapField* f) {
   int value =
      f->getTemp4() - (MAXMATERIALVALUE * additionalResourceFreeFieldsPercentageMaterial / 100);
