...
const char* diplomaticStateNames[diplomaticStateNum + 1] = {"War",         "Truce",    "Peace",
                                                            "Association", "Alliance", NULL};

DiplomaticStateVector::DiplomaticStateVector(Player& _player) : player(_player) {}

void DiplomaticStateVector::turnBegins() {
   for (QueuedStateChanges::iterator i = queuedStateChanges.begin(); i != queuedStateChanges.end();
        ++i) {
      if (getState(i->first) > i->second) {
         // we are changing to a more aggressive state, which doesn't need the confirmation of the
         // counterpart
         setState(i->first, i->second);
         player.getParentMap()->player[i->first].diplomacy.setState(player.getPosition(),
                                                                    i->second);
      } else {
         // we are browsing through the queued changes of the target player to look if he accepted
         // our proposal
...
void DiplomaticStateVector::write(tnstream& stream) const {
   ...
   for (QueuedStateChanges::const_iterator i = queuedStateChanges.begin();
        i != queuedStateChanges.end(); ++i) {
      stream.writeInt(i->first);
      stream.writeInt(int(i->second));
   }
}

const char* Player ::playerStatusNames[6] = {"Human Player", "AI player", "off",
                                             "Supervisor",   "suspended", NULL};
...
void Player ::setParentMap(GameMap* map, int pos) {
   ...
   parentMap->sigPlayerUserInteractionBegins.connect(
      sigc::mem_fun(*this, &Player::userInteractionBegins));
...
DI_Color Player ::getColor() const {
   if (color == DI_Color(0)) {
      switch (player) {
         case 0:
            return DI_Color(0xe0, 0, 0);
         case 1:
            return DI_Color(0, 0x71, 0xdb);
         case 2:
            return DI_Color(0xbc, 0xb3, 0);
         case 3:
            return DI_Color(0, 0xaa, 0);
         case 4:
            return DI_Color(0xbc, 0, 0);
         case 5:
            return DI_Color(0xb2, 0, 0xb2);
         case 6:
            return DI_Color(0, 0, 0xaa);
         case 7:
            return DI_Color(0xbc, 0x67, 0);
         case 8:
            return DI_Color(0xaa, 0xaa, 0xaa);
      };
...
void Player::swap(Player& secondPlayer) {
   ...
   for (int i = 0; i < parentMap->xsize * parentMap->ysize; i++) {
      ...
      for (MapField::MineContainer::iterator i = fld->mines.begin(); i != fld->mines.end(); i++)
         ...
         else if (i->player == secondPlayer.player)
            i->player = player;
      if (fld->resourceview) {
         ...
         if (fld->resourceview->visible & (1 << secondPlayer.player))
            fld->resourceview->setview(player,
                                       fld->resourceview->materialvisible[secondPlayer.player],
                                       fld->resourceview->fuelvisible[secondPlayer.player]);
         else
   ...
   for (int i = 0; i < parentMap->getPlayerCount(); ++i)
      if (i != a) {
         ...
         swapData(parentMap->tribute.payStatusLastTurn[i][a],
                  parentMap->tribute.payStatusLastTurn[i][b]);
         swapData(parentMap->tribute.payStatusLastTurn[a][i],
                  parentMap->tribute.payStatusLastTurn[b][i]);
      }
}
...
void DiplomaticStateVector::swap(int secondPlayer) {
   ...
   for (int i = 0; i < player.getParentMap()->getPlayerCount(); ++i) {
      ...
      DiplomaticStates fir =
         player.getParentMap()->getPlayer(i).diplomacy.getState(player.getPosition());
