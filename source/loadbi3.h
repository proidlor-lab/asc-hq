...
/** Imports a Battle Isle map
  ...
  \param errorOutput Pointer to a string which will store all warning and error messages from the
  import. May be NULL. \param __fakeMap If true ASC tries to make the map look like the Battle Isle
  map, generating new ObjectType and TerrainType instances if necessary. This must not be used by
  any program other than map2pcx, since the maps can not be played. This is only used to generate
                   the images of the Battle Isle Map Archive.
*/
extern void importbattleislemap(const ASCString& path, const ASCString& mapfilename,
                                TerrainType::Weather* trrn, const ASCString& importTable,
                                ASCString* errorOutput = NULL, bool __fakeMap = false);
...
*/
extern void insertbattleislemap(int x, int y, const ASCString& path, const ASCString& mapfilename,
                                const ASCString& importTable);

//! returns the path to the BI3 installation
extern ASCString getbi3path();

//! to prevent the appearnce of the dialog "choose import translation table" when running in batch
//! mode, a default translation table can be specified
