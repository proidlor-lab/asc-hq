...
void MemoryMeasurement::measure(GameMap* gamemap) {
   ...
   for (int i = 0; i < gamemap->getPlayerCount(); ++i) {
      ...
      for (Player::BuildingList::iterator i = p.buildingList.begin(); i != p.buildingList.end();
           ++i)
...
ASCString MemoryMeasurement::Node::prettyPrint(int indent) {
   ...
   result +=
      "#indent=" + ASCString::toString(indent * 20) + "," + ASCString::toString(indent * 20) + "#";
   result += name + ": " + ASCString::toString(usage / 1024) + " / " +
             ASCString::toString(sum() / 1024) + "\n";
   for (deallocating_vector<Node*>::iterator i = childs.begin(); i != childs.end(); ++i)
...
MemoryMeasurement::Node* MemoryMeasurement::measure(ItemRepository<T>& repository) {
   ...
   node->usage +=
      for_each(repository.getVector().begin(), repository.getVector().end(), MemorySum<T>()).size;
...
struct ObjectStorage : public unary_function<T, void> {
   ...
   ASCString print() {
      ...
      for (Storage::iterator i = storage.begin(); i != storage.end(); ++i) {
         ...
         s += ASCString::toString(i->second / 1024) + " " + o->name +
              "(ID: " + ASCString::toString(i->first) + "; " + o->filename + ")\n";
      }
...
ASCString MemoryMeasurement::getResult() {
   ...
   // for_each ( objectTypeRepository.getVector().begin(), objectTypeRepository.getVector().end(),
   // ObjectStorage<ObjectType*>() );
   result += for_each(objectTypeRepository.getVector().begin(),
                      objectTypeRepository.getVector().end(), ObjectStorage<ObjectType*>())
                .print();
   return result;
}
