...
void checkbi3dir(void) {
   ...
   do {
      ...
      for (int i = 0; i < libs_to_load; i++) {
         ...
         if (!exist(filename)) {
            filename = CGameOptions::Instance()->BI3directory + "LIB" + pathdelimitterstring +
                       LIBFiles[i].Name;
            if (!exist(filename)) {
...
class Bi3MapTranslationTable {
  public:
   //! The first entry is the picture number that is going to be replaced by the picture number of
   //! the second entry.
   ...
   //! This is a special translation for the fields that must be translated to a terrain AND an
   //! additional object
   ...
   /** These BI object pictures can be translated to up to four ASC objects. The first entry is
      again the BI picture number, the following four the pictures for the ASC objects. A -1 is used
      if the entry is not used. It does not matter if you use " -1, 1500, -1  , -1 " or " 1500, -1
      , -1,  -1" or any other order. */
...
void Bi3MapTranslationTable ::runTextIO(PropertyContainer& pc) {
   ...
   for (int i = 0; i < _terraintranslation.size() / 2; i++)
      terraintranslation.push_back(
         make_pair(_terraintranslation[i * 2], _terraintranslation[i * 2 + 1]));
   ...
   for (int i = 0; i < _object2IDtranslation.size() / 2; i++)
      object2IDtranslate.push_back(
         make_pair(_object2IDtranslation[i * 2], _object2IDtranslation[i * 2 + 1]));
}

PointerVector<Bi3MapTranslationTable*> bi3ImportTables;

Bi3MapTranslationTable* findTable(const ASCString& filename) {
   for (PointerVector<Bi3MapTranslationTable*>::iterator i = bi3ImportTables.begin();
        i != bi3ImportTables.end(); ++i)
...
class tloadBImap {
   ...
   struct TMISSPart {
      ...
      Uint8 Recs2Win;  // { Wieviel Gewinnrecords aus map erf llt werden m ssen, damit man gewinnt
                       // (f r jeden player gleich) }
      Uint8 Stuff2[3];
      Uint8 Landscape;
  ...
  public:
   void LoadFromFile(const char* path, const char* AFileName, TerrainType::Weather* trrn,
                     string* errorOutput);
...
int translateunits[bi3unitnum][2] = {
   {1201, 26}, {1270, 26}, {1202, 13}, {1200, 6},  {1203, -1}, {1204, 16}, {1205, 17}, {1206, 4},
   {1207, 65}, {1208, -1}, {1209, 7},  {1210, 66}, {1211, 2},  {1212, 3},  {1213, 17}, {1214, 51},
   {1215, 17}, {1216, 2},  {1217, 10}, {1218, 11}, {1219, 16}, {1220, 61}, {1221, 49}, {1222, 56},
   {1276, -1}, {1223, 25}, {1224, 72}, {1225, 57}, {1226, 58}, {1227, 7},  {1228, 59}, {1241, 9},
   {1242, 22}, {1244, 19}, {1243, 29}, {1245, 55}, {1246, 19}, {1271, 21}, {1247, 52}, {1248, 31},
   {1251, 15}, {1252, 15}, {1256, 39}, {1257, 42}, {1258, 47}, {1259, 14}, {1260, 36}, {1261, 34},
   {1262, 47}, {1263, 37}, {1264, 38}, {1265, 5},  {1229, 18}, {1277, -1}, {1249, -1}, {1230, 1},
   {1231, 66}, {1266, 5},  {1232, 64}, {1233, -1}, {1234, 4},  {1235, 4},  {1267, 35}, {1250, 29}};
...
void tloadBImap ::ReadACTNPart(void) {
   ...
   for (int i = 0; i < 6; i++)
      if (OrgMissRec.WhoPlays & (1 << i))
         actmap->player[convcol(1 << i)].stat =
            Player::PlayerStatus((OrgMissRec.PlayType >> i) & 1);
      else
      ...
      for (X = 0; X < Size.X; X++) {
         ...
         for (int i = 0; i < translationTable->terrain2idTranslation.size(); i++) {
            if (Line[X] == translationTable->terrain2idTranslation[i].BIpic) {
               pterraintype trrn = terrainTypeRepository.getObject_byID(
                  translationTable->terrain2idTranslation[i].terrainid);
         ...
         if (!found)
            for (int j = 0; j < translationTable->terraincombixlat.size(); j++)
               if (Line[X] == translationTable->terraincombixlat[j].bigraph) {
                  pterraintype trrn = terrainTypeRepository.getObject_byID(
                     translationTable->terraincombixlat[j].terrainid);
                  if (trrn) {
                     fld->typ = trrn->weather[translationTable->terraincombixlat[j].terrainweather];
                     ObjectType* obj = NULL;
                     if (translationTable->terraincombixlat[j].objectid > 0)
                        obj = objectTypeRepository.getObject_byID(
                           translationTable->terraincombixlat[j].objectid);
                     if (obj)
            ...
            for (int pass = 0; pass < 2 && !found_without_force; pass++) {
               for (int i = 0; i < translationTable->object2IDtranslate.size(); i++)
                  if (xlt[m] == translationTable->object2IDtranslate[i].first) {
                     ObjectType* obj = objectTypeRepository.getObject_byID(
                        translationTable->object2IDtranslate[i].second);
                     if (obj) {
                        MapField* fld = getfield(newx, newy);
                        if (pass == 1 || obj->getFieldModification(fld->getWeather())
                                            .terrainaccess.accessible(fld->bdt)) {
                     ...
                     if (obj)
                        for (int ww = 0; ww < cwettertypennum; ww++)
                           if (obj->weather.test(ww))
                              for (unsigned int j = 0; j < obj->weatherPicture[ww].images.size();
                                   j++)
                                 if (obj->weatherPicture[ww].bi3pic[j] == xlt[m] && !(found & 2) &&
                                     !(GraphicSetManager::Instance().getMode(xlt[m]) & 256)) {
                                    MapField* fld = getfield(newx, newy);
                                    if (pass == 1 || obj->getFieldModification(fld->getWeather())
                                                        .terrainaccess.accessible(fld->bdt)) {
                                       fld->addobject(obj, 0, true);
...
void tloadBImap ::ReadSHOPPart(void) {
                           ...
                           for (int m = 0; m < 4; m++)
                              for (int n = 0; n < 6; n++)
                                 if (bld->fieldExists(BuildingType::LocalCoordinate(m, n))) {
                                    MapCoordinate pos =
                                       bld->getFieldCoordinate(MapCoordinate(newx, newy),
                                                               BuildingType::LocalCoordinate(m, n));
                                    ...
                                    if (!fld2)
                                       ...
                                       if (bld->terrainaccess.accessible(fld2->bdt) > 0)
                                          terrainmatch++;
                                       if (bld->getBIPicture(BuildingType::LocalCoordinate(m, n), w,
                                                             p) == fld2->getTempw())
                                          objmatch++;
                                    }
                                    cnt++;
            ...
            if (bldlistnum) {
               for (int j = 0; j < bldlistnum - 1;)
                  if ((bldlist[j].objectmatch < bldlist[j + 1].objectmatch) ||
                      (bldlist[j].objectmatch == bldlist[j + 1].objectmatch &&
                       bldlist[j].terrainmatch < bldlist[j + 1].terrainmatch)) {
                     blds tmp = bldlist[j];
                     ...
                     for (int p = 0; p < maxbuildingpicnum; p++)
                        if (!found) {
                           ...
                           for (int m = 0; m < 4; m++)
                              for (int n = 0; n < 6; n++)
                                 if (bld->fieldExists(BuildingType::LocalCoordinate(m, n)))
                                    if (bld->getPicture(BuildingType::LocalCoordinate(m, n), w, p)
                                           .valid()) {
                                       MapCoordinate pos = bld->getFieldCoordinate(
                                          MapCoordinate(newx, newy),
                                          BuildingType::LocalCoordinate(m, n));
                                       MapField* fld2 = getfield(pos.x, pos.y);
                                       if (fld2->getTempw() !=
                                           bld->getBIPicture(BuildingType::LocalCoordinate(m, n), w,
                                                             p))
                                          match = 0;
                                    }
                           if (match)
               ...
               if (found) {
                  putbuilding(actmap, MapCoordinate(xoffset + newx, yoffset + newy), 0,
                              bldlist[actpos].bld, bldlist[actpos].bld->construction_steps - 1, 1);
                  if (fld->building) {
                     for (int m = 0; m < 4; m++)
                        for (int n = 0; n < 6; n++)
                           if (fld->building->getPicture(BuildingType::LocalCoordinate(m, n))
                                  .valid()) {
                              fld->building->getField(BuildingType::LocalCoordinate(m, n))
                                 ->setTemp3(0);
                           }
            ...
            if (fld->building) {
               ...
               if (found == 254) {
                  ...
                  if (firstmissingbuilding) {
                     strcat(missing,
                            "The buildings at the following coordinates could not be found:\n ");
   ...
   //   if ( TPWMtextfile )
   //      strcat ( missing, "\nThe names of the buildings could not be read because the text file
   //      is TPWM encoded. Please decode it first if you want to keep the names of the shops !\n");
   ...
   if (strncmp(buf, "TPWM", 4) == 0) {
      ...
      while (outptr < txtsize) {
         ...
         if (fread(&code, 1, 1, fp) != 1)
            throw treadafterend(filename);
         Uint8 bitsset = ((code >> 7) & 1) + ((code >> 6) & 1) + ((code >> 5) & 1) +
                         ((code >> 4) & 1) + ((code >> 3) & 1) + ((code >> 2) & 1) +
                         ((code >> 1) & 1) + ((code >> 0) & 1);
...
void tloadBImap ::LoadFromFile(const char* path, const char* AFileName, TerrainType::Weather* trrn,
                               string* errorOutput) {
...
void importbattleislemap(const ASCString& path, const ASCString& mapfilename,
                         TerrainType::Weather* trrn, const ASCString& importTable,
                         ASCString* errorOutput, bool fakemap) {
...
void insertbattleislemap(int x, int y, const ASCString& path, const ASCString& mapfilename,
