...
WeatherDialog::WeatherDialog()
   : ASC_PG_Dialog(NULL, PG_Rect(100, 50, xsize, ysize), "Weather Configuration", SHOW_CLOSE) {
   // Random Mode
   randomMode =
      new PG_CheckButton(this,
                         PG_Rect(GuiDimension::getLeftIndent(), GuiDimension::getTopOffSet(),
                                 xsize / 3, GetTextHeight() * 2),
                         "Random Mode");
   ...
   int seedModeYPos =
      ScreenToClient(0, randomMode->y).y + GetTextHeight() + GuiDimension::getTopOffSet();
   seedMode = new PG_CheckButton(
      this, PG_Rect(GuiDimension::getLeftIndent(), seedModeYPos, xsize / 3, GetTextHeight() * 2),
      "Seed is Set");
   ...
   defaultWeatherLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftOffSet(), defaultWeatherYPos, 10, GetTextHeight() * 2),
      "Default Weather");
   defaultWeatherLabel->SetSizeByText();
   weatherTypes = new PG_DropDown(
      this, PG_Rect(xsize - GuiDimension::getLineEditWidth() * 5, defaultWeatherYPos,
                    GuiDimension::getLineEditWidth() * 4, GetTextHeight() * 2));
   ...
   areaSpawnsLabel = new PG_Label(
      this,
      PG_Rect(GuiDimension::getLeftIndent(), areaSpawnsLabelYPos, xsize / 3, GetTextHeight() * 2),
      "Amount of Spawns:");
   areaSpawnsLabel->SetSizeByText();

   areaSpawnsValue =
      new PG_LineEdit(this, PG_Rect(valueXPos, areaSpawnsLabelYPos,
   ...
   nthTurnLabel = new PG_Label(
      this,
      PG_Rect(GuiDimension::getLeftIndent(), nthTurnLabelYPos, xsize / 3, GetTextHeight() * 2),
      "Weather changes each nth Turn:");
   nthTurnLabel->SetSizeByText();
   nthTurnValue =
      new PG_LineEdit(this, PG_Rect(valueXPos, nthTurnLabelYPos, GuiDimension::getLineEditWidth(),
                                    GetTextHeight() * 2));
   ...
   windSpeedFieldRatioLabel =
      new PG_Label(this,
                   PG_Rect(GuiDimension::getLeftIndent(), windSpeedFieldRatioLabelYPos, xsize / 3,
                           GetTextHeight() * 2),
                   "WindSpeed to Fields Ratio:");
   windSpeedFieldRatioLabel->SetSizeByText();
   windSpeedFieldRatioValue =
      new PG_LineEdit(this, PG_Rect(valueXPos, windSpeedFieldRatioLabelYPos,
                                    GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   windSpeedFieldRatioValue->SetText(
      ASCString::toString(actmap->weatherSystem->getWindspeed2FieldRatio()));

   // LowerSize
   int lowerSizeLimitsYPos = windSpeedFieldRatioLabelYPos + GetTextHeight() * 2;
   lowerSizeLimitsLabel = new PG_Label(
      this,
      PG_Rect(GuiDimension::getLeftIndent(), lowerSizeLimitsYPos, xsize / 3, GetTextHeight() * 2),
      "Lower Area Size rel. to Mapsize:");
   lowerSizeLimitsLabel->SetSizeByText();
   lowerSizeLimitsValue =
      new PG_LineEdit(this, PG_Rect(valueXPos, lowerSizeLimitsYPos,
                                    GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   ...
   upperSizeLimitsLabel = new PG_Label(
      this,
      PG_Rect(GuiDimension::getLeftIndent(), upperSizeLimitsYPos, xsize / 3, GetTextHeight() * 2),
      "Upper Area Size rel. to Mapsize:");
   upperSizeLimitsLabel->SetSizeByText();
   upperSizeLimitsValue =
      new PG_LineEdit(this, PG_Rect(valueXPos, upperSizeLimitsYPos,
                                    GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   ...
   lowerDurationLimitLabel =
      new PG_Label(this,
                   PG_Rect(GuiDimension::getLeftIndent(), lowerDurationLimitYPos, xsize / 3,
                           GetTextHeight() * 2),
                   "Lower Duration Limit:");
   lowerDurationLimitLabel->SetSizeByText();
   lowerDurationLimitValue =
      new PG_LineEdit(this, PG_Rect(valueXPos, lowerDurationLimitYPos,
                                    GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   lowerDurationLimitValue->SetText(
      ASCString::toString(actmap->weatherSystem->getLowerDurationLimit()));

   // Upper Duration Limit
   int upperDurationLimitYPos = lowerDurationLimitYPos + GetTextHeight() * 2;
   upperDurationLimitLabel =
      new PG_Label(this,
                   PG_Rect(GuiDimension::getLeftIndent(), upperDurationLimitYPos, xsize / 3,
                           GetTextHeight() * 2),
                   "Upper Duration Limit:");
   upperDurationLimitLabel->SetSizeByText();
   upperDurationLimitValue =
      new PG_LineEdit(this, PG_Rect(valueXPos, upperDurationLimitYPos,
                                    GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   upperDurationLimitValue->SetText(
      ASCString::toString(actmap->weatherSystem->getUpperDurationLimit()));
   // FallOut
   int fallOutYPos = upperDurationLimitYPos + GetTextHeight() * 2 + GuiDimension::getTopOffSet();
   fallOutLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftIndent(), fallOutYPos, xsize / 3, GetTextHeight() * 2),
      "Fallout Chances:");
   fallOutLabel->SetSizeByText();

   fallOutButton = new PG_Button(
      this, PG_Rect(valueXPos, fallOutYPos, (Width() - valueXPos) / 2, 30), "Edit", 100);
   ...
   windSpeedLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftIndent(), windSpeedYPos, xsize / 3, GetTextHeight() * 2),
      "WindSpeed Chances:");
   windSpeedLabel->SetSizeByText();

   windSpeedButton = new PG_Button(
      this, PG_Rect(valueXPos, windSpeedYPos, (Width() - valueXPos) / 2, 30), "Edit", 100);
   windSpeedButton->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &WeatherDialog::editWindSpeed)));

   // WindDirection
   int windDirYPos = windSpeedYPos + GetTextHeight() + GuiDimension::getTopOffSet();
   windDirectionLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftIndent(), windDirYPos, xsize / 3, GetTextHeight() * 2),
      "WindDirection Chances:");
   windDirectionLabel->SetSizeByText();

   windDirectionButton = new PG_Button(
      this, PG_Rect(valueXPos, windDirYPos, (Width() - valueXPos) / 2, 30), "Edit", 100);
   windDirectionButton->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &WeatherDialog::editWindDirection)));

   // eventAreas
   int eventAreasYPos = windDirYPos + GetTextHeight() + GuiDimension::getTopOffSet();
   eventAreasLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftIndent(), eventAreasYPos, xsize / 3, GetTextHeight() * 2),
      "Event driven weather:");
   eventAreasLabel->SetSizeByText();
   eventAreasButton = new PG_Button(
      this, PG_Rect(valueXPos, eventAreasYPos, (Width() - valueXPos) / 2, 30), "Edit", 100);
   eventAreasButton->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &WeatherDialog::editEventAreas)));

   // eventWindChanges
   int eventWindChangesYPos = eventAreasYPos + GetTextHeight() + GuiDimension::getTopOffSet();
   eventWindChangesLabel = new PG_Label(
      this,
      PG_Rect(GuiDimension::getLeftIndent(), eventWindChangesYPos, xsize / 3, GetTextHeight() * 2),
      "Event driven wind changes:");
   eventWindChangesLabel->SetSizeByText();
   eventWindChangesButton = new PG_Button(
      this, PG_Rect(valueXPos, eventWindChangesYPos, (Width() - valueXPos) / 2, 30), "Edit", 100);
   eventWindChangesButton->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &WeatherDialog::editEventWindChanges)));

   // Ok and Cancel Button
   (new PG_Button(this, PG_Rect(30, Height() - 40, (Width() - 70) / 2, 30), "OK", 100))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &WeatherDialog::buttonEvent)));
   (new PG_Button(this, PG_Rect(Width() / 2 + 5, Height() - 40, (Width() - 70) / 2, 30), "Cancel",
                  101))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &WeatherDialog::closeWindow)));
...
bool WeatherDialog::buttonEvent(PG_Button* button) {
   ...
   actmap->weatherSystem->setDefaultFallout(
      static_cast<FalloutType>(this->weatherTypes->GetSelectedItemIndex()));
   if (!((nthTurnValue->GetText() == "") || (windSpeedFieldRatioValue->GetText() == "") ||
         (areaSpawnsValue->GetText() == "") || (lowerSizeLimitsValue->GetText() == "") ||
      ...
      actmap->weatherSystem->setRandomSizeBorders(atof(lowerSizeLimitsValue->GetText()),
                                                  atof(upperSizeLimitsValue->GetText()));
   }
...
EventAreasDialog::EventAreasDialog()
   : ASC_PG_Dialog(NULL, PG_Rect(100, 100, xSize, ySize), "Edit eventdriven Areas", SHOW_CLOSE) {
   int xPos1 = 20;
   // EventBox
   ...
   // remove Button
   removeButton =
      new PG_Button(this, PG_Rect(Width() - (xPos1 + 70), yPosButtons, 70, 35), "Remove", 90);
   removeButton->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &EventAreasDialog::buttonRemove)));

   // Ok and Cancel Button
   (new PG_Button(this,
                  PG_Rect((Width() - GuiDimension::getButtonWidth()) / 2, Height() - 40,
                          GuiDimension::getButtonWidth(), 30),
                  "OK", 100))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &EventAreasDialog::buttonEvent)));
   //(new PG_Button(this, PG_Rect(Width()/2+5, Height()-40, (Width()-70)/2, 30), "Cancel",
   //101))->sigClick.connect( sigc::hide(sigc::mem_fun( *this, &EventAreasDialog::closeWindow )));
...
EventWindChangesDialog::EventWindChangesDialog()
   : ASC_PG_Dialog(NULL, PG_Rect(100, 100, xSize, ySize), "Edit eventdriven WindChange",
                   SHOW_CLOSE) {
   int xPos1 = 20;
   // EventBox
   eventList = new PG_ListBox(this, PG_Rect(xPos1, 20, 350, 200));
   ...
   addButton->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &EventWindChangesDialog::buttonAdd)));

   // remove Button
   removeButton =
      new PG_Button(this, PG_Rect(Width() - (xPos1 + 50), yPosButtons, 50, 35), "Remove", 90);
   removeButton->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &EventWindChangesDialog::buttonRemove)));
   ...
   (new PG_Button(this,
                  PG_Rect((Width() - GuiDimension::getButtonWidth()) / 2, Height() - 40,
                          GuiDimension::getButtonWidth(), 30),
                  "OK", 100))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &EventWindChangesDialog::buttonEvent)));
   //(new PG_Button(this, PG_Rect(Width()/2+5, Height()-40, (Width()-70)/2, 30), "Cancel",
   //101))->sigClick.connect( sigc::hide(sigc::mem_fun( *this, &EventWindChangeDialog::closeWindow
   //)));
...
void EventWindChangesDialog::addNewWindInformation(WindInformation* wi) {
   currentList.push_back(wi);
   new PG_ListBoxItem(eventList, 20, wi->getInformation(), NULL, wi);
   actmap->weatherSystem->addGlobalWindChange(wi->getWindData().speed, wi->getWindData().direction,
                                              wi->getTurn());
}
...
AddWindChangeDialog::AddWindChangeDialog(EventWindChangesDialog* ewcd)
   : ASC_PG_Dialog(ewcd, PG_Rect(0, 0, xSize, ySize), "Add Weather Area", SHOW_CLOSE) {
   int valueXPos = static_cast<int>(xSize * 0.75);

   turnLabel = new PG_Label(
      this,
      PG_Rect(GuiDimension::getLeftOffSet(), GuiDimension::getTopOffSet(), 10, GetTextHeight() * 2),
      "Turn");
   turnLabel->SetSizeByText();
   turnValue =
      new PG_LineEdit(this, PG_Rect(valueXPos, GuiDimension::getTopOffSet(),
                                    GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   turnValue->SetValidKeys("1234567890");

   int speedYPos = GuiDimension::getTopOffSet() + GetTextHeight() * 2;
   speedLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftOffSet(), speedYPos, 10, GetTextHeight() * 2), "Speed");
   speedLabel->SetSizeByText();
   speedValue = new PG_LineEdit(
      this, PG_Rect(valueXPos, speedYPos, GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   speedValue->SetValidKeys("1234567890");

   int directionYPos = speedYPos + +GetTextHeight() * 2;
   directionLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftOffSet(), directionYPos, 10, GetTextHeight() * 2),
      "Wind Direction");
   directionLabel->SetSizeByText();
   directionBox =
      new PG_DropDown(this, PG_Rect(valueXPos - valueXPos / 2, directionYPos,
                                    GuiDimension::getLineEditWidth() * 4, GetTextHeight() * 2));
   ...
   // Ok and Cancel Button
   (new PG_Button(this, PG_Rect(30, Height() - 40, (Width() - 70) / 2, 30), "OK", 100))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &AddWindChangeDialog::buttonEvent)));
   (new PG_Button(this, PG_Rect(Width() / 2 + 5, Height() - 40, (Width() - 70) / 2, 30), "Cancel",
                  101))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &AddWindChangeDialog::closeWindow)));
...
bool AddWindChangeDialog::buttonEvent(PG_Button* button) {
   if (!(speedValue->GetText() == "") || (turnValue->GetText() == "")) {
...
//*********************************************************************************************************
WeatherAreaInformation::WeatherAreaInformation(WeatherArea* wa, GameTime t, int d, FalloutType f)
   : weatherArea(wa), time(t), duration(d), fallOut(f) {}
WeatherAreaInformation::WeatherAreaInformation(WeatherArea* wa, GameTime t)
   : weatherArea(wa), time(t) {}
...
AddWeatherAreaDialog::AddWeatherAreaDialog(EventAreasDialog* ead)
   : ASC_PG_Dialog(ead, PG_Rect(0, 0, xSize, ySize), "Add Weather Area", SHOW_CLOSE) {
   int valueXPos = static_cast<int>(xSize * 0.75);
   ...
   durationLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftOffSet(), durationYPos, 10, GetTextHeight() * 2),
      "Duration");
   durationLabel->SetSizeByText();
   durationValue =
      new PG_LineEdit(this, PG_Rect(valueXPos, durationYPos, GuiDimension::getLineEditWidth(),
                                    GetTextHeight() * 2));
   durationValue->SetValidKeys("1234567890");

   int xCoordYPos = durationYPos + +GetTextHeight() * 2;
   xCoordLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftOffSet(), xCoordYPos, 10, GetTextHeight() * 2),
      "xPosition");
   xCoordLabel->SetSizeByText();
   xCoordValue = new PG_LineEdit(
      this, PG_Rect(valueXPos, xCoordYPos, GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   xCoordValue->SetValidKeys("1234567890");

   int yCoordYPos = xCoordYPos + +GetTextHeight() * 2;
   yCoordLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftOffSet(), yCoordYPos, 10, GetTextHeight() * 2),
      "yPosition");
   yCoordLabel->SetSizeByText();
   yCoordValue = new PG_LineEdit(
      this, PG_Rect(valueXPos, yCoordYPos, GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   yCoordValue->SetValidKeys("1234567890");

   int widthYPos = yCoordYPos + +GetTextHeight() * 2;
   widthLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftOffSet(), widthYPos, 10, GetTextHeight() * 2), "Width");
   widthLabel->SetSizeByText();
   widthValue = new PG_LineEdit(
      this, PG_Rect(valueXPos, widthYPos, GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   widthValue->SetValidKeys("1234567890");

   int heightYPos = widthYPos + +GetTextHeight() * 2;
   heightLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftOffSet(), heightYPos, 10, GetTextHeight() * 2), "Height");
   heightLabel->SetSizeByText();
   heightValue = new PG_LineEdit(
      this, PG_Rect(valueXPos, heightYPos, GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   heightValue->SetValidKeys("1234567890");

   int heightClustered = heightYPos + GetTextHeight() * 2;
   clusteredMode =
      new PG_CheckButton(this,
                         PG_Rect(GuiDimension::getLeftIndent(), heightClustered,
                                 GuiDimension::getLineEditWidth(), GetTextHeight() * 2),
                         "Clustered Mode");
   clusteredMode->SetSizeByText();

   int wTypesYPos = heightClustered + GetTextHeight() * 2;
   wTypesLabel = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftOffSet(), wTypesYPos, 10, GetTextHeight() * 2),
      "Weather Type");
   wTypesLabel->SetSizeByText();
   weatherTypes =
      new PG_DropDown(this, PG_Rect(valueXPos - valueXPos / 2, wTypesYPos,
                                    GuiDimension::getLineEditWidth() * 4, GetTextHeight() * 2));
   ...
   // Ok and Cancel Button
   (new PG_Button(this, PG_Rect(30, Height() - 40, (Width() - 70) / 2, 30), "OK", 100))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &AddWeatherAreaDialog::buttonEvent)));
   (new PG_Button(this, PG_Rect(Width() / 2 + 5, Height() - 40, (Width() - 70) / 2, 30), "Cancel",
                  101))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &AddWeatherAreaDialog::closeWindow)));
...
bool AddWeatherAreaDialog::buttonEvent(PG_Button* button) {
   if (!(xCoordValue->GetText() == "") || (yCoordValue->GetText() == "") ||
       (widthValue->GetText() == "") || (heightValue->GetText() == "") ||
       (durationValue->GetText() == "") || (turnValue->GetText() == "")) {
      FalloutType t = static_cast<FalloutType>(weatherTypes->GetSelectedItemIndex());
      WeatherArea* newWeatherArea = new WeatherArea(
         actmap, atoi(xCoordValue->GetText()), atoi(yCoordValue->GetText()),
         atoi(widthValue->GetText()), atoi(heightValue->GetText()), atoi(durationValue->GetText()),
         t, actmap->weatherSystem->createRandomValue(), clusteredMode->GetPressed());
      GameTime time;
...
ChanceSettingsDialog::ChanceSettingsDialog(std::string title)
   : ASC_PG_Dialog(NULL, PG_Rect(100, 100, xsize, ysize), title, SHOW_CLOSE) {}
...
void ChanceSettingsDialog::buildUpForm(const vector<string>& labelVec) {
   ...
   for (int i = 0; i < labelVec.size(); i++) {
      ...
      PG_Label* label = new PG_Label(
         this, PG_Rect(GuiDimension::getLeftIndent(), yPos, xsize / 3, GetTextHeight() * 2),
         labelVec[i]);
      label->SetSizeByText();
      labels.push_back(label);
      PG_LineEdit* value = new PG_LineEdit(
         this, PG_Rect(valueXPos, yPos, GuiDimension::getLineEditWidth(), GetTextHeight() * 2));
   ...
   note = new PG_Label(
      this, PG_Rect(GuiDimension::getLeftIndent(), noteYPos, xsize / 3, GetTextHeight() * 2),
      "The sum of chances must equal 100");
   note->SetSizeByText();
   // Ok and Cancel Button
   (new PG_Button(this, PG_Rect(30, Height() - 40, (Width() - 70) / 2, 30), "OK", 100))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ChanceSettingsDialog::buttonEvent)));
   (new PG_Button(this, PG_Rect(Width() / 2 + 5, Height() - 40, (Width() - 70) / 2, 30), "Cancel",
                  101))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ChanceSettingsDialog::closeWindow)));
...
WindDirectionSettingsDialog::WindDirectionSettingsDialog()
   : ChanceSettingsDialog("Fallout Chances") {
   vector<string> labels;
