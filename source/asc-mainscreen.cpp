...
class UnitMovementRangeLayer : public MapLayer, public sigc::trackable {
   ...
   void markField(const MapCoordinate& pos) { fields[pos] |= 1; }
   ...
   bool addUnit(Vehicle* veh) {
      if (fieldvisiblenow(getfield(veh->xpos, veh->ypos))) {
         ...
         for (AStar3D::VisitedContainer::iterator i = pathfinder.visited.begin();
              i != pathfinder.visited.end(); ++i)
...
class VisibilityLayer : public MapLayer {
   void renderText(const MapRenderer::FieldRenderInfo& fieldInfo, SPoint pos,
...
void VisibilityLayer::paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                                       const SPoint& pos) {
   if (fieldVisibility(fieldInfo.fld, fieldInfo.gamemap->getPlayerView()) >= visible_now) {
      ...
      for (int i = 0; i < fieldInfo.gamemap->getPlayerCount(); ++i)
         if (fieldInfo.gamemap->getPlayer(i).diplomacy.sharesView(
                fieldInfo.gamemap->getPlayerView())) {
      ...
      renderText(fieldInfo, SPoint(pos.x + 10, pos.y + 31),
                 ASCString::toString(fieldInfo.fld->getjamming()) + "T");
#endif
   }
}

void VisibilityLayer::renderText(const MapRenderer::FieldRenderInfo& fieldInfo, SPoint pos,
                                 const ASCString& text) {
   ...
   font->SetColor(0xffffff);
   PG_FontEngine::RenderText(fieldInfo.surface.getBaseSurface(),
                             PG_Rect(pos.x, pos.y, fieldsizex, fieldsizey), pos.x + 1, pos.y + 13,
                             text, font);

   font->SetColor(0);
   PG_FontEngine::RenderText(fieldInfo.surface.getBaseSurface(),
                             PG_Rect(pos.x, pos.y, fieldsizex, fieldsizey), pos.x, pos.y + 12, text,
                             font);
}

ASC_MainScreenWidget::ASC_MainScreenWidget(PG_Application& application)
   : MainScreenWidget(application),
     standardActionsLocked(0),
     guiHost(NULL),
     menu(NULL),
     unitInfoPanel(NULL),
     windInfoPanel(NULL),
   ...
   int counter = 0;
   for (CGameOptions::PanelDataContainer::iterator i = CGameOptions::Instance()->panelData.begin();
        i != CGameOptions::Instance()->panelData.end(); ++i) {
      if (i->second.visible) {
...
void ASC_MainScreenWidget::spawnPanel(Panels panel) {
   ...
   if (panel == ButtonPanel) {
      if (!guiHost || !CGameOptions::Instance()->cacheASCGUI) {
         delete guiHost;
         guiHost =
            new NewGuiHost(this, mapDisplay, PG_Rect(Width() - 170, Height() - 200, 170, 200));
...
// bool Menu::eventKeyDown(const SDL_KeyboardEvent* key)
{
   ...
   if (!mod) {
      switch (key->keysym.sym) {
         ...
         case SDLK_5:
            executeUserAction(ua_showCargoLayer);
            return true;
         ...
         case SDLK_0:
            executeUserAction(ua_visibilityInfo);
            return true;
         ...
         case SDLK_PLUS:
         case SDLK_KP_PLUS:
            executeUserAction(ua_increase_zoom);
            return true;

         case SDLK_MINUS:
         case SDLK_KP_MINUS:
            executeUserAction(ua_decrease_zoom);
            return true;

         case SDLK_F10: {
            PG_ScrollBar* sb = new PG_ScrollBar(NULL, PG_Rect(20, 50, 15, 400),
                                                PG_ScrollBar::VERTICAL, -1, "DLGScrollbar");
            sb->SetPageSize(10);
            sb->SetRange(0, 100);
            sb->Show();
   ...
   if (mod & KMOD_CTRL) {
      switch (key->keysym.sym) {
         case SDLK_0:
            executeUserAction(ua_writescreentopcx);
            return true;

         case SDLK_1:
            executeUserAction(ua_viewUnitinfoOverlay);
            return true;

         case SDLK_2:
            executeUserAction(ua_viewUnitexperienceOverlay);
            return true;
