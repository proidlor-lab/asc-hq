...
bool authenticateUser(GameMap* actmap, bool allowCancel = true, bool lockView = true,
                      bool throwOnFailure = false) {
   for (int p = 0; p < 8; p++)
      actmap->player[p].existanceAtBeginOfTurn =
         actmap->player[p].exist() && actmap->player[p].stat != Player::off;
   ...
   if (humannum > 1) {
      ...
      if ((!actmap->player[actmap->actplayer].passwordcrc.empty() &&
           actmap->player[actmap->actplayer].passwordcrc !=
              CGameOptions::Instance()->getDefaultPassword()) ||
          firstRound) {
         bool stat;
         actmap->setPlayerView(
            actmap->actplayer);  // the idle handler of enterpassword starts generating the overview
                                 // map, so we need to have the correct view prior to enterpassword
         do {
            stat = enterpassword(actmap->player[actmap->actplayer].passwordcrc, specifyPassword,
                                 allowCancel, true, actmap->player[actmap->actplayer].getName());
            if (!stat) {
               if (throwOnFailure) {
                  delete actmap;
                  throw NoMapLoaded();
         ...
         } while (actmap->player[actmap->actplayer].passwordcrc.empty() && stat &&
                  viewtextquery(910, "warning", "~e~nter password",
                                "~c~ontinue without password") == 0); /* enddo */
      } else {
...
void runai(GameMap* actmap, int playerView, MapDisplayInterface* display) {
   ...
   if (!actmap->player[actmap->actplayer].ai) {
      ...
      displayLogMessage(1, ASCString("DEBUG: Creating AI for player ") +
                              ASCString::toString(actmap->actplayer) +
                              ", aiType field = " + ASCString::toString(playerAiType) + "\n");

      AIFactory::AIType aiType = static_cast<AIFactory::AIType>(playerAiType);
      actmap->player[actmap->actplayer].ai = AIFactory::createAI(aiType, actmap, actmap->actplayer);
   }
   ...
   virtual void playerSkipped(Player& player) {
      if (!player.exist() || player.stat == Player::off || player.stat == Player::suspended) {
         ...
         if (player.getParentMap()->replayinfo)
            if (player.getParentMap()->replayinfo->map[i] &&
                player.getParentMap()->replayinfo->guidata[i]) {
...
void iterateToNextPlayer(GameMap* actmap, bool saveNetwork, int lastPlayer, int lastTurn,
   ...
   do {
      ...
      if (actmap->player[nextPlayer].stat == Player::human ||
          actmap->player[nextPlayer].stat == Player::supervisor) {
...
void next_turn(GameMap* gamemap, const NextTurnStrategy& nextTurnStrategy,
               MapDisplayInterface* display, int playerView) {
      ...
      if (currentPlayer.getParentMap()->player[i].exist())
         if (currentPlayer.getParentMap()->actplayer != i)
            if (currentPlayer.getParentMap()->player[i].ASCversion > 0)
               if ((currentPlayer.getParentMap()->player[i].ASCversion & 0xffffff00) >
                   getNumericVersion()) {
                  new Message(
                     ASCString("Player ") + currentPlayer.getParentMap()->player[i].getName() +
                        ...
                        "version of ASC.",
                     currentPlayer.getParentMap(), 1 << currentPlayer.getParentMap()->actplayer);
                  return;
               }
}
...
GameMap* continueNetworkGame(bool mostRecent) {
   ...
   StatusMessageWindowHolder smw =
      MessagingHub::Instance().infoMessageWindow("loading " + filename);
...
GameMap* continueNetworkGame(const ASCString& filename) {
   FileTransfer ft;
   std::unique_ptr<GameMap> newMap(
      mapLoadingExceptionChecker(filename, MapLoadingFunction(&ft, &FileTransfer::loadPBEMFile)));
   if (!newMap.get())
      return NULL;
...
void checkforvictory(GameMap* gamemap, bool hasTurnControl) {
   ...
   if (!gamemap->continueplaying && gamemap->state == GameMap::State::Normal) {
      ...
      if (plnum <= 1) {
         if (gamemap->player[gamemap->actplayer].ai &&
             gamemap->player[gamemap->actplayer].ai->isRunning()) {
