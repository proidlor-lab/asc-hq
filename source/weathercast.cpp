...
WeatherPanel::WeatherPanel(PG_Widget* parent, const PG_Rect& r, const ASCString& panelName_,
                           bool loadTheme)
   : Panel(parent, r, panelName_, loadTheme), counter(0) {
   weatherSystem = actmap->weatherSystem;
   turnLabel = new PG_Label(
      this,
      PG_Rect(Width() - 108, (Height() - GuiDimension::getButtonHeight()) - GetTextHeight() * 2, 10,
              GetTextHeight() * 2),
      "Turn: +0");
   turnLabel->SetSizeByText();
   //    turnLabelWidth = turnLabel->Width();//????
   forward =
      new PG_Button(this, PG_Rect(Width() - 60, Height() - GuiDimension::getButtonHeight(), 55, 35),
                    "forward", 90);
   forward->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &WeatherPanel::buttonForward)));
   back = new PG_Button(
      this, PG_Rect(Width() - (60 + 10) - 60, Height() - GuiDimension::getButtonHeight(), 55, 35),
      "back", 90);
   back->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &WeatherPanel::buttonBack)));

   int windRoseYPos = GuiDimension::getTopOffSet();
   windRoseImage = new PG_Image(this, PG_Point(50, windRoseYPos),
                                IconRepository::getIcon("windrose.png").GetSurface(), false,
                                PG_Draw::BkMode(PG_Draw::TILE), "g");

   int windBarYPos = windRoseYPos + windRoseImage->Height();
   bgw =
      new BarGraphWidget(this, PG_Rect(55, windBarYPos + 5, 50, 12), BarGraphWidget::Direction(0));
   windBar = new PG_Image(this, PG_Point(50, windBarYPos),
                          IconRepository::getIcon("wind_bar.png").GetSurface(), false,
                          PG_Draw::BkMode(PG_Draw::TILE), "g");
   ...
   windspeedLabel = new PG_Label(
      this, PG_Rect(427, windBarYPos + windBar->Height() - 16, 10, GetTextHeight() * 2));
   ...
   if (counter < weatherSystem->getMaxForecast()) {
      ...
      for (int i = 0; i < weatherSystem->getActiveWeatherAreasSize(); i++) {
         ...
         if (wa->getDuration() - counter > 0) {
            ...
            wAccu = updateWindAccu(wAccu, windSpeed, windStack.front().direction,
                                   weatherSystem->getWindspeed2FieldRatio());
   ...
   if (counter > 0) {
      ...
      for (int i = 0; i < weatherSystem->getActiveWeatherAreasSize(); i++) {
         ...
         if (wa->getDuration() - counter > 0) {
            ...
            wAccu = updateWindAccu(wAccu, windSpeed,
                                   static_cast<Direction>((windStack.front().direction + 3) % 6),
                                   weatherSystem->getWindspeed2FieldRatio());
            warea2WindAccu[wa] = wAccu;
         }
      }
...
void WeatherPanel::painter(const PG_Rect& src, const ASCString& name, const PG_Rect& dst) {
   ...
   // generateWeatherMap(actmap->time.turn() + counter);
   {
      ...
      Surface ws =
         IconRepository::getIcon(weathernames[actmap->weatherSystem->getDefaultFalloutType()]);
      ws.strech(60, 25);
...
WindAccu WeatherPanel::updateWindAccu(const WindAccu& ac, unsigned int windspeed,
                                      Direction windDirection, float ratio) {
...
Weathercast::Weathercast(const WeatherSystem& ws)
   : ASC_PG_Dialog(NULL, PG_Rect(100, 100, xSize, ySize), "Weathercast", MODAL),
     weatherSystem(ws),
     currentZoomX(1.0),
     currentZoomY(1.0),
     mapYPos(30),
...
bool Weathercast::mouseClick(SPoint pos) {
   MapCoordinate mc = OverviewMapImage::surface2map(
      SPoint(float(pos.x) / currentZoomX, float(pos.y) / currentZoomX));
...
void Weathercast::painter(const PG_Rect& src, const ASCString& name, const PG_Rect& dst) {
   ...
   if (actmap && actmap->overviewMapHolder) {
      ...
      rectangle<4>(screen, SPoint(dstPoint.x + ul.x, dstPoint.y + ul.y), lr.x - ul.x, lr.y - ul.y,
                   ColorMerger_Set<4>(0xff), ColorMerger_Set<4>(0xff));
   }
}
...
void Weathercast::generateWeatherMap(int turn) {
   for (int i = 0; i < actmap->weatherSystem->getActiveWeatherAreasSize(); i++) {
      ...
      if ((wa->getDuration() > (turn - (actmap->time.turn() + 1))) &&
          (wa->getCenterField()->isOnMap(actmap))) {
...
void Weathercast::paintWeatherArea(const WeatherArea* wa, int vMove, int hMove) {
   ...
   if ((wa->getCenterPos().x + hMove > 0) && (wa->getCenterPos().y + vMove > 0)) {
      ...
      SPoint pixCenter = OverviewMapImage::map2surface(
         MapCoordinate(((wa->getCenterPos().x + hMove) * currentZoomX),
                       (((wa->getCenterPos().y + vMove) * currentZoomY))));
      ...
      blitter.blit(wSurface, screen,
                   SPoint(transformedPoint.x,
                          transformedPoint.y));  // SPoint(transformedPoint.x, transformedPoint.y));
   ...
   /*for(int i = (wa->getCenterPos().x - wa->getWidth()/2); i < wa->getCenterPos().x +
   wa->getWidth(); i++){ for(int j = (wa->getCenterPos().y - wa->getHeight()/2); j <
   wa->getCenterPos().y + wa->getHeight()/2; j++){ SPoint pixCenter =
   OverviewMapImage::map2surface(MapCoordinate(i,j)); OverviewMapImage::fill(s, pixCenter, LRAINOR);
     }
   }*/
}
