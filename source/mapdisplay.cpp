...
class ContainerInfoLayer : public MapLayer {
   ...
   bool hasCargo(const ContainerBase* c) {
      for (ContainerBase::Cargo::const_iterator i = c->getCargo().begin(); i != c->getCargo().end();
           ++i)
         if (*i)
            return true;
      return false;
      ...
      for (ContainerBase::Cargo::const_iterator i = c->getCargo().begin(); i != c->getCargo().end();
         ...
         if (*i) {
            if ((*i)->getOwner() == viewingPlayer)
               return true;
            else if (hasOwnCargo(*i, viewingPlayer))
               return true;
         }
      return false;
   };
...
void ContainerInfoLayer::paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                                          const SPoint& pos) {
   if (fieldInfo.visibility >= visible_ago) {
      if (fieldInfo.fld->vehicle ||
          (fieldInfo.fld->building && fieldInfo.fld->bdt.test(cbbuildingentry))) {
         ContainerBase* c = fieldInfo.fld->getContainer();
         if (hasCargo(c)) {
            if (c->getOwner() == fieldInfo.playerView)
               fieldInfo.surface.Blit(marker, pos);
            else if (hasOwnCargo(c, fieldInfo.playerView))
               fieldInfo.surface.Blit(marker, pos);
         }
      }
   }
...
class ResourceGraphLayer : public MapLayer {
   void paintBar(const MapRenderer::FieldRenderInfo& fieldInfo, const SPoint& pos, int row,
                 int amount, int color) {
      /*
      int length = amount / 10;
      int maxlength = 255/10;
      if ( amount )
         paintFilledRectangle<4>( fieldInfo.surface, SPoint( pos.x + 10, pos.y + 2 + row*12),
      length, 5, ColorMerger_ColoredOverwrite<4>( color )); if ( length < maxlength )
         paintFilledRectangle<4>( fieldInfo.surface, SPoint( pos.x + 10 + length, pos.y + 2 +
      row*12), maxlength-length, 5, ColorMerger_ColoredOverwrite<4>( 0x888888 ));
      */

      int length = amount * 28 / 255;
      int maxlength = 28;
      if (amount)
         paintFilledRectangle<4>(fieldInfo.surface, SPoint(pos.x + 10, pos.y + 12 + row * 15),
                                 length, 8, ColorMerger_ColoredOverwrite<4>(color));
      if (length < maxlength)
         paintFilledRectangle<4>(fieldInfo.surface,
                                 SPoint(pos.x + 10 + length, pos.y + 12 + row * 15),
                                 maxlength - length, 8, ColorMerger_ColoredOverwrite<4>(0));
   };
...
void ResourceGraphLayer::paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                                          const SPoint& pos) {
#ifndef karteneditor
   if (fieldInfo.visibility >= visible_ago) {
      bool visible = false;
      ...
      if (fieldInfo.playerView >= 0)
         if (fieldInfo.fld->resourceview &&
             (fieldInfo.fld->resourceview->visible & (1 << fieldInfo.playerView)))
            visible = true;

      if (visible) {
         if (fieldInfo.playerView >= 0 &&
             fieldInfo.gamemap->getPlayer(fieldInfo.playerView).stat == Player::supervisor) {
            ...
            paintBar(fieldInfo, pos, 0,
                     fieldInfo.fld->resourceview->materialvisible[max(fieldInfo.playerView, 0)],
                     Resources::materialColor);
            paintBar(fieldInfo, pos, 1,
                     fieldInfo.fld->resourceview->fuelvisible[max(fieldInfo.playerView, 0)],
                     Resources::fuelColor);
         }
      }
   }
...
void PipeLayer::paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                                 const SPoint& pos) {
   if (!pipeline)
      return;
      ...
      if (fieldInfo.fld->building) {
         ...
         if (fieldInfo.fld->bdt.test(cbpipeline)) {
            ...
            if (o)
               ...
               bool objfound = false;
               for (MapField::ObjectContainer::iterator i = fieldInfo.fld->objects.begin();
                    i != fieldInfo.fld->objects.end(); ++i)
...
void ReactionFireLayer::paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                                         const SPoint& pos) {
   if (fieldInfo.visibility > visible_ago) {
      if (fieldInfo.fld->vehicle &&
          fieldInfo.fld->vehicle->reactionfire.getStatus() != Vehicle::ReactionFire::off &&
          fieldInfo.fld->vehicle->getOwner() == fieldInfo.playerView) {
...
class UnitInfoLayer : public MapLayer {
   Surface& image;
   void paintBar(const MapRenderer::FieldRenderInfo& fieldInfo, const SPoint& pos, int position,
                 int max, int FromTop, int color, bool OverrideColor) {
      ...
      if (OverrideColor == true) {
         ...
         paintcolor = color;
      paintFilledRectangle<4>(fieldInfo.surface, SPoint(pos.x + 9, pos.y + FromTop), length, 3,
                              ColorMerger_ColoredOverwrite<4>(paintcolor));
   };
...
void UnitInfoLayer::paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                                     const SPoint& pos) {
   if (fieldInfo.visibility > visible_ago) {
      if (fieldInfo.fld->vehicle) {
         if ((fieldInfo.fld->vehicle->getOwner() == fieldInfo.playerView) ||
             (fieldInfo.visibility == visible_all) ||
             ((fieldInfo.fld->vehicle->height >= chschwimmend) &&
              (fieldInfo.fld->vehicle->height <= chhochfliegend))) {
            MegaBlitter<colorDepth, colorDepth, ColorTransform_None, ColorMerger_AlphaMerge>
               blitter;
            // paint the BGimage
            blitter.blit(image, fieldInfo.surface, pos);
            // paint the bars
            ...
            // 2. fuel
            paintBar(fieldInfo, pos, fieldInfo.fld->vehicle->getTank().fuel,
                     fieldInfo.fld->vehicle->getStorageCapacity().fuel, 4, 0xFFB700, false);
         }
      }
   }
}
...
void UnitTrainingLayer::paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                                         const SPoint& pos) {
   if (fieldInfo.visibility > visible_ago) {
      if (fieldInfo.fld->vehicle) {
         if ((fieldInfo.fld->vehicle->getOwner() == fieldInfo.playerView) ||
...
class WeaponRange : public SearchFields {
  public:
   ...
   void testfield(const MapCoordinate& mc) { gamemap->getField(mc)->setTempw(1); };
   ...
   if (fieldvisiblenow(getfield(veh->xpos, veh->ypos)))
      for (int i = 0; i < veh->typ->weapons.count; i++) {
         if (veh->typ->weapons.weapon[i].shootable()) {
            initsearch(veh->getPosition(), veh->typ->weapons.weapon[i].maxdistance / minmalq,
                       (veh->typ->weapons.weapon[i].mindistance + maxmalq - 1) / maxmalq);
            startsearch();
            found++;
         }
      }
...
void MapRenderer::paintUnitOntoField(const MapRenderer::FieldRenderInfo& fieldInfo,
                                     int binaryUnitHeight, const SPoint& pos, Vehicle* vehicle) {
   if (vehicle && (vehicle->height == binaryUnitHeight))
      if ((vehicle->getOwner() == fieldInfo.playerView) || (fieldInfo.visibility == visible_all) ||
          ((vehicle->height >= chschwimmend) && (vehicle->height <= chhochfliegend)))
         vehicle->paint(fieldInfo.surface, pos);
}

void MapRenderer::paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                                   const SPoint& pos) {
   ...
   if (fieldInfo.visibility > visible_ago) {
      /* display buildings */
      if (fld->building && (fld->building->typ->height & binaryUnitHeight) &&
          fld->building->visible)
         if ((fieldInfo.visibility == visible_all) ||
             (fld->building->typ->height >= chschwimmend) ||
             (fld->building->getOwner() == fieldInfo.playerView))
            fld->building->paintSingleField(fieldInfo.surface, pos,
                                            fld->building->getLocalCoordinate(fieldInfo.pos));
   ...
   if (layer & 1)
      for (MapField::ObjectContainer::iterator o = fld->objects.begin(); o != fld->objects.end();
         ...
         if (fieldInfo.visibility > visible_ago ||
             (o->typ->visibleago && fieldInfo.visibility >= visible_ago))
   ...
   if (fieldInfo.visibility > visible_ago) {
      ...
      if (fieldInfo.visibility == visible_all)
         if (!fld->mines.empty() && layer == 7) {
            for (MapField::MineContainer::const_iterator i = fld->mines.begin();
                 i != fld->mines.end(); ++i)
      ...
      if (layer == 18) {
         ...
         else if (fld->getaTemp2() && tempsvisible)
         ...
         /*
                         ...
                         if ( fld->a.temp && tempsvisible )
                            ...
                            if ( fld->a.temp2 && tempsvisible )
                               putspriteimage(  r, yp, xlatpict ( &xlattables.a.dark2 ,
            cursor.markfield));

         */
      } else if (fieldInfo.visibility == visible_not) {
...
void MapRenderer::paintTerrain(Surface& surf, GameMap* actmap, int playerView,
                               const ViewPort& viewPort, const MapCoordinate& offset) {
...
void benchMapDisplay() { /*
                           ...
                           ASCString s;
                           s.format("update map: %d \nupdate widget: %d \n%f fps", t2-t,t3-t2, 20.0
                           / float(t3-t) * 100 ); displaymessage(s, 1 );
                           */
}
...
MapDisplayPG::MapDisplayPG(MainScreenWidget* parent, const PG_Rect r)
   ...
   PG_Application::GetApp()->sigKeyDown.connect(
      sigc::mem_fun(*this, &MapDisplayPG::keyboardHandler));
   ...
   MapRenderer::additionalItemDisplayHook.connect(
      sigc::mem_fun(*this, &MapDisplayPG::displayAddons));
...
void MapDisplayPG::setNewZoom(int zoom) {
   ...
   delete surface;
   surface = new Surface(Surface::createSurface(
      field.numx * fielddistx + 2 * surfaceBorder,
      (field.numy - 1) * fielddisty + fieldysize + 2 * surfaceBorder, colorDepth * 8));
...
class PixSel
   : public SourcePixelSelector_CacheZoom<pixelSize,
...
void MapDisplayPG::blitInternalSurface(SDL_Surface* dest, const SPoint& pnt,
   ...
   if (zoom != 100) {
      ...
      MegaBlitter<colorDepth, colorDepth, ColorTransform_None, ColorMerger_AlphaOverwrite, PixSel,
      ...
      // SourcePixelSelector
      blitter.setSrcRectangle(upperLeftSourceBlitCorner, int(float(Width()) / fzoom),
                              int(float(Height()) / fzoom));
      ...
      MegaBlitter<colorDepth, colorDepth, ColorTransform_None, ColorMerger_AlphaOverwrite,
                  SourcePixelSelector_DirectRectangle, TargetPixelSelector_Rect>
         blitter;
      blitter.initSource(*surface);
...
void MapDisplayPG::displayCursor(const PG_Rect& dst) {
   ...
   if (x >= field.viewPort.x1 && x < field.viewPort.x2 && y >= field.viewPort.y1 &&
       y < field.viewPort.y2 && x >= 0 && y >= 0 && x < actmap->xsize && y < actmap->ysize) {
...
SPoint MapDisplayPG::internal2widget(const SPoint& pos) {
   return SPoint(int(ceil(float(pos.x - surfaceBorder) * zoom / 100.0)),
                 int(ceil(float(pos.y - surfaceBorder) * zoom / 100.0)));
}
...
void filterQueuedZoomEvents() {
   ...
   while (PG_Application::GetEventSupplier()->PeepEvent(&event) &&
          ...
          (event.button.button == CGameOptions::Instance()->mouse.zoomoutbutton ||
           event.button.button == CGameOptions::Instance()->mouse.zoominbutton))
...
bool MapDisplayPG::eventMouseButtonDown(const SDL_MouseButtonEvent* button) {
   ...
   if (button->type == SDL_MOUSEBUTTONDOWN &&
       button->button == CGameOptions::Instance()->mouse.zoomoutbutton) {
      changeZoom(10);
   ...
   if (button->type == SDL_MOUSEBUTTONDOWN &&
       button->button == CGameOptions::Instance()->mouse.zoominbutton) {
      changeZoom(-10);
   ...
   if (button->type == SDL_MOUSEBUTTONDOWN &&
       button->button == CGameOptions::Instance()->mouse.fieldmarkbutton) {
   ...
   if (button->type == SDL_MOUSEBUTTONDOWN &&
       button->button == CGameOptions::Instance()->mouse.centerbutton) {
...
bool MapDisplayPG::eventMouseMotion(const SDL_MouseMotionEvent* button) {
   ...
   if (button->type == SDL_MOUSEMOTION &&
       (button->state == SDL_BUTTON(CGameOptions::Instance()->mouse.fieldmarkbutton))) {
      bool changed = cursor.pos() != mc;
      if (changed) {
         MapCoordinate oldpos = cursor.pos();
         cursor.pos() = mc;
...
bool MapDisplayPG::fieldCompletelyInView(const MapCoordinate& pos) {
   ...
   SPoint s2 = widget2screen(internal2widget(internal + SPoint(fieldsizex, fieldsizey)));
   return (s.y >= my_ypos && s2.y < my_ypos + my_height) &&
          (s.x >= my_xpos && s2.x < my_xpos + my_width);
}

bool MapDisplayPG::fieldInView(const MapCoordinate& mc) {
   if (mc.x < offset.x || mc.y < offset.y || mc.x >= offset.x + field.numx ||
       mc.y >= offset.y + field.numy || !fieldCompletelyInView(mc))
      return false;
   else
      return true;
}
...
Surface MapDisplayPG::createMovementBufferSurface() {
   Surface s = Surface::createSurface(2 * surfaceBorder + effectiveMovementSurfaceWidth,
                                      2 * surfaceBorder + effectiveMovementSurfaceHeight,
                                      8 * colorDepth, 0x00ffffff);
   ...
   if (!movementMask[0].mask.valid())
      for (int dir = 0; dir < sidenum; ++dir) {
         ...
         for (int i = 0; i < sidenum; ++i)
            movementMask[dir].mask.Blit(icons.fieldShape,
                                        getFieldPos2(getNeighbouringFieldCoordinate(start, i)));
         for (int i = 0; i < sidenum; ++i)
            movementMask[dir].mask.Blit(icons.fieldShape,
                                        getFieldPos2(getNeighbouringFieldCoordinate(dest, i)));
         ...
         pix.x += fieldsizex / 2;
         pix.y += fieldsizey / 2;
         movementMask[dir].mask.SetColorKey(SDL_SRCCOLORKEY,
                                            movementMask[dir].mask.GetPixel(pix) &
                                               ~movementMask[dir].mask.GetPixelFormat().Amask());
      }
}
...
class SourcePixelSelector_DirectSubRectangle {
  ...
  protected:
   SourcePixelSelector_DirectSubRectangle()
      : x(0), y(0), x1(0), y1(0), w(0), h(0), outerwidth(0), pointer(NULL), surface(NULL){};
   ...
   int getSourcePixelSkip() {
      ...
      else if (y >= innerRect.y + innerRect.h)
         return outerwidth - (x - x1);
      else if (x < innerRect.x)
         return innerRect.x - x;
      else if (x >= innerRect.x + innerRect.w)
         return outerwidth - (x - x1);
      else
         return 0;
   };
  ...
  public:
   ...
   void setInnerSrcRectangle(const PG_Rect& rect) { innerRect = rect; };
};
...
void MapDisplayPG::displayMovementStep(Movement& movement, int percentage) {
   ...
   for (int pass = 0; pass <= 18; pass++) {
      for (int i = 0; i < movement.actualFieldNum; ++i) {
         ...
         if (fieldRenderInfo.fld) {
            fieldRenderInfo.visibility =
               fieldVisibility(fieldRenderInfo.fld, fieldRenderInfo.playerView);
         ...
         if (!(pass & 1))
            if (movement.veh->height & (1 << ((pass - 2) / 2))) {
               ...
               if (movement.fromShadow >= 0 && movement.toShadow >= 0)
                  shadow = movement.fromShadow +
                           (movement.toShadow - movement.fromShadow) * percentage / 100;
...
#ifdef debugmapdisplay
   MegaBlitter<colorDepth, colorDepth, ColorTransform_None, ColorMerger_AlphaOverwrite,
               SourcePixelSelector_Plain, TargetPixelSelector_Valid>
      alphaBlitter;
   alphaBlitter.blit(*movement.mask, *surface, movement.maskPosition);
#endif

   PG_Rect targetArea(widget2screen(SPoint(0, 0)).x, widget2screen(SPoint(0, 0)).y, Width(),
                      Height());
   Surface s = Surface::Wrap(PG_Application::GetScreen());
   ...
   if (zoom != 100) {
      float fzoom = float(zoom) / 100.0;
      MegaBlitter<colorDepth, colorDepth, ColorTransform_None, ColorMerger_AlphaOverwrite,
                  MovePixSel, TargetPixelSelector_Rect>
         blitter;
      blitter.setZoom(fzoom);
      blitter.initSource(*surface);

      blitter.setSrcRectangle(SPoint(getFieldPosX(0, 0), getFieldPosY(0, 0)),
                              int(float(Width()) / fzoom), int(float(Height()) / fzoom));
      ...
      MegaBlitter<colorDepth, colorDepth, ColorTransform_None, ColorMerger_AlphaOverwrite,
                  SourcePixelSelector_DirectSubRectangle, TargetPixelSelector_Rect>
         blitter;
      blitter.initSource(*surface);
...
#ifdef debugmapdisplay
   rectangle<4>(s, SPoint(targetArea.x, targetArea.y), targetArea.w, targetArea.h,
                ColorMerger_ColoredOverwrite<4>(0x00ffff),
                ColorMerger_ColoredOverwrite<4>(0x00ffff));
   rectangle<4>(s, SPoint(movement.blitViewPortScreen.x, movement.blitViewPortScreen.y),
                movement.blitViewPortScreen.w, movement.blitViewPortScreen.h,
                ColorMerger_ColoredOverwrite<4>(0x0000ff),
                ColorMerger_ColoredOverwrite<4>(0x0000ff));
   PG_Application::UpdateRect(PG_Application::GetScreen(), 0, 0, PG_Application::GetScreen()->w,
                              PG_Application::GetScreen()->h);
#else
   PG_Application::UpdateRect(PG_Application::GetScreen(), movement.blitViewPortScreen.x,
                              movement.blitViewPortScreen.y, movement.blitViewPortScreen.w,
                              movement.blitViewPortScreen.h);
#endif
}
...
void MapDisplayPG::displayUnitMovement(GameMap* actmap, Vehicle* veh, const MapCoordinate3D& from,
                                       const MapCoordinate3D& to, int duration) {
   static int col = 0xff;
   ...
   if (from.x == to.x && from.y == to.y && from.getBitmappedHeight() <= chschwimmend &&
       to.getBitmappedHeight() <= chschwimmend)
   ...
   movement.blitViewPortInternal =
      PG_Rect(p1.x, p1.y, max(movement.from.x, movement.to.x) - p1.x + maxShadowWidth + fieldsizex,
              max(movement.from.y, movement.to.y) - p1.y + maxShadowWidth + fieldsizey);

   movement.blitViewPortInternal =
      movement.blitViewPortInternal.IntersectRect(PG_Rect(0, 0, surface->w(), surface->h()));
   movement.targetBlitPos = widget2screen(SPoint(0, 0));
   SPoint a = widget2screen(
      internal2widget(SPoint(movement.blitViewPortInternal.x, movement.blitViewPortInternal.y)));
   SPoint b = widget2screen(
      internal2widget(SPoint(movement.blitViewPortInternal.x + movement.blitViewPortInternal.w,
                             movement.blitViewPortInternal.y + movement.blitViewPortInternal.h)));

   movement.blitViewPortScreen = PG_Rect(a.x, a.y, b.x - a.x + 1, b.y - a.y + 1);
   movement.blitViewPortScreen = movement.blitViewPortScreen.IntersectRect(
...
#ifdef debugmapdisplay
   cout << (float(loopCounter) / float(ticker - loopStartTicker) * 100) << " / "
        << (float(loopCounter) / float(ticker - startTime) * 100) << " fps \n";
#endif
}
...
void MapDisplayPG::displayAddons(Surface& surf, int pass) {
   if (additionalUnit)
      if (pass == bitmappedHeight2pass(additionalUnit->height)) {
         ...
         if (p.x >= 0 && p.y >= 0 &&
             p.x + fieldsizex + ContainerBase::calcShadowDist(getFirstBit(chsatellit)) < surf.w() &&
             p.y + fieldsizey + ContainerBase::calcShadowDist(getFirstBit(chsatellit)) < surf.h())
...
bool MapDisplayPG::keyboardHandler(PG_MessageObject* o, const SDL_KeyboardEvent* keyEvent) {
   ...
   if (keyEvent->type == SDL_KEYDOWN) {
      if (!disableKeyboardCursorMovement) {
         ...
         if ((keyEvent->keysym.sym == SDLK_UP && keyStates[SDLK_UP]) ||
             (keyEvent->keysym.sym == SDLK_KP8 && keyStates[SDLK_KP8])) {
            moveCursor(0, 1);
            return true;
         }
         if ((keyEvent->keysym.sym == SDLK_DOWN && keyStates[SDLK_DOWN]) ||
             (keyEvent->keysym.sym == SDLK_KP2 && keyStates[SDLK_KP2])) {
            moveCursor(4, 1);
            return true;
         }
