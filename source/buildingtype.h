...
class BuildingType : public ContainerBaseType {
   ...
   */
   ...
   //! when the building is destroyed, it can leave rubble objects behind. If set to 0 no objects
   //! are being created
   DestructionObjects destructionObjects;

   //! the position of the entrance, which is the field of the building where units can enter and
   //! leave
   ...
   //! Each stage has a separate picture. Range is 1 to 8
   int construction_steps;

   int getMoveMalusType() const { return 11; };

   //! bitmapped: units on these levels of height may be refuelled when standing next to the
   //! buildings entry
   int externalloadheight;

   const Surface& getPicture(const LocalCoordinate& localCoordinate, int weather = 0,
                             int constructionStep = 0) const;
   void paint(Surface& s, SPoint pos, const PlayerColor& player, int weather = 0,
              int constructionStep = 0) const;
   void paint(Surface& s, SPoint pos) const;
   void paintSingleField(Surface& s, SPoint pos, const LocalCoordinate& localCoordinate,
                         const PlayerColor& player, int weather = 0,
                         int constructionStep = 0) const;
   void paintSingleField(Surface& s, SPoint pos, const LocalCoordinate& localCoordinate,
                         int weather = 0, int constructionStep = 0) const;

   int getBIPicture(const LocalCoordinate& localCoordinate, int weather = 0,
                    int constructionStep = 0) const;

   //! returns whether this building covers the given field
   bool fieldExists(const LocalCoordinate& localCoordinate) const {
      return field_Exists[localCoordinate.x][localCoordinate.y];
   };
   ...
   /** returns the Mapcoordinate of a buildings field
       ...
       \param localCoordinate The relative coordinate of the buildings segment for which is global
      MapCoordinate is going to be calculated and returned.
   */
   MapCoordinate getFieldCoordinate(const MapCoordinate& entryOnMap,
                                    const LocalCoordinate& localCoordinate) const;

   //! converts a global coordinate into a local coordinate.
   LocalCoordinate getLocalCoordinate(const MapCoordinate& entryOnMap,
