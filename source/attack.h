...
class tfight : public AttackFormula {
  ...
  public:
   ...
   //! Performs the calculation of the attack. The result is only stored in the av and dv structures
   //! and is not written to the map
   void calc(void);
...
class UnitAttacksSomething : public tfight {
  ...
  public:
   Vehicle* getAttackingUnit() { return _attackingunit; };
};
...
class tunitattacksunit : public UnitAttacksSomething {
  ...
  public:
   /*! Calculates the fight if one unit attacks another units.
       ...
       \param weapon  The number of the weapon which the attacking unit attacks with. If it is -1,
      the best weapon is chosen.
   */
   tunitattacksunit(Vehicle*& attackingunit, Vehicle*& attackedunit, bool respond = true,
                    int weapon = -1, bool reactionfire = false);
   void setup(Vehicle*& attackingunit, Vehicle*& attackedunit, bool respond, int weapon);
   void setresult(const Context& context);

   void visit(FightVisitor& visitor) { visitor.visit(*this); };
   Vehicle* getTarget() { return _attackedunit; };
...
class tunitattacksbuilding : public UnitAttacksSomething {
   ...
   */
   ...
   void visit(FightVisitor& visitor) { visitor.visit(*this); };
   Building* getTarget() { return _attackedbuilding; };
};
...
class tmineattacksunit : public tfight {
  ...
  public:
   ...
   /*! Calculates the fight if a unit drives onto a mine.
       ...
       \param minenum The number of a specific mine which explodes. If -1 , all mines on this field
      which are able to attack the unit will explode. \param attackedunit The unit which moved onto
      the minefield.
   */
 ...
 */
extern bool attackpossible2u(const Vehicle* attacker, const Vehicle* target,
                             AttackWeap* attackweap = NULL,
                             int targetheight = -1);  // distance is not evaluated
...
/*! \brief Is attacker able to attack target ? Distance is assumed one field.
     ...
     \param targetHeight if != -1 , assume the target unit was on this height (bitmapped!)
*/
extern bool attackpossible28(const Vehicle* attacker, const Vehicle* target,
                             AttackWeap* attackweap = NULL,
                             int targetHeight = -1);  // distance is fixed as 1 field
...
/*! \brief Is attacker able to attack target ? Actual distance used.
     ...
     \param attackweap if != NULL, detailed information about the weapons which can perform
                          the attack are written to attackweap
*/
extern bool attackpossible2n(const Vehicle* attacker, const Vehicle* target,
                             AttackWeap* attackweap = NULL);
...
class WeapDist {
  public:
   static float getWeaponStrength(const SingleWeapon* weap, int targetFieldWeather = 0,
                                  int dist = -1, int attacker_height = -1, int defender_height = -1,
                                  int reldiff = -1);
};
