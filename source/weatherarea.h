...
/**
@brief A Vector2D is a line (without a defined start point) in a 2 dimensional space and is
deccribed by its x- and y component
...
class WeatherArea {
  ...
  public:
   WeatherArea(GameMap* map, int xCenter, int yCenter, int width, int height, int duration,
               FalloutType fType, unsigned int seedValue, bool clustered = true);
   ...
   MapCoordinate getCenterPos() const { return center; }
...
class WeatherSystem : public sigc::trackable {
  ...
  public:
   ...
   // Once the read order of GameMap is changed and fields + weather are read inside of
   // gamemap::read get rid of this

   static int legacyWindSpeed;
   static int legacyWindDirection;

   WeatherSystem(GameMap* map);
   WeatherSystem(GameMap* map, int areaSpawns, float windspeed2FieldRatio,
                 unsigned int timeInterval = 6, WeatherSystemMode mode = EVENTMODE,
   ...
   FalloutType getDefaultFalloutType() const { return defaultFallout; };

   inline int getCurrentWindSpeed() const { return windspeed; }

   inline Direction getCurrentWindDirection() const { return globalWindDirection; }

   int getMaxForecast() const;

   bool isSeedValueSet() { return seedValueIsSet; }
   inline int getSpawnsAmount() { return areaSpawnAmount; }

   inline void setEventMode(const WeatherSystemMode& mode) { currentMode = mode; }

   inline int getTimeInterval() { return timeInterval; }

   inline void setTimeInterval(int i) { timeInterval = i; }

   inline void setWindSpeed2FieldRatio(float ratio) { windspeed2FieldRatio = ratio; }

   inline void setSpawnAmount(int a) { areaSpawnAmount = a; }

   void setLowerDurationLimit(int udl) { lowerRandomDuration = udl; }

   void setUpperDurationLimit(int ldl) { upperRandomDuration = ldl; }

   inline const WeatherSystemMode getEventMode() const { return currentMode; }
   inline float getWindspeed2FieldRatio() const { return windspeed2FieldRatio; }
   inline const int getFalloutPercentage(int i) const { return falloutPercentages[i]; }

   inline const int getWindSpeedPercentage(int i) const { return windSpeedPercentages[i]; }

   inline const int getWindDirPercentage(int i) const { return windDirPercentages[i]; }

   inline const float getLowerSizeLimit() const { return lowerRandomSize; }

   inline const float getUpperSizeLimit() const { return upperRandomSize; }

   const int getLowerDurationLimit() const { return lowerRandomDuration; }

   const int getUpperDurationLimit() const { return upperRandomDuration; }
