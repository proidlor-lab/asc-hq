...
void process_BI_map(const ASCString& filename) {
   ...
   while (!s.empty()) {
      string errormsg;
      importbattleislemap(outputDir.c_str(), s.c_str(),
                          terrainTypeRepository.getObject_byPos(0)->weather[0], &errormsg, true);
      if (!errormsg.empty() && 0)
      ...
      printf("%sµ%sµ%dµ%dµ%dµ%dµ%dµ%d\n", s.c_str(), actmap->maptitle.c_str(), actmap->xsize * 2,
             actmap->ysize / 2, width, height, playernum, ainum);
...
void process_ASC_map(const ASCString& filename) {
   ...
   while (!s.empty()) {
      ...
      printf("%sµ%sµ%dµ%dµ%dµ%dµ%dµ%dµ%sµ%sµ%sµ%sµ%dµ%s\n", s.c_str(), actmap->maptitle.c_str(),
             actmap->xsize, actmap->ysize, width, height, playernum, ainum,
             actmap->archivalInformation.author.c_str(),
             actmap->archivalInformation.description.c_str(),
             actmap->archivalInformation.requirements.c_str(),
             actmap->archivalInformation.tags.c_str(), actmap->_resourcemode, datebuf);
