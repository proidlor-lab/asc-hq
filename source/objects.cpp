...
const char* MineNames[cminenum] = {"antipersonnel mine", "antitank mine", "antisub mine",
                                   "antiship mine"};
...
void MineType::paint(MineTypes type, int player, Surface& surf, const SPoint& pos) {
   ...
   if (!images[type]) {
      switch (type) {
         case cmantipersonnelmine:
            images[type] = &IconRepository::getIcon("antipersonellmine.png");
            break;
         case cmantitankmine:
            images[type] = &IconRepository::getIcon("antitankmine.png");
            break;
         case cmfloatmine:
         case cmmooredmine:
            images[type] = &IconRepository::getIcon("antishipmine.png");
            break;
      };
   }
   if (images[type]) {
      if (type != cmmooredmine)
         megaBlitter<ColorTransform_None, ColorMerger_AlphaOverwrite, SourcePixelSelector_Plain,
                     TargetPixelSelector_All>(*images[type], surf, pos, nullParam, nullParam,
                                              nullParam, nullParam);
      else
         megaBlitter<ColorTransform_None, ColorMerger_AlphaMixer, SourcePixelSelector_Plain,
                     TargetPixelSelector_All>(*images[type], surf, pos, nullParam, nullParam,
                                              nullParam, nullParam);
   }
}
