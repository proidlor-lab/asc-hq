...
class tnetwork {
  public:
   struct {
      Uint8
         compposition;  // Nr. des Computers, an dem der SPieler spielt    => network.computernames
      int codewordcrc;
   } player[8];
...
void readLegacyNetworkData(tnstream& stream) {
   ...
   for (int i = 0; i < 8; i++)
      if (network.computer[i].name)
         stream.readString();
   // #else
   //         errorMessage("Unable to load map in old file format on this computer
   //         architecture\nPlease convert this file to the new file format on a 32 Bit little
   //         endian machine and try again"); throw tfileerror();
   // #endif
}
