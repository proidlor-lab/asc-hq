...
class ASC_PBEM : public ServerInterface {
  ...
  public:
    ...
    */
   bool createGame(ASCString fileName, const Uint8* data, const int size, ASCString gameName,
                   ASCString fileNamePattern, char* roles, int* players, int projectID = -1,
                   int turn = 1, int currentSlot = 1);

   /** @return a vector of TGameInfo structs containing the current games of the logged in player
    * @param myTurnOnly if true, only return info about games where it's the turn of the currenty
    * logged in player
    */
  ...
  private:
   ...
   /** @param parameters even amount of parameters _required_, in the format [0]paramName [1]value
    * [2]paramName [3]value ... may contain 0 entries
    *
