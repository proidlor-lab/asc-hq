...
class AuthenticationDialog : public ASC_PG_Dialog {
  ...
  public:
   AuthenticationDialog()
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 300, 150), "Logging in to server...") {
      propertyEditor = new ASC_PropertyEditor(
         this,
         PG_Rect(10, GetTitlebarHeight() + 20, Width() - 20, Height() - GetTitlebarHeight() - 50),
         "PropertyEditor", 50);

      new PG_PropertyField_String<ASCString>(propertyEditor, "Username", &name);
      (new PG_PropertyField_String<ASCString>(propertyEditor, "Default Password", &password))
         ->SetPassHidden('*');
...
PBEMServer ::PBEMServer() : filename("myFileName"), gameID(-1) {}
...
const bool r1 = networkTransferMechanismFactory::Instance().registerClass(
   PBEMServer::mechanismID(), ObjectCreator<GameTransferMechanism, PBEMServer>);
}
