...
int ASC_PBEM_writeBuffer(Uint8* buffer, const Uint8* source, int startPos, int bufferSize,
                         int sourceSize) {
   int i = 0;
...
size_t ASC_PBEM_readInternal(Uint8* outboundBuffer, size_t size, size_t nitems, void* infoStruct) {
   ...
   switch (control->step) {
      case 0:  // header of first part
      {
         ...
         // end of header has to be a \n<space>\n for the server to reliably identify it as empty
         // line
         int sent = ASC_PBEM_writeBuffer(outboundBuffer, (const Uint8*) header.c_str(),
                                         control->sent_step, size * nitems, header.size());
      ...
      case 1:  // first part
      {
         int sent = ASC_PBEM_writeBuffer(outboundBuffer, control->data, control->sent_step,
                                         size * nitems, control->size);
      ...
      default:  // footer: "--" + boundary + "--"
      {
         ...
         if (control->parameters.size() > parameterIndex) {
            ...
            int sent = ASC_PBEM_writeBuffer(outboundBuffer, (const Uint8*) data.c_str(),
                                            control->sent_step, size * nitems, data.size());
...
ASC_PBEM::ASC_PBEM(ASCString serverBase) {
   ...
   if (statusCode == 200 && serverVersion >= MIN_SERVER_VERSION &&
...
bool ASC_PBEM::request(ASCString url, std::vector<ASCString> parameters) {
   ...
   ASCString
      encodedRequest;  // lifetime considerations require this parameter to be _outside_ the loop
   ...
   ASCString cookieString =
      "cookie: JSESSIONID=" + sessionID +
      ";";  // lifetime considerations require this parameter to be _outside_ the loop
   ASCString acceptString = "Accept: text/plain";  // lifetime considerations require this parameter
                                                   // to be _outside_ the loop
...
bool ASC_PBEM::request(ASCString url, ASC_PBEM_FileUploadControl* fileUploadControl) {
   ...
   ASCString cookieString =
      "cookie: JSESSIONID=" + sessionID +
      ";";  // lifetime considerations require this parameter to be _outside_ the loop
   ASCString contentString =
      "content-type: multipart/form-data; boundary=" + fileUploadControl->boundary;
   ASCString acceptString = "Accept: text/plain";  // lifetime considerations require this parameter
                                                   // to be _outside_ the loop
   if (sessionID.length() > 0) {
      headers = curl_slist_append(headers, cookieString.c_str());
...
void ASC_PBEM::parseHeaderLine(ASCString line) {
   if (line.find("HTTP") == 0) {
      ...
      if (line.find("JSESSIONID") != ASCString::npos) {
         ...
         if (sessionID.find("=") == ASCString::npos)
            return;
...
bool ASC_PBEM::login(ASCString user, ASCString passwd) {
   ...
   if (!request("tools/login", parameters))
      return false;
...
bool ASC_PBEM::createAccount(ASCString user, ASCString passwd, ASCString email) {
   ...
   if (!request("tools/create", parameters))
      return false;
...
bool ASC_PBEM::activateAccount(ASCString user, ASCString code) {
   ...
   if (!request("tools/activate", parameters))
      return false;
...
bool ASC_PBEM::uploadFile(ASCString fileName, const Uint8* data, const int size, const int gameID) {
   ...
   if (!request("tools/upload", &control))
      return false;

   return statusCode == 201;
}

bool ASC_PBEM::createGame(ASCString fileName, const Uint8* data, const int size, ASCString gameName,
                          ASCString fileNamePattern, char* roles, int* players, int projectID,
                          int turn, int currentSlot) {
   if (!loggedIn)
      return false;
   ...
   if (!request("tools/newgame", &control))
      return false;
...
TFileData* ASC_PBEM::downloadGame(TGameInfo game) {
   ...
   */
   ...
   if (statusCode == 200) {
      ...
      ASC_PBEM_writeBuffer(data->fileData, (const Uint8*) bodyString.c_str(), 0, bodyString.size(),
                           bodyString.size());
...
TGameInfo ASC_PBEM::parseGameInfoLine(ASCString line) {
   ...
   info.projectID =
      atoi(line.substr(currentIndex, line.find('\t', currentIndex) - currentIndex).c_str());
   currentIndex = line.find('\t', currentIndex) + 1;
   info.currentSlot =
      atoi(line.substr(currentIndex, line.find('\t', currentIndex) - currentIndex).c_str());
   currentIndex = line.find('\t', currentIndex) + 1;
   info.turn =
      atoi(line.substr(currentIndex, line.find('\t', currentIndex) - currentIndex).c_str());
   currentIndex = line.find('\t', currentIndex) + 1;
   info.currentSaveGameName =
      line.substr(currentIndex, line.find('\t', currentIndex) - currentIndex);
   currentIndex = line.find('\t', currentIndex) + 1;
...
int nmain(int argc, char* argv[]) {
   ...
   }else
   {
      ...
      if( asc_pbem.createGame( fileName, fileName.c_str(), fileName.size(), gameName,
      fileNamePattern, roles, players, projectID, turn, currentSlot ) )
      {
