...
class ASCString : public ASCCharTString {
  public:
   /*!
       charT is an alias to ASCStringHelpers::charT, the character type used to represent strings
      internally.
   */
   ...
   /*!
       NoncharT is an alias to ASCStringHelpers::NoncharT, the complementary character type used to
      convert strings not using charT types.
   */
...
*/
inline ASCString::ASCString() : ASCCharTString() {}
...
*/
inline ASCString::ASCString(const charT* pS) : ASCCharTString(pS) {}
...
*/
inline ASCString::ASCString(const charT* pS, size_type n) : ASCCharTString(pS, n) {}
...
*/
inline ASCString::ASCString(size_type n, charT c) : ASCCharTString(n, c) {}

/*!
    \overload ASCString::ASCString ( const_iterator first, const_iterator last )

    \param first a const iterator delimiting the begining of a range of \a charT element types to be
   used to construct this ASCString.

    \param last a const iterator delimiting the ending of a range of \a charT element types to be
   used to construct this ASCString.
*/
...
*/
inline ASCString::ASCString(const ASCCharTString& s) : ASCCharTString(s) {}
...
/*!
    ...
    \param s a const reference to an ASCAdaptatorString object to be used to construct this
   ASCString.

    \exception range_error
    The range_error exception is raised when not all the characters of ASCAdaptatorString can be
   converted to ASCCharTString's characters.
*/
inline ASCString::ASCString(const ASCAdaptatorString& s) : ASCCharTString() {
   ...
   if (stNumCharConverted != s.length()) {
      throw range_error(
         "ASCString::ASCString( const ASCAdaptatorString& s ) ==> unable to convert all characters "
         "from ASCAdaptatorString to ASCCharTString type.");
   }
...
inline ASCString::ASCString(const ASCAdaptatorString& s, size_type pos, size_type n)
   ...
   if (stNumCharConverted != stLen) {
      throw range_error(
         "ASCString::ASCString( const ASCAdaptatorString& s, size_type pos, size_type n ) ==> "
         "unable to convert all characters from ASCAdaptatorString to ASCCharTString type.");
   }
...
*/
inline int ASCString::compare_ci(size_type p0, size_type n0, const ASCCharTString& s, size_type pos,
                                 size_type n) {
...
/*!
    \overload int ASCString::compare_ci ( size_type p0, size_type n0, const charT* pS, size_type pos
   ) const
