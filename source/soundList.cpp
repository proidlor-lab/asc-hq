...
SoundLoopManager ::SoundLoopManager(Sound* snd, bool _active) : sound(snd), active(_active) {
...
void SoundList::readLine(PropertyContainer& pc, const ASCString& name, SoundList::Sample sample,
                         int subtype) {
...
Sound* SoundList::getSound(Sample snd, int subType, const ASCString& label, int height) {
   ...
   if (label.find(".") != ASCString::npos) {
      ...
      for (vector<SoundAssignment>::iterator i = soundAssignments.begin();
           i != soundAssignments.end(); i++)
         if (snd == i->sample && subType == i->subType) {
            if (height >= 0 && height <= 7)
               if (i->snd.find(heightTags[height]) != i->snd.end()) {
                  displayLogMessage(10, ASCString(" SoundList::getSound(2) : heightlabel ") +
                                           heightTags[height] +
                                           " found, returning matching sound \n");
                  return i->snd[heightTags[height]];
               }

            if (newlabel.empty() || i->snd.find(newlabel) == i->snd.end()) {
               displayLogMessage(10, " SoundList::getSound(2) : label " + label +
                                        " not found, returning default sound \n");
               if (!i->defaultSound)
                  displayLogMessage(
                     10, "  SoundList::getSound(2) : no default sound registered !!!! \n");
