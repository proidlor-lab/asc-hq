...
extern void circularFieldIterator(GameMap* gamemap, const MapCoordinate& center, int startDist,
...
extern inline int getnextdx(int dir, int y) {
   return ((y & 1) == (dir <= 2)) * dxLookup[dir];
}

static const int dyLookup[] = {-2, -1, 1, 2, 1, -1};
extern inline int getnextdy(int dir) {
   return dyLookup[dir];
}

//! returns the coordinate of the field that is adjecent to the given field in the direction of
//! direc
...
class WindMovement {
  ...
  public:
   ...
   inline int getDist(int dir) {
      assert(dir >= 0 && dir <= 5);
      return wm[dir];
   };
   inline int getDist(int dir) const {
      assert(dir >= 0 && dir <= 5);
      return wm[dir];
   };
};
