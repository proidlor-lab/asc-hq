...
void tsearchreactionfireingunits ::findOffensiveUnits(Vehicle* vehicle, int height, int x1, int y1,
                                                      int x2, int y2) {
   ...
   for (int player = 0; player < gamemap->getPlayerCount(); ++player) {
      if (gamemap->getPlayer(vehicle).diplomacy.isHostile(player)) {
         for (Player::VehicleList::iterator i = gamemap->getPlayer(vehicle).vehicleList.begin();
              i != gamemap->getPlayer(vehicle).vehicleList.end(); ++i) {
...
int tsearchreactionfireingunits ::attack(Vehicle* attacker, Vehicle* target,
   ...
   if (attacker->reactionfire.canPerformAttack(target)) {
      ...
      if (atw->count) {  // && (attacker->reactionfire.enemiesAttackable & (1 << (target->color /
                         // 8)))) {
         ...
         if (num >= 0) {
            int visibility = 0;
            if (context.display) {
               MessagingHub::Instance().statusInformation("attacking with weapon " +
                                                          ASCString::toString(atw->num[num]));

               if (fieldvisiblenow(gamemap->getField(attacker->xpos, attacker->ypos),
                                   gamemap->getPlayerView())) {
                  ++visibility;
...
int tsearchreactionfireingunits ::checkfield(const MapCoordinate3D& pos, Vehicle*& vehicle,
                                             const Context& context) {
   int attacks = 0;
   int result = 0;
...
int tsearchreactionfireingunits ::finalCheck(int currentPlayer, const Context& context) {
   ...
   for (int player = 0; player < gamemap->getPlayerCount(); ++player) {
      if (gamemap->getPlayer(currentPlayer).diplomacy.isHostile(player)) {
         ...
         for (Player::VehicleList::iterator exposedTarget =
                 gamemap->getPlayer(currentPlayer).vehicleList.begin();
              exposedTarget != gamemap->getPlayer(currentPlayer).vehicleList.end();
              ++exposedTarget) {
            if (fieldvisiblenow(gamemap->getField((*exposedTarget)->getPosition()), player)) {
               if (visibleUnits.find(*exposedTarget) == visibleUnits.end() ||
                   !(visibleUnits[*exposedTarget] & (1 << player))) {
                  if ((*exposedTarget)
                         ->getMap()
                         ->getField((*exposedTarget)->getPosition())
                         ->unitHere(*exposedTarget))
                     exposedTargets.push_back((*exposedTarget)->networkid);
               }
            }
         }

         for (list<int>::iterator exposedTargetID = exposedTargets.begin();
              exposedTargetID != exposedTargets.end(); ++exposedTargetID) {
            Vehicle* exposedTarget = gamemap->getUnit(*exposedTargetID);
            if (exposedTarget) {
               bool destroyed = false;

               for (Player::VehicleList::iterator enemyUnit =
                       gamemap->getPlayer(player).vehicleList.begin();
                    enemyUnit != gamemap->getPlayer(player).vehicleList.end() && !destroyed;
                    ++enemyUnit) {
                  if ((*enemyUnit)->reactionfire.canPerformAttack(exposedTarget)) {
                     if (beeline(*enemyUnit, exposedTarget) <=
                         maxshootdist[exposedTarget->getPosition().getNumericalHeight()]) {
