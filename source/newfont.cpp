/*! \file newfont.cpp
    ...
    Although the file name contains is called newfont, the time when this was actually the case is
   long past (1993)...
...
pfont loadfont(tnstream* stream) {
   ...
   for (i = 0; i < 256; i++) {
      ...
      for (int j = 0; j < 256; j++) {
         font2->kerning[j][i] =
            font1->kerning[int(font1->kernchartable[i])][int(font1->kernchartable[j])];
...
void showtext3(const char* txt, int xpos, int ypos) {
   // collategraphicoperations cgo ( xpos, ypos, max ( activefontsettings.height,
   // activefontsettings->font.height ),;
   ...
   while (txt[i] != 0) {
      ...
      if (s[0] != 0) {
         ...
         } else if (orgjus == centertext) {
            newx = xpos + (activefontsettings.length - gettextwdth(s2, NULL)) / 2 +
                   gettextwdth(s3, NULL) - gettextwdth(s, NULL) + 1;
         } else if (orgjus == righttext) {
            newx = xpos + 1 + (activefontsettings.length - gettextwdth(s2, NULL)) +
                   gettextwdth(s3, NULL) - gettextwdth(s, NULL);
         }
...
class InitNewfont {
  public:
   InitNewfont(void) { memset(&activefontsettings, 0, sizeof(activefontsettings)); };
} initnewfont;
