...
/** describes the behavier that next_turn() shall do when it finds out that the current player is
...
/*! Ends the turn of the current player and runs AI until a player is human again
    \param playerView -2 = detect automatically; -1 = don't display anything; 0-7 = this player is
   watching
*/
extern void next_turn(GameMap* gameMap, const NextTurnStrategy& nextTurnStrategy,
                      MapDisplayInterface* display, int playerView = -2);
...
//! this checks if one player has been using a new ASC version than the current player. If this is
//! the case, the current player is notified and asked to upgrade
