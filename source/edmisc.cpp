...
void placeCurrentItem() {
   if (selection.getSelection() && actmap->getCursor().valid()) {
      if (selection.getSelection()->supportMultiFieldPlacement() && selection.brushSize > 1) {
         circularFieldIterator(
            actmap, actmap->getCursor(), 0, selection.brushSize - 1,
            FieldIterationFunctor(selection.getSelection(), &MapComponent::vPlace));
      } else {
...
void tputresourcesdlg ::init(void) {
   ...
   addbutton("ma~x~ resources ( at center )", xsize / 2 + 5, 80, xsize - 20 - 5, 100, 2, 1, 3,
             true);
...
void tputresourcesdlg ::run(void) {
   ...
   if (status == 11) {
      ...
      pr.init(actmap->getCursor().x, actmap->getCursor().y, dist, resourcetype ? 1 : 2, maxresource,
              minresource);
   }
}
...
void tplayerchange::anzeige(void) {
   ...
   for (int i = 0; i <= 8; i++) {
      if (i == sel1)
         ...
         rectangle(x1 + 16, y1 + 51 + i * 30, x1 + 154, y1 + 79 + i * 30, 28);
      else
   ...
   switch (id) {
      ...
      case 2:
         ...
         /*     case 3: {
                    ...
                    if ( ( sel != 255) && ( sel != sel1 ) && ( sel1 != 255 ) &&
                       ( actmap->player[sel].firstvehicle == NULL ) && (
            actmap->player[sel].firstbuilding == NULL )   ) {
      ...
      case 5: {
         if ((sel1 != 255) && (sel2 != sel1) && (sel2 != 255)) {
            ...
            if (sel1 != 8)
               for (Player::VehicleList::iterator i = actmap->player[sel2].vehicleList.begin();
                    i != actmap->player[sel2].vehicleList.end();) {
                  (*i)->color = sel1 * 8;
...
void k_loadmap(void) {
   ...
   if (!s1.empty()) {
      StatusMessageWindowHolder smw =
         MessagingHub::Instance().infoMessageWindow("loading map " + s1);
          ...
          if ( terrainaccessible(ffield,ffield->vehicle) )
               {
                  ...
                  if (auswahlf != NULL) {
                     ...
                     while ( ! ( ( ( ( ffield->vehicle->height & ffield->vehicle->typ->height ) > 0)
&& (terrainaccessible(ffield,ffield->vehicle) == 2) ) ) && (ffield->vehicle->height != 0) )
                     ...
                     else ffield->vehicle->setMovement (
ffield->vehicle->typ->movement[getFirstBit(ffield->vehicle->height)] ); ffield->vehicle->direction =
auswahld;
                  }
            }
            else
...
class PolygonEditor : public SelectFromMap {
  protected:
   void showFieldMarking(const CoordinateList& coordinateList) {
      Poly_gon poly;
      for (CoordinateList::const_iterator i = coordinateList.begin(); i != coordinateList.end();
           ++i)
         ...
         for (int r = 0; r < 3; r++)
            if (r != changed_resource)
               if (mplus.resource(changed_resource)) {
                  int a = mplus.resource(r) * plus.resource(changed_resource) /
                          mplus.resource(changed_resource);
            ...
            for (int r = 0; r < 3; r++)
               if (r != changed_resource)
                  if (gbde.typ->maxplus.resource(changed_resource)) {
                     int a = gbde.typ->maxplus.resource(r) * mplus.resource(changed_resource) /
                             gbde.typ->maxplus.resource(changed_resource);
...
); if ( ( ( taste == ct_enter ) || (action == 1 ) ) && (redline != 255 ) ) { ppolystructure pps;
...
void EditAiParam::buttonpressed(int id) {
   switch (id) {
      case 20:
         aiv.setTask(AiParameter::Task(
            selectString(aiv.getTask(), "Select Task", AItasks, AiParameter::taskNum)));
         break;
      case 21: {
         AiParameter::JobList j;
         j.push_back(AiParameter::Job(
            selectString(aiv.getJob(), "Select Job", AIjobs, AiParameter::jobNum)));
         aiv.setJob(j);
      } break;
      /*
...
class SelectUnitSetFilter : public ASC_PG_Dialog {
  ...
  public:
   SelectUnitSetFilter()
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 300, 500), "Visible UnitSet Filter") {
      listbox =
         (new MultiListBox(this, PG_Rect(10, 30, Width() - 20, Height() - 80)))->getListBox();

      for (int i = 0; i < ItemFiltrationSystem::itemFilters.size(); i++) {
         PG_ListBoxItem* item =
            new PG_ListBoxItem(listbox, 20, ItemFiltrationSystem::itemFilters[i]->name);
...
VehicleType* UnitTypeTransformation ::transformvehicletype(const VehicleType* type, int unitsetnum,
                                                           int translationnum) {
   for (int i = 0; i < unitSets[unitsetnum]->transtab[translationnum]->translation.size(); i++)
      if (unitSets[unitsetnum]->transtab[translationnum]->translation[i].from == type->id) {
         VehicleType* tp = vehicleTypeRepository.getObject_byID(
            unitSets[unitsetnum]->transtab[translationnum]->translation[i].to);
         if (tp)
            return tp;
      }
...
void UnitTypeTransformation ::transformvehicle(Vehicle* veh, int unitsetnum, int translationnum) {
   for (ContainerBase::Cargo::const_iterator i = veh->getCargo().begin();
        i != veh->getCargo().end(); ++i)
      if (*i)
...
void UnitTypeTransformation ::run(void) {
   ...
   for (int y = 0; y < actmap->ysize; y++)
      for (int x = 0; x < actmap->xsize; x++) {
         ...
         if (fld->building && (fld->bdt & getTerrainBitType(cbbuildingentry)).any()) {
            for (ContainerBase::Cargo::const_iterator i = fld->building->getCargo().begin();
                 i != fld->building->getCargo().end(); ++i)
               if (*i)
   ...
   if (vehicleTypesNotTransformed.size()) {
      ...
      for (set<int>::const_iterator i = vehicleTypesNotTransformed.begin();
           i != vehicleTypesNotTransformed.end(); ++i) {
         s += "\n ID ";
...
void transformMap() {
   ...
   if (o2ot_translation.size() % 3) {
      displaymessage("Map Translation : Object2ObjTerrainTranslation - Invalid number of entries ",
                     1);
      return;
   }
      ...
      for (int x = 0; x < actmap->xsize; x++) {
         ...
         for (int i = 0; i < terraintranslation.size() / 2; i++)
            if (fld->typ->terraintype->id == terraintranslation[i * 2]) {
               TerrainType* tt =
                  terrainTypeRepository.getObject_byID(terraintranslation[i * 2 + 1]);
               if (tt) {
         ...
         for (int i = 0; i < terrainobjtranslation.size() / 3; i++)
            if (fld->typ->terraintype->id == terrainobjtranslation[i * 3]) {
               TerrainType* tt =
                  terrainTypeRepository.getObject_byID(terrainobjtranslation[i * 3 + 1]);
               if (tt) {
                  TerrainType::Weather* tw = tt->weather[fld->getWeather()];
                  if (!tw)
                     tw = tt->weather[0];
                  fld->typ = tw;
                  fld->addobject(
                     objectTypeRepository.getObject_byID(terrainobjtranslation[i * 3 + 2]), -1,
                     true);
                  fld->setparams();
               }
            }
         ...
         for (int j = 0; j < fld->objects.size(); ++j)
            for (int i = 0; i < objecttranslation.size() / 2; i++)
               if (fld->objects[j].typ->id == objecttranslation[i * 2]) {
                  ObjectType* ot =
                     objectTypeRepository.getObject_byID(objecttranslation[i * 2 + 1]);
                  if (ot) {
         ...
         for (int b = 0; b < buildingtranslation.size() / 2; ++b)
            if ((fld->bdt & getTerrainBitType(cbbuildingentry)).any() && fld->building &&
                fld->building->typ->id == buildingtranslation[b * 2]) {
               BuildingType* newtype =
                  buildingTypeRepository.getObject_byID(buildingtranslation[b * 2 + 1]);
               if (newtype) {
                  Building* bld = fld->building;
         ...
         for (int j = 0; j < fld->objects.size(); ++j)
            for (int i = 0; i < o2ot_translation.size() / 3; i++)
               if (fld->objects[j].typ->id == o2ot_translation[i * 3]) {
                  fld->removeObject(fld->objects[j].typ, true);
                  TerrainType* tt =
                     terrainTypeRepository.getObject_byID(o2ot_translation[i * 3 + 2]);
                  if (tt) {
                     TerrainType::Weather* tw = tt->weather[fld->getWeather()];
                     if (!tw)
                        tw = tt->weather[0];
   ...
   for (int p = 0; p < actmap->getPlayerCount(); ++p) {
      ...
      for (Player::VehicleList::iterator i = actmap->getPlayer(p).vehicleList.begin();
           i != actmap->getPlayer(p).vehicleList.end(); ++i) {
      ...
      for (Player::BuildingList::iterator i = actmap->getPlayer(p).buildingList.begin();
           i != actmap->getPlayer(p).buildingList.end(); ++i) {
         ContainerBase::Production prod = (*i)->getProduction();
         for (ContainerBase::Production::iterator j = prod.begin(); j != prod.end(); ++j)
            if (*j && transform((*j)->id, vehicletranslation))
               *j = transform((*j)->id, vehicletranslation);
      ...
      for (vector<ASCString>::iterator tat = techAdapterTranslation.begin();
           tat != techAdapterTranslation.end();) {
         ASCString from = *(tat++);
         ASCString to = *(tat++);

         vector<ASCString> triggeredAdapters(r.triggeredTechAdapter.begin(),
                                             r.triggeredTechAdapter.end());
...
void EditArchivalInformation::init() {
   ...
   addbutton("~R~equirements ( see http://asc-hq.sf.net/req.php for possible values)", 20, 220,
             xsize - 20, 240, 1, 1, 13, true);
...
void resourceComparison() {
   ...
   for (int i = 0; i < 9; i++) {
      ...
      Resources plus;
      Resources have;
      for (Player::BuildingList::iterator b = actmap->player[i].buildingList.begin();
           b != actmap->player[i].buildingList.end(); ++b) {
...
void readClipboard() {
   ASCString filename =
      selectFile(ASCString(clipboardFileExtension) + ";" + oldClipboardFileExtension, true);
   if (!filename.empty()) {
      try {
            ...
            if (res.first == 0) {
               ...
               for (int i = 0; i < technologyRepository.getNum(); ++i) {
                  ...
                  if (find(devTech.begin(), devTech.end(), t->id) == devTech.end())
                     if (!ItemFiltrationSystem::isFiltered(ItemFiltrationSystem::Technology,
                                                           t->id)) {
               ...
               vector<int>::iterator p =
                  find(devTech.begin(), devTech.end(), techIds[techs[res.second]]);
               devTech.erase(p);
            }
         } while (res.first != 2);
      }
   ...
   do {
      ...
      for (int i = 0; i < 8; ++i) {
         ...
         player.push_back(s + " " + actmap->player[i].getName() + " (" +
                          strrr(actmap->player[i].research.progress) + " Points)");
      }

      playerRes = chooseString("Choose Player", player, buttonsP);
      if (playerRes.first == 0 && playerRes.second >= 0)
         actmap->player[playerRes.second].research.progress =
            editInt("Points", actmap->player[playerRes.second].research.progress);
...
void generateTechTree() {
   ...
   if (!filename.empty()) {
      ...
      for ( int i = 1; i < techIds.size();  )
         if ( technologyRepository.getObject_byID( techIds[i-1] )->name >
      technologyRepository.getObject_byID( techIds[i] )->name ) { int a = techIds[i]; int b =
      techIds[i-1]; techs[i] = printTech ( b ); techs[i-1] = printTech ( a ); techIds[i] = b;
            techIds[i-1] = a;
            if ( i > 1 )
               --i;
         } else
      ...
      if (leavesUp) {
         ...
         if (r.first == 0) {
            ...
            for (int i = 0; i < technologyRepository.getNum(); ++i) {
               ...
               if (!ItemFiltrationSystem::isFiltered(ItemFiltrationSystem::Technology, t->id)) {
                  vector<int> stack;
                  if (t->techDependency.findInheritanceLevel(techIds[r.second], stack,
                                                             techs[r.second]) > 0)
  ...
  public:
   ItemLocator() : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 400, 200), "Item Locator") {
      ...
      AddStandardButton("cancel")->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &ItemLocator::cancel)));
...
void copyVehicleData(Vehicle* source, Vehicle* target, GameMap* targetMap, int* playerTranslation) {
   ...
   for (ContainerBase::Production::const_iterator i = source->getProduction().begin();
        i != source->getProduction().end(); ++i)
      target->addProductionLine(*i);
}

void copyBuildingData(Building* source, Building* target, GameMap* targetMap,
                      int* playerTranslation, bool mirrorUnits) {
   for (int i = 0; i < waffenanzahl; i++)
      target->ammo[i] = source->ammo[i];
   target->name = source->name;
...
void copyFieldStep1(MapField* sourceField, MapField* targetField, bool mirrorTerrain,
                    bool mirrorResources, bool mirrorWeather) {
   ...
   if (mirrorWeather)
      targetField->setWeather(sourceField->getWeather());
}

void copyFieldStep2(MapField* sourceField, MapField* targetField, GameMap* targetMap,
                    int* directionTranslation, int* playerTranslation, bool mirrorObjects,
                    bool mirrorBuildings, bool mirrorUnits, bool mirrorMines) {
   if (mirrorObjects) {
   ...
   if (mirrorBuildings) {
      if (sourceField->building != NULL && sourceField->building->getEntryField() == sourceField) {
         ...
         Building* newBuilding =
            new Building(targetMap, MapCoordinate(targetField->getx(), targetField->gety()),
                         sourceField->building->typ, playerID);
         copyBuildingData(sourceField->building, newBuilding, targetMap, playerTranslation,
                          mirrorUnits);
      }
   }
   ...
   if (mirrorUnits) {
      if (sourceField->vehicle != NULL) {
         ...
         targetField->vehicle->direction =
            directionTranslation[int(sourceField->vehicle->direction)];
      }
   }
   ...
   if (mirrorMines) {
      for (int i = 0; i < sourceField->mines.size(); i++) {
         ...
         targetField->putmine(playerTranslation[sourceMine.player], sourceMine.type,
                              sourceMine.strength);
      }
   }
}
...
void MirrorMap::mirrorX() {
   ...
   for (int x = 0; x < maxX; x++) {
      for (int y = 0; y < actmap->ysize; y++) {
         ...
         if (y % 2 == 0)
            targetX++;
         if (targetX >= actmap->xsize || targetX == x)
            continue;
   ...
   for (int x = 0; x < maxX; x++) {
      for (int y = 0; y < actmap->ysize; y++) {
         ...
         copyFieldStep2(sourceField, targetField, actmap, directionTranslation, playerTranslation,
                        mirrorObjects, mirrorBuildings, mirrorUnits, mirrorMines);
      }
   }
   doneMirrorMap = true;
}
...
CopyMap::CopyMap()
   : FieldAddressing(actmap), ASC_PG_Dialog(NULL, PG_Rect(30, 30, 550, 400), "Paste Options") {
   addressingMode = poly;
   mapStartX = -1;
   mapStartY = -1;
   mapEndX = -1;
   ...
   for (int i = 0; i < 6; i++)
      directionTranslation[i] = i;
   playerTranslation = new int[9];
   for (int i = 0; i < 9; i++)
      playerTranslation[i] = i;
   ...
   PG_Label* label =
      new PG_Label(this, PG_Rect(startX, startY + dialogLine * (lineHeight + lineSpacing),
                                 startX2 - xSpacer - startX, lineHeight));
   label->SetAlignment(PG_Label::LEFT);
   label->SetText("Terrain");
   mirrorTerrain =
      new PG_CheckButton(this, PG_Rect(startX2, startY + dialogLine * (lineHeight + lineSpacing),
                                       startX3 - xSpacer - startX2, lineHeight));
   mirrorTerrain->SetPressed();
   dialogLine++;

   label = new PG_Label(this, PG_Rect(startX, startY + dialogLine * (lineHeight + lineSpacing),
                                      startX2 - xSpacer - startX, lineHeight));
   label->SetAlignment(PG_Label::LEFT);
   ...
   PG_Button* ok = new PG_Button(this,
                                 PG_Rect(startX, startY + dialogLine * (lineHeight + lineSpacing),
                                         startX2 - xSpacer - startX, lineHeight),
                                 "Paste");
   ...
   label = new PG_Label(this, PG_Rect(startX3, startY + dialogLine * (lineHeight + lineSpacing),
                                      startX4 - xSpacer - startX, lineHeight));
   label->SetAlignment(PG_Label::LEFT);
   label->SetText("playerTranslation0");
   playerTranslation0 =
      new PG_LineEdit(this, PG_Rect(startX4, startY + dialogLine * (lineHeight + lineSpacing),
                                    this->Width() - xSpacer - startX4, lineHeight));
...
bool CopyMap::paste() {
   ...
   if (mirrorY->GetPressed()) {
      int directionTranslationNew[6] = {directionTranslation[3], directionTranslation[2],
                                        directionTranslation[1], directionTranslation[0],
                                        directionTranslation[5], directionTranslation[4]};

      for (int i = 0; i < 6; i++)
         directionTranslation[i] = directionTranslationNew[i];
   }

   if (mirrorX->GetPressed()) {
      int directionTranslationNew[6] = {directionTranslation[0], directionTranslation[5],
                                        directionTranslation[4], directionTranslation[3],
                                        directionTranslation[2], directionTranslation[1]};
   ...
   for (int x = 0; x < map->xsize; x++) {
      for (int y = 0; y < map->ysize; y++) {
         if (fieldCopied[x + y * sizeX]) {
            ...
            if (fieldX >= actmap->xsize)
               continue;
            if (fieldY >= actmap->ysize)
               continue;

            MapField* target = actmap->getField(fieldX, fieldY);
            MapField* source = map->getField(x, y);

            copyFieldStep1(source, target, mirrorTerrain->GetPressed(),
                           mirrorResources->GetPressed(), mirrorWeather->GetPressed());
         }
      }
   }
   ...
   for (int x = 0; x < map->xsize; x++) {
      for (int y = 0; y < map->ysize; y++) {
         if (fieldCopied[x + y * sizeX]) {
            ...
            copyFieldStep2(source, target, actmap, directionTranslation, playerTranslation,
                           mirrorObjects->GetPressed(), mirrorBuildings->GetPressed(),
                           mirrorUnits->GetPressed(), mirrorMines->GetPressed());
         }
      }
   }
...
void CopyMap::fieldOperator(const MapCoordinate& point) {
   ...
   if (copyStep == 0) {
      ...
      copyFieldStep2(source, target, map, directionTranslation, playerTranslation, true, true, true,
                     true);
   }
}
