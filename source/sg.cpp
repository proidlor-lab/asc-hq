...
bool normalizePlayerSpec( const std::string& input, const std::string& fallback, std::string& normalized, ASCString& errorMessage )
{
...
struct HeadlessResult {
   ...
   int winner;
   int rounds;
   HeadlessResult() : finished(false), draw(false), turnLimitReached(false), winner(-1), rounds(0) {}
};
...
static int runHeadlessMode( Cmdline& cl )
{
   ...
   if ( outcome.turnLimitReached ) {
      ...
      summary.format( "Headless simulation finished after %d rounds. Winner: Player %d (%s).", outcome.rounds, outcome.winner + 1, winner.getName().c_str() );
   }
   ...
   if ( !trackedPlayers.empty() ) {
      ...
      for ( std::vector<int>::const_iterator it = trackedPlayers.begin(); it != trackedPlayers.end(); ++it ) {
         int idx = *it;
         ...
         for ( std::vector<int>::const_iterator jt = trackedPlayers.begin(); jt != trackedPlayers.end(); ++jt ) {
            int opp = *jt;
            if ( opp < 0 || opp >= static_cast<int>( statsData.size() ))
               continue;
            if ( opp >= static_cast<int>( damageMatrix.size() ) || idx >= static_cast<int>( damageMatrix.size() ))
               continue;
            if ( opp >= static_cast<int>( damageMatrix[idx].size() ) || idx >= static_cast<int>( damageMatrix[opp].size() ))
               continue;
            if ( opp >= static_cast<int>( killMatrix.size() ) || idx >= static_cast<int>( killMatrix.size() ))
               continue;
            if ( opp >= static_cast<int>( killMatrix[idx].size() ) || idx >= static_cast<int>( killMatrix[opp].size() ))
               continue;
            if ( idx == opp )
               continue;
