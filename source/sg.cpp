...
/*!
   ...
   we decided to use the term vehicle instead. But now, with ASC written in C++, 'unit' is also
   used.
   ...
   weapon systems, accessible terrain etc, while the vehicle stores things like remaining movement
   for this turn, ammo, fuel and cargo.
...
extern void runai(GameMap* actmap, int playerView, MapDisplayInterface* display);
#endif
...
bool loadGameFromFile(const ASCString& filename) {
   GameMap* m =
      mapLoadingExceptionChecker(filename, MapLoadingFunction(tsavegameloaders::loadGameFromFile));
   if (!m)
      return false;
   ...
   if (actmap->replayinfo) {
      ...
      if (actmap->replayinfo->guidata[actmap->actplayer])
         actmap->replayinfo->actmemstream =
            new MemoryStream(actmap->replayinfo->guidata[actmap->actplayer], tnstream::appending);
      else {
         actmap->replayinfo->guidata[actmap->actplayer] = new MemoryStreamStorage;
         actmap->replayinfo->actmemstream =
            new MemoryStream(actmap->replayinfo->guidata[actmap->actplayer], tnstream::writing);
      }
   }
   ...
   if (gameToLoad && gameToLoad[0]) {
      try {
         ...
         if (err.expected < err.found)
            displaymessage(
               "File/module %s has invalid version.\nExpected version %d\nFound version %d\nPlease "
               "install the latest version from www.asc-hq.org",
               2, err.getFileName().c_str(), err.expected, err.found);
         else
            displaymessage(
               "File/module %s has invalid version.\nExpected version %d\nFound version %d\nThis "
...
void changePassword(GameMap* gamemap) {
   ...
   } while (gamemap->player[gamemap->actplayer].passwordcrc.empty() && success &&
            viewtextquery(910, "warning", "~e~nter password", "~c~ontinue without password") ==
               0); /* enddo */
}
...
void showUsedPackages() {
   ...
   if (actmap->packageData) {
      ASCString s;
      for (PackageData::Packages::const_iterator i = actmap->packageData->packages.begin();
           i != actmap->packageData->packages.end(); ++i) {
...
class CommandAllianceSetupStrategy : public AllianceSetupWidget::ApplyStrategy {
   ...
   virtual void setState(GameMap* map, int actingPlayer, int towardsPlayer,
...
void executeUserAction(tuseractions action) {
   switch (action) {
      ...
      case ua_toggleunitshading: {
         CGameOptions::Instance()->units_gray_after_move =
            !CGameOptions::Instance()->units_gray_after_move;
      ...
      case ua_computerturn:
         if (maintainencecheck()) {
            displaymessage(
               "This function is under development and for programmers only\n"
      ...
      case ua_showResearchStatus: {
         ...
         if (actmap->player[actmap->actplayer].research.activetechnology)
            s += ASCString(" / ") +
                 strrr(actmap->player[actmap->actplayer].research.activetechnology->researchpoints);
         s += "\n\n";
         ...
         for (vector<int>::iterator i =
                 actmap->player[actmap->actplayer].research.developedTechnologies.begin();
              i != actmap->player[actmap->actplayer].research.developedTechnologies.end(); ++i) {
      ...
      case ua_unitweightinfo:
         if (fieldvisiblenow(getSelectedField())) {
            ...
            if (eht &&
                actmap->player[actmap->actplayer].diplomacy.getState(eht->getOwner()) >= PEACE_SV)
         ...
         break;
      case ua_GameStatus:
         infoMessage("Current game time is:\n turn " + ASCString::toString(actmap->time.turn()) +
                     " , move " + ASCString::toString(actmap->time.move()));
         break;
      case ua_soundDialog:
         soundSettings(NULL);
         break;
      case ua_reloadDlgTheme:
         ...
         // soundSettings( NULL );
         break;
      case ua_viewButtonPanel:
         mainScreenWidget->spawnPanel(ASC_MainScreenWidget::ButtonPanel);
         break;
      case ua_viewWindPanel:
         mainScreenWidget->spawnPanel(ASC_MainScreenWidget::WindInfo);
         break;
      case ua_clearImageCache:
         IconRepository::clear();
         break;
      case ua_viewUnitInfoPanel:
         mainScreenWidget->spawnPanel(ASC_MainScreenWidget::UnitInfo);
         break;
      case ua_viewOverviewMapPanel:
         mainScreenWidget->spawnPanel(ASC_MainScreenWidget::OverviewMap);
         break;
      case ua_viewMapControlPanel:
         mainScreenWidget->spawnPanel(ASC_MainScreenWidget::MapControl);
         break;
         //      case ua_viewActionPanel: mainScreenWidget->spawnPanel(
         //      ASC_MainScreenWidget::ActionInfo );
         //         break;
      case ua_vehicleinfo:
         unitInfoDialog();
         break;
      ...
      case ua_viewterraininfo:
         if (fieldvisiblenow(actmap->getField(actmap->getCursor())))
            viewterraininfo(actmap, actmap->getCursor(),
                            fieldVisibility(actmap->getField(actmap->getCursor())) == visible_all);
         break;
      case ua_testMessages:
         MessagingHub::Instance().message(MessagingHubBase::InfoMessage,
         ...
         MessagingHub::Instance().message(MessagingHubBase::FatalError,
                                          "This is an fatal error message. Game will be exited.");
         break;
      ...
      case ua_showTechAdapter: {
         ViewFormattedText vft("TechAdapter",
                               actmap->getCurrentPlayer().research.listTriggeredTechAdapter(),
                               PG_Rect(-1, -1, 300, 500));
         vft.Show();
         vft.RunModal();
      }; break;
      case ua_showUnitEndurance:
         showUnitEndurance();
         break;

      case ua_getMemoryFootprint:
         showMemoryFootprint();
         break;

      case ua_showMiningPower:
         viewMiningPower();
         break;

      case ua_emailOptions:
         editEmailOptions();
         break;

      case ua_createReminder:
         newreminder();
         break;
      ...
      case ua_writeLuaCommands:
         writeLuaCommands();
         break;

      case ua_chooseTechnology:
         chooseTechnologyIfAvail(actmap->getCurrentPlayer());
         break;

      case ua_actionManager:
         actionManager(actmap);
         break;

      case ua_runLuaCommands:
         selectAndRunLuaScript();
         break;

      case ua_unitAiOptions:
         showUnitAiProperties();
         break;

      case ua_showUsedPackages:
         showUsedPackages();
         break;

      case ua_runOpenTasks:
         runOpenTasks();
         break;
      ...
      case ua_createUnitCostList:
         createUnitCostList();
         break;

      case ua_eventInfo:
         viewEventInfo(actmap);
         break;
...
struct HeadlessGuard {
   HeadlessGuard() { setHeadlessMode(true); }
   ~HeadlessGuard() { setHeadlessMode(false); }
};
...
struct HeadlessStatsGuard {
   bool active;
   HeadlessStatsGuard() : active(true) {}
   ~HeadlessStatsGuard() {
      if (active)
         headlessStatsEnd();
   }
...
bool normalizePlayerSpec(const std::string& input, const std::string& fallback,
                         std::string& normalized, ASCString& errorMessage) {
   ...
   if (lower == "classic" || lower == "mcts_balanced" || lower == "mcts-balanced" ||
       lower == "mcts_aggressive" || lower == "mcts-aggressive" || lower == "mcts_defensive" ||
       lower == "mcts-defensive" || lower == "mcts_fast" || lower == "mcts-fast" ||
   ...
   errorMessage = ASCString("Unsupported AI specification: ") + value +
                  "\nValid options: classic, mcts_balanced, mcts_aggressive, mcts_defensive, "
                  "mcts_fast, mcts_deep";
   return false;
}
...
int countLivingParticipants(const GameMap& map, const std::vector<int>& participants,
                            int* lastAlive) {
   int alive = 0;
   int last = -1;
   for (std::vector<int>::const_iterator it = participants.begin(); it != participants.end();
        ++it) {
      int idx = *it;
      if (idx < 0 || idx >= map.getPlayerCount())
         continue;
      const Player& player = map.player[idx];
      if (player.stat == Player::off)
         continue;
      if (player.exist()) {
         ++alive;
         last = idx;
...
struct HeadlessResult {
   bool finished;
   bool draw;
   bool turnLimitReached;
   int winner;
   int rounds;
   HeadlessResult()
      : finished(false), draw(false), turnLimitReached(false), winner(-1), rounds(0) {}
};

HeadlessResult makeResult(int rounds, int aliveCount, int lastAlive, bool limitReached) {
   HeadlessResult result;
   result.finished = true;
   result.draw = (aliveCount == 0);
   result.turnLimitReached = limitReached;
   result.winner = (aliveCount == 1) ? lastAlive : -1;
   result.rounds = rounds;
   return result;
}

bool executeAiTurn(GameMap* map, int playerIndex, ASCString& errorMessage) {
   bool turnOpened = false;
   try {
      map->actplayer = playerIndex;
      map->setPlayerView(playerIndex);
      map->beginTurn();
      turnOpened = true;
      if (map->player[playerIndex].stat != Player::off && map->player[playerIndex].exist())
         runai(map, playerIndex, NULL);
      map->endTurn();
      turnOpened = false;
      return true;
   } catch (const ASCmsgException& msg) {
      ...
      errorMessage = msg.getMessage();
   } catch (const ActionResult& result) {
      if (turnOpened) {
         try {
            map->endTurn();
         } catch (...) {}
      ...
      errorMessage = ASCString("Headless mode aborted: ") + ex.what();
   } catch (...) {
      if (turnOpened) {
         try {
            map->endTurn();
      ...
      errorMessage = "Headless mode aborted due to unknown error";
   }
   return false;
}
...
static int runHeadlessMode(Cmdline& cl) {
   HeadlessGuard headlessGuard;

   if (cl.l().empty()) {
      errorMessage("Headless mode requires --mapfile (or --load) to be specified.");
      return 1;
   }
   ...
   std::unique_ptr<GameMap> mapHolder(
      mapLoadingExceptionChecker(cl.l(), MapLoadingFunction(tmaploaders::loadmap)));
   if (!mapHolder.get()) {
      errorMessage("Unable to load map for headless mode");
      return 1;
   }

   mapHolder->levelfinished = false;
   if (mapHolder->replayinfo) {
      delete mapHolder->replayinfo;
      mapHolder->replayinfo = NULL;
   }

   computeview(mapHolder.get());
   deployMapPlayingHooks(mapHolder.get());

   actmap = mapHolder.release();
   ActmapGuard mapGuard;
   ...
   while (true) {
      int lastAlive = -1;
      int aliveCount = countLivingParticipants(*actmap, participants, &lastAlive);
      if (aliveCount <= 1) {
         outcome = makeResult(rounds, aliveCount, lastAlive, false);
         break;
      }
      ...
      for (std::vector<int>::const_iterator it = participants.begin(); it != participants.end();
         ...
         int idx = *it;
         ASCString turnError;
         if (!executeAiTurn(actmap, idx, turnError)) {
            errorMessage(turnError);
            return 2;
         }

         aliveCount = countLivingParticipants(*actmap, participants, &lastAlive);
         if (aliveCount <= 1) {
            outcome = makeResult(rounds, aliveCount, lastAlive, false);
            finishedEarly = true;
            break;
      ...
      if (turnLimit > 0 && rounds >= turnLimit) {
         int remainingLast = -1;
         int remainingAlive = countLivingParticipants(*actmap, participants, &remainingLast);
         outcome = makeResult(rounds, remainingAlive, remainingLast, true);
         break;
      }
   }

   ASCString summary;
   if (outcome.turnLimitReached) {
      summary.format("Headless simulation reached the turn limit after %d rounds.", outcome.rounds);
   } else if (outcome.draw) {
      summary.format("Headless simulation ended in a draw after %d rounds.", outcome.rounds);
   } else {
      const Player& winner = actmap->player[outcome.winner];
      summary.format("Headless simulation finished after %d rounds. Winner: Player %d (%s).",
                     outcome.rounds, outcome.winner + 1, winner.getName().c_str());
   }

   ASCString baseSummary = summary;

   const std::vector<HeadlessPlayerStats>& statsData = headlessStatsData();
   const std::vector<int>& trackedPlayers = headlessStatsParticipants();
   const std::vector<std::vector<long long>>& damageMatrix = headlessStatsDamageMatrix();
   const std::vector<std::vector<long long>>& killMatrix = headlessStatsKillMatrix();

   std::ostringstream statsStream;
   if (!trackedPlayers.empty()) {
      statsStream << "Statistics:" << '\n';
      for (std::vector<int>::const_iterator it = trackedPlayers.begin(); it != trackedPlayers.end();
           ++it) {
         int idx = *it;
         if (idx < 0 || idx >= static_cast<int>(statsData.size()))
            continue;

         const Player& player = actmap->player[idx];
         const HeadlessPlayerStats& stats = statsData[idx];

         long long participantDamageDone = 0;
         long long participantDamageTaken = 0;
         long long participantKills = 0;
         long long participantLosses = 0;

         for (std::vector<int>::const_iterator jt = trackedPlayers.begin();
              jt != trackedPlayers.end(); ++jt) {
            int opp = *jt;
            if (opp < 0 || opp >= static_cast<int>(statsData.size()))
               continue;
            if (opp >= static_cast<int>(damageMatrix.size()) ||
                idx >= static_cast<int>(damageMatrix.size()))
               continue;
            if (opp >= static_cast<int>(damageMatrix[idx].size()) ||
                idx >= static_cast<int>(damageMatrix[opp].size()))
               continue;
            if (opp >= static_cast<int>(killMatrix.size()) ||
                idx >= static_cast<int>(killMatrix.size()))
               continue;
            if (opp >= static_cast<int>(killMatrix[idx].size()) ||
                idx >= static_cast<int>(killMatrix[opp].size()))
               continue;
            if (idx == opp)
               continue;

            participantDamageDone += damageMatrix[idx][opp];
            participantDamageTaken += damageMatrix[opp][idx];
            participantKills += killMatrix[idx][opp];
            participantLosses += killMatrix[opp][idx];
         }

         long long externalDamageDone = stats.damageDone - participantDamageDone;
         long long externalDamageTaken = stats.damageTaken - participantDamageTaken;
         long long externalLosses = stats.unitsLost - participantLosses;
         if (externalLosses < 0)
            externalLosses = 0;

         statsStream << "Player " << (idx + 1) << " (" << player.getName().c_str() << "): ";
         statsStream << "damage done " << participantDamageDone;
         if (externalDamageDone > 0)
            statsStream << " (+" << externalDamageDone << " vs non-participants)";
         statsStream << ", damage taken " << participantDamageTaken;
         if (externalDamageTaken > 0)
            statsStream << " (+" << externalDamageTaken << " from non-participants)";
         statsStream << ", units destroyed " << participantKills;
         statsStream << ", units lost " << participantLosses;
         if (externalLosses > 0)
            statsStream << " (+" << externalLosses << " losses vs non-participants)";
         statsStream << '\n';

         if (stats.reactionFireDamageDone > 0 || stats.reactionFireDamageTaken > 0 ||
             stats.reactionFireKills > 0 || stats.reactionFireLosses > 0) {
            statsStream << "    reaction fire: damage done " << stats.reactionFireDamageDone;
            statsStream << ", damage taken " << stats.reactionFireDamageTaken;
            statsStream << ", kills " << stats.reactionFireKills;
            statsStream << ", losses " << stats.reactionFireLosses << '\n';
         }
      }
   }
   ...
   if (!statsText.empty()) {
      summary += "\n";
      summary += ASCString(statsText.c_str());
   }

   std::ostringstream logStream;
   std::time_t now = std::time(NULL);
   char timeBuffer[64];
   if (std::strftime(timeBuffer, sizeof(timeBuffer), "%Y-%m-%d %H:%M:%S", std::localtime(&now)))
      logStream << "Timestamp: " << timeBuffer << '\n';

   logStream << "Map: " << cl.l() << '\n';
   logStream << "Participants: player 1 -> " << player1Spec << ", player 2 -> " << player2Spec
             << '\n';
   logStream << "Result: " << baseSummary.c_str() << '\n';
   logStream << "Rounds: " << outcome.rounds;
   if (outcome.turnLimitReached && turnLimit > 0)
      logStream << " (turn limit " << turnLimit << " reached)";
   logStream << '\n';
   if (!statsText.empty())
      logStream << statsText;
   logStream << '\n';
   ...
   {
      std::ofstream statsFile(statsLogPath.c_str(), std::ios::app);
      if (statsFile) {
         statsFile << logStream.str();
         statsFile.flush();
         logWritten = true;
      } else {
         ASCString logError = "Failed to write statistics logfile ";
         logError += statsLogPath.c_str();
         displayLogMessage(0, logError);
      }
   }

   if (logWritten) {
      summary += "\nStatistics logfile: ";
      summary += statsLogPath.c_str();
   }

   displayLogMessage(1, summary);
   infoMessage(summary);
...
class ResourceLogger : public sigc::trackable {
  ...
  public:
   ResourceLogger() {
      ...
      MessagingHub::Instance().logCategorizedMessage.connect(
         sigc::mem_fun(*this, &ResourceLogger::message));
...
ScreenResolutionSetup::ScreenResolutionSetup(Cmdline& commandLine) : cli(commandLine) {
   ...
   if (CGameOptions::Instance()->forceWindowedMode &&
       !cli.f())  // cl->f == force fullscreen command line param
      fullscreen = false;
...
int main(int argc, char* argv[]) {
   ...
   bool headlessMode = cl->headless();

   bool muteEffects = CGameOptions::Instance()->sound.muteEffects || headlessMode;
   bool muteMusic = CGameOptions::Instance()->sound.muteMusic || headlessMode;
   bool disableSound = cl->q() || CGameOptions::Instance()->sound.off || headlessMode;

   SoundSystem soundSystem(muteEffects, muteMusic, disableSound);
   if (headlessMode) {
      soundSystem.setMusicVolume(0);
      soundSystem.setEffectVolume(0);
   } else {
   ...
   if (!app.InitScreen(screenResolutionSetup.getWidth(), screenResolutionSetup.getHeight(), 32,
                       flags)) {
