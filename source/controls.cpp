...
bool checkUnitsForCrash(Player& player, ASCString& text) {
   ...
   for (Player::VehicleList::const_iterator i = player.vehicleList.begin();
...
pair<int, int> calcMoveMalus(const MapCoordinate3D& start, const MapCoordinate3D& dest,
                             const Vehicle* vehicle, WindMovement* wm, bool* inhibitAttack,
   ...
   if (start.getNumericalHeight() >= 0 && dest.getNumericalHeight() >= 0) {
      ...
      if ((start.getNumericalHeight() != dest.getNumericalHeight()) && !container2container) {
         const VehicleType::HeightChangeMethod* hcm = vehicle->getHeightChange(
            start.getNumericalHeight() < dest.getNumericalHeight() ? 1 : -1,
            start.getBitmappedHeight());
         ...
         if (start.getNumericalHeight() >= 4)
            movecost = maxmalq;
         else if (start.getNumericalHeight() <= 1) {
   ...
   } else if (dest.getNumericalHeight() >= 0) {
      // moving out of container
      int mm;
      const ContainerBaseType::TransportationIO* unloadSystem =
         vehicle->getMap()
            ->getField(start.x, start.y)
            ->getContainer()
            ->vehicleUnloadSystem(vehicle->typ, dest.getBitmappedHeight());
      if (unloadSystem)
   ...
   if (checkHemming && dest.getNumericalHeight() >= 0)
      for (int c = 0; c < sidenum; c++) {
         ...
         if (fld) {
            ...
            if (fld->vehicle) {
               if (vehicle->getMap()->getPlayer(vehicle).diplomacy.isHostile(
                      fld->vehicle->getOwner()))
   ...
   for (int i = 0; i < 3; i++)
      if (!getMap()->isResourceGlobal(i)) {
         if (netcontrol & (cnet_moveenergyout << i)) {
            ...
            npop(netcontrol);
         } else if (netcontrol & (cnet_storeenergy << i)) {
            npush(netcontrol);
            netcontrol |= (cnet_stopenergyoutput << i);
            actstorage.resource(i) +=
               getResource(getStorageCapacity().resource(i) - actstorage.resource(i), i, false);
            npop(netcontrol);
         }
      }
}
...
/*   modes: 0 = energy   ohne abbuchen
            ...
            2 = fuel     ohne abbuchen
             ...
             +16         plus zurueckliefern                 <  diese Bits schliessen sich
   gegenseitig aus
...
int Building ::putResource(int need, int resourcetype, bool queryonly, int scope, int player) {
   if (need < 0)
      ...
      int placed;
      {
         ...
         placed =
            putresource.getresource(entryPosition.x, entryPosition.y, resourcetype, need, queryonly,
                                    player >= 0 ? player : getMap()->actplayer, scope);
      }
...
int Building ::getResource(int need, int resourcetype, bool queryonly, int scope, int player) {
   if (need < 0)
      ...
      int got;
      {
         ...
         got = gr.getresource(entryPosition.x, entryPosition.y, resourcetype, need, queryonly,
                              player >= 0 ? player : getMap()->actplayer, scope);
      }
      // if ( !queryonly && got > 0 )
      //   resourceChanged();
...
int Building ::getAvailableResource(int need, int resourcetype, int scope) const {
   ...
   {
      GetResource gr(getMap(), scope);
      got = gr.getresource(entryPosition.x, entryPosition.y, resourcetype, need, true,
                           getMap()->actplayer, scope);
   }
   return got;
}
