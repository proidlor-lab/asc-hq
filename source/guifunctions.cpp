...
class AttackGui : public GuiIconHandler, public GuiFunction, public sigc::trackable {
  ...
  public:
   AttackGui() { GameMap::sigMapDeletion.connect(sigc::mem_fun(*this, &AttackGui::mapDeleted)); };
...
ASCString AttackGui::getName(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   if (p.first->target == AttackWeap::vehicle)
      battle = new tunitattacksunit(attacker, actmap->getField(pos)->vehicle, true,
   ...
   ASCString result;
   result.format(
      "%s; eff strength: %d; damage inflicted to enemy: %d, making a total of ~%d~; own damage "
      "will be +%d = %d",
      cwaffentypen[getFirstBit(p.first->typ[p.second])], battle->av.strength,
      battle->dv.damage - dd, battle->dv.damage, battle->av.damage - ad, battle->av.damage);
   return result;
}
...
void MovementBase::execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
   if (!commandPending()) {
      // This situation should have already been covered by MovementBase::available(), but there
      // were crash reports with vehicle being NULL
      ...
      if (CGameOptions::Instance()->fastmove || move->getPath().size()) {
         ...
         move->calcPath();
         actmap->cleartemps(7);
         for (AStar3D::Path::const_iterator i = move->getPath().begin(); i != move->getPath().end();
              ++i)
            actmap->getField(*i)->setaTemp(1);
         displaymap();
      }
...
void EndTurn::execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
   if (!CGameOptions::Instance()->endturnquestion ||
       (choice_dlg("do you really want to end your turn ?", "~y~es", "~n~o") == 1)) {
...
void Attack::execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   for (i = attack->getAttackableBuildings().begin(); i != attack->getAttackableBuildings().end();
        i++)
      actmap->getField(i->first)->setaTemp(1);
...
class PowerSwitch : public GuiFunction {
  ...
  public:
   ...
   void execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
      std::unique_ptr<PowerGenerationSwitchCommand> pgsc(
         new PowerGenerationSwitchCommand(actmap->getField(pos)->vehicle));
...
bool DestructBuilding::available(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   if (!commandPending()) {
      ...
      DestructBuildingCommand* dbc =
         dynamic_cast<DestructBuildingCommand*>(NewGuiHost::pendingCommand);
      if (dbc) {
   ...
   bool available(const MapCoordinate& pos, ContainerBase* subject, int num) {
      ...
      if (fieldvisiblenow(fld) && fld->getContainer()) {
         if (!containeractive && !commandPending()) {
            ...
            if (fld->building && (player.diplomacy.isAllied(actmap->actplayer) ||
                                  actmap->getNeutralPlayerNum() == fld->building->getOwner() ||
                                  actmap->getCurrentPlayer().stat == Player::supervisor)) {
               if (fld->building->getCompletion() == fld->building->typ->construction_steps - 1)
                  return true;
            } else {
               if (fld->vehicle && fld->vehicle->typ->maxLoadableUnits &&
                   (player.diplomacy.isAllied(actmap->actplayer) ||
                    actmap->getCurrentPlayer().stat == Player::supervisor))
                  return true;
            }
         }
  ...
  public:
   bool available(const MapCoordinate& pos, ContainerBase* subject, int num) {
      if (!commandPending()) {
         ...
         if (eht)
            if (eht->getOwner() == actmap->actplayer)
               return JumpDriveCommand::available(eht).getAvailability() >=
                      ActionAvailability::partially;
      } else {
  ...
  public:
   bool available(const MapCoordinate& pos, ContainerBase* subject, int num) {
      ...
      if (!commandPending()) {
         ...
         if (NewGuiHost::pendingCommand) {
            RepairUnitCommand* service =
               dynamic_cast<RepairUnitCommand*>(NewGuiHost::pendingCommand);
   ...
   void execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
      if (!commandPending()) {
         std::unique_ptr<RepairUnitCommand> rp(
            new RepairUnitCommand(actmap->getField(pos)->vehicle));
  ...
  public:
      ...
      if (!commandPending()) {
         ...
         if (NewGuiHost::pendingCommand) {
            ...
            if (service && fld->getContainer()) {
               ...
               return find(destinations.begin(), destinations.end(), fld->getContainer()) !=
                      destinations.end();
            }
         }
      }
   ...
   void execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
      if (!NewGuiHost::pendingCommand) {
         ...
         const ServiceTargetSearcher::Targets& destinations = service->getDestinations();
         for (ServiceTargetSearcher::Targets::const_iterator i = destinations.begin();
...
class PutMineStage2 : public GuiFunction {
  ...
  protected:
   PutMineStage2(MineTypes type) { this->type = type; };
...
Surface buildGuiIcon(const Surface& image, bool remove = false) {
   ...
   if (o.GetPixelFormat().BitsPerPixel() == 32) {
      MegaBlitter<4, 4, ColorTransform_None, ColorMerger_PlainOverwrite,
                  SourcePixelSelector_DirectZoom>
         blitter;
      blitter.setSize(o.w(), o.h(), s.w(), s.h());
      blitter.blit(o, s,
                   SPoint(int((s.w() - blitter.getZoomX() * o.w()) / 2),
                          int((s.h() - blitter.getZoomY() * o.h()) / 2)));
   } else {
      MegaBlitter<1, 4, ColorTransform_None, ColorMerger_AlphaOverwrite,
                  SourcePixelSelector_DirectZoom>
         blitter;
      blitter.setSize(o.w(), o.h(), s.w(), s.h());
      blitter.blit(o, s,
...
ASCString ObjectBuildingGui::getName(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   ASCString result;
   if (num >= 0) {
      result.format("Build %s (%d Material, %d Fuel, %d Movepoints)", objtype->name.c_str(),
                    objtype->buildcost.material, objtype->buildcost.fuel, objtype->build_movecost);
      // num = -num;
   } else
      result.format("Remove %s (%d Material, %d Fuel, %d Movepoints)", objtype->name.c_str(),
                    objtype->removecost.material, objtype->removecost.fuel,
                    objtype->remove_movecost);

   return result;
}

void ObjectBuildingGui::addButton(int& num, const MapCoordinate& mc, ContainerBase* subject,
                                  int id) {
   GuiButton* b = host->getButton(num);
...
bool BuildVehicleCommand::available(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   if (NewGuiHost::pendingCommand) {
      ConstructUnitCommand* construct =
         dynamic_cast<ConstructUnitCommand*>(NewGuiHost::pendingCommand);
      if (construct)
...
void BuildVehicleCommand::execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   if (!construct) {
      ...
      VehicleProduction_SelectionWindow fsw(NULL, PG_Rect(10, 10, 450, 550), subject, buildables,
                                            false);
      fsw.Show();
      fsw.RunModal();
      const VehicleType* v = fsw.getVehicletype();
      if (v) {
         for (ConstructUnitCommand::Producables::const_iterator i = buildables.begin();
              i != buildables.end(); ++i)
            if (i->type == v) {
               ...
               if (i->prerequisites.getValue() & (ConstructUnitCommand::Lack::Movement)) {
                  ...
                  if (constructor)
                     message +=
                        "\nRequired: " +
                        ASCString::toString(constructor->typ->unitConstructionMoveCostPercentage *
                                            constructor->maxMovement() / 100) +
                        " points";
...
Surface generate_gui_build_icon(BuildingType* bld) {
   ...
   */
   ...
   MegaBlitter<4, 4, ColorTransform_None, ColorMerger_AlphaOverwrite, SourcePixelSelector_Rectangle>
      blitter;
...
class ConstructBuilding : public GuiFunction {
  public:
   ...
   ASCString getName(const MapCoordinate& pos, ContainerBase* subject, int num) {
      ConstructBuildingCommand* build =
         dynamic_cast<ConstructBuildingCommand*>(NewGuiHost::pendingCommand);
      if (!build)
...
class BuildingConstructionSelection : public BuildingTypeSelectionItemFactory {
  ...
  public:
   BuildingConstructionSelection(Resources plantResources, const Container& types,
                                 const Player& player)
      : BuildingTypeSelectionItemFactory(plantResources, types, player), selectedType(NULL){};
...
void ConstructBuilding::execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
   ...
   if (!build) {
      ...
      for (ConstructBuildingCommand::Producables::const_iterator i = producables.begin();
           i != producables.end(); ++i)
         if (!(i->prerequisites.getValue() &
               (ConstructBuildingCommand::Lack::Research | ConstructBuildingCommand::Lack::Level)))
      ...
      BuildingConstructionSelection* bcs = new BuildingConstructionSelection(
         fld->vehicle->getResource(Resources(maxint, maxint, maxint), true), buildings,
         fld->vehicle->getOwningPlayer());
      ItemSelectorWindow isw(NULL, PG_Rect(-1, -1, 550, 650), "Select Building", bcs);
      isw.Show();
      isw.RunModal();
      isw.Hide();

      if (bcs->getSelectedType() &&
          cbc->buildingProductionPrerequisites(bcs->getSelectedType()).ok()) {
...
class InternalAmmoTransferDialog : public GuiFunction {
  public:
   bool available(const MapCoordinate& pos, ContainerBase* subject, int num) {
      ...
      if (subject &&
          subject->getMap()->getPlayer(subject).diplomacy.isAllied(subject->getMap()->actplayer))
         if (!commandPending()) {
  ...
  public:
   bool available(const MapCoordinate& pos, ContainerBase* subject, int num) {
      ...
      if (fieldvisiblenow(fld) && c && !commandPending() &&
          actmap->getPlayer(c).diplomacy.isAllied(actmap->actplayer))
   ...
   void execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
      ...
      if (choice_dlg("do you really want to destruct this unit?", "~y~es", "~n~o") == 1) {
         std::unique_ptr<DestructUnitCommand> destructor(
            new DestructUnitCommand(fld->getContainer()));
   ...
   bool checkForKey(const SDL_KeyboardEvent* key, int modifier, int num) { return false; };
