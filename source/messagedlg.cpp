...
class NewMessage : public ASC_PG_Dialog {
   ...
   bool insertCoordinates() {
      ...
      for (SelectFromMap::CoordinateList::iterator i = coordinates.begin(); i != coordinates.end();
           ++i) {
...
NewMessage ::NewMessage(GameMap* gamemap, Message* msg, bool reminder)
   : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 600, 500), "new message") {
   ...
   if (!reminder) {
      ...
      to = new PlayerSelector(this, PG_Rect(50, 30, 150, 150), gamemap, true,
                              1 << gamemap->actplayer);
      if (msg)
         to->setSelection(msg->to);

      new PG_Label(this, PG_Rect(210, 30, 30, 20), "CC:");
      cc = new PlayerSelector(this, PG_Rect(240, 30, 150, 150), gamemap, true,
                              1 << gamemap->actplayer);
      if (msg)
   ...
   AddStandardButton("Cancel")->sigClick.connect(
      sigc::hide(sigc::mem_fun(*this, &NewMessage::cancel)));
   AddStandardButton("");
   AddStandardButton("Coordinates")
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &NewMessage::insertCoordinates)));
...
class IngameMessageViewer : public ASC_PG_Dialog {
  ...
  public:
   IngameMessageViewer(const ASCString& title, const Message& msg,
                       PG_Rect rect = PG_Rect(50, 50, 500, 400), bool autoHeader = true)
      : ASC_PG_Dialog(NULL, rect, title), message(NULL), from(NULL), to(NULL), cc(NULL) {
      keepMessage = false;
      int footerHeight;
      /* if ( !buttonText.empty() ) { */
      if (msg.reminder) {
   ...
   bool getKeepMessage() { return keepMessage; }

   ~IngameMessageViewer() { displayLogMessage(9, "~IngameMessageViewer\n"); }
};
...
class MessageLineWidget : public SelectionWidget {
  ...
  public:
   MessageLineWidget(PG_Widget* parent, const PG_Point& pos, int width, Message* msg,
                     GameMap* gamemap);

   ASCString getName() const {
      if (message->from > 0 && message->from <= 255)
         return map->getPlayer(getFirstBit(message->from)).getName();
      else if (message->from & (1 << 9))
         return "system";
      return "";
   };
...
MessageLineWidget::MessageLineWidget(PG_Widget* parent, const PG_Point& pos, int width,
                                     Message* msg, GameMap* gamemap)
   : SelectionWidget(parent, PG_Rect(pos.x, pos.y, width, 20)), message(msg), map(gamemap) {
#ifndef ctime_r
...
MessageListItemFactory::MessageListItemFactory(const MessagePntrContainer& messages, GameMap* map)
   : messageContainer(messages), gamemap(map) {
   restart();
};
...
class MessageSelectionWindow : public ASC_PG_Dialog {
  ...
  public:
   MessageSelectionWindow(PG_Widget* parent, const PG_Rect& r, const MessagePntrContainer& messages,
                          GameMap* g, bool editable);
};
...
void MessageSelectionWindow::messageSelected(Message* msg) {
   if (edit) {
      ...
      if (!viewer) {
         PG_Rect r(my_xpos + Width(), my_ypos, min(PG_Application::GetScreenWidth() / 2, 500),
                   Height());
...
MessageSelectionWindow::MessageSelectionWindow(PG_Widget* parent, const PG_Rect& r,
                                               const MessagePntrContainer& messages,
                                               GameMap* gamemap, bool editable)
   ...
   ItemSelectorWidget* isw = new ItemSelectorWidget(
      this, PG_Rect(10, GetTitlebarHeight(), r.Width() - 10, r.Height() - GetTitlebarHeight()),
      factory);
   isw->sigQuitModal.connect(sigc::mem_fun(*this, &ItemSelectorWindow::QuitModal));
};

void viewmessages(const char* title, const MessagePntrContainer& msg,
                  bool editable)  // mode : 0 verschickte ; 1 empfangene
{
...
void checkJournal(Player& player) {
   ...
   if (actmap->lastjournalchange.abstime)
      if ((actmap->lastjournalchange.turn() == actmap->time.turn()) ||
          (actmap->lastjournalchange.turn() == actmap->time.turn() - 1 &&
           actmap->lastjournalchange.move() > actmap->actplayer))
         viewjournal(false);
}
...
void viewunreadmessages(Player& player) {
   /** this is hack to prevent several message windows popping up at once.
       The problem is that the idle handler of the first message viewer may trigger or evaluate
      further events, which may themself view new messages
       */
   ...
   if ((player.stat == Player::human || player.stat == Player::supervisor) &&
       player.getParentMap()->getPlayerView() == player.getPosition()) {
