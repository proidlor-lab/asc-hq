...
/*  Datenaufbau des triggerData fieldes: [ hi 16 Bit ] [ low 16 Bit ] [ 32 bit Integer ] [ Pointer ]
   [ low 24 Bit       ]  [ high 8 Bit ] 'turn/move',                            move           turn
   'building/unit     ',Kartened/Spiel Building/Vehicle disk               ypos           xpos
   'technology researched',                                             Tech. ID
   'event',                                                             Event ID
   'tribut required' Hoehe des Tributes      Spieler, von dem Tribut gefordert wird 'all enemy *.*'
   Bit 0: alle nicht allierten Bit 1: alle, die ?ber die folgenden Bits festgelegt werden, ob
   alliiert oder nicht Bit 2 : Spieler 0
                                                                          ...
                                                                          Bit 9 : Spieler 7
...
+     Twindchange
         intensitt[3]         ( fuer tieffliegend, normalfliegend und hochfliegend ; -1 steht fuer
keine Aenderung ) Richtung[3]           ( dito )
 ...
 Treinforcements        ( alles unter DATA )
        int num      // ein int , der die Anzahl der vehicle angibt. Die vehicle, die ein
Transporter geladen hat, werden NICHT mitgezhlt. ���> die vehicle, mit tspfldloaders::writeunit in
einen memory-stream geschrieben.
 ...
 Talliancechange
      Data : Array[8][8] of int                      // status der Allianzen. Sollte vorerst
symetrisch bleiben, also nur jeweils 7 Werte abfragen. Vorerst einfach Zahlwerte eingeben. 256 steht
fuer unverndert, 257 fuer umkehrung
...
Wenn Data != NULL ist, MUss datasize die Groesse des Speicherbereichs, auf den Data zeigt,
beinhalten.
...
class PrehistoricEventStructure {
  public:
   ...
   int datasize;
   pevent next;
   int conn;  // wird nur im Spiel gebraucht, BIt 0 gibt an, das andere events abh"nging sind von
              // diesem
   ...
   // Werte ungleich -1 bedeuten automatisch, dass das event bereits erf?llt ist und evt. nur noch
   // die Zeit abzuwait ist
   ...
   /* Funktionsweise der verzoegerten Events:
      Sobald die Trigger erf?llt sind, wird triggertime[0] ausgef?llt. Dadurch wird das event
      ausgeloest, sobald das Spiel diese Zeit erreicht ist, unabh"ngig vom Zustand des mapes (
...
const char* cconnections[6] = {"destroyed",
                               "conquered",
                               "lost",
                               "seen",
                               "area entererd by any unit",
                               "area entered by specific unit"};

const char* ceventtriggerconn[8] = {"AND ", "OR ", "NOT ", "( ", "(( ", ")) ", ") ", "Clear "};

//! All actions that can be performed by events
const char* ceventactions[ceventactionnum] = {"message",
                                              "weather change",
                                              "new technology discovered",
                                              "lose campaign",
                                              "run script + next map",
                                              "new technology researchable",
                                              "map change",
                                              "discarded [ was erase event ]",
                                              "end campaign",
                                              "next map",
                                              "reinforcement",
                                              "weather change completed",
                                              "new vehicle developed",
                                              "palette change",
                                              "alliance change",
                                              "wind change",
                                              "nothing",
...
const char* ceventtrigger[ceventtriggernum] = {"*NONE*",
                                               "turn/move >=",
                                               "building conquered",
                                               "building lost",
                                               "building destroyed",
                                               ...
                                               "all units lost",
                                               "all enemy buildings destroyed/captured",
                                               "all buildings lost",
                                               "energy tribute <",
                                               "material tribute <",
                                               "fuel tribute <",
                                               "any unit enters polygon",
                                               "specific unit enters polygon",
                                               "building is seen",
                                               "irrelevant (used internally)"};

Event* readOldEvent(tnstream* stream, GameMap* gamemap, map<int, int>& eventTranslation,
...
void readOldEventLists(tnstream* stream, bool passedEvents, GameMap* spfld) {
   ...
   if (sizeof(int*) != 4) {
      displaymessage("Unable to load map " + stream->getLocation() +
                        " in old file format on a non-32 bit machine\nPlease convert this file to "
                        "the new file format on a 32 bit little endian machine and try again",
                     1);
      throw tfileerror();
   }
   ...
   for (map<EventTriggered*, int>::iterator i = eventTriggerEvents.begin();
        i != eventTriggerEvents.end(); ++i) {
...
#else
   displaymessage("Unable to load map " + stream->getLocation() +
                     " in old file format on a big endian machine\nPlease convert this file to the "
                     "new file format on a little endian machine and try again",
                  1);
   throw tfileerror();
#endif
}
...
PrehistoricEventStructure::LargeTriggerData::PolygonEntered ::PolygonEntered(
   const PrehistoricEventStructure::LargeTriggerData::PolygonEntered& poly) {
...
PrehistoricEventStructure::LargeTriggerData ::LargeTriggerData(const LargeTriggerData& data) {
   ...
   if (data.unitpolygon) {
      unitpolygon =
         new PrehistoricEventStructure::LargeTriggerData::PolygonEntered(*data.unitpolygon);
   } else
      unitpolygon = NULL;
}
...
Event* readOldEvent(tnstream* stream, GameMap* gamemap, map<int, int>& eventTranslation,
   ...
   switch (event1.a.action) {
      case cewindchange:
         ea = new WindChange(event1.intdata[0], event1.intdata[3]);
         break;
      case cegameparamchange:
         ea = new ChangeGameParameter(event1.intdata[0], event1.intdata[1]);
         break;
      case cemessage:
         ea = new DisplayMessage(event1.a.saveas);
         break;
      case cenothing:
         ea = new Action_Nothing();
         break;
      case cechangebuildingdamage:
         ea = new ChangeBuildingDamage(MapCoordinate(event1.intdata[0], event1.intdata[1]),
                                       event1.intdata[2]);
         break;
      case ceweatherchangecomplete:
         ea = new MapChangeCompleted();
         break;
      case cenextmap:
         ea = new NextMap(event1.a.saveas);
         break;
      case celosecampaign:
         ea = new LoseMap();
         break;
      ...
      case ceellipse:
         ea = new DisplayEllipse(event1.intdata[0], event1.intdata[1], event1.intdata[2],
                                 event1.intdata[3], event1.intdata[4], event1.intdata[5]);
         break;
      case ceremoveellipse:
         ea = new RemoveEllipse();
         break;
      case ceweatherchange: {
         ...
         if (event1.intdata[1] == 0) {
            ...
            for (int i = 0; i < event1.intdata[3]; ++i)
               poly.vertex.push_back(
                  MapCoordinate(event1.intdata[4 + 2 * i], event1.intdata[5 + 2 * i]));
      ...
      case cemapchange: {
         ...
         for (int i = 0; i < event1.intdata[4]; ++i)
            poly.vertex.push_back(
               MapCoordinate(event1.intdata[5 + 2 * i], event1.intdata[6 + 2 * i]));
      ...
      default:
         displaymessage("The event action %s has not been converted and will be missing!", 1,
                        ceventactions[int(event1.a.action)]);
   };
      ...
      if (event1.trigger[m]) {
         if ((event1.trigger[m] == ceventt_buildingconquered) ||
            ...
            switch (event1.trigger[m]) {
               case ceventt_buildingconquered:
                  et = new BuildingConquered(MapCoordinate(xpos, ypos));
                  break;
               case ceventt_buildinglost:
                  et = new BuildingLost(MapCoordinate(xpos, ypos));
                  break;
               case ceventt_buildingdestroyed:
                  et = new BuildingDestroyed(MapCoordinate(xpos, ypos));
                  break;
               case ceventt_building_seen:
                  et = new BuildingSeen(MapCoordinate(xpos, ypos));
                  break;
            };
         }
         ...
         if ((event1.trigger[m] == ceventt_unitconquered) ||
            ...
            if (version == 0) {
               ...
               displaymessage("The event trigger %s has not been converted and will be missing!", 1,
                              ceventtrigger[event1.trigger[m]]);
               ...
               switch (event1.trigger[m]) {
                  case ceventt_unitconquered:
                     et = new UnitConquered(event1.trigger_data[m]->networkid);
                     break;
                  case ceventt_unitlost:
                     et = new UnitLost(event1.trigger_data[m]->networkid);
                     break;
                  case ceventt_unitdestroyed:
                     et = new UnitDestroyed(event1.trigger_data[m]->networkid);
                     break;
               };
            }
         }
         ...
         if (event1.trigger[m] == ceventt_turn) {
            ...
            et = new TurnPassed(event1.trigger_data[m]->time.turn(),
                                event1.trigger_data[m]->time.move());
         }
         if (event1.trigger[m] == ceventt_any_unit_enters_polygon ||
             event1.trigger[m] == ceventt_specific_unit_enters_polygon) {
            stream->readInt();  // i
            event1.trigger_data[m]->unitpolygon =
               new PrehistoricEventStructure::LargeTriggerData::PolygonEntered;
            stream->readdata2(*event1.trigger_data[m]->unitpolygon);
            event1.trigger_data[m]->unitpolygon->data =
               new int[event1.trigger_data[m]->unitpolygon->dataSize];
            stream->readdata(event1.trigger_data[m]->unitpolygon->data,
                             event1.trigger_data[m]->unitpolygon->dataSize * sizeof(int));
            if (event1.trigger_data[m]
                   ->unitpolygon
                   ->vehiclenetworkid) {  // || event1.trigger_data[m]->unitpolygon->dummy
               ...
               event1.trigger_data[m]->unitpolygon->vehiclenetworkid =
                  event1.trigger_data[m]->unitpolygon->tempnwid;
            }

            Poly_gon pg;
            for (int i = 0; i < event1.trigger_data[m]->unitpolygon->data[1]; ++i)
               pg.vertex.push_back(
                  MapCoordinate(event1.trigger_data[m]->unitpolygon->data[2 + 2 * i],
                                event1.trigger_data[m]->unitpolygon->data[3 + 2 * i]));

            if (event1.trigger[m] == ceventt_any_unit_enters_polygon) {
               AnyUnitEntersPolygon* auep =
                  new AnyUnitEntersPolygon(event1.trigger_data[m]->unitpolygon->color);
               auep->setPolygon(pg);
               et = auep;
            } else {
               SpecificUnitEntersPolygon* suep = new SpecificUnitEntersPolygon(
                  event1.trigger_data[m]->unitpolygon->vehiclenetworkid);
               suep->setPolygon(pg);
               et = suep;
            }
         ...
         if (event1.triggerconnect[m] & (ceventtrigger_klammerauf | ceventtrigger_2klammerauf |
                                         ceventtrigger_klammerzu | ceventtrigger_2klammerzu))
         ...
         if (et) {
            ...
            displaymessage("The trigger %d (%s) for event %s could not be converted !", 1, m,
                           ceventtrigger[event1.trigger[m]], event1.description);
      }
   }
   ...
   if (missingConnection)
      displaymessage(
         "The connection of the triggers for event %s could not be completely converted !", 1,
         event1.description);
