...
//! convenience-class which automatically determines the Player of units, buildings and other game
//! objects
class PlayerID {
   int num;
...
class DiplomaticStateVector : public sigc::trackable {
  ...
  public:
   ...
   bool sharesView(PlayerID receivingPlayer) const {
      return getState(receivingPlayer.getID()) >= PEACE_SV;
   };
   bool isAllied(PlayerID towardsPlayer) const {
      return getState(towardsPlayer.getID()) >= ALLIANCE;
   };
...
class Player : public sigc::trackable {
  ...
  public:
   ...
   //! did the player exist when the turn started? Required for checking if a player has been
   //! terminated
   ...
   //! This does not mean that there really ARE events that are ready to be executed
   int queuedEvents;
   ...
   //! The time this player ended his turns. This is very informative in email games with > 2
   //! players to find out who is delaying the game.
