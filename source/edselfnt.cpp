...
bool buildingComp(const BuildingType* v1, const BuildingType* v2) {
   ...
   return (id1 < id2) || (id1 == id2 && v1->name < v2->name);
}
...
void MapComponent::displayClip(PG_Widget* parent, SDL_Surface* surface, const PG_Rect& src,
                               const PG_Rect& dst) const {
   if (!getClippingSurface().valid())
      getClippingSurface() =
         Surface::createSurface(displayWidth() + 10, displayHeight() + fontHeight + 10, 32, 0);
   ...
   PG_FontEngine::RenderText( getClippingSurface().getBaseSurface(), blitRect, blitRect.x,
   blitRect.y+fontProvidingWidget ->GetFontAscender(), getItemType()->getName(), fontProvidingWidget
   ->GetFont() );
   */
   ...
   PG_FontEngine::RenderText(surface, dst, x, y, getItemType()->getName(),
                             fontProvidingWidget->GetFont());
}
...
int VehicleItem::place(GameMap* gamemap, const MapCoordinate& mc, const VehicleType* v, int owner) {
   ...
   for (int i = 0; i < 9; ++i) {
      if (fld->vehicle->typ->height & (1 << i)) {
         ...
         if (terrainaccessible(fld, fld->vehicle) ==
             2)  //  || gamemap->getgameparameter( cgp_movefrominvalidfields)
            break;
      }
...
int BuildingItem::place(const MapCoordinate& mc) const {
   ...
   for (int x = 0; x < 4; x++)
      for (int y = 0; y < 6; y++)
         if (bld->fieldExists(BuildingType::LocalCoordinate(x, y))) {
            MapCoordinate mc =
               bld->getFieldCoordinate(actmap->getCursor(), BuildingType::LocalCoordinate(x, y));
   ...
   putbuilding(actmap, actmap->getCursor(), selection.getPlayer() * 8, bld,
               bld->construction_steps);
   return 0;
}
...
void BuildingItem::display(Surface& s, const SPoint& pos) const {
   if (!fullSizeImage.valid())
      fullSizeImage =
         Surface::createSurface(displayWidth() * 2 + 10, displayHeight() * 2 + 30, 32, 0);
   fullSizeImage.FillTransparent();
   if (actmap)
      bld->paint(fullSizeImage, SPoint(0, 0),
                 actmap->getPlayer(selection.getPlayer()).getPlayerColor());
   else
      bld->paint(fullSizeImage, SPoint(0, 0));

   MegaBlitter<colorDepth, colorDepth, ColorTransform_None, ColorMerger_AlphaOverwrite,
               SourcePixelSelector_DirectZoom, TargetPixelSelector_Valid>
      blitter;
   blitter.setZoom(0.5);
...
int ObjectItem::place(const MapCoordinate& mc) const {
   if (!actmap->getField(mc)->addobject(item))
      if (SDL_GetKeyState(NULL)[SDLK_LCTRL] || SDL_GetKeyState(NULL)[SDLK_RCTRL] ||
          choice_dlg("object cannot be built here\n(bypass this dialog by pressing <ctrl>)",
                     "~c~ancel", "~i~gnore") == 2)
...
int MineItem::place(const MapCoordinate& mc) const {
   actmap->getField(mc)->putmine(selection.getPlayer(), MineTypes(item->id),
                                 MineBasePunch[item->id - 1]);
   return 0;
}
...
class CargoItemFactory : public MapItemTypeWidgetFactory<MapItemTypeWidget<VehicleType>> {
  ...
  public:
   CargoItemFactory(ContainerBase* container_)
      : Parent(vehicleTypeRepository), container(container_){};
      ...
      if (mapItemWidget->getItem()) {
         ...
         if (container->doesVehicleFit(unit)) {
            ...
            if (!container->doesVehicleFit(unit)) {
               ...
               unit->tank.fuel = 0;
               displaymessage(
                  "Warning:\nThe unit you just set could not be loaded with full material and "
...
void addCargo(ContainerBase* container) {
   ItemSelectorWindow isw(NULL, PG_Rect(100, 100, 280, 600), "cargo",
                          new CargoItemFactory(container));
   isw.Show();
   isw.RunModal();
}
...
class ProductionItemFactory : public MapItemTypeWidgetFactory<MapItemTypeWidget<VehicleType>> {
  ...
  public:
   ProductionItemFactory(ContainerBase* container_)
      : Parent(vehicleTypeRepository), container(container_), selectedItem(NULL){};
   ...
   void itemSelected(const SelectionWidget* widget, bool mouse) { itemMarked(widget); }

   const VehicleType* getSelectedVehicleType() { return selectedItem; }
};
...
class AvailableProductionItemFactory : public SelectionItemFactory, public sigc::trackable {
  ...
  public:
   AvailableProductionItemFactory(ContainerBase* container_, ContainerBase::Production& prod)
      : container(container_), selectedItem(NULL), production(prod) {
      restart();
   };
   ...
   void itemMarked(const SelectionWidget* widget) {
      ...
      const VehicleTypeBaseWidget* mapItemWidget =
         dynamic_cast<const VehicleTypeBaseWidget*>(widget);
      assert(mapItemWidget);
...
class ProductionEditorWindow : public ASC_PG_Dialog {
  ...
  public:
   ProductionEditorWindow(ContainerBase* container)
      : ASC_PG_Dialog(NULL, PG_Rect(20, 20, 700, 550), "Unit Production") {
      ...
      allTypes = new ItemSelectorWidget(
         this, PG_Rect(border, 50, (my_width - centerSpace) / 2 - 2 * border, my_height - 100),
         allTypesFactory);

      productionFactory = new AvailableProductionItemFactory(container, production);
      productionWidget =
         new ItemSelectorWidget(this,
                                PG_Rect((my_width + centerSpace) / 2 + border, 50,
                                        (my_width - centerSpace) / 2 - 2 * border, my_height - 100),
                                productionFactory);

      PG_Button* addB =
         new PG_Button(this, PG_Rect((my_width - centerSpace) / 2, 100, centerSpace, 30), "->");
      addB->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ProductionEditorWindow::addOne)));

      PG_Button* removeB =
         new PG_Button(this, PG_Rect((my_width - centerSpace) / 2, 140, centerSpace, 30), "<-");
      removeB->sigClick.connect(
         sigc::hide(sigc::mem_fun(*this, &ProductionEditorWindow::removeOne)));
