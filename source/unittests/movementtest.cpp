...
void testHeightChangeGUI() {
   ...
   assertOrThrow(veh->height ==
                 16);  // because low level flight can be reached with less movement consumption
}
...
void testPathFinding() {
...

   {
      ...
      for (set<MapCoordinate3D>::iterator i = muc->getReachableFields().begin();
           i != muc->getReachableFields().end(); ++i) {
...

   {
      ...
      // make entries unique - there shouldn't be any duplicates, but at the time of writing this
      // test findAllAccessibleFields does return some duplicates
      ...
      for ( vector<MapCoordinate3D>::iterator i = fields.begin(); i!= fields.end(); ++i ) {
         cout << "assertOrThrow ( s.find( MapCoordinate3D(" << i->x << "," << i->y << "," <<
      i->getBitmappedHeight() << ")) != s.end() );\n";
      }
      */
   ...
   /* now testing the docking. After the shuttle has reached orbit, the trooper will move from
    * shuttle through the space station to the other shuttle, although troopers as such cannot move
    * in orbit
    */
   {
