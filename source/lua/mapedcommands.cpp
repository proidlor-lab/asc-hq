...
Building* placeBuilding(GameMap* map, const MapCoordinate& pos, const BuildingType* bld,
                        int owner) {
...
class LuaFieldSearcher : public SearchFields {
  public:
   ...
   LuaFieldSearcher(GameMap* _gamemap)
      : SearchFields(_gamemap){

        };

  protected:
   void testfield(const MapCoordinate& pos) { fields.push_back(pos); };
};
