...
class ExperienceOverview : public PG_Widget {
  ...
  protected:
   ...
   static PG_Rect getSize(const PG_Point& pos) {
      ...
      return PG_Rect(pos.x, pos.y, columns * s.w(),
                     (AttackFormula::experienceIcons + columns - 1) / columns * s.h());
   }

  public:
   ExperienceOverview(const PG_Point& pos, int exp = -1) : PG_Widget(NULL, getSize(pos), true) {
      // PG_Application::GetApp()->sigMouseButtonUp.connect( sigc::mem_fun( *this,
      // &ExperienceOverview::QuitModal ));
   }
   ...
   void eventDraw(SDL_Surface* surface, const PG_Rect& rect) {
      ...
      for (int i = 0; i < AttackFormula::experienceIcons; ++i)
         s2.Blit(IconRepository::getIcon("experience" + ASCString::toString(i) + ".png"),
                 SPoint(i % columns * width, i / columns * height));
   }
};

DashboardPanel::DashboardPanel(PG_Widget* parent, const PG_Rect& r, const ASCString& panelName_,
                               bool loadTheme = true)
   ...
   ContainerBase::anyContainerDestroyed.connect(
      sigc::mem_fun(*this, &DashboardPanel::containerDeleted));

   GameMap::sigMapDeletion.connect(sigc::mem_fun(*this, &DashboardPanel::reset));

   PG_Widget* w = parent->FindChild("unitexp", true);
   if (w)
      w->sigMouseButtonDown.connect(
         sigc::hide(sigc::hide(sigc::mem_fun(*this, &DashboardPanel::viewExperienceOverview))));
};
...
void DashboardPanel::painter(const PG_Rect& src, const ASCString& name, const PG_Rect& dst) {
   ...
   if (name == "windarrow") {
      if (actmap && actmap->weather.windSpeed > 0) {
         MegaBlitter<4, colorDepth, ColorTransform_None, ColorMerger_AlphaOverwrite,
                     SourcePixelSelector_DirectRotation>
            blitter;
   ...
   if (name == "field_weather") {
      ...
      if (actmap && mc.valid() && fieldvisiblenow(actmap->getField(mc), actmap->getPlayerView())) {
         ...
         blitter.blit(IconRepository::getIcon(weathernames[actmap->getField(mc)->getWeather()]),
                      screen, SPoint(dst.x, dst.y));
      }
      return;
   }

   if (veh && !fieldvisiblenow(veh->getMap()->getField(veh->getPosition()),
                               veh->getMap()->getPlayerView()))
      return;
   ...
   if (name == "unitexp") {
      ...
      screen.Blit(IconRepository::getIcon("experience" + ASCString::toString(idx) + ".png"),
                  SPoint(dst.x, dst.y));
   }
   ...
   if (name == "unit_level") {
      ...
      for (int i = 0; i < 8; ++i) {
         if (height1 & (1 << i)) {
            ...
            blitter.blit(IconRepository::getIcon("height-b" + ASCString::toString(i) + ".png"),
                         screen, SPoint(dst.x, dst.y + (7 - i) * 13));
         } else if (height2 & (1 << i))
            screen.Blit(IconRepository::getIcon("height-a" + ASCString::toString(i) + ".png"),
                        SPoint(dst.x, dst.y + (7 - i) * 13));
      }
   }
      ...
      for (int i = 0; i < veh->typ->weapons.count; ++i) {
         if (!veh->typ->weapons.weapon[i].service() && pos < 10) {
            if (name == "symbol_weapon" + ASCString::toString(pos))
               screen.Blit(
                  IconRepository::getIcon(SingleWeapon::getIconFileName(
                                             veh->typ->weapons.weapon[i].getScalarWeaponType()) +
                                          "-small.png"),
                  SPoint(dst.x, dst.y));
...
void DashboardPanel::showUnitData(Vehicle* veh, Building* bld, MapField* fld, bool redraw) {
   ...
   if (veh) {
      ...
      setLabelText("unitattackincrease",
                   int(af.strength_experience(veh->getExperience_offensive()) * 100));
      setLabelText("unitdefenseincrease",
                   int(af.defense_experience(veh->getExperience_defensive()) * 100));
      ...
      setBargraphValue("unitfuel", veh->getStorageCapacity().fuel
                                      ? float(veh->getTank().fuel) / veh->getStorageCapacity().fuel
                                      : 0);
      setLabelText("unitfuelstatus", veh->getTank().fuel);
      setBargraphValue("unitmaterial",
                       veh->getStorageCapacity().material
                          ? float(veh->getTank().material) / veh->getStorageCapacity().material
                          : 0);
      setLabelText("unitmaterialstatus", veh->getTank().material);
      setBargraphValue("unitenergy",
                       veh->getStorageCapacity().energy
                          ? float(veh->getTank().energy) / veh->getStorageCapacity().energy
                          : 0);
      ...
      for (int i = 0; i < veh->typ->weapons.count; ++i) {
         if (!veh->typ->weapons.weapon[i].service() && pos < 10) {
            ...
            if (veh->typ->hasFunction(ContainerBaseType::NoReactionfire) ||
                !veh->typ->weapons.weapon[i].shootable() ||
                veh->typ->weapons.weapon[i].getScalarWeaponType() == cwminen)
            ...
            setBargraphValue("bar_ammo" + ps,
                             veh->typ->weapons.weapon[i].count
                                ? float(veh->ammo[i]) / veh->typ->weapons.weapon[i].count
                                : 0);
            ++pos;
         }
      }
...
WindInfoPanel::WindInfoPanel(PG_Widget* parent, const PG_Rect& r)
   : DashboardPanel(parent, r, "WindInfo") {}

UnitInfoPanel::UnitInfoPanel(PG_Widget* parent, const PG_Rect& r)
   : DashboardPanel(parent, r, "UnitInfo") {
   ...
   if (l) {
      l->sigMouseButtonUp.connect(
         sigc::hide(sigc::hide(sigc::mem_fun(*this, &UnitInfoPanel::unitNaming))));
   }

   VehicleTypeSelectionItemFactory::showVehicleInfo.connect(
      sigc::mem_fun(*this, &UnitInfoPanel::showUnitInfo));
}
...
bool UnitInfoPanel::unitNaming() {
   ...
   if (container && RenameContainerCommand::avail(container) &&
       container->getOwner() == container->getMap()->actplayer) {
...
class WeaponInfoLine : public PG_Image {
  ...
  public:
   WeaponInfoLine(WeaponInfoPanel* parent, const PG_Point& p, SDL_Surface* image,
                  const SingleWeapon* weap, const VehicleType* vehicle)
   ...
   void painter(const PG_Rect& src, const ASCString& name, const PG_Rect& dst) {
      ...
      if (name == "weapon_symbol1")
         screen.Blit(
            IconRepository::getIcon(SingleWeapon::getIconFileName(weapon->getScalarWeaponType()) +
                                    "-small.png"),
            SPoint(dst.x, dst.y));
...
WeaponInfoPanel::WeaponInfoPanel(PG_Widget* parent, const Vehicle* veh, const VehicleType* vt)
   : Panel(parent, PG_Rect::null, "WeaponInfo"), weaponCount(0) {
   SetName(name);
   ...
   for (int i = 0; i < weaponCount; ++i) {
      ...
      WeaponInfoLine* lineWidget =
         new WeaponInfoLine(this, PG_Point(0, lineStartY + i * line.h()), line.getBaseSurface(),
                            displayedWeapons[i], vt);
   ...
   setLabelText("weapon_moveaftershoot",
                vt->hasFunction(ContainerBaseType::MoveAfterAttack) ? "yes" : "no");
...
void WeaponInfoPanel::showWeapon(const SingleWeapon* weap) {
   ...
   if (weap) {
      setImage("weapon_symbol2",
               IconRepository::getIcon(SingleWeapon::getIconFileName(weap->getScalarWeaponType()) +
                                       "-small.png"));
      show("weapon_symbol2");
   } else
      hide("weapon_symbol2");
...
MapInfoPanel::MapInfoPanel(PG_Widget* parent, const PG_Rect& r, MapDisplayPG* mapDisplay)
   : DashboardPanel(parent, r, "MapInfo"), zoomSlider(NULL), changeActive(false) {
   assert(mapDisplay);
...
ActionInfoPanel::ActionInfoPanel(PG_Widget* parent, const PG_Rect& r)
   : DashboardPanel(parent, r, "ActionInfo") {
