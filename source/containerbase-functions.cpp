...
class GenericWorkerFactory : public ContainerBase::WorkClassFactory {
   bool available(const ContainerBase* cnt) { return cnt->baseType->hasFunction(f); };

   ContainerBase::Work* produce(ContainerBase* cnt, bool queryOnly) { return new T(cnt); };
};

class BiResourceGenerationFactory : public ContainerBase::WorkClassFactory {
   bool available(const ContainerBase* cnt) { return true; };
...
const bool r1 = ContainerBase::registerWorkClassFactory(
   new GenericWorkerFactory<MatterConverter, ContainerBaseType::MatterConverter>());
const bool r2 = ContainerBase::registerWorkClassFactory(
   new GenericWorkerFactory<ResourceSink, ContainerBaseType::ResourceSink>);
const bool r3 = ContainerBase::registerWorkClassFactory(
   new GenericWorkerFactory<WindPowerplant, ContainerBaseType::WindPowerPlant>);
const bool r4 = ContainerBase::registerWorkClassFactory(
   new GenericWorkerFactory<SolarPowerplant, ContainerBaseType::SolarPowerPlant>);
...
void AutoHarvestObjects::harvestObject(const MapCoordinate& pos, const ObjectType* obj) {
   ...
   if (object != NULL) {
      ...
      if (base->getResource(removeCost, true, 1, base->getOwner()) == removeCost) {
         ...
         logMessage("ResourceWork", "AutoHarvester " + tos(base->getPosition()) +
                                       ": harvested object " + tos(obj->id) + " on field " +
                                       tos(pos) + " and got " + removeBenefit.toString());
      }
   }
}
...
void AutoHarvestObjects::processField(const MapCoordinate& pos) {
   ...
   for (vector<IntRange>::const_iterator i = base->baseType->autoHarvest.objectsHarvestable.begin();
        i != base->baseType->autoHarvest.objectsHarvestable.end(); ++i)
   ...
   for (vector<IntRange>::const_iterator i =
           base->baseType->autoHarvest.objectGroupsHarvestable.begin();
        i != base->baseType->autoHarvest.objectGroupsHarvestable.end(); ++i)
...
bool AutoHarvestObjects::run() {
   ...
   circularFieldIterator(base->getMap(), base->getPosition(), 0, base->baseType->autoHarvest.range,
                         FieldIterationFunctor(this, &AutoHarvestObjects::iterateField));
...
GetMiningInfo ::GetMiningInfo(const ContainerBase* container)
   : SearchFields(container->getMap()), miningStation(container) {
   run();
}
...
bool MatterConverter ::run() {
   ...
   logMessage("ResourceWork", "MatterConverter " + tos(bld->getPosition()) +
                                 ": still needs to do " + tos(perc) + "% of work");
   ...
   logMessage("ResourceWork", "MatterConverter " + tos(bld->getPosition()) +
                                 ": checked storage, limiting to " + tos(perc) + "% of work");
   ...
   logMessage("ResourceWork", "MatterConverter " + tos(bld->getPosition()) +
                                 ": checked required res, limiting to " + tos(perc) + "% of work");
   ...
   for (int r = 0; r < 3; r++)
      if (bld->plus.resource(r) > 0) {
         ...
         logMessage("ResourceWork", "MatterConverter " + tos(bld->getPosition()) + ": generated " +
                                       tos(toput) + " of " + Resources::name(r) +
                                       ", successfully stored " + tos(didput) + " of it");

      } else {
         if (bld->plus.resource(r) < 0) {
            int toget = -bld->plus.resource(r) * perc / 100;
            int got = bld->getResource(toget, r, false, 1, bld->getOwner());
            logMessage("ResourceWork", "MatterConverter " + tos(bld->getPosition()) + ": needed " +
                                          tos(toget) + " of " + Resources::name(r) +
                                          ", successfully got " + tos(got) + " of it");
         }
      }

   percentage -= perc;

   logMessage("ResourceWork", "MatterConverter " + tos(bld->getPosition()) + ": did " + tos(perc) +
                                 "% of work, " + tos(percentage) + "% left");
...
MiningStation ::MiningStation(ContainerBase* bld_, bool justQuery_)
   : SearchFields(bld_->getMap()), bld(bld_), justQuery(justQuery_) {
   int counter = 0;
   for (int r = 1; r < 3; r++) {
      if (bld->plus.resource(r) > 0) {
         ++counter;
         if (counter == 2)
            fatalError(
               ASCString("A mining station can only produce ONE kind of resource; building ID") +
               strrr(bld->baseType->id) + " is violating this");
   ...
   if (!justQuery) {
      ...
      for (int r = 0; r < 3; ++r)
         if (spaceAvail.resource(r) < 0) {
            warningMessage(
               ASCString("map corruption detected; building space availability is negative! ") +
               resourceNames[r]);
   ...
   if (!justQuery) {
      ...
      for (int r = 0; r < 3; ++r)
         if (powerAvail.resource(r) < 0) {
            warningMessage(
               ASCString(
                  "map corruption detected; available power for mining station is negative! ") +
               resourceNames[r]);
      ...
      for (int r = 1; r < 3; r++) {
         if (toExtract_thisTurn.resource(r) > 0) {
            ...
            if (r == 1) {
               ...
               // double(bld->baseType->efficiencymaterial) / 1024;
               resourceFactor = resource_material_factor;
               fieldResource = &fld->material;
            } else {
               // resourceFactor = resource_fuel_factor * double(bld->baseType->efficiencyfuel) /
               // 1024;
            ...
            perc = min(perc, (*fieldResource * resourceFactor) /
                                (toExtract_thisTurn.resource(r) * distEfficiency));

            perc = min(perc, float(double(spaceAvail.resource(r)) /
                                   (toExtract_thisTurn.resource(r) * distEfficiency)));

            for (int i = 0; i < 3; ++i)
               if (usageRatio[i] * toExtract_thisTurn.resource(r) > 0)
                  perc = min(perc, float(double(powerAvail.resource(i)) / usageRatio[i] *
                                         toExtract_thisTurn.resource(r)));
            ...
            if (!justQuery)
               *fieldResource -=
                  int(toExtract_thisTurn.resource(r) * perc * distEfficiency / resourceFactor);

            int ex = int(ceil(toExtract_thisTurn.resource(r) * perc * distEfficiency));
            actuallyExtracted.resource(r) += ex;
            spaceAvail.resource(r) -= ex;
            for (int i = 0; i < 3; ++i) {
               if (spaceAvail.resource(i) <
                   -2)  // we allow for small deviations due to rounding errors
