...
const char* cdirections[WeatherSystem::WindDirNum] = {"North", "NorthEast", "SouthEast",
...
WeatherArea::WeatherArea(GameMap* m, int xCenter, int yCenter, int w, int h, int d,
                         FalloutType fallout, unsigned int sV, bool clustered)
   : map(m),
     ...
     seedValue(sV),
     verticalWindAccu(0),
     horizontalWindAccu(0),
     clustered(clustered) {
   WeatherField* wf;
   for (int i = 0; i < width * height; i++) {
      int x = i % width;
      int y = i / width;
      wf = new WeatherField(MapCoordinate(x + (center.x - width / 2), y + (center.y - height / 2)),
                            this);
      area.push_back(wf);
   }
   createWeatherFields();
...
WeatherArea::WeatherArea(GameMap* m, int xCenter, int yCenter, int r)
   : map(m), center(xCenter, yCenter), width(r), height(r), radius(r) {}
...
void WeatherArea::updateMovementVector(unsigned int windspeed, Direction windDirection,
                                       double ratio) {
   if (windDirection == N) {
...
void WeatherArea::read(tnstream& inputStream) {
   ...
   for (int i = 0; i < size; i++) {
      ...
      WeatherField* wf = new WeatherField(
         MapCoordinate(x + (center.x - width / 2), y + (center.y - height / 2)), this);
...
int WeatherArea::calculateCornerPointValue(int a, int b, int c) {
   ...
   cornerValue =
      cornerValue + static_cast<int>((createRandomValue(MAXOFFSET) * 1.3 * createAlgebraicSign()));
   if (cornerValue < 0) {
      cornerValue = 0;
...
int WeatherArea::calculateDiamondPointValue(int a, int b, int c, int d) {
   int diamondValue =
      (area[a]->getValue() + area[b]->getValue() + area[c]->getValue() + area[c]->getValue()) / 4;
   diamondValue =
      diamondValue + static_cast<int>((createRandomValue(MAXOFFSET) * 1.3 * createAlgebraicSign()));
   if (diamondValue < 0) {
      diamondValue = 0;
...
WeatherField::WeatherField(MapCoordinate mapPos, const WeatherArea* area)
   : posInArea(mapPos), mapField(0) {
...
bool WeatherField::isOnMap(const GameMap* map) const {
   if ((posInArea.x >= 0) && (posInArea.x < map->xsize) && (posInArea.y >= 0) &&
       (posInArea.y < map->ysize))
      return true;
   else
      return false;
}
...
WeatherSystem::WeatherSystem(GameMap* map, int spawn, float ws2fr, unsigned int tInterval,
                             WeatherSystemMode mode, FalloutType defaultFT)
   : timeInterval(tInterval),
     windspeed2FieldRatio(ws2fr),
     areaSpawnAmount(spawn),
     maxForecast(5),
     gameMap(map),
     currentMode(mode),
     windspeed(0),
     globalWindDirection(S),
     lowerRandomSize(0.25),
     upperRandomSize(0.8),
     access2RandCount(0),
     defaultFallout(defaultFT),
     lowerRandomDuration(tInterval),
     upperRandomDuration(tInterval * 3) {
   seedValue = time(0);
   seedValueIsSet = true;
...
void WeatherSystem::randomWeatherChange(GameTime currentTime, Direction windDirection, int delay) {
   ...
   int ypos;
   int width =
      static_cast<int>(gameMap->xsize * createRandomValue(lowerRandomSize, upperRandomSize));
   int height =
      static_cast<int>(gameMap->ysize * createRandomValue(lowerRandomSize, upperRandomSize));
   ...
   newArea = new WeatherArea(
      gameMap, xpos, ypos, width, height,
      static_cast<unsigned int>(createRandomValue(lowerRandomDuration, upperRandomDuration)), ft,
      createRandomValue());
   // cout << "added area: " << ft << " turn: " << t.turn() << " expected dir: " << windDirection <<
   // endl;
...
void WeatherSystem::update() {
   ...
   startTime.set(1, 0);
   Direction newDirection;
   if ((currentMode == RANDOMMODE) &&
       ((currentTime.turn() % timeInterval == 0) || (currentTime.abstime == startTime.abstime))) {
   ...
   for (WeatherAreaList::iterator it = activeWeatherAreas.begin(); it != activeWeatherAreas.end();
        it++) {
      if ((*it)->getDuration() == 0) {
...
void WeatherSystem::addGlobalWindChange(int speed, Direction direction,
                                        int turn) throw(IllegalValueException) {
...
void WeatherSystem::write(tnstream& outputStream) const {
   ...
   for (WeatherAreaList::const_iterator i = activeWeatherAreas.begin();
        i != activeWeatherAreas.end(); ++i) {
      (*i)->write(outputStream);
   }
