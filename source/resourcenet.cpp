...
void MapNetwork ::searchAllVehiclesNextToBuildings(int player) {
   pass++;
   for (Player::VehicleList::iterator j = actmap->player[player].vehicleList.begin();
        j != actmap->player[player].vehicleList.end(); j++) {
   ...
   if (globalsearch() == 2) {
      for (int i = 0; i < 8; i++)
         if (actmap->player[i].exist()) {
            for (Player::BuildingList::iterator j = actmap->player[i].buildingList.begin();
                 j != actmap->player[i].buildingList.end(); j++)
               checkbuilding(*j);
...
int StaticResourceNet ::getresource(int x, int y, int resource, int _need, int _queryonly,
                                    int _player, int _scope) {
...
GetResource ::GetResource(GameMap* gamemap, int scope) : StaticResourceNet(gamemap, scope) {
...
void GetResource ::checkbuilding(Building* b) {
   if (b->color / 8 == player) {
      if ((b->netcontrol & (cnet_stopenergyoutput << resourcetype)) == 0) {
         ...
         if (b->actstorage.resource(resourcetype) < 0) {
            displaymessage("map corruption detected; building %s storage for %d/%d is negative!", 1,
                           resourceNames[resourcetype], b->getEntry().x, b->getEntry().y);
   ...
   } else if (b->color / 8 < 8) {  // no neutral buildings
      ...
      int gettable = actmap->tribute.avail[b->color / 8][player].resource(resourcetype) -
                     tributegot[resourcetype][b->color / 8];
      if (gettable > 0) {
         int toget = need - got;
         if (toget > gettable)
            toget = gettable;
...
void PutResource ::checkbuilding(Building* b) {
   if (b->color / 8 == player) {
      if ((b->netcontrol & (cnet_stopenergyinput << resourcetype)) == 0) {
         ...
         if (b->getStorageCapacity().resource(resourcetype) - b->actstorage.resource(resourcetype) <
             tostore)
            tostore = b->getStorageCapacity().resource(resourcetype) -
                      b->actstorage.resource(resourcetype);
...
int PutTribute ::puttribute(Building* start, int resource, int _queryonly, int _forplayer,
                            int _fromplayer, int _scope) {
   startbuilding = start;
   targplayer = _forplayer;
   return getresource(startbuilding->getEntry().x, startbuilding->getEntry().y, resource, 0,
                      _queryonly, _fromplayer, _scope);
}
         ...
         if (targplayer != player)
            if (actmap->player[player].exist()) {
               ...
               for (int resourcetype = 0; resourcetype < 3; resourcetype++) {
                  got[resourcetype] =
                     actmap->tribute.paid[targplayer][player].resource(resourcetype) -
                     actmap->tribute.payStatusLastTurn[targplayer][player].resource(resourcetype);
                  topay[resourcetype] =
                     actmap->tribute.avail[player][targplayer].resource(resourcetype) +
                     got[resourcetype];

                  if (!actmap->isResourceGlobal(resourcetype)) {
                     for (Player::BuildingList::iterator j =
                             actmap->player[player].buildingList.begin();
                          j != actmap->player[player].buildingList.end() &&
                          topay[resourcetype] > got[resourcetype];
                          j++) {
                        PutTribute pt(actmap);
                        got[resourcetype] +=
                           pt.puttribute(*j, resourcetype, 0, targplayer, player, 1);
                     }
                  } else {
                     int i;
               ...
               actmap->tribute.payStatusLastTurn[targplayer][player] =
                  actmap->tribute.paid[targplayer][player];
               if (topay[0] || topay[1] || topay[2] || got[0] || got[1] || got[2]) {
                  ...
                  ASCString msg;
                  msg.format(getmessage(10020), actmap->player[player].getName().c_str(),
                             topayStr.c_str(), gotStr.c_str());
                  ...
                  if (got[0] || got[1] || got[2]) {
                     msg = "";
                     gotStr = gt.toString();
                     msg.format(getmessage(10021), gotStr.c_str(),
                                actmap->player[targplayer].getName().c_str());
...
bool compareMapResources(GameMap* currentMap, GameMap* replaymap, int player, ASCString* log) {
   ...
   for (int r = 0; r < 3; ++r) {
      if (currentMap->isResourceGlobal(r)) {
         if (currentMap->bi_resource[player].resource(r) !=
             replaymap->bi_resource[player].resource(r)) {
            diff = true;
            if (log) {
               s.format(
                  "Global resource mismatch: %d %s available after replay, but %d available in "
                  "actual map\n",
                  replaymap->bi_resource[player].resource(r), resourceNames[r],
                  currentMap->bi_resource[player].resource(r));
               *log += s;
            }
         }
      } else {
         GetConnectedBuildings::BuildingContainer cb;
         for (Player::BuildingList::iterator b = currentMap->player[player].buildingList.begin();
              b != currentMap->player[player].buildingList.end(); ++b) {
            Building* b1 = *b;
            ...
            if (!b1 || !b2) {
               ...
               if (find(cb.begin(), cb.end(), b1) == cb.end()) {
                  ...
                  if (ab1 != ab2) {
                     ...
                     if (log) {
                        s.format(
                           "Building (%d,%d) resource mismatch: %d %s available after replay, but "
                           "%d available in actual map\n",
   ...
   for (Player::VehicleList::iterator v = currentMap->player[player].vehicleList.begin();
        v != currentMap->player[player].vehicleList.end(); ++v) {
      Vehicle* v1 = *v;
      ...
      if (!v1 || !v2) {
         ...
         if (v1->typ != v2->typ) {
            ...
            if (log) {
               s.format(
                  "Vehicles with NWID %d don't have matching type, probably production "
                  "inconsistency. (%s <-> %s)\n",
            ...
            for (int r = 0; r < 3; ++r) {
               ...
               if (av1 != av2) {
                  ...
                  if (log) {
                     s.format(
                        "Vehicle %s (%d,%d) resource mismatch: %d %s available after replay, but "
                        "%d available in actual map\n",
                        v1->typ->getName().c_str(), v1->getPosition().x, v1->getPosition().y, av2,
                        resourceNames[r], av1);
                     *log += s;
                  }
               }
            ...
            if (v1->damage != v2->damage) {
               ...
               if (log) {
                  s.format(
                     "Vehicle (%d,%d) damage mismatch: %d after replay, but %d in actual map\n",
                     v1->getPosition().x, v1->getPosition().y, v2->damage, v1->damage);
                  *log += s;
               }
            }
            if (v1->getPosition().x != v2->getPosition().x ||
                v1->getPosition().y != v2->getPosition().y) {
               diff = true;
               if (log) {
   ...
   if (currentMap->player[player].vehicleList.size() !=
       replaymap->player[player].vehicleList.size()) {
      diff = true;
      if (log) {
         s.format("The number of units differ. Replay: %d ; actual map: %d",
                  replaymap->player[player].vehicleList.size(),
                  currentMap->player[player].vehicleList.size());
         *log += s;
         ...
         for (Player::VehicleList::iterator i = more->player[player].vehicleList.begin();
              i != more->player[player].vehicleList.end(); ++i)
            if (!less->getUnit((*i)->networkid)) {
               s.format("\nType: %s at %d/%d, nwid: %d", (*i)->getName().c_str(),
                        (*i)->getPosition().x, (*i)->getPosition().y, (*i)->networkid);
               *log += s;
            }
      }
   }

   if (currentMap->player[player].buildingList.size() !=
       replaymap->player[player].buildingList.size()) {
      diff = true;
      if (log) {
         s.format("The number of buildings differ. Replay: %d ; actual map: %d",
                  replaymap->player[player].buildingList.size(),
                  currentMap->player[player].buildingList.size());
         *log += s;
      }
   }

   if (currentMap->player[player].research.progress !=
       replaymap->player[player].research.progress) {
      diff = true;
      if (log) {
         s.format("Research points mismatch! Replay: %d ; actual map: %d",
                  replaymap->player[player].research.progress,
                  currentMap->player[player].research.progress);
         *log += s;
      }
   }

   sort(currentMap->player[player].research.developedTechnologies.begin(),
        currentMap->player[player].research.developedTechnologies.end());
   sort(replaymap->player[player].research.developedTechnologies.begin(),
        replaymap->player[player].research.developedTechnologies.end());
   if (replaymap->player[player].research.developedTechnologies.size() !=
       currentMap->player[player].research.developedTechnologies.size()) {
      diff = true;
      if (log) {
      ...
      for (int i = 0; i < replaymap->player[player].research.developedTechnologies.size(); ++i)
         if (replaymap->player[player].research.developedTechnologies[i] !=
             currentMap->player[player].research.developedTechnologies[i]) {
            diff = true;
            if (log) {
