...
class GenericGuiFunction : public GuiFunction {
     ...
     public:
        ...
        GenericGuiFunction( Surface icon_, Availability availability, Execution execution, const
ASCString& iconName ) : name(iconName), icon(icon_), avail(availability), exec(execution) {};
...
class GuiIconHandler {
  ...
  public:
   ...
   /** registers a user function. Icons are displayed in the order that they were registered.
      By passing an object here, the GuiIconHandler will obtain ownership of the object and delete
