...
void generategrayxlattable(ppixelxlattable tab, Uint8 offset, Uint8 size, dacpalette256* pal) {
   for (int b = 0; b <= 255; b++) {
      //		(*tab)[b] = (Uint8) (offset + size - 1 - ((*activepalette256)[b][0] +
      //(*activepalette256)[b][1] + (*activepalette256)[b][2]) * size / 192);
      (*tab)[b] =
         (Uint8) (offset + size - 1 -
                  (0.299 * (*pal)[b][0] + 0.587 * (*pal)[b][1] + 0.114 * (*pal)[b][2]) * size / 64);
   }
}
   ...
   for (int y = 0; y < fieldysize; y++) {
      for (int x = 0; x < fieldxsize; x++) {
         ...
         if (organgle != 0 && organgle != -180 && organgle != 180) {
            ...
            nx = -dx;
            ny = -dy;
         } else if (organgle == 180 || organgle == -180) {
            nx = dx;
            ny = dy;
         }
...
void putpixel(int x1, int y1, int color) {
   ...
   if (agmp->byteperpix == 1)
      ...
      if (agmp->windowstatus == 100) {
         Uint8* pc =
            (Uint8*) (agmp->linearaddress + x1 * agmp->byteperpix + y1 * agmp->scanlinelength);
         int alpha = color >> 24;
         if (alpha == 0) {
            ...
            pc[agmp->redfieldposition / 8] = pc[agmp->redfieldposition / 8] * alpha / alphabase +
                                             (color & 0xff) * (alphabase - alpha) / alphabase;
            pc[agmp->greenfieldposition / 8] =
               pc[agmp->greenfieldposition / 8] * alpha / alphabase +
               ((color >> 8) & 0xff) * (alphabase - alpha) / alphabase;
            pc[agmp->bluefieldposition / 8] =
               pc[agmp->bluefieldposition / 8] * alpha / alphabase +
               ((color >> 16) & 0xff) * (alphabase - alpha) / alphabase;
         }
      } else {
         /*
          ...
          if ( alpha == 0 ) {
             ...
             pc[ agmp->redfieldposition/8 ] = pc[ agmp->redfieldposition/8 ] * alpha / alphabase +
          (color & 0xff) * (alphabase - alpha ) / alphabase; pc[ agmp->greenfieldposition/8 ] = pc[
          agmp->greenfieldposition/8 ] * alpha / alphabase + ((color >> 8 ) & 0xff) * (alphabase -
          alpha ) / alphabase; pc[ agmp->bluefieldposition/8 ] = pc[ agmp->bluefieldposition/8 ] *
          alpha / alphabase + ((color >> 16) & 0xff) * (alphabase - alpha ) / alphabase;
          }
         */
      }
   }
...
void putmask(int x1, int y1, void* vbuf, int newtransparence) {
   ...
   Uint8* basemembuf =
      (Uint8*) (agmp->linearaddress + y1 * agmp->bytesperscanline + x1 * agmp->byteperpix);
...
void tvirtualdisplay ::init(int x, int y, int color, int depth) {
   ...
   if (depth == 24 || depth == 32) {
      ...
      surface = agmp->surface =
         new Surface(Surface::CreateSurface(buf, x, y, depth, 0xff, 0xff00, 0xff0000));
   } else {
      surface = agmp->surface =
         new Surface(Surface::CreateSurface(buf, x, y, depth, agmp->scanlinelength));
...
void bar(int x1, int y1, int x2, int y2, Uint8 color) {
   ...
   if (agmp->windowstatus == 100) {
      ...
      Uint8* buf =
         (Uint8*) (agmp->scanlinelength * y1 + x1 * agmp->byteperpix + agmp->linearaddress);
      for (int y = y1; y <= y2; y++) {
         for (int x = x1; x <= x2; x++)
            *(buf++) = color;
...
void showtext(const char* text, int x, int y, int textcol) {
   ...
   else if (activefontsettings.height > activefontsettings.font->height) {
      fontheight = activefontsettings.font->height;
   ...
   if (!activefontsettings.length)
      ...
      } else if (activefontsettings.justify == 1) {
         if (activefontsettings.length > length) {
      ...
      } else if (activefontsettings.justify == 2) {
         rightextralength = 0;
         if (activefontsettings.length > length)
   ...
   collategraphicoperations cgo(x, y, x + length + leftextralength + rightextralength,
                                y + fontheight + extraheight);
         ...
         for (cx = characterwidth[ps]; cx > 0 && x < length; cx--) {
            ...
            if (pix) {
               if (textcol != -1)
                  ...
                  *fb = pix;
            } else if (activefontsettings.background != 255) {
               if (suppressbkgr)
                  suppressbkgr--;
               else
                  *fb = activefontsettings.background;
      ...
      for (int y = 0; y < s.h(); y++)
         for (int x = 0; x < s.w(); x++) {
            ...
            if (alpha < 128)
               ...
               buf[4 + s.w() * y + x] =
                  truecolor2pal_table[(red >> 2) + ((green >> 2) << 6) + ((blue >> 2) << 12)];
         }
   }
