...
class Building : public ContainerBase {
  ...
  public:
   ...
   //! a bitmapped variable containing the status of the resource-net connection. \see
   //! execnetcontrol() \deprecated
   int netcontrol;

   //! bitmapped: are there events that are triggered by actions affecting this building
   int connection;

   //! is the building visible? Building can be made invisible, but this feature should be used only
   ...
   Building(GameMap* map, const MapCoordinate& entryPosition, const BuildingType* type, int player,
            bool setupImages = true, bool chainToField = true);
  ...
  public:
   ...
   //! returns the picture of the building. It may depend on the current weather of the fields the
   ...
   /** Sets the level of completion of the building.
       \param completion range: 0 .. typ->construction_steps-1 . If completion ==
   typ->construction_steps-1 the building is completed and working. \param setupImages Are the
   building image pointer of the fields the building is standing on updated? \see
   ...
   **/
   ...
   //! returns a name for the building. If the building itself has a name, it will be returned. If
   //! it doesn't, the name of the building type will be returned.
