...
class MapField {
  ...
  public:
   ...
   /** mineral resources on this field.
       \note that mineral resources are different from #Resources , there is a factor of
      #resource_fuel_factor and #resource_material_factor in between
   */
   Uint8 fuel, material;
   ...
   //! can this field be seen be the player. Variable is bitmapped; two bits for each player. These
   //! two bits can have the states defined in ::VisibilityStates
   Uint16 visible;
   ...
   //! the mineral resources that were seen by a player on this field; since the actual amount may
   //! have decreased since the player looked, this value is not identical to the fuel and material
   //! fields.
   ...
   //! returns the nth mine. This function should only be used by legacy code; new code should store
   //! an iterator instead of an index
   Mine& getMine(int n);
   ...
   **/
   ...
   /** removes all objects of the given type from the field
       ...
       \param force remove the object even if there are obstacles on the field (like a building
      standing on the object) \returns if the removal was successful
   */
   bool removeObject(const ObjectType* obj, bool force = false,
                     ObjectRemovalStrategy* objectRemovalStrategy = NULL);

   //! sorts the objects. Since objects can be on different levels of height, the lower one must be
   //! displayed first
   ...
   //! the terraintype properties. They determine which units can move over the field. This variable
   //! is recalculated from the terraintype and objects each time something on the field changes
   //! (#setparams)
   TerrainBits bdt;
   ...
   //! this method should be used instead of comparing the pointers to the unit
   ...
   //! put a mine of type typ for player owner and a punch of strength on the field. Strength is an
   //! absolute value (unlike the basestrength of a mine or the punch of the mine-weapon, which are
   //! just factors)
   ...
   **/
   ...
   /** The visibility status for all players is stored in a bitmapped variable. This functions
      changes the status in this variable for a single player \param valtoset the value that is
      going to be written into the visibility variable \param actplayer the player for which the
      view is changed
   */
