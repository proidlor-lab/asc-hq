...
void TerrainType::Weather::paint(Surface& s, SPoint pos) {
   ...
   megaBlitter<ColorTransform_None, ColorMerger_AlphaOverwrite, SourcePixelSelector_Plain,
               TargetPixelSelector_All>(*img, s, pos, nullParam, nullParam, nullParam, nullParam);
}
...
int TerrainAccess ::accessible(const TerrainBits& bts) const {
   if ((terrain & bts).any() && (terrainreq & bts) == terrainreq && (terrainnot & bts).none())
      return 1;
   else if ((terrainkill & bts).any())
      return -1;
   else
      return 0;
}
...
void TerrainType::Weather::runTextIO(PropertyContainer& pc) {
   ...
   if (!bi3pics) {
      ...
      if (pc.isReading()) {
         int operations;
         pc.addNamedInteger("GraphicOperations", operations, graphicOperationNum, graphicOperations,
                            0);
         if (operations == 1)
            snowify(image);
...
TerrainBits getTerrainBitType(TerrainBitTypes tbt) {
   ...
   switch (tbt) {
      ...
      case cbwater3:
         ...
         break;
      case cbwater:
         tb.setInt((1 << 22)    // very shallow water
                   | (1 << 0)   // shallow water
                   | (1 << 12)  // water
                   | (1 << 13)  // deep water
                   | (1 << 31)  // river
      ...
      case cbfrozenwater:
         ...
         break;
      case cbicebreaking:
         tb |= getTerrainBitType(cbfrozenwater) | getTerrainBitType(cbsnow1) |
