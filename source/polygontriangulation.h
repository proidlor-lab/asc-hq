...
class PolygonPainter {
  ...
  protected:
   ...
   virtual bool checkline(typename Poly::Point a, typename Poly::Point b, typename Poly::Point d,
...
template <class Poly>
bool PolygonPainter<Poly>::checkline(typename Poly::Point a, typename Poly::Point b,
                                     typename Poly::Point d, typename Poly::Point e) {
   double r, s, t;
   ...
   if (poly.vertex.size() > 1 && poly.vertex.size() < 3)
      ...
      for (i = 0; i < poly.vertex.size(); i++)
         for (j = 0; j < poly.vertex.size(); j++)
            if (i != j)
               if (poly.vertex[i].x == poly.vertex[j].x && poly.vertex[i].y == poly.vertex[j].y)
                  return false;

      k = poly.vertex.size() - 1;
      for (i = 0; i < k; i++) {
         for (j = i + 2; j < k; j++)
            if (checkline(poly.vertex[i], poly.vertex[i + 1], poly.vertex[j], poly.vertex[j + 1]) ==
                false)
               return false;
         if (i > 0 && j == k)
            if (checkline(poly.vertex[i], poly.vertex[i + 1], poly.vertex[j], poly.vertex[0]) ==
                false)
               return false;
      } /* endfor */
      return true;
...
int PolygonPainter<Poly>::getpolydir(const Poly& a) {
   ...
   for (p = 0; p < a.vertex.size(); p++) {
      if (p == 0)
         alpha = getsegmentdir(a.vertex[p].x - a.vertex[a.vertex.size() - 1].x,
                               a.vertex[p].y - a.vertex[a.vertex.size() - 1].y);
      else
         alpha =
            getsegmentdir(a.vertex[p].x - a.vertex[p - 1].x, a.vertex[p].y - a.vertex[p - 1].y);
