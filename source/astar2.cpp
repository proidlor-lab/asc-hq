...
class OpenContainer {
   typedef boost::heap::pairing_heap<AStar3D::Node, boost::heap::compare<greater<AStar3D::Node>>,
                                     boost::heap::stable<true>,
                                     boost::heap::constant_time_size<false>>
      storage_t;
   typedef boost::unordered_map<MapCoordinate3D, storage_t::handle_type,
  ...
  public:
   ...
   void pushOrUpdate(const AStar3D::Node& n) {
      ...
      } else if (((*i->second).gval > n.gval) ||
                 ((*i->second).gval == n.gval && (*i->second).hasAttacked && !n.hasAttacked)) {
         storage.increase(i->second, n);
      }
   }
};

static inline int windbeeline(const MapCoordinate& start, const MapCoordinate& dest,
...
AStar3D ::AStar3D(GameMap* actmap_, Vehicle* veh_, bool markTemps_, int maxDistance)
   ...
   */
   ...
   if ((veh->typ->height & (chtieffliegend | chfliegend | chhochfliegend)) &&
...
AStar3D::DistanceType AStar3D::dist(const MapCoordinate3D& a, const MapCoordinate3D& b) {
   ...
   DistanceType dist;
   if (b.valid()) {
      if (wind && ((b.getBitmappedHeight() | a.getBitmappedHeight()) &
                   (chtieffliegend | chfliegend | chhochfliegend))) {
...
AStar3D::DistanceType AStar3D::getMoveCost(const MapCoordinate3D& start,
                                           const MapCoordinate3D& dest, const Vehicle* vehicle,
                                           bool& hasAttacked) {
   ...
   if (start.getNumericalHeight() < 0)
      return movecost;
   else if (!vehicleSpeedFactor[start.getNumericalHeight()])
      return longestPath;
   else
      return movecost / vehicleSpeedFactor[start.getNumericalHeight()];
}

int AStar3D::initNode(Node& newN, const Node* oldN_ptr, const MapCoordinate3D& newpos,
                      const vector<MapCoordinate3D>& B, bool disableAttack, bool enter, bool dock) {
   int fa;
   ...
   if (!dock) {
      fa = fieldAccessible(actmap->getField(newN.h), veh, newN.h.getBitmappedHeight(),
                           &newN.hasAttacked);
      if (!fa)
         return 0;
      newN.canStop = fa >= 2;
...
const int* getDirectionOrder(int x, int y, int x2, int y2) {
   ...
   static const Directions directions[3][3] = {
      {{5, 0, 4, 1, 3, 2}, {0, 1, 5, 2, 4, 3}, {1, 0, 2, 5, 3, 4}},
   ...
   while (!open.empty()) {
      ...
      for (vector<MapCoordinate3D>::const_iterator i = B.begin(); i != B.end(); i++)
         if (N_ptr->h == *i) {
            if (N_ptr->h.getNumericalHeight() == -1 ||
                !(oldFld->building || (oldFld->vehicle && oldFld->vehicle != veh))) {
               found = true;
               break;
            }
         }
      ...
      if (N_ptr->h.getNumericalHeight() == -1) {
         ...
         if (!operationLimiter || operationLimiter->allowLeavingContainer()) {
            for (int dir = 0; dir < 6; dir++) {
               ...
               if (actmap->getField(pos)) {
                  ...
                  for (int i = 0; i < 8; i++)
                     if (h & (1 << i)) {
                        const ContainerBaseType::TransportationIO* tio =
                           oldFld->getContainer()->vehicleUnloadSystem(veh->typ, 1 << i);
                        if (tio) {
                           ...
                           if (N2.canStop && actmap->getField(N2.h)->getContainer() &&
                               actmap->getField(N2.h)->vehicle != veh) {
                              // there's an container on the field that can be entered. This means,
                              // the unit can't stop 'over' the container...
                              N2.canStop = false;
            ...
            for (int dir = 0; dir < 6; dir++) {
               if (dock) {
                  ...
                  if (fld && fld->getContainer() &&
                      (fld->getContainer() != oldFld->getContainer())) {
                     ...
                     if (receivingHeight & dock)
                        if (fld->getContainer()->getOwner() == oldFld->getContainer()->getOwner() &&
                           ...
                           if (!fld->building ||
                               (fld->bdt & getTerrainBitType(cbbuildingentry)).any()) {
                              pos.setNumericalHeight(-1);
                              Node N2 = Node();

                              const ContainerBaseType::TransportationIO* dockingSystem =
                                 oldFld->getContainer()->getDockingSystem(veh, receivingHeight);
                              bool inhibitAttack =
                                 dockingSystem != NULL ? dockingSystem->disableAttack : false;
         ...
         if (!operationLimiter || operationLimiter->allowMovement()) {
            // this check is only necessary for the very first field (gval==0) in which the unit
            // starts. The unit may not be able to leave it, for example if a ship is frozen in ice
            if (terrainaccessible(oldFld, veh) ||
                veh->getMap()->getgameparameter(cgp_movefrominvalidfields) || N_ptr->gval > 0) {
               for (int dci = 0; dci < 6; dci++) {
                  ...
                  if (N2.canStop && actmap->getField(N2.h)->getContainer() &&
                      actmap->getField(N2.h)->vehicle != veh) {
                     // there's an container on the field that can be entered. This means, the unit
                     // can't stop 'over' the container...
                     ...
                     if (!visited.find(N2.h))
                        open.pushOrUpdate(N2);
                  } else if (!veh->typ->hasFunction(ContainerBaseType::OnlyMoveToAndFromTransports))
                     open.pushOrUpdate(N2);
               }
            }
         }
         // and now change the units' height. That's only possible on fields where the unit can stop
         // it's movement

         if ((!operationLimiter || operationLimiter->allowHeightChange()) &&
             !(veh->typ->hasFunction(ContainerBaseType::OnlyMoveToAndFromTransports)))
            if ((fieldAccessible(oldFld, veh, N_ptr->h.getBitmappedHeight()) == 2) ||
                actmap->getgameparameter(cgp_movefrominvalidfields))
               for (int heightDelta = -1; heightDelta <= 1; heightDelta += 2) {
                  const VehicleType::HeightChangeMethod* hcm =
                     veh->getHeightChange(heightDelta, N_ptr->h.getBitmappedHeight());
                  if (hcm) {
                     for (int dir = 0; (dir < 6 && hcm->dist) || (dir < 1 && !hcm->dist); dir++) {
                        ...
                        for (int step = 0; step <= hcm->dist; step++) {
                           ...
                           if (!fieldAccessible(fld, veh, N_ptr->h.getBitmappedHeight()) &&
                           ...
                           if (!fieldAccessible(
                                  fld, veh,
                                  1 << (N_ptr->h.getNumericalHeight() + hcm->heightDelta)))
                              access = false;
                        ...
                        MapField* fld = actmap->getField(newpos);
                        if (fld && access) {
                           newpos.setNumericalHeight(newpos.getNumericalHeight() +
                                                     hcm->heightDelta);
                           bool enter = actmap->getField(newpos)->getContainer() &&
                                        actmap->getField(newpos)->vehicle != veh;
                           Node N2 = Node();
...
void AStar3D::PathPoint::read(tnstream& stream) {
   ...
   dist = stream.readInt();
   enterHeight = stream.readInt(), hasAttacked = stream.readInt();
}
