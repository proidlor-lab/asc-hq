...
void tspfldloaders::readmessagelist(MessagePntrContainer& lst) {
   ...
   while (i) {
      // MessageContainer::iterator mi = find ( spfld->messages.begin(), spfld->messages.end(),
      // MessageIDequals ( i ));
      MessageContainer::iterator mi = spfld->messages.end();
      for (MessageContainer::iterator mi2 = spfld->messages.begin(); mi2 != spfld->messages.end();
           mi2++)
         if ((*mi2)->id == i)
            mi = mi2;
      ...
      if (mi == spfld->messages.end())
         warningMessage(
            "message list corrupted !\nplease report this bug!\nthe game will continue, but some "
            "messages will probably be missing\nand other instabilities may occur.");
      lst.push_back(*mi);
      i = stream->readInt();
   }
}
...
void checkForUniqueUnitIDs(GameMap* gamemap) {
   ...
   for (int p = 0; p < gamemap->getPlayerCount(); ++p)
      for (Player::VehicleList::iterator i = gamemap->getPlayer(p).vehicleList.begin();
           i != gamemap->getPlayer(p).vehicleList.end(); ++i)
         if (units[(*i)->networkid]++ > 0) {
            warningMessage(
               "unit with duplicate network ids: " + ASCString::toString((*i)->networkid) +
   ...
   do {
      ...
      */
      ...
      if (b4 & csm_newobject) {
         ...
         for (MapField::MineContainer::iterator m = fld->mines.begin(); m != fld->mines.end();
              m++) {
   ...
   do {
      ...
      if (cnt2 == 0) {
         ...
         if (b1 & csm_vehicle) {
            ...
            if (fld2->vehicle->getOwner() == 8)  // neutral units are not allowed, it was a bug that
                                                 // they could be placed on some maps
               delete fld2->vehicle;
         }
         ...
         if (b4 & csm_newobject) {
            ...
            for (int i = 0; i < minenum; i++) {
               ...
               if (objectversion == 1) {
                  ...
                  int lifetime = spfld->getgameparameter(
                     GameParameter(cgp_antipersonnelmine_lifetime + m.type - 1));
                  if (lifetime > 0 && endtime > 0)
                     m.lifetimer = endtime - spfld->time.turn() +
                                   spfld->getgameparameter(
                                      GameParameter(cgp_antipersonnelmine_lifetime + m.type - 1));
                  else if (lifetime > 0)
...
void weatherSystemRequired() {
   throw ASCmsgException(
      "This file can not be loaded, since it contains data of the discontinued weather generator");
}
...
int tnetworkloaders::savenwgame(tnstream* strm, const GameMap* gamemap) {
   ...
   spfld = const_cast<GameMap*>(gamemap);  // yes, this is bad, but spfld can't be made constant
                                           // because it is also used for loading
...
GameMap* tnetworkloaders::loadnwgame(tnstream* strm) {
   ...
   for (int i = 0; i < 8; ++i)
      for (Player::BuildingList::iterator b = spfld->player[i].buildingList.begin();
           b != spfld->player[i].buildingList.end(); ++b)
         for (int a = 0; a < waffenanzahl; ++a)
            if ((*b)->ammo[a] > 32000) {
               ASCString s;
               s.format("Player %s had %d ammo of type %s in the building at %d/%d",
                        spfld->player[i].getName().c_str(), (*b)->ammo[a], cwaffentypen[a],
                        (*b)->getEntry().x, (*b)->getEntry().y);
...
void writeMessageFile(GameMap* gamemap, tnstream& stream) {
   ...
   for (GameMap::Events::const_iterator i = gamemap->events.begin(); i != gamemap->events.end();
        ++i) {
      ...
      if (!s.empty()) {
         ...
         stream.writeString("asc.setLocalizedEventMessage( map, " + ASCString::toString((*i)->id) +
                               ", message )\n\n",
                            false);
      }
   }
   ...
   for (int p = 0; p <= gamemap->getPlayerCount(); ++p) {
      ...
      for (Player::BuildingList::const_iterator b = player.buildingList.begin();
           b != player.buildingList.end(); ++b) {
         if (!(*b)->name.empty()) {
            stream.writeString("--- ===== Building " +
                                  ASCString::toString((*b)->getIdentification()) + "  ======= \n",
                               false);
            ASCString f;
...
void savemap(const ASCString& name, GameMap* gamemap) {
   ...
   try {
      ...
      if (CGameOptions::Instance()->saveEventMessagesExternal &&
          gamemap->nativeMessageLanguage.length()) {
         tn_file_buf_stream messages(name + "." + gamemap->nativeMessageLanguage,
                                     tnstream::writing);
...
GameMap* mapLoadingExceptionChecker(const ASCString& filename, MapLoadingFunction loader) {
   ...
   try {
      ...
      displaymessage("The file %s is corrupted.\nPlease obtain a new copy of that file", 1,
                     filename.c_str());
      return NULL;
   } catch (const tfileerror& err) {
      displaymessage("error reading map filename %s ", 1, err.getFileName().c_str());
