...
BuildingType ::BuildingType(void) {
   ...
   static const float matrix[] = {1, 0, 0, 0, 1, 0, 0, 0, 1};
...
int BuildingType ::getBIPicture(const LocalCoordinate& localCoordinate, int weather,
                                int constructionStep) const {
   return bi_picture[weather][constructionStep][localCoordinate.x][localCoordinate.y];
}

const Surface& BuildingType ::getPicture(const LocalCoordinate& localCoordinate, int weather,
                                         int constructionStep) const {
   ...
   if (bi_picture[weather][constructionStep][localCoordinate.x][localCoordinate.y] <= 0)
      ...
      return GraphicSetManager::Instance().getPic(
         bi_picture[weather][constructionStep][localCoordinate.x][localCoordinate.y]);
}

void BuildingType::paint(Surface& s, SPoint pos, const PlayerColor& player, int weather,
                         int constructionStep) const {
   for (int x = 0; x < 4; x++)
      for (int y = 0; y < 6; y++)
...
void BuildingType::paintSingleField(Surface& s, SPoint pos, const LocalCoordinate& localCoordinate,
                                    const PlayerColor& player, int weather,
                                    int constructionStep) const {
   megaBlitter<ColorTransform_PlayerCol, ColorMerger_AlphaOverwrite, SourcePixelSelector_Plain,
               TargetPixelSelector_All>(
      getPicture(localCoordinate, weather, constructionStep), s,
      SPoint(pos.x + localCoordinate.x * fielddistx + (localCoordinate.y & 1) * fielddisthalfx,
             pos.y + localCoordinate.y * fielddisty),
      player, nullParam, nullParam, nullParam);

   // s.Blit( getPicture(localCoordinate,weather,constructionStep),SPoint( pos.x + localCoordinate.x
   // * fielddistx + ( localCoordinate.y & 1 ) * fielddisthalfx, pos.y + localCoordinate.y *
   // fielddisty));
}

void BuildingType::paintSingleField(Surface& s, SPoint pos, const LocalCoordinate& localCoordinate,
                                    int weather, int constructionStep) const {
...
MapCoordinate BuildingType ::getFieldCoordinate(const MapCoordinate& entryOnMap,
                                                const LocalCoordinate& localCoordinates) const {
...
BuildingType::LocalCoordinate BuildingType::getLocalCoordinate(const MapCoordinate& entryOnMap,
                                                               const MapCoordinate& field) const {
 ...
 */
   ...
   for (DestructionObjects::const_iterator i = destructionObjects.begin();
        i != destructionObjects.end(); ++i) {
                        ...
                        for (int y = 0; y < 6; y++)
                           if (w_picture[w][c][x][y].valid())
                              megaBlitter<ColorTransform_None, ColorMerger_AlphaOverwrite,
                                          SourcePixelSelector_Plain, TargetPixelSelector_All>(
                                 w_picture[w][c][x][y], s,
                                 SPoint(500 * c + x * fielddistx + (y & 1) * fielddisthalfx,
                                        y * fielddisty),
                                 nullParam, nullParam, nullParam, nullParam);
                  ASCString file =
                     extractFileName_withoutSuffix(filename) + weatherAbbrev[w] + ".png";
                  pc.addImage(weatherTags[w], s, file, false);
                  pc.addString(ASCString(weatherTags[w]) + "_OriginalImageFilename",
                               originalImageFilename[w]);
               }
         } else {
            for (int w = 0; w < cwettertypennum; w++)
               if (weatherBits.test(w)) {
                  ...
                  for (int c = 0; c < construction_steps; c++)
                     for (Fields::iterator i = fields.begin(); i != fields.end(); i++) {
                        ...
                        if (depth == 8) {
                           img.Blit(s, SDLmm::SRect(SPoint(xx, yy), fieldsizex, fieldsizey),
                                    SPoint(0, 0));
                           applyFieldMask(img);
                        } else {
                           MegaBlitter<4, 4, ColorTransform_None, ColorMerger_PlainOverwrite,
                              ...
                              blitter;
                           blitter.setSrcRectangle(
                              SDLmm::SRect(SPoint(xx, yy), fieldsizex, fieldsizey));
...
void BuildingType::convertOldFunctions(int abilities, const ASCString& location) {
   features.reset();
   if (abilities & 1)
      warningMessage(location + ": The HQ function for buildings is not supported any more");
   if (abilities & 2)
      features.set(TrainingCenter);
   if (abilities & (1 << 3))
      features.set(InternalVehicleProduction);
   if (abilities & (1 << 4))
      features.set(AmmoProduction);
   if (abilities & (1 << 8))
      features.set(InternalUnitRepair);
   if (abilities & (1 << 9))
      features.set(RecycleUnits);
   if (abilities & (1 << 10))
      features.set(Research);
   if (abilities & (1 << 11))
      features.set(Sonar);
   if (abilities & (1 << 12))
      features.set(WindPowerPlant);
   if (abilities & (1 << 13))
      features.set(SolarPowerPlant);
   if (abilities & (1 << 14))
      features.set(MatterConverter);
   if (abilities & (1 << 15))
      features.set(MiningStation);
   if (abilities & (1 << 16)) {
   ...
   if (abilities & (1 << 17))
      features.set(ProduceNonLeavableUnits);
   if (abilities & (1 << 18))
      features.set(ResourceSink);
   if (abilities & (1 << 19)) {
      features.set(ExternalMaterialTransfer);
      features.set(ExternalFuelTransfer);
   }
   if (abilities & (1 << 20))
      features.set(ExternalAmmoTransfer);
   if (abilities & (1 << 21))
      features.set(NoObjectChaining);
   if (abilities & (1 << 22))
      features.set(SelfDestructOnConquer);
   if (abilities & (1 << 23))
