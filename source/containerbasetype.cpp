...
void ContainerBaseType ::TransportationIO ::runTextIO(PropertyContainer& pc) {
   ...
   pc.addTagInteger("CategoriesNOT", vehicleCategoriesLoadable, cmovemalitypenum, unitCategoryTags,
                    true);
   ...
   if (pc.find("RequireUnitFunction")) {
      int r = 0;
      pc.addTagInteger("RequireUnitFunction", r, VehicleType::legacyVehicleFunctionNum,
                       vehicleAbilities, 0);
      requiresUnitFeature = VehicleType::convertOldFunctions(r, pc.getFileName());
   } else if (pc.find("RequiresUnitFeature") || !pc.isReading())
      pc.addTagArray("RequiresUnitFeature", requiresUnitFeature, ContainerBaseType::functionNum,
                     containerFunctionTags);
   else
...
void ContainerBaseType ::runTextIO(PropertyContainer& pc) {
   ...
   pc.addTagInteger("CategoriesNOT", vehicleCategoriesStorable, cmovemalitypenum, unitCategoryTags,
                    -1, true);
   pc.closeBracket();
   ...
   pc.addTagInteger("CategoriesProduceable", vehicleCategoriesProduceable, cmovemalitypenum,
                    unitCategoryTags, -1);
...
int ContainerBaseType ::vehicleUnloadable(const VehicleType* vehicleType, int carrierHeight) const {
   ...
   if (vehicleFit(vehicleType))
      for (ContainerBaseType::EntranceSystems::const_iterator i = entranceSystems.begin();
           i != entranceSystems.end(); i++)
