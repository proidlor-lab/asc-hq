...
Resources StandardUnitCostCalculator ::productionCost(const VehicleType* vehicle) {
   ...
   for (int T = 0; T < vehicle->entranceSystems.size(); ++T)
      if (((vehicle->entranceSystems[T].container_height < chtieffliegend &&
            (vehicle->entranceSystems[T].height_abs &
             (chtieffliegend | chfliegend | chhochfliegend | chsatellit))) ||
           ((vehicle->entranceSystems[T].container_height &
             (chfliegend | chhochfliegend | chsatellit)) &&
            (vehicle->entranceSystems[T].height_abs &
             (chfliegend | chhochfliegend | chsatellit)))) &&
   ...
   if (vehicle->movemalustyp == MoveMalusType::trooper) {
      ...
      typecostm += vehicle->armor * 6;
              ...
              vehicle->movemalustyp == MoveMalusType::medium_wheeled_vehicle) {
      typecoste += vehicle->armor * 7;
      typecostm += vehicle->armor * 7;
   } else if (vehicle->movemalustyp == MoveMalusType::medium_tracked_vehicle ||
              vehicle->movemalustyp == MoveMalusType::heavy_wheeled_vehicle ||
              vehicle->movemalustyp == MoveMalusType::rail_vehicle ||
              vehicle->movemalustyp == MoveMalusType::structure) {
      ...
      typecostm += vehicle->armor * 16;
              ...
              vehicle->movemalustyp == MoveMalusType::heavy_aircraft) {
   ...
   if (vehicle->height & chgetaucht) {
      if (vehicle->movemalustyp == MoveMalusType::light_ship ||
          vehicle->movemalustyp == MoveMalusType::medium_ship ||
          vehicle->movemalustyp == MoveMalusType::heavy_ship) {
         typecoste += vehicle->armor * 2;
         typecostm += vehicle->armor * 2;
      } else {
         if (vehicle->movemalustyp == MoveMalusType::trooper) {
   ...
   if (vehicle->weapons.count > 0) {
      for (int W = 0; W < vehicle->weapons.count; ++W) {
         ...
         if (vehicle->weapons.weapon[W].getScalarWeaponType() == cwmachinegunn ||
             vehicle->weapons.weapon[W].getScalarWeaponType() == cwsmallmissilen ||
             vehicle->weapons.weapon[W].getScalarWeaponType() == cwbombn) {
            if (vehicle->weapons.weapon[W].shootable()) {
         ...
         if (vehicle->weapons.weapon[W].getScalarWeaponType() == cwlargemissilen ||
             vehicle->weapons.weapon[W].getScalarWeaponType() == cwtorpedon) {
            if (vehicle->weapons.weapon[W].shootable()) {
         ...
         if (vehicle->weapons.weapon[W].getScalarWeaponType() == cwcannonn ||
             vehicle->weapons.weapon[W].getScalarWeaponType() == cwcruisemissilen) {
            if (vehicle->weapons.weapon[W].shootable()) {
         ...
         if (vehicle->weapons.weapon[W].getScalarWeaponType() == cwlasern &&
             vehicle->weapons.weapon[W].shootable()) {
         ...
         // Move during reaction fire(MDRF) - Move After Attack(MAM) - No Attack After Move(NAAM) -
         // ReactionFire(RF)
         int weaponspecial = 0;
         ...
         int weaponMDRF =
            vehicle->weapons.weapon[W].reactionFireShots * weaponsinglecostm * maxmoverange / 400;

         if (vehicle->weapons.weapon[W].shootable()) {
            if (vehicle->hasFunction(ContainerBaseType::MoveWithReactionFire)) {
               if (vehicle->wait) {
                  if (vehicle->hasFunction(
                         ContainerBaseType::MoveAfterAttack)) {  // MDRF+NAAM+MAM
                                                                 // (Defkind,Spear,Stahlschwein)
                  ...
                  if (vehicle->hasFunction(
                         ContainerBaseType::MoveAfterAttack)) {  // MDRF+MAM      (Druk, Innocence,
                                                                 // Skjold, PHM, Jub-O)
               ...
               if (vehicle->wait) {
                  if (vehicle->hasFunction(
                         ContainerBaseType::MoveAfterAttack)) {  // NAAM+MAM      (Coma2, K5,
                                                                 // PzH2000, Pulsar)
   ...
   if (vehicle->jamming > 0 && vehicle->hasFunction(ContainerBaseType::JamsOnlyOwnField)) {
      if (vehicle->jamming < 31) {
         typecoste +=
            vehicle->jamming * 20;  //  fuer Trooper oder eigenschaftsbedingt (klein, schnell)
         typecostm += vehicle->jamming * 10;
      } else {
         typecoste += vehicle->jamming * 60;  //  fuer alle hoeherwirkenden Stealthverfahren,
                                              //  Anstrich, besondere Konstruktion, tarnfeld usw.
   ...
   if (vehicle->hasFunction(ContainerBaseType::InternalVehicleProduction) ||
       vehicle->hasFunction(ContainerBaseType::ExternalVehicleProduction)) {
      specialcoste += 1000;
      specialcostm += 500;
   }
   ...
   // Reparaturfahrzeug
   if (vehicle->hasFunction(ContainerBaseType::InternalUnitRepair) ||
       vehicle->hasFunction(ContainerBaseType::ExternalRepair)) {
      specialcoste += 1000;
      specialcostm += 500;
   }
   ...
   if (vehicle->hasFunction(ContainerBaseType::Sonar) &&
       (vehicle->movemalustyp == MoveMalusType::trooper)) {
