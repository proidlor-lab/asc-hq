...
ActionResult MoveUnit::runAction(const Context& context) {
   ...
   if (oldfield->vehicle == vehicle)
      (new UnitFieldRegistration(vehicle, vehicle->getPosition(),
                                 UnitFieldRegistration::RemoveView))
         ->execute(context);

   {
      ActionResult res = (new UnitFieldRegistration(vehicle, vehicle->getPosition(),
                                                    UnitFieldRegistration::UnregisterOnField))
                            ->execute(context);
      if (!res.successful())
         return res;
   }
   ...
   SoundLoopManager slm(
      SoundList::getInstance().getSound(SoundList::moving, vehicle->typ->movemalustyp,
                                        vehicle->typ->movementSoundLabel, soundHeight),
      false);
   ...
   while (pos != stop && vehicle && cancelmovement != 1) {
      ...
      bool container2container =
         pos->getNumericalHeight() == -1 && next->getNumericalHeight() == -1;
      pair<int, int> mm = calcMoveMalus(*pos, next->getRealPos(), vehicle, wind.get(),
                                        &inhibitAttack, container2container);
      movedist += mm.first;
      fueldist += mm.second;
      ...
      if (next->getRealHeight() != pos->getRealHeight() && next->getRealHeight() >= 0)
         (new ChangeUnitProperty(vehicle, ChangeUnitProperty::Height, 1 << next->getRealHeight()))
            ->execute(context);
      ...
      do {
         ...
         if (vehicle) {
            ...
            (new UnitFieldRegistration(vehicle, to, UnitFieldRegistration::AddView))
               ->execute(context);
...

         {
            ...
            if (dir >= 0 && dir <= 5)
               (new ChangeUnitProperty(vehicle, ChangeUnitProperty::Direction, dir))
                  ->execute(context);
         }

         if (context.display) {
            // displaymap();
            if (next == stop && to.x == next->x &&
                to.y == next->y)  // the unit will reach its destination
               slm.fadeOut(CGameOptions::Instance()->movespeed * 10);
            context.display->displayMovingUnit(
               from, to, vehicle, pathStep, pathStepNum,
               MapDisplayInterface::SoundStartCallback(&slm, &SoundLoopManager::activate),
               context.display->getUnitMovementDuration());
         ...
         printTimer(5);
         {
            ...
            if (context.viewingPlayer >= 0)
               fieldsWidthChangedVisibility =
                  evaluateviewcalculation(getMap(), 1 << context.viewingPlayer, false, &context);
            else
         ...
         if (vehicle) {
            ...
            if (vehicle && dest->mineattacks(vehicle)) {
               ...
               if (context.display && (fieldvisiblenow(dest, context.viewingPlayer) ||
                                       dest->mineowner() == context.viewingPlayer))
         ...
         if (vehicle)
            if (!(stop->x == to.x && stop->y == to.y && next == stop))
               (new UnitFieldRegistration(vehicle, to, UnitFieldRegistration::RemoveView))
                  ->execute(context);
         ...
         if (vehicle) {
            ...
            if ((dest->connection & cconnection_areaentered_specificunit) &&
                (vehicle->connection & cconnection_areaentered_specificunit))
               fieldCrossed(context);
            dest->secondvehicle = NULL;
         }
         printTimer(8);
   ...
   if (vehicle) {
      ...
      if (vehicle->typ->movement[getFirstBit(orgHeight)]) {
         if (orgHeight != vehicle->height) {
            ...
            int move = int(floor(vehicle->maxMovement() * float(orgMovement) /
                                    float(vehicle->typ->movement[getFirstBit(orgHeight)]) +
                                 0.5));
            (new ChangeUnitMovement(vehicle, move, false, ChangeUnitMovement::NONE))
               ->execute(context);
         }

         int nm = int(floor(vehicle->maxMovement() * float(newMovement) /
                               float(vehicle->typ->movement[getFirstBit(orgHeight)]) +
                            0.5));
      ...
      (new ConsumeResource(vehicle,
                           Resources(0, 0, fueldist * vehicle->typ->fuelConsumption / maxmalq)))
         ->execute(context);
      ...
      if (vehicle) {
         if ((fld->vehicle == NULL) && (fld->building == NULL)) {
            if (!vehicle->isViewing()) {
               (new UnitFieldRegistration(vehicle, *pos, UnitFieldRegistration::AddView))
                  ->execute(context);
               viewInputChanged = true;
               ...
               */
               ...
               (new UnitFieldRegistration(vehicle, *pos, UnitFieldRegistration::RegisterOnField))
                  ->execute(context);
               ...
               for (int i = 0; i < getMap()->getPlayerCount(); ++i)
                  evaluatevisibilityfield(getMap(), fld, i, -1,
                                          getMap()->getgameparameter(cgp_initialMapVisibility));
            ...
            if (vehicle->isViewing()) {
               (new UnitFieldRegistration(vehicle, *pos, UnitFieldRegistration::RemoveView))
                  ->execute(context);
               viewInputChanged = true;
            }
            ...
            if (cn->getOwner() != vehicle->getOwner() && fld->building &&
                getMap()->getPlayer(fld->building).diplomacy.isHostile(vehicle)) {
               (new ConvertContainer(fld->building, vehicle->getOwner()))->execute(context);
               if (fieldvisiblenow(fld, context.viewingPlayer) ||
                   context.viewingPlayer == vehicle->getOwner())
