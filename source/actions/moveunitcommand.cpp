...
MoveUnitCommand ::MoveUnitCommand(Vehicle* unit)
   ...
   if (unit)
      unit->getMap()->sigCoordinateShift.connect(
         sigc::mem_fun(*this, &MoveUnitCommand::changeCoordinates));
}
...
class PathFinder : public AStar3D {
   ...
   */
   void getMovementFields(set<MapCoordinate3D>& reachableFields,
                          set<MapCoordinate3D>& reachableFieldsIndirect, int height);
};
...
void PathFinder ::getMovementFields(set<MapCoordinate3D>& reachableFields,
   ...
   for (AStar3D::VisitedContainer::iterator i = visited.begin(); i != visited.end(); ++i) {
      AStar3D::Node node = *i;
      if (node.h.x != veh->getPosition().x || node.h.y != veh->getPosition().y ||
          node.h.getNumericalHeight() != unitHeight) {
   ...
   for (Fields::iterator i = fields.begin(); i != fields.end();) {
      ...
      while (i != fields.end() && i->first == key) {
         if (i->second->gval < move ||
             (i->second->gval == move &&
              abs(i->second->h.getNumericalHeight() - orgHeight) < abs(height - orgHeight)))
...
ActionResult MoveUnitCommand::searchFields(int height, int capabilities) {
   ...
   if ((capabilities | flags) & LimitVerticalDirection) {
      if (getVerticalDirection() == 0) {
         HeightChangeLimitation* hcl =
            new HeightChangeLimitation(!(capabilities & DisableHeightChange));
         ...
         pf.getMovementFields(reachableFields, reachableFieldsIndirect, h);
         limiter =
            NULL;  // the unit shouldn't take any detours anyway to reach the destination,
...
bool MoveUnitCommand::isFieldReachable(const MapCoordinate& pos, bool direct) {
   if (direct) {
      for (set<MapCoordinate3D>::iterator i = reachableFields.begin(); i != reachableFields.end();
           ++i)
         if (i->x == pos.x && i->y == pos.y)
            return true;
   } else {
      for (set<MapCoordinate3D>::iterator i = reachableFieldsIndirect.begin();
           i != reachableFieldsIndirect.end(); ++i)
         if (i->x == pos.x && i->y == pos.y)
            return true;
   }
   return false;
...
*/
   ...
   if ((getUnit()->getPosition() == destination) ||
       (getUnit()->getPosition().x == destination.x &&
        getUnit()->getPosition().y == destination.y && destination.getNumericalHeight() == -1))
   ...
   if (!multiTurnMovement &&
       (path.rbegin()->x != destination.x || path.rbegin()->y != destination.y))
      return ActionResult(105);
...
ASCString MoveUnitCommand ::getCommandString() const {
   ASCString c;
   c.format("unitMovement ( map, %d, asc.MapCoordinate( %d, %d), %d )", getUnitID(), destination.x,
            destination.y, destination.getNumericalHeight());
   return c;
}
