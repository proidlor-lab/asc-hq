...
UnitFieldRegistration::UnitFieldRegistration(Vehicle* vehicle, const MapCoordinate3D& pos,
                                             Operation operation, ContainerBase* carrier)
...
ASCString UnitFieldRegistration::getOpName() const {
   switch (operation) {
      case RegisterOnField:
         return "RegisterOnField";
      case UnregisterOnField:
         return "UnregisterOnField";
      case AddView:
         return "AddView";
      case RemoveView:
         return "RemoveView";
...
ActionResult UnitFieldRegistration::runAction(const Context& context) {
   ...
   if (operation == Position3D || operation == Position) {
      for (Vehicle::Cargo::const_iterator i = veh->getCargo().begin(); i != veh->getCargo().end();
           ++i)
         if (*i) {
            std::unique_ptr<UnitFieldRegistration> ufr4(
               new UnitFieldRegistration(*i, position, Position));
