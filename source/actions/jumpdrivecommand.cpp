...
ActionAvailability JumpDriveCommand ::available(const Vehicle* unit) {
   ...
   if (unit->typ->jumpDrive.movementConsumptionPercentage == 100) {
      ...
      if (needed > unit->getMovement(false))
         avail.set(ActionAvailability::partially, "Unit has not enough movement left, need " +
                                                     ASCString::toString(needed) + " points");
   }
   ...
   if ((unit->reactionfire.getStatus() != Vehicle::ReactionFire::off) &&
       !unit->typ->hasFunction(VehicleType::MoveWithReactionFire))
      avail.set(ActionAvailability::partially, "Unit has reaction fire enabled");

   if (!(unit->height & unit->typ->jumpDrive.height))
      avail.set(ActionAvailability::partially,
                "Unit needs to be on this height: " + heightToString(unit->typ->jumpDrive.height));

   if (unit->getResource(unit->typ->jumpDrive.consumption) < unit->typ->jumpDrive.consumption)
      avail.set(ActionAvailability::partially, "Unit is missing resources. Required: " +
                                                  unit->typ->jumpDrive.consumption.toString());

   return avail;
}

JumpDriveCommand ::JumpDriveCommand(Vehicle* unit) : UnitCommand(unit) {}
...
ActionResult JumpDriveCommand::go(const Context& context) {
   ...
   std::unique_ptr<UnitFieldRegistration> ufr1(
      new UnitFieldRegistration(unit, unit->getPosition(), UnitFieldRegistration::RemoveView));
   ...
   std::unique_ptr<UnitFieldRegistration> ufr2(new UnitFieldRegistration(
      unit, unit->getPosition(), UnitFieldRegistration::UnregisterOnField));
   ...
   std::unique_ptr<UnitFieldRegistration> ufr3(
      new UnitFieldRegistration(unit, dest3D, UnitFieldRegistration::Position3D));
   ...
   std::unique_ptr<UnitFieldRegistration> ufr4(
      new UnitFieldRegistration(unit, dest3D, UnitFieldRegistration::RegisterOnField));
   ...
   if (context.display)
      context.display->playPositionalSound(dest3D,
                                           SoundList::getInstance().getSound(SoundList::jumpdrive));

   int newmovement;
   if (unit->typ->jumpDrive.movementConsumptionPercentage >= 100)
      newmovement = 0;
   else
      newmovement = unit->getMovement(false, false) -
                    unit->maxMovement() * unit->typ->jumpDrive.movementConsumptionPercentage / 100;

   std::unique_ptr<ChangeUnitMovement> cum(
      new ChangeUnitMovement(unit, newmovement, false, ChangeUnitMovement::ALLFULL));
   ...
   std::unique_ptr<UnitFieldRegistration> ufr5(
      new UnitFieldRegistration(unit, dest3D, UnitFieldRegistration::AddView));
...
ASCString JumpDriveCommand ::getCommandString() const {
   ASCString c;
   c.format("unitJump ( map, %d, asc.MapCoordinate( %d, %d ) )", getUnitID(), destination.x,
            destination.y);
   return c;
}
