            ...
            if (unit->reactionfire.getStatus() == Vehicle::ReactionFire::off)
               if (unit->weapexist())
                  for (int i = 0; i < unit->typ->weapons.count; ++i)
                     if (unit->typ->weapons.weapon[i].offensive() &&
                         unit->typ->weapons.weapon[i].reactionFireShots)
                        return true;
   } else {
      if (unit->reactionfire.getStatus() != Vehicle::ReactionFire::off)
         return true;
...
ActionResult ReactionFireSwitchCommand::go(const Context& context) {
   ...
   if (newRFstate == true) {
      std::unique_ptr<ChangeUnitProperty> propChange(new ChangeUnitProperty(
         getUnit(), ChangeUnitProperty::ReactionFire, (int) Vehicle::ReactionFire::init2));
      ...
      std::unique_ptr<ChangeUnitProperty> propChange(new ChangeUnitProperty(
         getUnit(), ChangeUnitProperty::ReactionFire, (int) Vehicle::ReactionFire::off));
      ActionResult res = propChange->execute(context);
...
namespace {
const bool r1 =
   registerAction<ReactionFireSwitchCommand>(ActionRegistry::ReactionFireSwitchCommand);
}
