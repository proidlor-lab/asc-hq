...
DiscoverResources::DiscoverResources(Vehicle* unit) : UnitAction(unit) {
...
void DiscoverResources::testField(const MapCoordinate& mc) {
   ...
   if (!fld->building || fld->building->color == getMap()->actplayer * 8 ||
       fld->building->color == 8 * 8)
      if (!fld->vehicle || fld->vehicle->color == getMap()->actplayer * 8 ||
          fld->vehicle->color == 8 * 8) {
         if (!fld->resourceview)
            fld->resourceview = new MapField::Resourceview;

         for (int c = 0; c < 8; c++)
            if (shareview & (1 << c)) {
               if (!(fld->resourceview->visible & (1 << c)) ||
...
ActionResult DiscoverResources::runAction(const Context& context) {
   ...
   circularFieldIterator(getMap(), getUnit()->getPosition(), 0, getUnit()->typ->digrange,
                         FieldIterationFunctor(this, &DiscoverResources::testField));
