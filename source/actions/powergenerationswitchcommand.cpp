...
ActionResult PowerGenerationSwitchCommand::go(const Context& context) {
   ...
   std::unique_ptr<ChangeUnitProperty> propChange(
      new ChangeUnitProperty(getUnit(), ChangeUnitProperty::PowerGeneration, newState));
...
void PowerGenerationSwitchCommand ::readData(tnstream& stream) {
   ...
   if (version > PowerGenerationSwitchCommandVersion)
      throw tinvalidversion("PowerGenerationSwitchCommand", PowerGenerationSwitchCommandVersion,
                            version);
...
namespace {
const bool r1 =
   registerAction<PowerGenerationSwitchCommand>(ActionRegistry::PowerGenerationSwitchCommand);
}
