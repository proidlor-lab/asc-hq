...
RepairUnitCommand ::RepairUnitCommand(ContainerBase* container) : ContainerCommand(container) {}
...
vector<Vehicle*> RepairUnitCommand ::getExternalTargets() {
   ...
   for (ServiceTargetSearcher::Targets::const_iterator i = sts.getTargets().begin();
        i != sts.getTargets().end(); ++i)
      if (!(*i)->getCarrier()) {
...
ActionResult RepairUnitCommand::go(const Context& context) {
   ...
   std::unique_ptr<ChangeContainerProperty> propChange(
      new ChangeContainerProperty(target, ChangeContainerProperty::Damage, newDamage));
   ...
   int experience_o =
      target->getRepairExperienceValue(oldDamage, newDamage, true, Vehicle::experienceResolution);
   int experience_d =
      target->getRepairExperienceValue(oldDamage, newDamage, false, Vehicle::experienceResolution);

   if (experience_o != target->getExperience_offensive_raw()) {
      std::unique_ptr<ChangeUnitProperty> expChange(
         new ChangeUnitProperty(target, ChangeUnitProperty::ExperienceOffensive_Raw, experience_o));
   ...
   if (experience_d != target->getExperience_defensive_raw()) {
      std::unique_ptr<ChangeUnitProperty> expChange(
         new ChangeUnitProperty(target, ChangeUnitProperty::ExperienceDefensive_Raw, experience_d));
      ActionResult res = expChange->execute(context);
      if (res.successful())
         expChange.release();
      else
