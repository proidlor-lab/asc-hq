...
RegisterUnitRFTarget::RegisterUnitRFTarget(GameMap* gamemap, int vehicleID, int weapon,
                                           int targetUnitID)
...
ActionResult RegisterUnitRFTarget::undoAction(const Context& context) {
   ...
   veh->reactionfire.nonattackableUnits.erase(find(veh->reactionfire.nonattackableUnits.begin(),
                                                   veh->reactionfire.nonattackableUnits.end(),
                                                   targetUnitID));
   return ActionResult(0);
}
