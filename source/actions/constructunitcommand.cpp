...
ConstructUnitCommand::unitProductionPrerequisites(const VehicleType* type) const {
   int l = 0;
   ...
   if (mode == internal) {
      ...
      if (!type->techDependency.available(getMap()->getPlayer(getContainer()).research) &&
          getMap()->getgameparameter(cgp_produceOnlyResearchedStuffInternally))
         l |= Lack::Research;

      if (!getContainer()->vehicleUnloadable(type) &&
          !getContainer()->baseType->hasFunction(ContainerBaseType::ProduceNonLeavableUnits))
      ...
      if (veh->getMovement() <
          veh->maxMovement() * veh->typ->unitConstructionMoveCostPercentage / 100)
         l |= Lack::Movement;
...
ConstructUnitCommand::Producables ConstructUnitCommand ::getProduceableVehicles() {
   ...
   if (mode == internal) {
      ...
      for (ContainerBase::Production::const_iterator i = prod.begin(); i != prod.end(); ++i)
         entries.push_back(ProductionEntry(*i, getContainer()->getProductionCost(*i),
                                           unitProductionPrerequisites(*i)));
   } else if (mode == external) {
      ...
      if (veh) {
         ...
         for (int i = 0; i < veh->typ->vehiclesBuildable.size(); i++)
            for (int j = veh->typ->vehiclesBuildable[i].from;
                 j <= veh->typ->vehiclesBuildable[i].to; j++)
               if (veh->getMap()->getgameparameter(cgp_forbid_unitunit_construction) == 0 ||
                   veh->getMap()->unitProduction.check(j)) {
                  VehicleType* v = veh->getMap()->getvehicletype_byid(j);
                  if (v)
                     entries.push_back(ProductionEntry(v,
                                                       veh->getExternalVehicleConstructionCost(v),
                                                       unitProductionPrerequisites(v)));
               }
      }
   }
   return entries;
...
vector<MapCoordinate> ConstructUnitCommand::getFields() {
   ...
   if (vehicleTypeID > 0 && veh) {
      circularFieldIterator(veh->getMap(), veh->getPosition(),
                            veh->typ->unitConstructionMaxDistance,
                            veh->typ->unitConstructionMinDistance,
                            FieldIterationFunctor(this, &ConstructUnitCommand::fieldChecker));
      for (map<MapCoordinate, vector<int>>::const_iterator i = unitsConstructable.begin();
           i != unitsConstructable.end(); ++i)
...
ActionResult ConstructUnitCommand::go(const Context& context) {
   ...
   if (mode == external) {
      SpawnUnit* spawnUnit =
         new SpawnUnit(getMap(), position, vehicleTypeID, getContainer()->getOwner());
      ...
      if (veh) {
         res =
            (new ChangeUnitMovement(
                veh, veh->maxMovement() * veh->typ->unitConstructionMoveCostPercentage / 100, true))
               ->execute(context);
         if (!res.successful())
            return res;
      }
      ...
      if (getMap()->getgameparameter(cgp_bi3_training) >= 1) {
         ...
         for (Player::BuildingList::iterator bi =
                 getMap()->player[getMap()->actplayer].buildingList.begin();
              bi != getMap()->player[getMap()->actplayer].buildingList.end(); bi++)
            if ((*bi)->typ->hasFunction(ContainerBaseType::TrainingCenter))
               cnt++;

         Vehicle* vehicle = spawnUnit->getUnit();
         int expOff =
            vehicle->getExperience_offensive() + cnt * getMap()->getgameparameter(cgp_bi3_training);
         if (expOff > maxunitexperience)
            expOff = maxunitexperience;
         vehicle->setExperience_offensive(expOff);

         int expDef =
            vehicle->getExperience_defensive() + cnt * getMap()->getgameparameter(cgp_bi3_training);
         if (expDef > maxunitexperience)
            expDef = maxunitexperience;
         vehicle->setExperience_offensive(expDef);
      }
   }
...
ASCString ConstructUnitCommand ::getCommandString() const {
   ASCString c;
   c.format("constructUnit ( map, %d, asc.MapCoordinate( %d, %d), %d )", getContainerID(), target.x,
            target.y, vehicleTypeID);
   return c;
}
