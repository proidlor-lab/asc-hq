...
ActionResult AttackCommand::searchTargets() {
   ...
   if (unit->typ->wait && getUnit()->hasMoved() &&
       unit->reactionfire.getStatus() != Vehicle::ReactionFire::ready)
   ...
   circularFieldIterator(getMap(), unit->getPosition(), maxdist, mindist,
                         FieldIterationFunctor(this, &AttackCommand::fieldChecker));

   if (attackableUnits.size() + attackableBuildings.size() + attackableObjects.size() +
       attackableUnitsKamikaze.size()) {
      setState(Evaluated);
...
void AttackCommand ::fieldChecker(const MapCoordinate& pos) {
   if (fieldvisiblenow(getMap()->getField(pos))) {
      if (!kamikaze) {
         ...
         if (atw->count > 0) {
            switch (atw->target) {
               case AttackWeap::vehicle:
                  attackableUnits[pos] = *atw;
                  break;
               case AttackWeap::building:
                  attackableBuildings[pos] = *atw;
                  break;
               case AttackWeap::object:
                  attackableObjects[pos] = *atw;
                  break;
               default:;
            } /* endswitch */
         }
         ...
         if (fieldvisiblenow(fld)) {
            ...
            if (eht != NULL)
               if (((getUnit()->height >= chtieffliegend) && (eht->height <= getUnit()->height) &&
                    (eht->height >= chschwimmend)) ||
                   ((getUnit()->height == chfahrend) && (eht->height == chfahrend)) ||
                   ((getUnit()->height == chschwimmend) && (eht->height == chschwimmend)) ||
                   ((getUnit()->height == chgetaucht) && (eht->height >= chgetaucht) &&
                    (eht->height <= chschwimmend))) {
...
ActionResult AttackCommand::go(const Context& context) {
   ...
   } else if (targetBuilding.valid() && getMap()->getField(targetBuilding)->building) {
   ...
   ActionResult res =
      (new VehicleAttackAction(getMap(), getUnitID(), targetPosition, weapon))->execute(context);
   if (res.successful())
      setState(Finished);
   else
      setState(Failed);
...
ASCString AttackCommand ::getCommandString() const {
   ASCString c;
   c.format("unitAttack ( map, %d, asc.MapCoordinate(%d, %d), %d )", getUnitID(), target.x,
            target.y, weapon);
   return c;
}
