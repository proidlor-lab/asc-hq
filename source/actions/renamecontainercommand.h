...
#ifndef RenameContainerCommandH
#define RenameContainerCommandH
...
class RenameContainerCommand : public ContainerCommand {
  public:
   static bool avail(const ContainerBase* container);

  private:
   RenameContainerCommand(GameMap* map) : ContainerCommand(map){};
   template <class Child>
   friend GameAction* GameActionCreator(GameMap* map);

   ASCString publicName;
   ASCString privateName;
   ASCString originalPublicName;
   ASCString originalPrivateName;
