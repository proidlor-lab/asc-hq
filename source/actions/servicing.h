...
class Transferrable : public sigc::trackable {
  ...
  public:
   ...
   \param avail If true, the amount is limited by the resources which can actually provided by the
   other unit. If false, return the storage capacity
    */
   ...
   //! the id is used to identify the transferrable when serializng to disk. In each service
   //! operation, the id must be unique through all transferrables
   virtual int getID() = 0;
...
class TransferHandler : public sigc::trackable, protected ServiceChecker {
  ...
  public:
   ...
   using ServiceChecker::ignoreDistance;
   using ServiceChecker::ignoreHeight;
