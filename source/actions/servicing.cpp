...
TransferLimitation getTransferLimitation(const ContainerBase* target) {
   ...
   if (target->getMap()->player[target->getMap()->actplayer].diplomacy.getState(
          target->getOwner()) >= PEACE)
      return GET;
...
class ResourceTransferrable : public Transferrable {
  ...
  public:
   ResourceTransferrable(int resource, ResourceWatch& src, ResourceWatch& dst,
...
void ResourceTransferrable::executeTransfer(ContainerBase* from, ContainerBase* to, int amount,
...
ResourceTransferrable::ResourceTransferrable(int resource, ResourceWatch& src, ResourceWatch& dst,
                                             bool isExchangable)
...
int ResourceTransferrable::getMin(ContainerBase* c, bool avail) {
   if (avail) {
      int space = getOpposingResourceWatch(c).limit().resource(resourceType) -
                  getOpposingResourceWatch(c).avail().resource(resourceType);
      if (space > getAvail(c))
         return 0;
      else
...
int ResourceTransferrable::transfer(ContainerBase* target, int delta) {
   if (delta < 0)
      ...
      delta =
         min(delta, getResourceWatch(target).limit().resource(resourceType) - getAvail(target));
...
void ResourceTransferrable::commit(const Context& context) {
   ...
   executeTransfer(source.getContainer(), target,
                   getAmount(target) - target->getResource(maxint, resourceType, true), context);
}
...
int AmmoTransferrable::get(ContainerBase* c, int toGet, bool queryOnly) {
   ...
   if (allowAmmoProduction && toProduce > 0 && weaponAmmo[ammoType] &&
       c->baseType->hasFunction(ContainerBaseType::AmmoProduction)) {
      for (int r = 0; r < resourceTypeNum; ++r) {
         if (ammoProductionCost[ammoType][r]) {
            int produceable =
               getResourceWatch(c).avail().resource(r) / ammoProductionCost[ammoType][r];
...
void AmmoTransferrable::executeTransfer(ContainerBase* from, ContainerBase* to, int amount,
                                        const Context& context) {
   if (amount < 0)
      executeTransfer(to, from, -amount, context);
   else if (amount > 0) {
      std::unique_ptr<ConsumeAmmo> ca1(new ConsumeAmmo(from, ammoType, -1, amount));
      ...
      std::unique_ptr<ConsumeAmmo> ca2(new ConsumeAmmo(to, ammoType, -1, -amount));
...
AmmoTransferrable::AmmoTransferrable(int ammo, ResourceWatch& src, ResourceWatch& dst,
                                     bool& allowProduction)
   : Transferrable(src, dst), ammoType(ammo), allowAmmoProduction(allowProduction) {
...
void AmmoTransferrable::commit(const Context& context) {
   executeTransfer(source.getContainer(), dest.getContainer(),
                   destAmmo - orgAmmo[dest.getContainer()], context);
}
   ...
   if (serviceWeapon)
      if ((ignoreChecks & ignoreHeight) || (serviceWeapon->sourceheight & source->getHeight()))
         if ((ignoreChecks & ignoreHeight) || (serviceWeapon->targ & dest->getHeight())) {
            ...
            if ((ignoreChecks & ignoreDistance) ||
                (serviceWeapon->mindistance <= dist && serviceWeapon->maxdistance >= dist))
               if (serviceWeapon->targetingAccuracy[dest->baseType->getMoveMalusType()] > 0)
                  if ((ignoreChecks & ignoreHeight) ||
                      (serviceWeapon->efficiency[6 + getheightdelta(source, dest)] > 0))
                     if (!((source->getHeight() & (chtieffliegend | chfliegend | chhochfliegend)) &&
                           dest->baseType->hasFunction(ContainerBaseType::NoInairRefuelling)))
                        return true;
         }
...
ServiceChecker ::ServiceChecker(ContainerBase* src, int skipChecks)
   : source(src), ignoreChecks(skipChecks) {}
...
void ServiceChecker ::check(ContainerBase* dest) {
   ...
   static ContainerBaseType::ContainerFunctions resourceVehicleFunctions[resourceTypeNum] = {
      ContainerBaseType::ExternalEnergyTransfer, ContainerBaseType::ExternalMaterialTransfer,
   ...
   for (int r = 0; r < resourceTypeNum; r++) {
      if (externalTransfer) {
         ...
         if (!srcVehicle) {  // it's a building
            ...
            assert(bld);
            bool active = source->baseType->hasFunction(resourceVehicleFunctions[r]) ||
                          dest->baseType->hasFunction(resourceVehicleFunctions[r]);
            if ((ignoreChecks & ignoreHeight) || (bld->typ->externalloadheight & dest->getHeight()))
               if ((ignoreChecks & ignoreDistance) ||
                   beeline(source->getPosition(), dest->getPosition()) < 20)
   ...
   /* it is important that the ammo transfers are in front of the resource transfers, because ammo
      production affects resource amounts and their prelimarny commitment would cause
      inconsistencies */
   ...
   for (int a = 0; a < weaponTypeNum; ++a) {
      if (source->maxAmmo(a) && dest->maxAmmo(a)) {
         if (weaponAmmo[a]) {
            if (externalTransfer) {
               if (source->baseType->hasFunction(ContainerBaseType::ExternalAmmoTransfer) ||
                   dest->baseType->hasFunction(ContainerBaseType::ExternalAmmoTransfer)) {
                  Vehicle* srcVehicle = dynamic_cast<Vehicle*>(source);
                  if (!srcVehicle) {  // it's a building
                     Building* bld = dynamic_cast<Building*>(source);
                     assert(bld);
                     if ((ignoreChecks & ignoreHeight) ||
                         (bld->typ->externalloadheight & dest->getHeight()))
                        if ((ignoreChecks & ignoreDistance) ||
                            beeline(source->getPosition(), dest->getPosition()) < 20)
                           ammo(dest, a);
                  } else {
...
ServiceTargetSearcher::ServiceTargetSearcher(ContainerBase* src, int checkFlags)
   : ServiceChecker(src), checks(checkFlags) {
   gamemap = src->getMap();
}
...
bool ServiceTargetSearcher::externallyAvailable() {
   ...
   if (srcVehicle) {
      ...
      if (srcVehicle->reactionfire.getStatus() == Vehicle::ReactionFire::off ||
          srcVehicle->baseType->hasFunction(ContainerBaseType::MoveWithReactionFire))
      ...
      if (source->getMap()->getField(source->getPosition())->unitHere(srcVehicle)) {
         ...
         if (weap)
            circularFieldIterator(
               source->getMap(), source->getPosition(), weap->maxdistance / maxmalq,
               (weap->mindistance + maxmalq - 1) / maxmalq,
               FieldIterationFunctor(this, &ServiceTargetSearcher::fieldChecker));
      }
   } else
      circularFieldIterator(source->getMap(), source->getPosition(), 1, 1,
                            FieldIterationFunctor(this, &ServiceTargetSearcher::fieldChecker));

   for (ContainerBase::Cargo::const_iterator i = source->getCargo().begin();
        i != source->getCargo().end(); ++i)
      if (*i)
         check(*i);
};
...
TransferHandler::TransferHandler(ContainerBase* src, ContainerBase* dst, int flags)
   : ServiceChecker(src, flags), sourceRes(src), destRes(dst), source(src), dest(dst) {
   allowProduction = CGameOptions::Instance()->autoproduceammunition;

   if (dest->getMap()->player[dest->getMap()->actplayer].diplomacy.getState(dest->getOwner()) <=
       TRUCE)
      return;
...
bool TransferHandler::ammoProductionPossible() {
   return source->baseType->hasFunction(ContainerBaseType::AmmoProduction) ||
          dest->baseType->hasFunction(ContainerBaseType::AmmoProduction);
}
