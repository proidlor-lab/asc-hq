...
SpawnBuilding::SpawnBuilding(GameMap* gamemap, const MapCoordinate& position, int buildingTypeID,
                             int owner)
   ...
   for (int a = 0; a < 4; a++)
      for (int b = 0; b < 6; b++)
         if (buildingType->fieldExists(BuildingType::LocalCoordinate(a, b))) {
            MapField* field = getMap()->getField(
               buildingType->getFieldCoordinate(pos, BuildingType::LocalCoordinate(a, b)));
            if (field == NULL)
