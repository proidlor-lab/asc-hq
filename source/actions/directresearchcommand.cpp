...
void DirectResearchCommand::readData(tnstream& stream) {
   ...
   if (version < 1 || version > directResearchCommandStreamVersion)
      throw tinvalidversion("directResearchCommandStreamVersion",
                            directResearchCommandStreamVersion, version);
...
ActionResult DirectResearchCommand::undoAction(const Context& context) {
   ...
   for (vector<int>::const_iterator i = immediatelyResearchedTechnologies.begin();
        i != immediatelyResearchedTechnologies.end(); ++i) {
      Technology* t = technologyRepository.getObject_byID(*i);
      if (!t)
         return ActionResult(21005, ASCString::toString(*i));

      vector<int>::iterator j =
         find(research.developedTechnologies.begin(), research.developedTechnologies.end(), *i);
   ...
   for (vector<ASCString>::iterator i = techAdaptersMadeAvailable.begin();
        i != techAdaptersMadeAvailable.end(); ++i) {
      Research::TriggeredTechAdapter::iterator ta =
         find(research.triggeredTechAdapter.begin(), research.triggeredTechAdapter.end(), *i);
...
ASCString DirectResearchCommand::getDescription() const {
   ASCString s = "Choose technology " + ASCString::toString(targetTechnologyID) + " for player " +
                 ASCString::toString(player);
   return s;
}
