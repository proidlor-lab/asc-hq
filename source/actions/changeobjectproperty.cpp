...
ChangeObjectProperty::ChangeObjectProperty(GameMap* map, MapCoordinate& pos, Object* object,
                                           Property property, int value, bool valueIsAbsolute)
...
ASCString ChangeObjectProperty::getDescription() const {
   ASCString res = "Change property " + getPropertyName(property) + " of object with id " +
                   ASCString::toString(objectID) + " on field " + position.toString(false);
...
int ChangeObjectProperty::getProperty() {
   switch (property) {
      case Damage:
         return getObject()->damage;
   };
   throw ActionResult(21503);
}

ActionResult ChangeObjectProperty::setProperty(Property property, int value,
