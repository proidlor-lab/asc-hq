...
SpawnObject::SpawnObject(GameMap* gamemap, const MapCoordinate& position, int objectID,
                         int direction)
...
class ActionObjectRemovalStrategy : public MapField::ObjectRemovalStrategy {
  ...
  public:
   ...
   bool getImmediateRemoval() { return immediateRemoval; }
};
...
ActionResult SpawnObject::undoAction(const Context& context) {
   ...
   } else if (!objectImmediatelyDisappearsAgain)
