...
SpawnUnit::SpawnUnit(GameMap* gamemap, const MapCoordinate3D& position, int vehicleTypeID,
                     int owner, int unitID)
   : GameAction(gamemap),
     pos(position),
     networkid(-1),
     carrierID(0),
     carrier(false),
     mapNetworkIdCounterBefore(0),
     mapNetworkIdCounterAfter(0) {
...
ActionResult SpawnUnit::runAction(const Context& context) {
   ...
   if (!carrier) {
      ...
      ActionResult res = (new UnitFieldRegistration(v, pos, UnitFieldRegistration::RegisterOnField))
                            ->execute(context);
      if (res.successful())
      ...
      ActionResult res = (new UnitFieldRegistration(
                             v, pos, UnitFieldRegistration::RegisterInCarrier, hostingCarrier))
                            ->execute(context);
      return res;
   }
