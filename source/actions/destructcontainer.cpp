   ...
   if (veh) {
      ...
      if (!veh->typ->wreckageObject.empty() && getMap()->state != GameMap::Destruction &&
         ...
         if (fieldRegistration == FIRST || fieldRegistration == SECOND) {
            for (vector<int>::const_iterator i = veh->typ->wreckageObject.begin();
                 i != veh->typ->wreckageObject.end(); ++i) {
         ...
         for (int j = 0; j < BuildingType::ydimension; j++)
            if (bld->typ->fieldExists(BuildingType::LocalCoordinate(i, j))) {
               ...
               if (fld) {
                  ...
                  pair<J, J> b =
                     bld->typ->destructionObjects.equal_range(BuildingType::LocalCoordinate(i, j));
                  for (J o = b.first; o != b.second; ++o)
                     (new SpawnObject(getMap(),
                                      bld->getFieldCoordinates(BuildingType::LocalCoordinate(i, j)),
                                      o->second))
                        ->execute(context);
               }
            }
