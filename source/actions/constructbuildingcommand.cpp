...
Resources ConstructBuildingCommand::getProductionCost(const BuildingType* bld) const {
   ...
   return Resources(0, bld->productionCost.material * mf / 100,
                    bld->productionCost.fuel * ff / 100);
}

ConstructBuildingCommand::Lack
ConstructBuildingCommand::buildingProductionPrerequisites(const BuildingType* type) const {
   int l = 0;
   ...
   for (int i = 0; i < buildingTypeRepository.getNum(); i++) {
      ...
      for (int j = 0; j < getUnit()->typ->buildingsBuildable.size(); j++)
         if (getUnit()->typ->buildingsBuildable[j].from <= bld->id &&
            ...
            entries.push_back(
               ProductionEntry(bld, getProductionCost(bld), buildingProductionPrerequisites(bld)));
         }
   }
   return entries;
}
      ...
      for (int y1 = 0; y1 <= 5; y1++)
         for (int x1 = 0; x1 <= 3; x1++)
            if (bld->fieldExists(BuildingType::LocalCoordinate(x1, y1))) {
               MapField* fld = getMap()->getField(
                  bld->getFieldCoordinate(entry, BuildingType::LocalCoordinate(x1, y1)));
               if (fld) {
                  ...
                  if (fld->building != NULL) {
                     if (fld->building->typ != bld)
                        b = false;
                     if (fld->building->getCompletion() ==
                         fld->building->typ->construction_steps - 1)
                        b = false;
...
ActionResult ConstructBuildingCommand::go(const Context& context) {
   ...
   std::unique_ptr<SpawnBuilding> sb(
      new SpawnBuilding(getMap(), target, buildingTypeID, getUnit()->getOwner()));
...
ASCString ConstructBuildingCommand ::getCommandString() const {
   ASCString c;
   c.format("constructBuilding ( map, %d, asc.MapCoordinate(%d, %d), %d )", getUnitID(), target.x,
            target.y, buildingTypeID);
   return c;
}
