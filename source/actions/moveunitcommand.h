...
class MoveUnitCommand : public UnitCommand, public TaskInterface, public sigc::trackable {
  ...
  public:
   ...
   enum SeachCapabilities {
      NoInterrupt = 1,  //!< the unit will not interrupt the movement if it runs onto a mine of into
                        //!< reaction fire
      DisableHeightChange =
         2,  //!< the unit will not change its height even if that would provide a shortcut
   ...
   //! defines whether we want to end up either at the same level of height (0), lower(<0), or
   //! heigher(>0)
