...
ChangeDiplomaticState::ChangeDiplomaticState(Player& fromplayer, Player& towardsPlayer, int queue,
...
ActionResult ChangeDiplomaticState::runAction(const Context& context) {
   ...
   if (queued > 0) {
      DiplomaticStateVector::QueuedStateChanges::iterator i =
         d.queuedStateChanges.find(towardsPlayer);
...
ASCString ChangeDiplomaticState ::getCommandString() const {
   ASCString c;
   c.format("changediplomaticstate ( %d %d %d %d )", fromPlayer, towardsPlayer, queued,
            (int) newState);
   return c;
}
...
ASCString ChangeDiplomaticState::getDescription() const {
   ASCString s = "Change diplomatic state between " + ASCString::toString(fromPlayer) + " and " +
                 ASCString::toString(towardsPlayer) + " to " + diplomaticStateNames[newState];
   return s;
}
