...
bool PutObjectCommand ::avail(Vehicle* eht) {
   if (eht)
      if (eht->getOwner() == eht->getMap()->actplayer)
         if (eht->typ->objectsBuildable.size() || eht->typ->objectsRemovable.size() ||
             eht->typ->objectGroupsBuildable.size() || eht->typ->objectGroupsRemovable.size())
            if (!eht->attacked)
               return true;
   return false;
}

PutObjectCommand ::PutObjectCommand(Vehicle* unit) : UnitCommand(unit), object(-1), mode(Build) {}

vector<MapCoordinate> PutObjectCommand::getFields() {
   vector<MapCoordinate> fields;
   for (map<MapCoordinate, set<int>>::iterator i = objectsCreatable.begin();
        i != objectsCreatable.end(); ++i)
      fields.push_back(i->first);

   for (map<MapCoordinate, set<int>>::iterator i = objectsRemovable.begin();
        i != objectsRemovable.end(); ++i)
...
ActionResult PutObjectCommand::searchFields() {
   ...
   circularFieldIterator(getMap(), getUnit()->getPosition(), 1, 1,
                         FieldIterationFunctor(this, &PutObjectCommand::fieldChecker));
...
bool PutObjectCommand::checkObject(MapField* fld, ObjectType* objtype, Mode mode) {
   ...
   if (mode == Build) {
      if (objtype->getFieldModification(fld->getWeather()).terrainaccess.accessible(fld->bdt) > 0 &&
         ...
         //            && !getheightdelta ( getFirstBit( actvehicle->height),
         //            getFirstBit(objtype->getEffectiveHeight())) ) {

         if (veh->getResource(objtype->buildcost, true) == objtype->buildcost &&
             veh->getMovement() >= objtype->build_movecost)
            return true;
      }
   } else {
...
void PutObjectCommand ::fieldChecker(const MapCoordinate& pos) {
   ...
   if (fieldvisiblenow(getMap()->getField(pos))) {
      ...
      for (int i = 0; i < veh->typ->objectsBuildable.size(); i++)
         for (int j = veh->typ->objectsBuildable[i].from; j <= veh->typ->objectsBuildable[i].to;
              j++) {
      ...
      for (int i = 0; i < veh->typ->objectGroupsBuildable.size(); i++)
         for (int j = veh->typ->objectGroupsBuildable[i].from;
              j <= veh->typ->objectGroupsBuildable[i].to; j++)
            for (int k = 0; k < objectTypeRepository.getNum(); k++) {
               ObjectType* objtype = objectTypeRepository.getObject_byPos(k);
               if (objtype->groupID == j)
                  if (checkObject(fld, objtype, Build))
                     if (find(objectsCreatable[pos].begin(), objectsCreatable[pos].end(),
                              objtype->id) == objectsCreatable[pos].end())
                        objectsCreatable[pos].insert(objtype->id);
            }

      for (int i = 0; i < veh->typ->objectsRemovable.size(); i++)
         for (int j = veh->typ->objectsRemovable[i].from; j <= veh->typ->objectsRemovable[i].to;
              j++) {
            ObjectType* objType = getMap()->getobjecttype_byid(j);
      ...
      for (int i = 0; i < veh->typ->objectGroupsRemovable.size(); i++)
         for (int j = veh->typ->objectGroupsRemovable[i].from;
              j <= veh->typ->objectGroupsRemovable[i].to; j++)
            for (int k = 0; k < objectTypeRepository.getNum(); k++) {
               ObjectType* objtype = objectTypeRepository.getObject_byPos(k);
               if (objtype->groupID == j)
                  if (checkObject(fld, objtype, Remove))
                     if (find(objectsRemovable[pos].begin(), objectsRemovable[pos].end(),
                              objtype->id) == objectsRemovable[pos].end())
                        objectsRemovable[pos].insert(objtype->id);
            }
   }
}
...
ActionResult PutObjectCommand::go(const Context& context) {
   ...
   bool objectAffectsVisibility = obj->basicjamming_plus || obj->viewbonus_plus ||
                                  obj->viewbonus_abs != -1 || obj->basicjamming_abs != -1;
   ...
   if (res.successful())
      res = (new ConsumeResource(getUnit(), mode == Build ? obj->buildcost : obj->removecost))
               ->execute(context);

   if (res.successful())
      res = (new ChangeUnitMovement(
                getUnit(), mode == Build ? obj->build_movecost : obj->remove_movecost, true))
               ->execute(context);
...
ASCString PutObjectCommand ::getCommandString() const {
   ASCString c;
   if (mode == Build)
      c.format("unitPutObject ( map, %d, asc.MapCoordinate( %d, %d), %d )", getUnitID(), target.x,
               target.y, object);
   else
      c.format("unitRemoveObject ( map, %d, asc.MapCoordinate( %d, %d), %d )", getUnitID(),
               target.x, target.y, object);
   return c;
}
