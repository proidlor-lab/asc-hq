/*! \file basegfx.h
   \brief basegfx.h is the interface for the legacy graphic routines (of which some are platform
   dependent).
...
struct tgraphmodeparameters {
   ...
   int actsetpage;   //!< only used in DOS where it may be necessary to access the graphic memory in
                     //!< 64 kB pages
   int windowstatus;  //!< determines whether the memory is a linear (windowstatus==100) or paged.
                      //!< When using SDL, the memory is always linear addressable.
   int granularity;   //!< the offset in graphics memory between two pages. Can be ignored nowadays
   int scanlinelength;    //!< the size of a line in byte (may be larger than resolutionx due to
                          //!< offscreen memory)
   int scanlinenumber;    //!< the number of lines (may be larger than resolutiony due to offscreen
                          //!< memory)
   ...
   Uint8 redfieldposition;    //!< RGB only: the position of the first red bit relative to the start
                              //!< of the pixel
   Uint8 greenmasksize;       //!< RGB only: the number of bits in the green component
   Uint8 greenfieldposition;  //!< RGB only: the position of the first green bit relative to the
                              //!< start of the pixel
   Uint8 bluemasksize;        //!< RGB only: the number of bits in the blue component
   Uint8 bluefieldposition;  //!< RGB only: the position of the first blue bit relative to the start
                             //!< of the pixel
   Uint8 bitperpix;          //!< the size of a pixel(?) in bits
   Uint8 memorymodel;        //!< unused
   int directscreenaccess;   //!< if 0 no call to an update function (which copies the buffer to the
                             //!< screen) is performed
   Surface* surface;
...
//! copy an image to a buffer. buffer must have been alloced before with malloc ( imagesize (
//! x1,y1,x2,y2 ))
...
//! puts the image pointed to by buffer on the screen. All pixels with color #255 are treated as
//! transparent
...
//! returns the position of the lowest bit of a which is set. This equals an inter logarithm to the
//! base of 2
...
//! puts a single pixel on the screen. This is one of the few functions that work in truecolor mode
//! too
extern void putpixel(int x1, int y1, int color);

//! gets the color of a single pixel from the screen. This is one of the few functions that work in
//! truecolor mode too
...
/** A translation table ( to be used by xlatpict ) is generated to translate any color of palette
     ...
     offset-size range. */
extern void generategrayxlattable(ppixelxlattable tab, Uint8 offset, Uint8 size,
                                  dacpalette256* pal);
...
//! Any new code should use SDLmm-Surfaces.
...
//! puts the image pointed to by tci to the screen. Both must be truecolor images. This function is
//! a quick and unoptimized hack!
...
// this variable determines whether the next call to initgraphics will open a window or a fullscreen
// session extern int fullscreen;
...
/** converts a SDLmm::Surface to an old style image buffer
    ...
    \param paletteTranslation If the source surface is 8 bit, convertSurface will convert the
   palette to the ASC palette if this is set to true.
*/
