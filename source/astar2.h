...
class AStar3D {
  public:
   ...
   class PathPoint : public MapCoordinate3D {
     ...
     public:
      PathPoint(const MapCoordinate3D& mc, int dist_, int enterHeight_, bool hasAttacked_)
         : MapCoordinate3D(mc), dist(dist_), enterHeight(enterHeight_), hasAttacked(hasAttacked_){};
      // PathPoint ( const MapCoordinate3D& mc ) : MapCoordinate3D(mc), dist(-1), enterHeight(-1),
      // hasAttacked(false) {};
      int getRealHeight() {
         if (getNumericalHeight() != -1)
            return getNumericalHeight();
         else
            return enterHeight;
      };
      MapCoordinate3D getRealPos() {
         MapCoordinate3D p;
         p.setnum(x, y, getRealHeight());
         return p;
      };
      int dist;
      int enterHeight;
   ...
   struct hash_MapCoordinate {
      size_t operator()(const MapCoordinate& h) const { return h.x ^ (h.y << 16); }
   };
  ...
  protected:
   ...
   virtual DistanceType getMoveCost(const MapCoordinate3D& start, const MapCoordinate3D& dest,
                                    const Vehicle* vehicle, bool& hasAttacked);
  ...
  protected:
   int initNode(Node& newN, const Node* oldN_ptr, const MapCoordinate3D& newpos,
                const vector<MapCoordinate3D>& B, bool disableAttack = false, bool enter = false,
                bool dock = false);
  ...
  public:
   ...
   //! searches for a path from the units current position to one of the dest fields and stores it
   //! in path
   ...
   */
   ...
   //! else true
   inline bool constructPath(Path& path, const MapCoordinate3D& pos) {
