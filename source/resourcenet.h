...
class ResourceNet : public MapNetwork {
  public:
   ResourceNet(GameMap* gamemap, int _scope = -1)
      : MapNetwork(gamemap, _scope != 0), resourcetype(-1), scope(_scope){};
...
class GetConnectedBuildings : public ResourceNet {
  ...
  public:
   ...
   GetConnectedBuildings(BuildingContainer& buildingContainer_, GameMap* gamemap, int resourceType)
      : ResourceNet(gamemap, gamemap->isResourceGlobal(resourceType) ? 2 : 1),
        buildingContainer(buildingContainer_) {
      resourcetype = 0;
   };
};
...
class PutTribute : public StaticResourceNet {
  ...
  public:
   ...
   int puttribute(Building* start, int resource, int _queryonly, int _forplayer, int _fromplayer,
                  int _scope);
};
...
extern bool compareMapResources(GameMap* currentMap, GameMap* replaymap, int player,
                                ASCString* log);
