...
int AttackFormula ::getIconIndex(int experience, bool offensive) {
   int idx;
   if (offensive)
      idx = (int) (experienceIcons * strength_experience(experience) /
                   strength_experience(maxunitexperience));
   else
      idx = (int) (experienceIcons * defense_experience(experience) /
                   defense_experience(maxunitexperience));
...
void tfight ::calc(void) {
   ...
   if (av.strength) {
      float absstrength =
         ...
         strength_damage(av.damage) * dv.hemming;

      float absdefense =
         float(dv.armor / armordivisor) *
...
tunitattacksunit ::tunitattacksunit(Vehicle*& attackingunit, Vehicle*& attackedunit, bool respond,
                                    int weapon, bool reactionfire)
   : UnitAttacksSomething(attackingunit->getMap()) {
   this->reactionfire = reactionfire;
   setup(attackingunit, attackedunit, respond, weapon);
}

void tunitattacksunit ::setup(Vehicle*& attackingunit, Vehicle*& attackedunit, bool respond,
                              int weapon) {
   ...
   av.strength = int(ceil(attackingunit->weapstrength[_weapon] *
                          WeapDist::getWeaponStrength(weap, targetWeather, dist,
                                                      attackingunit->height, attackedunit->height) *
                          attackingunit->typ->weapons.weapon[_weapon]
                             .targetingAccuracy[attackedunit->typ->movemalustyp] /
                          100));
   ...
   if (dist <= maxmalq && respond && !av.kamikaze &&
       fieldvisiblenow(field, attackedunit->getOwner())) {
      AttackWeap atw;
   ...
   if (respond) {
      ...
      int attackerWeather =
         attackingunit->getMap()->getField(attackingunit->getPosition())->getWeather();

      dv.strength =
         int(ceil(attackedunit->weapstrength[dv.weapnum] *
                  WeapDist::getWeaponStrength(weap, attackerWeather, dist, attackedunit->height,
                                              attackingunit->height) *
                  attackedunit->typ->weapons.weapon[dv.weapnum]
                     .targetingAccuracy[attackingunit->typ->movemalustyp] /
                  100));
   ...
   if (attackedunit->height <= chfahrend)
      dv.defensebonus = attackedunit->getMap()
                           ->getField(attackedunit->xpos, attackedunit->ypos)
                           ->getdefensebonus();
   else
      dv.defensebonus = 0;
...
void log(const Vehicle* attacker, const Vehicle* attackee) {
   if (CGameOptions::Instance()->logKillsToConsole) {
      ...
      if (!header) {
         msg +=
            "# Attacked player; Attacking player ; attacked unit type id ; attacked unit type name "
            "; attacked unit offensive experience ; turn \n";
         header = true;
      }

      msg += ASCString::toString(attackee->getOwner()) + ";" +
             ASCString::toString(attacker->getOwner()) + ";" +
             ASCString::toString(attackee->typ->id) + ";" + attackee->typ->name + ";" +
...
void tunitattacksunit ::setresult(const Context& context) {
   ...
   if (!reactionfire) {
      GameAction* a = new ChangeUnitProperty(
         _attackingunit, ChangeUnitProperty::ExperienceOffensive, av.experience_offensive);
      a->execute(context);

      GameAction* a2 = new ChangeUnitProperty(
         _attackingunit, ChangeUnitProperty::ExperienceDefensive, av.experience_defensive);
      a2->execute(context);
   }

   GameAction* b = new ConsumeAmmo(
      _attackingunit, _attackingunit->typ->weapons.weapon[av.weapnum].getScalarWeaponType(),
      av.weapnum, _attackingunit->ammo[av.weapnum] - av.weapcount);
   b->execute(context);
   ...
   if (_respond) {
      GameAction* d = new ChangeUnitProperty(_attackedunit, ChangeUnitProperty::ExperienceOffensive,
                                             dv.experience_offensive);
      d->execute(context);

      GameAction* e = new ConsumeAmmo(
         _attackedunit, _attackedunit->typ->weapons.weapon[dv.weapnum].getScalarWeaponType(),
         dv.weapnum, _attackedunit->ammo[dv.weapnum] - dv.weapcount);
      e->execute(context);
   }

   if (!reactionfire) {
      GameAction* d2 = new ChangeUnitProperty(
         _attackedunit, ChangeUnitProperty::ExperienceDefensive, dv.experience_defensive);
      d2->execute(context);
   }
...
void tunitattacksbuilding ::setup(Vehicle* attackingunit, int x, int y, int weapon) {
   ...
   av.strength =
      int(ceil(attackingunit->weapstrength[_weapon] *
               WeapDist::getWeaponStrength(weap, targetWeather, dist, attackingunit->height,
                                           _attackedbuilding->typ->height) *
...
tmineattacksunit ::tmineattacksunit(const MapCoordinate& mineposition, int minenum,
...
void tmineattacksunit ::setup(const MapCoordinate& position, int minenum, Vehicle*& attackedunit) {
   ...
   if (minenum == -1) {
      int cnt = 1;
      av.strength = 0;
      for (MapField::MineContainer::iterator m = mineposition->mines.begin();
           m != mineposition->mines.end(); m++)
...
void tmineattacksunit ::setresult(const Context& context) {
   ...
   if (_minenum == -1) {
      for (MapField::MineContainer::iterator m = _mineposition->mines.begin();
           m != _mineposition->mines.end(); ++m)
         if (m->attacksunit(_attackedunit))
            actions.push_back(new RemoveMine(_attackedunit->getMap(), position, m->identifier));
   } else {
      int counter = 0;
      for (MapField::MineContainer::iterator m = _mineposition->mines.begin();
           m != _mineposition->mines.end(); ++m, ++counter)
         if (counter == _minenum)
            actions.push_back(new RemoveMine(_attackedunit->getMap(), position, m->identifier));
   }
...
void tunitattacksobject ::setup(Vehicle* attackingunit, int obj_x, int obj_y, int weapon) {
   ...
   for (MapField::ObjectContainer::reverse_iterator o = targetField->objects.rbegin();
        o != targetField->objects.rend(); o++)
      if (o->typ->armor > 0) {
         _obji = &(*o);
         break;
      }
   ...
   const SingleWeapon* weap = &attackingunit->typ->weapons.weapon[_weapon];
   av.strength = int(
      ceil(attackingunit->weapstrength[_weapon] *
           WeapDist::getWeaponStrength(weap, targetField->getWeather(), dist, attackingunit->height,
                                       _obji->typ->getEffectiveHeight()) *
           attackingunit->typ->weapons.weapon[_weapon].targetingAccuracy[cmm_building] / 100));
...
void tunitattacksobject ::setresult(const Context& context) {
   ...
   if (dv.damage >= 100) {
      ...
      GameAction* g =
         new ChangeObjectProperty(map, position, _obji, ChangeObjectProperty::Damage, dv.damage);
      g->execute(context);
   }
}
...
AttackWeap* attackpossible(const Vehicle* attacker, int x, int y) {
   ...
   if (efield->getVehicle()) {
      ...
      if (attacker->getMap()->getPlayer(attacker).diplomacy.isHostile(
             efield->building->getOwner()) ||
          efield->building->color == 8 * 8)
                        ...
                        if (fieldvisiblenow(efield, attacker->color / 8)) {
                           ...
                           if (d <= attacker->typ->weapons.weapon[i].maxdistance)
                              if (d >= attacker->typ->weapons.weapon[i].mindistance) {
                                 if (attacker->height &
                                     attacker->typ->weapons.weapon[i].sourceheight)
                                    if (attacker->typ->weapons.weapon[i]
                                           .efficiency[6 +
                                                       getheightdelta(getFirstBit(attacker->height),
                                                                      getFirstBit(tm))])
                                       if (attacker->ammo[i] > 0) {
                                          atw->strength[atw->count] = attacker->weapstrength[i];
                                          atw->typ[atw->count] = 1
                                                                 << attacker->typ->weapons.weapon[i]
                                                                       .getScalarWeaponType();
                                          atw->num[atw->count] = i;
      ...
      int n = 0;
      for (MapField::ObjectContainer::iterator j = efield->objects.begin();
           j != efield->objects.end(); j++)
         if (j->typ->armor > 0)
            n++;
      ...
      if (n > 0)
         if ((efield->vehicle == NULL) && (efield->building == NULL)) {
            bool found = false;
            for (MapField::ObjectContainer::reverse_iterator j = efield->objects.rbegin();
                 j != efield->objects.rend(); ++j) {
               for (int i = 0; i <= attacker->typ->weapons.count - 1; i++)
                  if (attacker->typ->weapons.weapon[i].shootable())
                     if (attacker->typ->weapons.weapon[i].getScalarWeaponType() == cwcannonn ||
                         ...
                         attacker->typ->weapons.weapon[i].getScalarWeaponType() ==
                            cwcruisemissilen ||
                         attacker->typ->weapons.weapon[i].getScalarWeaponType() ==
                            cwlargemissilen ||
                         attacker->typ->weapons.weapon[i].getScalarWeaponType() ==
                           ...
                           if (fieldvisiblenow(efield, attacker->color / 8)) {
                              ...
                              if (d <= attacker->typ->weapons.weapon[i].maxdistance)
                                 if (d >= attacker->typ->weapons.weapon[i].mindistance) {
                                    if (attacker->height &
                                        attacker->typ->weapons.weapon[i].sourceheight)
                                       if (attacker->typ->weapons.weapon[i].targ &
                                           j->typ->getEffectiveHeight())
                                          if (attacker->typ->weapons.weapon[i]
                                                 .efficiency[6 +
                                                             getheightdelta(
                                                                getFirstBit(attacker->height),
                                                                getFirstBit(
                                                                   j->typ->getEffectiveHeight()))])
                                             if (attacker->ammo[i] > 0) {
                                                atw->strength[atw->count] =
                                                   attacker->weapstrength[i];
                                                atw->num[atw->count] = i;
                                                atw->typ[atw->count] =
                                                   1 << attacker->typ->weapons.weapon[i]
                                                           .getScalarWeaponType();
...
bool attackpossible2u(const Vehicle* attacker, const Vehicle* target, AttackWeap* atw,
                      int targetheight) {
   if (targetheight == -1)
                  ...
                  if (attacker->typ->weapons.weapon[i].offensive())
                     if ((1 << h) & attacker->typ->weapons.weapon[i].targ)
                        if (attacker->height & attacker->typ->weapons.weapon[i].sourceheight)
                           if (attacker->typ->weapons.weapon[i]
                                  .targetingAccuracy[target->typ->movemalustyp] > 0)
                              if (attacker->typ->weapons.weapon[i]
                                     .efficiency[6 +
                                                 getheightdelta(getFirstBit(attacker->height), h)])
                                 if (attacker->ammo[i] > 0) {
                                    result = true;
                                    if (atw) {
                                       atw->strength[atw->count] =
                                          attacker->weapstrength[i] *
                                          attacker->typ->weapons.weapon[i]
                                             .targetingAccuracy[target->typ->movemalustyp] /
                                          100;
                                       atw->num[atw->count] = i;
                                       atw->typ[atw->count] =
                                          1
...
bool attackpossible28(const Vehicle* attacker, const Vehicle* target, AttackWeap* atw,
                      int targetHeight) {
   if (targetHeight < 0)
      targetHeight = target->height;
   ...
   //   if ( attacker->getMap()->player[attacker->getOwner()].diplomacy.isHostile(
   //   target->getOwner() )  )
   for (int i = 0; i < attacker->typ->weapons.count; i++)
      if (attacker->typ->weapons.weapon[i].shootable())
         if (attacker->typ->weapons.weapon[i].offensive())
                  ...
                  if (minmalq >= attacker->typ->weapons.weapon[i].mindistance)
                     if (attacker->height & attacker->typ->weapons.weapon[i].sourceheight)
                        if (attacker->typ->weapons.weapon[i]
                               .targetingAccuracy[target->typ->movemalustyp] > 0)
                           if (attacker->typ->weapons.weapon[i]
                                  .efficiency[6 + getheightdelta(getFirstBit(attacker->height),
                                                                 getFirstBit(targetHeight))])
                              if (attacker->ammo[i] > 0) {
                                 result = true;
                                 if (atw) {
                                    atw->strength[atw->count] =
                                    ...
                                    atw->num[atw->count] = i;
                                    atw->typ[atw->count] =
                                       1 << attacker->typ->weapons.weapon[i].getScalarWeaponType();
                                    atw->target = AttackWeap::vehicle;
                                    atw->count++;
                                 }
                              }
...
bool attackpossible2n(const Vehicle* attacker, const Vehicle* target, AttackWeap* atw) {
   ...
   if (attacker->getMap()->player[attacker->getOwner()].diplomacy.isHostile(target->getOwner()))
      if (!attacker->attacked)
         if (!attacker->typ->wait || !attacker->hasMoved() ||
             attacker->reactionfire.getStatus() == Vehicle::ReactionFire::ready)
            for (int i = 0; i < attacker->typ->weapons.count; i++)
               if (attacker->typ->weapons.weapon[i].shootable())
                  if (attacker->typ->weapons.weapon[i].offensive())
...
float WeapDist::getWeaponStrength(const SingleWeapon* weap, int weather, int dist,
                                  int attacker_height, int defender_height, int reldiff) {
   /*
     int         translat[31]  = { 6, 255, 1, 3, 2, 4, 0, 5, 255, 255, 6, 255, 255, 255, 255, 255,
...
*/
   ...
   if (reldiff == -1) {
      if (dist < weap->mindistance || dist > weap->maxdistance) {
         warningMessage("tweapdist::getweapstrength: invalid range: \n min = " +
                        ASCString::toString(weap->mindistance) +
                        " ; max = " + ASCString::toString(weap->maxdistance) +
                        " ; req = " + ASCString::toString(dist));
         return 0;
      }
   ...
   if (attacker_height != -1 && defender_height != -1) {
      ...
      if (attacker_height >= chtieffliegend && weather != 0 && defender_height != -1) {
         int weatherRelevantHeightDelta =
            min(abs(getheightdelta(getFirstBit(attacker_height), getFirstBit(defender_height))), 3);
         if (weather == 1 || weather == 3)
            weatherFactor = 1 - 0.07 * weatherRelevantHeightDelta;
         else if (weather == 2 || weather == 4 || weather == 5)
   ...
   /*   if ( attacker_height != -1 && defender_height!= -1 ) {
         int hd = getheightdelta ( getFirstBit ( attacker_height ), getFirstBit ( defender_height
      )); return relstrength * weap->efficiency[6+hd] / 100 ; } else return relstrength ;
         */
}
