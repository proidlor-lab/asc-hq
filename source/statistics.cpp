...
double StatisticsCalculator::strength(const ContainerBase* c, bool recurse) {
   ...
   if (veh) {
      ...
      s *= (af.strength_experience(veh->getExperience_offensive()) +
            af.defense_experience(veh->getExperience_defensive())) +
           1.0;
...
ASCString getVisibilityStatistics(GameMap* actmap) {
   ...
   for (int i = 0; i < actmap->getPlayerCount(); i++) {
      if (actmap->player[i].exist()) {
         msg += ASCString("#fontsize=14#Player ") + ASCString::toString(i) + ": " +
                actmap->player[i].getName() + "#fontsize=12#\n";
         int notVisible = 0;
         int fogOfWar = 0;
         int visible = 0;
         int viewDominance = 0;
...
ASCString getPlayerStrength(GameMap* gamemap) {
   ...
   for (int i = 0; i < gamemap->getPlayerCount(); ++i) {
      ...
      Resources r;
      Resources total;
      for (Player::VehicleList::iterator j = gamemap->player[i].vehicleList.begin();
           j != gamemap->player[i].vehicleList.end(); ++j) {
         strength += StatisticsCalculator::strength(*j, false);
         r += (*j)->typ->productionCost;
         total += StatisticsCalculator::resource(*j, false);
      }

      for (Player::BuildingList::iterator j = gamemap->player[i].buildingList.begin();
           j != gamemap->player[i].buildingList.end(); ++j) {
         total += StatisticsCalculator::resource(*j, false);
      }

      message += ASCString("#fontsize=14#Player ") + ASCString::toString(i) + ": " +
                 gamemap->player[i].getName() + "#fontsize=12#\n";
      message += "strength: ";
      ASCString s;
      ...
      message +=
         "Unit count: " + ASCString::toString(int(gamemap->player[i].vehicleList.size())) + "\n";
