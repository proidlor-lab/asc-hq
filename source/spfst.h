...
//! puts a building onto the map. To be used by the map editor! For ingame usage, see
//! ConstructBuildingCommand or SpawnBuilding
extern void putbuilding(GameMap* gamemap, const MapCoordinate& entryPosition, int color,
                        const BuildingType* buildingtyp, int completion, int ignoreunits = 0);
...
*/
extern void calculateobject(int x, int y, bool mof, const ObjectType* obj, GameMap* gamemap);

extern void calculateobject(const MapCoordinate& pos, bool mof, const ObjectType* obj,
                            GameMap* gamemap);

/*! tests if the vehicle can move onto the field

   \param uheight the level of height for which the check should be done. Use -2 to use the current
   height of the unit \retval 0 unit cannot move there \retval 1 unit can pass over the field
   \retval 2 unit can stop its movement there
*/
extern int fieldAccessible(const MapField* field, const Vehicle* vehicle, int uheight = -2,
                           const bool* attacked = NULL, bool ignoreVisibility = false);
...
inline bool fieldvisiblenow(const MapField* pe, int player) {
   return fieldvisiblenow(pe, NULL, player);
};
inline bool fieldvisiblenow(const MapField* pe, Vehicle* veh) {
   return fieldvisiblenow(pe, veh, pe->getMap()->actplayer);
};
inline bool fieldvisiblenow(const MapField* pe) {
   return fieldvisiblenow(pe, pe->getMap()->actplayer);
};
//! }@
