...
const float repairEfficiencyVehicle[resourceTypeNum * resourceTypeNum] = {0, 0,   0, 0, 0.5,
                                                                          0, 0.5, 0, 1};
...
Vehicle ::Vehicle()
   : ContainerBase(NULL, NULL, 0),
     repairEfficiency(repairEfficiencyVehicle),
     typ(NULL),
     reactionfire(this) {}
...
Vehicle ::~Vehicle() {
   ...
   if (gamemap) {
      ...
      Player::VehicleList::iterator i =
         find(gamemap->player[c].vehicleList.begin(), gamemap->player[c].vehicleList.end(), this);
...
void Vehicle ::init(void) {
   ...
   if (typ) {
      ...
      if (typ->height & chfahrend)
         height = chfahrend;
      else if (typ->height & chschwimmend)
         height = chschwimmend;
...
int Vehicle::getRepairExperienceValue(int oldDamage, int newDamage, bool offensive,
                                      int roundToResolution) {
   /*
   This is an alternative Strategy that removes as much experience points as are necessary to
   achieve the desired decrease of attack bonus int decreasePercentage = (oldDamage - newDamage) *
   repairExperienceDecrease / 100;
   ...
   while ( experience > 0 && af.strength_experience( experience ) >= origPower* ( 100 -
   decreasePercentage)/100) experience--;
...
int Vehicle ::putResource(int amount, int resourcetype, bool queryonly, int scope, int player) {
   //  if units start using/storing resources that will not be stored in the unit itself, the
   //  replays will fail !
   ...
   if (amount < 0) {
      ...
      if (player != getOwner() &&
          !(getMap()->getPlayer(getOwner()).diplomacy.getState(player) >= PEACE))
         return 0;
...
int Vehicle ::getResource(int amount, int resourcetype, bool queryonly, int scope, int player) {
   ...
   if (amount < 0) {
      ...
      if (player != getOwner() &&
          (getMap()->getPlayer(getOwner()).diplomacy.getState(player) < PEACE) && !queryonly)
         return 0;
...
void Vehicle ::postRepair(int oldDamage, bool autoRepair) {
   experience_offensive =
      getRepairExperienceValue(oldDamage, damage, true, autoRepair ? 1 : experienceResolution);
   if (experience_offensive < 0)
      experience_offensive = 0;

   experience_defensive =
      getRepairExperienceValue(oldDamage, damage, false, autoRepair ? 1 : experienceResolution);
   ;
...
void Vehicle ::beginTurn() {
   // changing unit height to the height with the maximumem movement, to reduce the inaccuracies due
   // to rounding
...
void Vehicle ::endOwnTurn() {
   ...
   for (int w = 0; w < typ->weapons.count; w++)
      if (typ->weapons.weapon[w].gettype() & cwlaserb) {
         int cnt = 0;
         while (cnt < typ->weapons.weapon[w].laserRechargeRate &&
                ammo[w] < typ->weapons.weapon[w].count) {
            for (int r = 0; r < 3; r++)
               if (typ->weapons.weapon[w].laserRechargeCost.resource(r) < 0)
                  fatalError(" negative Laser recharge cost !");
            if (!(ContainerBase::getResource(typ->weapons.weapon[w].laserRechargeCost, 1) <
                  typ->weapons.weapon[w].laserRechargeCost)) {
      ...
      if (typ->movement[getFirstBit(height)]) {
         ...
         for (Cargo::iterator i = cargo.begin(); i != cargo.end(); ++i)
            if (*i) {
               ...
               (*i)->decreaseMovement(max(
                  1, int(ceil(lperc * double((*i)->typ->movement[getFirstBit((*i)->height)])))));
            }
      }
   _movement = newmove;
}
...
bool Vehicle::spawnMoveObjects(const MapCoordinate& start, const MapCoordinate& dest,
   ...
   if (typ->objectLayedByMovement.size() && (height == chfahrend || height == chschwimmend)) {
      ...
      for (int i = 0; i < typ->objectLayedByMovement.size(); i++)
         for (int id = typ->objectLayedByMovement[i].from; id <= typ->objectLayedByMovement[i].to;
              ++id) {
...
void Vehicle::ReactionFire::checkData() {
   // the size could have changed because a unit was saved to disk with 1 weapon system, the type
   // modified to 2 and the unit loaded again
...
void Vehicle::ReactionFire::endOwnTurn() {
   if (status != off) {
      if (status == init1a)
         status = init1b;
      else if (status == init2 || status == init1b)
         status = ready;
   }
...
bool Vehicle::ReactionFire::canPerformAttack(Vehicle* target) {
   if (!unit->getCarrier())
      if (unit->getMap()->getPlayer(unit).diplomacy.isHostile(target->getOwner()))
         if (getStatus() >= ready)
            if (find(nonattackableUnits.begin(), nonattackableUnits.end(), target->networkid) ==
                nonattackableUnits.end())
...
const VehicleType::HeightChangeMethod* Vehicle::getHeightChange(int dir, int height) const {
   ...
   for (int i = 0; i < typ->heightChangeMethodNum; i++)
      if (typ->heightChangeMethod[i].startHeight & height)
         if ((dir > 0 && typ->heightChangeMethod[i].heightDelta > 0) ||
             (dir < 0 && typ->heightChangeMethod[i].heightDelta < 0))
...
void Vehicle ::setnewposition(const MapCoordinate& mc, const Context& context) {
   ...
   (new UnitFieldRegistration(this, MapCoordinate3D(mc, 0), UnitFieldRegistration::Position))
      ->execute(context);
}
...
void Vehicle::registerForNewOwner(int player) {
   ...
   Player::VehicleList::iterator i = find(gamemap->player[oldcol].vehicleList.begin(),
                                          gamemap->player[oldcol].vehicleList.end(), this);
...
bool Vehicle ::vehicleconstructable(const VehicleType* tnk, int x, int y) {
   ...
   if ((tnk->height & height) ||
       ((tnk->height & (chfahrend | chschwimmend)) && (height & (chfahrend | chschwimmend)))) {
      int hgt = height;
      if (!(tnk->height & height))
         hgt = 1 << getFirstBit(tnk->height);
      if (terrainaccessible2(gamemap->getField(x, y), tnk->terrainaccess, hgt) > 0)
         if (getResource(getExternalVehicleConstructionCost(tnk), true) ==
             getExternalVehicleConstructionCost(tnk)) {
            int dist = beeline(x, y, xpos, ypos);
            if (dist <= maxmalq * typ->unitConstructionMaxDistance &&
                dist >= maxmalq * typ->unitConstructionMinDistance)
               return 1;
         }
   }
   return 0;
...
bool Vehicle :: buildingconstructable ( const BuildingType* building, bool checkResources )
{
   ...
   if ( hd != 0 ) // && !(hd ==-1 && (height == chschwimmend || height == chfahrend)) ???  what was
that ?? return 0;


   if ( (building->productionCost.material * mf / 100 <= tank.material   &&
building->productionCost.fuel * ff / 100 <= tank.fuel) || !checkResources ) { if ( typ->hasFunction(
ContainerBaseType::ConstructBuildings) ) for ( int i = 0; i < typ->buildingsBuildable.size(); i++ )
...
int Vehicle ::freeWeight() {
   ...
   if (fld->vehicle)
      ...
      for (Cargo::const_iterator i = fld->building->getCargo().begin();
...
void Vehicle ::postAttack(bool reactionFire) {
   ...
   else if (reactionfire.getStatus() == ReactionFire::off)
      setMovement(0);
}
...
void Vehicle::readData(tnstream& stream) {
   ...
   if (bm & cem_loading) {
      ...
      if (c) {
         for (int k = 0; k < c; k++) {
            ...
            addToCargo(v);
            if (v && v->reactionfire.getStatus() != Vehicle::ReactionFire::off &&
                !v->baseType->hasFunction(ContainerBaseType::MoveWithReactionFire))
               /* there are two reasons that a unit with enabled RF is put into a transport,
                  although the unit cannot move with RF:
                  - the unitType was changed after the unit was moved into the transport
                  - RF was enable with the mapeditor
                  In order for the user to have any change to get the unit out again, we disable RF
                  here */
   ...
   if (reactionfirestatus >= 8 &&
       reactionfireenemiesAttackable <= 4) {  // for transition from the old reactionfire system ( <
                                              // ASC1.2.0 ) to the new one ( >= ASC1.2.0 )
   ...
   if ((reactionfire.status != ReactionFire::off) &&
       baseType->hasFunction(ContainerBaseType::NoReactionfire))
      reactionfire.status = ReactionFire::off;
...
int Vehicle::getAmmo(int type, int num, bool queryOnly) {
   ...
   while (weap < typ->weapons.count && got < num) {
      if (typ->weapons.weapon[weap].getScalarWeaponType() == type &&
          typ->weapons.weapon[weap].canRefuel()) {
         int toget = min(num - got, ammo[weap]);
         if (!queryOnly)
            ammo[weap] -= toget;
         got += toget;
...
int Vehicle::putAmmo(int type, int num, bool queryOnly) {
   ...
   while (weap < typ->weapons.count && put < num) {
      if (typ->weapons.weapon[weap].getScalarWeaponType() == type &&
          typ->weapons.weapon[weap].shootable()) {
...
void Vehicle::paint(Surface& s, SPoint pos, int shadowDist) const {
   ...
   bool shaded =
      (!canMove() && !secondUnit) && maxMovement() && (color == gamemap->actplayer * 8) &&
      (attacked || !typ->weapons.count || CGameOptions::Instance()->units_gray_after_move);
#else
   bool shaded = 0;
#endif
...
Vehicle* Vehicle::newFromStream(GameMap* gamemap, tnstream& stream, int forceNetworkID) {
   ...
   // a forced networkID of -2 will prevent any ID from being assigned and the unit not being
   // registered in the ID cache
...
int UnitHooveringLogic::calcFuelUsage(const Vehicle* veh) {
   ...
   if (veh->maxMovement())
      return veh->getMovement(false, false) * FuelConsumption * veh->typ->fuelConsumption /
             (100 * minmalq);
   else
      return (veh->getMap()->weather.windSpeed * maxwindspeed / 256) * veh->typ->fuelConsumption /
             (minmalq * 64);
}
...
int UnitHooveringLogic::getEndurance(const VehicleType* veh, int height, int resourceModel) {
   ...
   int fields = minmalq * veh->getStorageCapacity(resourceModel).fuel * 100 /
                (veh->fuelConsumption * FuelConsumption);
   if (maxmove)
...
int UnitHooveringLogic::getEndurance(const Vehicle* veh) {
   ...
   int fields =
      (veh->getTank().fuel - fuelUsage) * 100 / (veh->typ->fuelConsumption * FuelConsumption);
   if (veh->maxMovement())
