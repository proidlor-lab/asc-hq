...
bool MapField ::addobject(const ObjectType* obj, int dir, bool force,
                          MapField::ObjectRemovalStrategy* objectRemovalStrategy) {
   if (!obj)
      return false;
...
bool MapField ::removeObject(const ObjectType* obj, bool force,
                             ObjectRemovalStrategy* objectRemovalStrategy) {
   if (!force && building)
      return false;
...
ASCString MapField ::getName() {
   ...
   for (ObjectContainer::iterator o = objects.begin(); o != objects.end(); o++) {
      if (o->typ->namingMethod == ObjectType::ReplaceTerrain)
         a = o->typ->getName();
      else if (o->typ->namingMethod == ObjectType::AddToTerrain)
         a += ", " + o->typ->getName();
   }
...
int MapField ::getmovemalus(const Vehicle* veh) {
   ...
   if (mnum) {
      ...
      if (movemalus < minmalq)
         fatalError("invalid movemalus for terraintype ID %d used on field %d / %d",
                    typ->terraintype->id, getx(), gety());
...
void MapField ::setparams(ObjectRemovalStrategy* objectRemovalStrategy) {
   ...
   for (ObjectContainer::iterator o = objects.begin(); o != objects.end(); o++) {
      ...
      for (i = 0; i < cmovemalitypenum; i++) {
         ...
         if ((o->typ->getFieldModification(getWeather()).movemalus_abs[i] != 0) &&
             (o->typ->getFieldModification(getWeather()).movemalus_abs[i] != -1))
