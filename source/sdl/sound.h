...
class Sound {
  public:
   ...
   Sound(const ASCString& startSoundFilename, const ASCString& continuousSoundFilename,
         int fadeIn = 0);
...
class SoundSystem {
  ...
  public:
   /** Sets up ASC's sound system.
      \param muteEffects The sound is going to be initialized, but no sounds played. Sounds can be
      enabled at runtime \param muteMusic The sound is going to be initialized, but no music played.
      Music can be enabled at runtime \param off  The sound system is not even going to be
      initiliazed. Can only be restartet by restarting ASC
   */
   ...
   //! Returns the sound effect volume. This is already normalized to the 0 .. 128 range of
   //! SDL_Mixer !
