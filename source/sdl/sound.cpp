...
ASCString SoundSystem ::getDiagnosticText() {
   ...
   text += "SDL_mixer compiled version: " + ASCString::toString((int) mixCompiled.major) + "." +
           ASCString::toString((int) mixCompiled.minor) + "." +
           ASCString::toString((int) mixCompiled.patch) + "\n";

   const SDL_version* mixVersion = Mix_Linked_Version();
   text += "SDL_mixer linked version: " + ASCString::toString((int) mixVersion->major) + "." +
           ASCString::toString((int) mixVersion->minor) + "." +
           ASCString::toString((int) mixVersion->patch) + "\n";
...
void SoundSystem ::nextTrack(void) {
   ...
   if (internalData->currentPlaylist) {
      ...
      if (!filename.empty()) {
         ...
         if (!internalData->musicBuf) {
            internalData->lastMusicMessage =
               "Could not load music file " + filename + " ; SDL reports error " + SDL_GetError();
...
pair<Sound_Sample*, Mix_Chunk*> loadWave(const ASCString& name) {
   ...
   try {
      ...
      if (sample)
         ...
         displayLogMessage(0, ASCString(" SoundSystem::loadWave") + name +
                                 " failed. Message: " + Sound_GetError() + "\n");
         return make_pair(sample, chunk);
      }
...
Sound::Sound(const ASCString& filename, int _fadeIn)
   : name(filename), fadeIn(_fadeIn), waitingForMainWave(false) {
...
Sound::Sound(const ASCString& startSoundFilename, const ASCString& continuousSoundFilename,
             int _fadeIn)
   : name(startSoundFilename), fadeIn(_fadeIn), waitingForMainWave(false) {
   internalData = new Sound_InternalData;
...
void Sound::playWait(void) {
   ...
   if (internalData->startwave) {
      ...
      } while (SoundSystem::instance->internalData->channel[channel] == this &&
               Mix_Playing(channel));
   }
