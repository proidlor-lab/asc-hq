...
class UnitHooveringLogic {
   //! the percentage of fuel that is required for a flying plane just hoovering in the air (unit:
   //! percentage)
  ...
  public:
   ...
   /** calculates the time until the unit crashes because of lack of fuel
       \returns time in turns or -1 if the unit can't crash because of missing fuel in its current
      height
   */
...
class Vehicle : public ContainerBase {
  ...
  public:
   ...
   int getRepairExperienceValue(int oldDamage, int newDamage, bool offensive,
  ...
  public:
   //! if events are triggered by this unit (if it is lost for example), this will set connection !=
   //! 0
   int connection;
   ...
   */
   ...
   //! The class that manages the reaction fire which causes a unit to attack approaching enemies
   //! even if it is not the unit's turn
   class ReactionFire {
   ...
   //! returns the movement points the unit has left for this turn. CheckFuel should almost always
   //! be true.
   int getMovement(bool checkFuel = true, bool checkRF = true) const;

   /** sets a new distance that the unit can move
   \param cargoDivisor : the cargo of this unit gets 1/cargodivisor the change that this unit is
   getting; if 0 the cargo is not touched ; -1 is default
    */
   ...
   Don't use something like "setmovement ( getmovement() - amount )", because getmovement may return
   a lower amount due to lack of fuel. */
   ...
   */
   ...
   Resources putResource(const Resources& res, bool queryonly, int scope = 1, int player = -1) {
      return ContainerBase::putResource(res, queryonly, scope, player);
   };
   Resources getResource(const Resources& res, bool queryonly, int scope = 1, int player = -1) {
      return ContainerBase::getResource(res, queryonly, scope, player);
   };
  ...
  private:
   /** if a unit (searchedInnerVehicle) is inside a transport, its payload depends also on the
      transport. Since transports can be arbitrarily stacked, this function is used to search the
      entire stack from outmost to the searched inner vehicle */
   int searchstackforfreeweight(Vehicle* searchedInnerVehicle);

  public:
   //! Returns the size of a unit. A size is equal to the weight of the unit without any cargo or
   //! carried resources.
   int size(void);
   ...
   */
   ...
   /** adds the units view to the map. The view must then be evaluated by functions like
      #evaluateviewcalculation ( GameMap*, int) \sa viewcalculation.cpp */
   void addview();

   /** removes the units view to the map. The view must then be evaluated by functions like
      #evaluateviewcalculation ( GameMap*, int) \sa viewcalculation.cpp */
   void removeview();
  ...
  public:
   ...
   /** returns the method for changing the height in the specified direction, or none if there is
      none. \param dir  +1 is up;  -1 is down \param height if != 0:  assume this level of height
