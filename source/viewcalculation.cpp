...
void tcomputeview::initviewcalculation(int view, int jamming, int sx, int sy, int _mode,
...
void tcomputeview::testfield(const MapCoordinate& mc) {
   ...
   if (f) {
      ...
      else if (height == chhochfliegend)
         baseJammingMultiplier = 50;
      else if (height == chfliegend)
...
#ifdef DEBUGVIEW
   if (efield->view[player].view < 0 || efield->view[player].sonar < 0 ||
       efield->view[player].satellite < 0 || efield->view[player].jamming < 0 ||
       efield->view[player].mine < 0)
      errorMessage("Warning: inconsistency in view calculation !!\n Please report this bug !");
#endif
}

void tcomputevehicleview::init(const Vehicle* eht,
                               int _mode)  // mode: +1 = add view  ;  -1 = remove view ); )
{
   player = eht->getOwner();
   ...
   tcomputeview::initviewcalculation(view, eht->typ->jamming, eht->xpos, eht->ypos, _mode,
                                     eht->height);
...
void clearvisibility(GameMap* gamemap) {
   ...
   for (int p = 0; p < gamemap->getPlayerCount(); p++) {
      for (Player::VehicleList::iterator i = gamemap->player[p].vehicleList.begin();
           i != gamemap->player[p].vehicleList.end(); i++)
         (*i)->resetview();
      for (Player::BuildingList::iterator b = gamemap->player[p].buildingList.begin();
           b != gamemap->player[p].buildingList.end(); b++)
         (*b)->resetview();
   }
...
VisibilityStates calcvisibilityfield(GameMap* gamemap, MapField* fld, int player, int add,
      ...
      for (int y = 0; y < gamemap->ysize; ++y)
         for (int x = 0; x < gamemap->xsize; ++x) {
            ...
            if (fld->visible != fld->getTemp3()) {
               // we are running under action control, the change shall be done by the action and
               // not here, so we are undoing our change for the moment
...
int evaluateviewcalculation(GameMap* gamemap, const MapCoordinate& pos, int distance,
                            int player_fieldcount_mask, bool disableShareView,
                            const Context* context) {
   ...
   for (int yy = y1; yy <= y2; yy++)
      for (int xx = x1; xx <= x2; xx++) {
         ...
         if ((fld->visible != oldview) && viewState) {
            // we are running under action control, the change shall be done by the action and not
            // here, so we are undoing our change for the moment
...
int computeview(GameMap* gamemap, int player_fieldcount_mask, bool disableShareView,
   ...
   for (int a = 0; a < gamemap->getPlayerCount(); a++)
      if (gamemap->player[a].exist()) {
         for (Player::VehicleList::iterator i = gamemap->player[a].vehicleList.begin();
              i != gamemap->player[a].vehicleList.end(); i++) {
            Vehicle* actvehicle = *i;
            if (actvehicle == gamemap->getField(actvehicle->getPosition())->vehicle)
               actvehicle->addview();
         }

         for (Player::BuildingList::iterator i = gamemap->player[a].buildingList.begin();
              i != gamemap->player[a].buildingList.end(); i++)
            (*i)->addview();
      }
...
RecalculateAreaView ::RecalculateAreaView(GameMap* gamemap, const MapCoordinate& pos, int range,
                                          const Context* context)
   : active(false) {
   position = pos;
   this->range = range;
...
void RecalculateAreaView::removeView() {
   circularFieldIterator(gamemap, position, 0, range,
                         FieldIterationFunctor(this, &RecalculateAreaView::removeFieldView));
   active = true;
}

void RecalculateAreaView::addView() {
   circularFieldIterator(gamemap, position, 0, range,
                         FieldIterationFunctor(this, &RecalculateAreaView::addFieldView));
...
void RecalculateAreaView::removeFieldView(const MapCoordinate& pos) {
   ...
   if ((fld->vehicle || fld->getBuildingEntrance()) &&
       fld->getContainer()->getOwner() < fld->getContainer()->getMap()->getPlayerCount()) {
      if (context)
         (new ViewRegistration(fld->getContainer(), ViewRegistration::RemoveView))
            ->execute(*context);
      else
...
void RecalculateAreaView::addFieldView(const MapCoordinate& pos) {
   ...
   if ((fld->vehicle || fld->getBuildingEntrance()) &&
       fld->getContainer()->getOwner() <
          fld->getContainer()->getMap()->getPlayerCount()) {  // excluding neutral buildings here
      if (context)
