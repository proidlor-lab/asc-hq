...
class CGameOptions {
  public:
   ...
   //! the mouse configuration. Mouse button are: bit 0 = left button ; bit 1 = right button ; bit 2
   //! = center button
   struct Mouse {
      // int scrollbutton;
      ...
      /** specifies the occasions when the small icons are positioned directly under the mouse
         pointer (instead of some pixel upwards) 0 = never 1 = always 2 = only if there is a
         building or a unit on the field or if the field is a destination for the current unit
         action (default)
      */
      ...
      //! marks a field and opens the small gui buttons with a single click (default: first click
      //! mark, second click: execute)
   ...
   struct SoundSettings {
      ...
      //! if the sound is off, it can only be reactivated by restarting asc, but doesn't lock the
      //! sound device
      bool off;
...
inline bool CGameOptions::isChanged() {
   return _changed;
}

inline void CGameOptions::setChanged(bool flag) {
   _changed = flag;
}
