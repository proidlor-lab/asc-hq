   ...
   for (int y = 0; y < s.h(); ++y) {
      ...
      for (int x = 0; x < s.w(); ++x) {
         ...
         if (((*i >> s.GetPixelFormat().Ashift()) & 0xff) != Surface::transparent) {
            ...
            *i = (nv << s.GetPixelFormat().Rshift()) | (nv << s.GetPixelFormat().Gshift()) |
                 (nv << s.GetPixelFormat().Bshift()) | (*i & s.GetPixelFormat().Amask());
         }
         c += 4;
      }
   }
...
bool imageEmpty(const Surface& s) {
   ...
   int colmask =
      s.GetPixelFormat().Rmask() | s.GetPixelFormat().Gmask() | s.GetPixelFormat().Bmask();
   int colref = (0xff << s.GetPixelFormat().Rshift()) | (0xff << s.GetPixelFormat().Gshift()) |
                (0xff << s.GetPixelFormat().Bshift());
...
vector<Surface> loadASCFieldImageArray(const ASCString& file, int num) {
   ...
   for (int i = 0; i < num; i++) {
      ...
      if (s2.GetPixelFormat().BitsPerPixel() != 8 || s.GetPixelFormat().BitsPerPixel() != 8) {
         ...
         MegaBlitter<1, 1, ColorTransform_None, ColorMerger_AlphaOverwrite,
                     SourcePixelSelector_Rectangle>
            blitter;
         blitter.setSrcRectangle(SDLmm::SRect(SPoint(x1, y1), fieldsizex, fieldsizey));
         blitter.blit(s, s2, SPoint(0, 0));
