...
void tdrawgettempline ::start(int x1, int y1, int x2, int y2) {
   ...
   */
   ...
   while (x != x2 || y != y2) {
      ...
      if (ldist2 > ldist) {
         fatalError("inconsistency in tdrawgettempline :: start ; parameters are %d/%d ; %d/%d ", 1,
                    x1, y1, x2, y2);
         return;
      }
#endif
...
void SearchFields::initsearch(const MapCoordinate& startPosition, int _firstDistance,
                              int _lastDistance) {
   cancelSearch = false;
      ...
      if (dist == 0) {
         ...
         for (int e = 0; e < 6; e++) {
            ...
            for (int c = 0; c < dist; c++) {
               if ((mc.x >= 0) && (mc.y >= 0) && (mc.x < gamemap->xsize) &&
                   (mc.y < gamemap->ysize)) {
                  testfield(mc);
                  if (cancelSearch)
                     return;
...
class SearchFieldsIterator : public SearchFields {
  ...
  protected:
   void testfield(const MapCoordinate& pos) { myFunctor(pos); };

  public:
   SearchFieldsIterator(GameMap* _gamemap, MyFunctor& functor)
      : SearchFields(_gamemap), myFunctor(functor){};
};

void circularFieldIterator(GameMap* gamemap, const MapCoordinate& center, int startDist,
                           int stopDist, FieldIterationFunctor functor) {
...
/*
Wenn Du Wind aus Westen hast und willst nach norden fliegen, dann darf der Flieger nicht Kurs auf
Norden nehmen, denn dann wÅrde er abgetrieben. Er muss stattdessen etwas gegen den Wind fliegen.

Diesen Winkel ermittel' ich durch plumpes ausprobieren, in WindMovement::WindMovement

Solange die Sollrichtung der Einheitenbewegung und die WindRichtungen Åbereinstimmen, brauche ich
nur den Fall fÅr 1 Windrichtung berechnen. Ich ermittel' dann fÅr die 6 Einheitenrichtungen den
Malus. FÅr andere Windrichtungen verschiebe ich einfach nur die Werte in die Tabelle (die letzten
beiden Anweisungen in WindMovement), da das entscheidende ja nur der Winkelunterschied zwischen
Wind- und Sollbewegungsrichtung ist.
*/
