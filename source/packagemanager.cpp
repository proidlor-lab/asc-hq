...
void PackageManager::checkGame(GameMap* game) {
   ...
   for (PackageData::Packages::const_iterator i = game->packageData->packages.begin();
        i != game->packageData->packages.end(); ++i)
      packageRepository.checkPackageDependency(i->second, game->packageData);
}

void PackageManager::processContainer(const ContainerBase* container,
                                      std::set<ASCString>& archives) {
   archives.insert(container->baseType->archive);
   for (ContainerBase::Production::const_iterator i = container->getProduction().begin();
        i != container->getProduction().end(); ++i)
      if (*i) {
...
void PackageManager::storeData(const GameMap* game) {
   ...
   for (int y = 0; y < game->ysize; ++y)
      for (int x = 0; x < game->xsize; ++x) {
         ...
         for (MapField::ObjectContainer::const_iterator o = fld->objects.begin();
              o != fld->objects.end(); ++o)
            archives.insert(o->typ->archive);
      }
   for (int p = 0; p <= 8; ++p) {
      const Player& pl = game->getPlayer(p);
      for (Player::VehicleList::const_iterator i = pl.vehicleList.begin();
           i != pl.vehicleList.end(); ++i)
         processContainer(*i, archives);

      for (Player::BuildingList::const_iterator i = pl.buildingList.begin();
           i != pl.buildingList.end(); ++i)
         processContainer(*i, archives);
   }

   game->packageData->packages.clear();
   for (PackageRepository::const_iterator i = packageRepository.begin();
        i != packageRepository.end(); ++i) {
