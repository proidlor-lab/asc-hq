...
void tviewanytext::init(const char* _title, const char* text, int xx1, int yy1, int xxsize,
                        int yysize) {
   ...
   if (textsizeycomplete >= textsizey) {
      ...
      addscrollbar(xsize - 30, starty, xsize - 15, ysize - 35, &textsizeycomplete, textsizey,
                   &tvt_starty, 1, 0);
      setscrollspeed(1, 1);
...
void tviewanytext::redraw(void) {
   ...
   tdialogbox::redraw();
   rahmen(true, x1 + 10, y1 + textstart - 2, x1 + xsize - rightspace,
          y1 + textstart + textsizey + 2);
...
void viewUnitSetinfo(void) {
   ...
   if (fld && fieldvisiblenow(fld) && fld->vehicle) {
      ...
      */
      ...
      s += "\nUnit owner: " + ASCString::toString(fld->vehicle->getOwner()) + " - " +
           actmap->getPlayer(fld->vehicle).getName();
...
pparagraph tparagraph ::erasechar(int c) {
   ...
   if (c == 1) {  // backspace
      if (cursor) {
         ...
         prev->join();
         return temp;  //  !!!###!!!   gef�llt mir eigentlich ?berhauptnicht, wird aber wohl laufen.
                       //  Sonst m?�te ich halt erasechar von au�en managen
      }
   }
   if (c == 2) {  // del
      if (cursor < size - 1) {
...
void tparagraph ::checkscrolldown(void) {
   if (next) {
      ...
      if (ps.line1num + linenum < maxlinenum)
         bar(winx1, winy1 + (ps.line1num + linenum + 1) * activefontsettings.height,
             winx1 + activefontsettings.length, winy1 + maxlinenum * activefontsettings.height,
             dblue);
   }
}
...
void tparagraph ::displaycursor(void) {
   if (cursor >= 0) {
      ...
      if (starty < winy2) {
         collategraphicoperations cgo(winx1 + cursorx - 2, starty, winx1 + cursorx + 2,
                                      starty + activefontsettings.height);
         xorline(winx1 + cursorx, starty + 1, winx1 + cursorx,
                 starty + activefontsettings.height - 1, blue);
         if (cursorx)
            xorline(winx1 + cursorx - 1, starty + 1, winx1 + cursorx - 1,
                    starty + activefontsettings.height - 1, blue);
...
void tmessagedlg ::run(void) {
   ...
   if (prntkey != cto_invvalue) {
      ...
      else if (taste == cto_entf)
         actparagraph->erasechar(2);
      else if (taste == cto_left)
         actparagraph = actparagraph->movecursor(-1, 0);
      else if (taste == cto_right)
         actparagraph = actparagraph->movecursor(1, 0);
      else if (taste == cto_up)
         actparagraph = actparagraph->movecursor(0, -1);
      else if (taste == cto_down)
         actparagraph = actparagraph->movecursor(0, 1);
      else if (taste == cto_enter)
         actparagraph = actparagraph->cut();
      else if (taste == cto_esc)
         printf("\a");
      else if (prntkey > 31 && prntkey < 256)
   ...
   for (int i = 0; i < 8; i++) {
      if (actmap->player[i].exist())
         if (actmap->actplayer != i) {
            ...
            // this is a leak! As this dialog will be scrapped before the ASC2 release,  I don't
            // care :->
            addbutton(strdup(actmap->player[i].getName().c_str()), x, y, x + 180, y + 15, 3, 0,
                      num + 3, true);
