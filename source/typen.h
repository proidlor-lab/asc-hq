...
//! The maximum number of number of different images for a building and a weather. For example, A
//! wind power plant might have 6 different images, depending on the direction of the wind.
...
class Resources {
  public:
   ...
   bool operator>=(const Resources& res) {
      return energy >= res.energy && material >= res.material && fuel >= res.fuel;
   };
...
/*!
  ...
  Since floating and ground based are assumed to be the same effective height, a simple subtraction
  isn't sufficient. Height is a numeric (and not bitmapped) value
 */
extern int getheightdelta(int height1, int height2);

//! a class for storing general key/values pairs. Intended as a mechanism to persist data from Lua
//! scripts
class Properties {
...
class deallocating_map : public std::map<T, U> {
  public:
   ~deallocating_map() {
      for (typename std::map<T, U>::iterator i = std::map<T, U>::begin();
           i != std::map<T, U>::end(); ++i)
         delete i->second;
   };
};
...
//! The movemalus type for a building. It is used for #SingleWeapon::targetingAccuracy . \see
//! MoveMalusType
...
//! The movement decrease in percent of the total movemnt that is used for attacking; only used for
//! units that can move after attacking
...
#define cnet_storeenergy \
   0x001  // es wird garantiert,  dass material immer das 2 und fuel das 4 fache von energy ist
...
#define destruct_building_fuel_usage \
   10  // beim Abreissen wird 10 * fuelconsumption Fuel fuelconsumptiont

#define dissectunitresearchpointsplus \
   2  // Beim dissectn einer vehicle wird der sovielte Teil der Researchpoints jeder unbekannten
      // Technologie gutgeschrieben

#define dissectunitresearchpointsplus2 \
   3  // Beim dissectn einer vehicle wird der sovielte Teil der Researchpoints jeder unbekannten
      // Technologie gutgeschrieben.
      // fuer die Technologie existieren aber bereits von einem anderen sezierten vehicletype
      // gutschriften.

//! The maximum number of fields a mining station can extract mineral resources from; Counted from
//! its entry
