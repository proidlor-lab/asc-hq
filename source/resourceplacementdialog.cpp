...
ResourcePlacementDialog::ResourcePlacementDialog()
   : ASC_PG_Dialog(NULL, PG_Rect(100, 100, xsize, ysize), "Place Resources", SHOW_CLOSE),
     ...
     materialRoughness(1),
     maxFuelOffSet(100),
     maxMaterialOffSet(100),
     fuelRoughnessWidget(NULL),
     materialRoughnessWidget(NULL),
     fuelOffsetWidget(NULL),
     materialOffsetWidget(NULL),
     d(0) {
   int xFuelWidgetPos = leftIndent;
   int yFuelWidgetPos = ysize / 10;
   int hFuelWidget = GetTextHeight() + 10;
   placeFuelWidget = new PG_CheckButton(
      this, PG_Rect(xFuelWidgetPos, yFuelWidgetPos, xsize / 3, GetTextHeight() * 2), "Fuel");
   placeFuelWidget->SetSizeByText();

   int xMaterialWidgetPos = xFuelWidgetPos;
   int yMaterialWidgetPos = yFuelWidgetPos + hFuelWidget;
   placeMaterialWidget = new PG_CheckButton(
      this, PG_Rect(xMaterialWidgetPos, yMaterialWidgetPos, xsize / 3, GetTextHeight() * 2),
      "Material");
   ...
   fuelRoughnessLabel = new PG_Label(
      this, PG_Rect(xFuelRoughnessLabelPos, yFuelWidgetPos, 40, hFuelWidget), "Fuel Roughness: ");
   fuelRoughnessLabel->SetSizeByText();

   materialRoughnessLabel =
      new PG_Label(this, PG_Rect(xFuelRoughnessLabelPos, yMaterialWidgetPos, 40, hFuelWidget),
                   "Material Roughness: ");
   ...
   fuelRoughnessWidget = new PG_LineEdit(
      this, PG_Rect(xFuelRoughnessWidgetPos, yFuelWidgetPos,
                    xsize - (xFuelRoughnessLabelPos + fuelRoughnessLabel->Width() + xOffSet),
                    hFuelWidget));
   fuelRoughnessWidget->SetValidKeys("1234567890.");
   fuelRoughnessWidget->SetText("1.0");

   materialRoughnessWidget = new PG_LineEdit(
      this, PG_Rect(xFuelRoughnessWidgetPos, yMaterialWidgetPos,
                    xsize - (xFuelRoughnessLabelPos + fuelRoughnessLabel->Width() + xOffSet),
                    hFuelWidget));
   ...
   fuelOffSetLabel = new PG_Label(
      this, PG_Rect(xFuelWidgetPos, yFuelOffSetLabelPos, 40, hFuelWidget), "Fuel Offset: ");
   fuelOffSetLabel->SetSizeByText();

   int xFuelOffSetValue = xFuelWidgetPos + fuelOffSetLabel->Width();
   fuelOffSetValue =
      new PG_LineEdit(this, PG_Rect(xFuelOffSetValue, yFuelOffSetLabelPos, 40, hFuelWidget));
   ...
   fuelOffSetSlider = new PG_Slider(this,
                                    PG_Rect(xFuelWidgetPos, yFuelOffSetSliderPos,
                                            xsize - (xFuelWidgetPos + leftIndent), hFuelWidget),
   ...
   fuelOffSetSlider->sigSlide.connect(
      sigc::mem_fun(*this, &ResourcePlacementDialog::scrollTrackEventFuel));

   // MaterialOffSet
   int yMaterialOffSetLabelPos = yFuelOffSetSliderPos + hFuelWidget;
   materialOffSetLabel = new PG_Label(
      this, PG_Rect(xFuelWidgetPos, yMaterialOffSetLabelPos, 40, hFuelWidget), "Material Offset: ");
   materialOffSetLabel->SetSizeByText();

   int xMaterialOffSetValue = xFuelWidgetPos + materialOffSetLabel->Width();
   materialOffSetValue = new PG_LineEdit(
      this, PG_Rect(xMaterialOffSetValue, yMaterialOffSetLabelPos, 40, hFuelWidget));
   ...
   materialOffSetSlider = new PG_Slider(this,
                                        PG_Rect(xFuelWidgetPos, yMaterialOffSetSliderPos,
                                                xsize - (xFuelWidgetPos + leftIndent), hFuelWidget),
                                        PG_ScrollBar::HORIZONTAL);
   materialOffSetSlider->SetRange(ResourcePlacement::MINMATERIALVALUE,
                                  ResourcePlacement::MAXMATERIALVALUE);
   materialOffSetSlider->SetPosition(atoi(materialOffSetValue->GetText()));
   materialOffSetSlider->sigSlide.connect(
      sigc::mem_fun(*this, &ResourcePlacementDialog::scrollTrackEventMaterial));

   // FuelFreeRatio
   int yFuelFreeRatioLabelPos = yMaterialOffSetSliderPos + hFuelWidget + yOffSet;
   fuelFreeRatioLabel =
      new PG_Label(this, PG_Rect(xFuelWidgetPos, yFuelFreeRatioLabelPos, 40, hFuelWidget),
                   "Fuel Free Fields Ratio: ");
   fuelFreeRatioLabel->SetSizeByText();

   int xFuelFreeRatioValue = xFuelWidgetPos + fuelFreeRatioLabel->Width();
   fuelFreeRatioValue =
      new PG_LineEdit(this, PG_Rect(xFuelFreeRatioValue, yFuelFreeRatioLabelPos, 40, hFuelWidget));
   ...
   fuelFreeRatioSlider = new PG_Slider(this,
                                       PG_Rect(xFuelWidgetPos, yFuelFreeRatioSliderPos,
                                               xsize - (xFuelWidgetPos + leftIndent), hFuelWidget),
                                       PG_ScrollBar::HORIZONTAL);
   fuelFreeRatioSlider->SetRange(0, 100);
   fuelFreeRatioSlider->SetPosition(atoi(fuelFreeRatioValue->GetText()));
   fuelFreeRatioSlider->sigSlide.connect(
      sigc::mem_fun(*this, &ResourcePlacementDialog::scrollTrackEventFuelFreeRatio));

   // MaterialFreeRatio
   int yMaterialFreeRatioLabelPos = yFuelFreeRatioSliderPos + hFuelWidget;
   materialFreeRatioLabel =
      new PG_Label(this, PG_Rect(xFuelWidgetPos, yMaterialFreeRatioLabelPos, 40, hFuelWidget),
                   "Material Free Fields Ratio: ");
   materialFreeRatioLabel->SetSizeByText();

   int xMaterialFreeRatioValue = xFuelWidgetPos + materialFreeRatioLabel->Width();
   materialFreeRatioValue = new PG_LineEdit(
      this, PG_Rect(xMaterialFreeRatioValue, yMaterialFreeRatioLabelPos, 40, hFuelWidget));
   ...
   materialFreeRatioSlider =
      new PG_Slider(this,
                    PG_Rect(xFuelWidgetPos, yMaterialFreeRatioSliderPos,
                            xsize - (xFuelWidgetPos + leftIndent), hFuelWidget),
                    PG_ScrollBar::HORIZONTAL);
   materialFreeRatioSlider->SetRange(0, 100);
   materialFreeRatioSlider->SetPosition(atoi(materialFreeRatioValue->GetText()));
   materialFreeRatioSlider->sigSlide.connect(
      sigc::mem_fun(*this, &ResourcePlacementDialog::scrollTrackEventMaterialFreeRatio));

   // Buttons OK and Cancel

   (new PG_Button(this, PG_Rect(30, Height() - 40, (Width() - 70) / 2, 30), "OK", 100))
      ->sigClick.connect(sigc::mem_fun(*this, &ResourcePlacementDialog::buttonEvent));
   (new PG_Button(this, PG_Rect(Width() / 2 + 5, Height() - 40, (Width() - 70) / 2, 30), "Cancel",
                  101))
      ->sigClick.connect(sigc::hide(sigc::mem_fun(*this, &ResourcePlacementDialog::closeWindow)));
...
bool ResourcePlacementDialog::buttonEvent(PG_Button* button) {
   if ((atof(fuelRoughnessWidget->GetText()) <= 0) || (atof(fuelRoughnessWidget->GetText()) >= 4) ||
       (atof(materialRoughnessWidget->GetText()) <= 0) ||
       (atof(materialRoughnessWidget->GetText()) >= 4)) {
      d = new PG_MessageBox(this, PG_Rect(0, 0, Width() / 2, int(Height() * 0.75)), "Error Report",
                            "Condition 0.0 < roughness < 4.0 not met",
                            PG_Rect(Width() / 4 - 25, int(Height() * 0.75) - 30, 50, 30), "OK");
      d->Show();

      return true;
   } else {
      quitModalLoop(1);
      ResourcePlacement rp(*actmap, atof(fuelRoughnessWidget->GetText()),
                           atof(materialRoughnessWidget->GetText()),
                           atoi(fuelOffSetValue->GetText()), atoi(materialOffSetValue->GetText()),
                           0, atoi(materialFreeRatioValue->GetText()));
