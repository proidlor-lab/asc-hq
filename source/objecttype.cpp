...
const char* ObjectType::namingMethodNames[namingMethodNum + 1] = {"ReplaceTerrain", "AddToTerrain",
                                                                  "UnNamed", NULL};
...
const OverviewMapImage* ObjectType ::getOverviewMapImage(int picnum, int weather) const {
   ...
   if (weatherPicture[weather].bi3pic[picnum] > 0)
      ...
      if (!weatherPicture[weather].overviewMapImage[picnum].valid())
         weatherPicture[weather].overviewMapImage[picnum].create(
            weatherPicture[weather].images[picnum]);
...
void ObjectType ::display(Surface& surface, const SPoint& pos, int dir, int weather) const {
   ...
   if (id == 4) {
      switch (dir) {
         case 68:
            realDisplay(surface, pos, 9, weather);
            break;
         case 34:
            realDisplay(surface, pos, 10, weather);
            break;
         case 17:
            realDisplay(surface, pos, 11, weather);
            break;
         case 136:
            realDisplay(surface, pos, 12, weather);
            break;
         case 0:
            realDisplay(surface, pos, 0, weather);
            break;
         default: {
            for (int i = 0; i <= 7; i++)
               if (dir & (1 << i))
...
void ObjectType::realDisplay(Surface& surface, const SPoint& pos, int dir, int weather) const {
   ...
   if (displayMethod == 1) {  // SHADOW: buried pipeline, tracks, ...
      ...
      if (dir != 0 && rotateImage) {
         megaBlitter<ColorTransform_None, ColorMerger_AlphaLighter,
                     SourcePixelSelector_CacheRotation, TargetPixelSelector_All>(
            s, surface, pos, nullParam, 0.7, make_pair(&s, directionangle[dir % 6]), nullParam);
      } else {
         megaBlitter<ColorTransform_None, ColorMerger_AlphaLighter, SourcePixelSelector_DirectFlip,
                     TargetPixelSelector_All>(s, surface, pos, nullParam, 0.7, flip, nullParam);
      }
   } else if (displayMethod == 2) {  // translation
      const Surface& s = getPicture(dir, weather);
      if (!s.valid())
         return;

      if (s.GetPixelFormat().BitsPerPixel() == 8) {
         MegaBlitter<1, 4, ColorTransform_None, ColorMerger_Alpha_XLAT_TableShifter,
                     SourcePixelSelector_DirectFlip, TargetPixelSelector_All,
                     ColorConverter_PassThrough>
            blitter;
         blitter.setFlipping(flip & 1, flip & 2);
         blitter.blit(s, surface, pos);
      }  // else
         // warning("objects with palette translation as display method may not have truecolor
         // images");
   } else if (displayMethod == 4) {
      const Surface& s = getPicture(dir, weather);
      if (!s.valid())
         return;

      if (dir != 0 && rotateImage) {
         megaBlitter<ColorTransform_None, ColorMerger_AlphaMixer, SourcePixelSelector_CacheRotation,
                     TargetPixelSelector_All>(s, surface, pos, nullParam, nullParam,
                                              make_pair(&s, directionangle[dir % 6]), nullParam);
      } else {
         megaBlitter<ColorTransform_None, ColorMerger_AlphaMixer, SourcePixelSelector_DirectFlip,
                     TargetPixelSelector_All>(s, surface, pos, nullParam, nullParam, flip,
                                              nullParam);
      }
   } else {
      bool disp = true;
      ...
      if (disp) {
         ...
         if (flip) {
            ...
            megaBlitter<ColorTransform_None, ColorMerger_AlphaMerge, SourcePixelSelector_DirectFlip,
                        TargetPixelSelector_All>(s, surface, pos, nullParam, nullParam, flip,
                                                 nullParam);
            // else
            //   megaBlitter<ColorTransform_None, ColorMerger_AlphaOverwrite,
            //   SourcePixelSelector_DirectFlip,TargetPixelSelector_All>(getPicture( dir, weather),
            //   surface, pos, nullParam,nullParam, flip, nullParam);
         } else {
            if (dir != 0 && rotateImage) {
               // if ( s.flags() & SDL_SRCALPHA )
               megaBlitter<ColorTransform_None, ColorMerger_AlphaMerge,
                           SourcePixelSelector_CacheRotation, TargetPixelSelector_All>(
                  s, surface, pos, nullParam, nullParam, make_pair(&s, directionangle[dir % 6]),
                  nullParam);
               // else
               //   megaBlitter<ColorTransform_None, ColorMerger_AlphaOverwrite,
               //   SourcePixelSelector_CacheRotation,TargetPixelSelector_All>(s, surface, pos,
               //   nullParam,nullParam,make_pair(&s,directionangle[dir]),nullParam);
            } else {
               // if ( s.flags() & SDL_SRCALPHA )
               megaBlitter<ColorTransform_None, ColorMerger_AlphaMerge, SourcePixelSelector_Plain,
                           TargetPixelSelector_All>(s, surface, pos, nullParam, nullParam,
                                                    nullParam, nullParam);
               // else
               //   megaBlitter<ColorTransform_None, ColorMerger_AlphaOverwrite,
               //   SourcePixelSelector_Plain,TargetPixelSelector_All>(getPicture( dir, weather),
               //   surface, pos, nullParam,nullParam,nullParam,nullParam);
            }
         }
      }
   }
...
int woodform[woodformnum] = {63, 30, 60, 39, 51, 28, 35, 48, 6,  57, 15, 14, 56, 7,
                             49, 47, 59, 31, 61, 60, 55, -1, -1, -1, -1, -1, -1, -1};

int SmoothTreesData0[] = {
   4,   7,    10,  101, 1,    0x0001, 243, 1,    0x0115, 243,  // die die garnicht gehen
   30,  0x3F, 30,  243, 0x3F, 60,     243, 0x3F, 39,     243, 0x3F, 51, 243, 0x3F, 28,
   243, 0x3F, 35,  243, 0x3F, 48,     243, 0x3F, 6,      243, 0x3F, 57, 243, 0x3F, 15,
   243, 0x3F, 14,  243, 0x3F, 56,     243, 0x3F, 7,      243, 0x3F, 49, 243, 0x3F, 47,
   243, 0x3F, 59,  243, 0x3F, 31,     243, 0x3F, 61,     243, 0x3F, 62, 243, 0x3F, 55,
   243, 0x3F, 23,  243, 0x3F, 46,     243, 0x3F, 29,     243, 0x3F, 58, 243, 0x3F, 53,
   243, 0x3F, 43,  243, 0x3F, 22,     243, 0x3F, 38,     243, 0x3F, 50, 243, 0x3F, 52,
   243, 7,    264, 265, 266,  267,    268, 269,  270};

int SmoothTreesData[] = {
   4,    7,    10,   101,  1,    0x0001, 243,  1,    0x0115, 243,  30,   0x3F, 30,   244,
   0x3F, 60,   245,  0x3F, 39,   246,    0x3F, 51,   247,    0x3F, 28,   248,  0x3F, 29,
   248,  0x3F, 35,   249,  0x3F, 43,     249,  0x3F, 48,     250,  0x3F, 50,   250,  0x3F,
   52,   250,  0x3F, 6,    251,  0x3F,   22,   251,  0x3F,   38,   251,  0x3F, 57,   252,
   0x3F, 15,   253,  0x3F, 14,   254,    0x3F, 46,   254,    0x3F, 56,   255,  0x3F, 58,
   255,  0x3F, 7,    256,  0x3F, 23,     256,  0x3F, 49,     257,  0x3F, 53,   257,  0x3F,
   47,   258,  0x3F, 59,   259,  0x3F,   31,   260,  0x3F,   61,   261,  0x3F, 62,   262,
   0x3F, 55,   263,  7,    264,  265,    266,  267,  268,    269,  270};

int UnSmoothTreesData[] = {4, 7, 8, 9, 1, 0x011C, 243, 0, 0, 1, 243};

int SmoothBanksData[] = {
   4,      7,      16,   77,   1,    0x0103, 95,  // was zu ersetzen ist         - blaues wasser }
   4,      0x010F, 95,  // was als 1 zu betrachten ist - wasser und strand }
   0x010E, 0,           // - Hafen }
   0x0109, 110,         // - Steine und Schilf }
   0x0107, 121,         // - Schilf }
   20,     0x3F,   59,   98,   0x3F, 51,     98,   0x3F, 47,   99,   0x3F, 39,   99,  // durch was
                                                                                      // ersetzen }
   0x3F,   31,     100,  0x3F, 30,   100,    0x3F, 61,   101,  0x3F, 60,   101,  0x3F, 55,
   102,    0x3F,   62,   103,  0x3F, 35,     104,  0x3F, 28,   105,  0x3F, 56,   106,  0x3F,
   48,     106,    0x3F, 49,   106,  0x3F,   57,   106,  0x3F, 14,   107,  0x3F, 15,   107,
   0x3F,   7,      107,  0x3F, 6,    107,
   0  // wenn nicht gefunden }
};

int UnSmoothBanksData[] = {
   4, 7, 8, 9, 1, 0x010C, 98,  // { alle str"nder ersetzen }
   0, 0, 1, 95                 // durch flaches wasser ersetzen }
};

int SmoothDarkBanksData[] = {
   4,      7,      16,     77,   1,      0x0103, 385,  // dunkels wasser }
   4,      0x0103, 385,                                // dunkles wasser }
   0x010A, 373,    0x0104, 449,  0x0104, 463,    20,   0x3F, 59,  373,  0x3F, 51,
   373,    0x3F,   47,     374,  0x3F,   39,     374,  // durch was ersetzen }
...
int UnSmoothDarkBanksData[] = {4, 7, 8, 9, 1, 0x010A, 373, 0, 0, 1, 385};
...
class Smoothing {
  ...
  public:
   ...
   MapField* getfield(int x, int y) { return actmap->getField(x, y); }
            ...
            if (TerObj) {
               ...
               if (obj && obj->typ->weather.test(0)) {
                  int Old = obj->dir;  // bipicnum
                                       //    Old:= TRawArrEck(Mission.ACTN[Y, X])[TerObj];  //
                                       //    bisherige Form / oder Bildnummer ?

                  if (IsInSetOfWord(
                         obj->typ->weatherPicture[0].bi3pic[obj->dir],
                         &SmoothData[P0])) {  // Nur die "allesWald"-fielder werden gesmootht
                     int Mask = GetNeighbourMask(X, Y, &SmoothData[P1], TerObj);
                     if (Mask < 63) {
                        int nw;
                        if (!SearchAndGetInt(Mask, &SmoothData[P2],
                                             &nw)) {  // Wenn kein passendes field gefunden wurde
   ...
   do {
      ...
      for (int y = 0; y < actmap->ysize; y++)
         for (int x = 0; x < actmap->xsize; x++) {
            ...
            for (MapField::ObjectContainer::iterator o = fld->objects.begin();
                 o != fld->objects.end(); o++)
               if (o->typ == woodObj) {
                  int c = 0;
...
int ObjectType ::getMemoryFootprint() const {
   ...
   for (int ww = 0; ww < cwettertypennum; ww++) {
      size += for_each(weatherPicture[ww].images.begin(), weatherPicture[ww].images.end(),
                       MemorySum<Surface>())
                 .size;
      size += for_each(weatherPicture[ww].overviewMapImage.begin(),
                       weatherPicture[ww].overviewMapImage.end(), MemorySum<OverviewMapImage>())
                 .size;
   }
...
void ObjectType ::FieldModification ::runTextIO(PropertyContainer& pc) {
   ...
   pc.addTagArray("TerrainProperties_Filter", terrain_and, terrainPropertyNum, terrainProperties,
                  true);
      ...
      if (weather.test(i)) {
         ...
         if (bi3pics) {
            ...
            if (pc.find("FlipPictures") || !pc.isReading()) {
               ...
               if (pc.isReading()) {
                  for (int j = 0; j < weatherPicture[i].images.size(); j++)
                     if (j < imgReferences.size() && imgReferences[j] >= 0 &&
                         imgReferences[j] < weatherPicture[i].images.size())
            ...
            if (pc.isReading()) {
               ...
               pc.addNamedInteger("GraphicOperations", operations, graphicOperationNum,
                                  graphicOperations, 0);
               if (operations == 1) {
                  for (int j = 0; j < weatherPicture[i].images.size(); j++)
                     if (processedImages.find(weatherPicture[i].images[j].getBaseSurface()) ==
                         processedImages.end()) {
         ...
         if (pc.find("DisplayMethod") || !pc.isReading()) {
            pc.addNamedInteger("DisplayMethod", displayMethod, objectDisplayingMethodNum,
                               objectDisplayingMethodTags);
            if (displayMethod == 2 && pc.isReading() &&
                weatherPicture[i].images[0].GetPixelFormat().BitsPerPixel() != 8)
               pc.error("Error parsing object " + name + " (ID=" + ASCString::toString(id) +
                        "): invalid image; displaymethod=translation is only a available for 8 Bit "
                        "images");
