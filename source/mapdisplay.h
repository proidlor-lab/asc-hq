...
class MapRenderer {
  ...
  protected:
   ...
   void paintUnitOntoField(const MapRenderer::FieldRenderInfo& fieldInfo, int binaryUnitHeight,
                           const SPoint& pos, Vehicle* vehicle);
   ...
   // return the position of a field on the internal surface
   int getFieldPosY(int x, int y) { return surfaceBorder + y * fielddisty; };

   SPoint getFieldPos(int x, int y) { return SPoint(getFieldPosX(x, y), getFieldPosY(x, y)); };
   ...
   void paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                         const SPoint& pos);
   void paintTerrain(Surface& surf, GameMap* actmap, int playerView, const ViewPort& viewPort,
...
class MapLayer {
  ...
  public:
   ...
   virtual void paintSingleField(const MapRenderer::FieldRenderInfo& fieldInfo, int layer,
                                 const SPoint& pos) = 0;
   virtual ~MapLayer(){};
};
...
class MapDisplayPG : public PG_Widget, protected MapRenderer {
  ...
  protected:
   ...
   bool eventKeyDown(const SDL_KeyboardEvent* key) { return keyboardHandler(NULL, key); };
  ...
  public:
   ...
   void displayUnitMovement(GameMap* actmap, Vehicle* veh, const MapCoordinate3D& from,
  ...
  public:
   ...
   /** Signal that is fired when the mouse is pressed on a valid field, after the cursor evaluation
      has been run. \param MapCoordinate the Coordinate of the field that was clicked \param SPoint
      the mouse position \param bool true if the cursor had been repositioned \param int  the button
      that was pressed \param int  the priority of the signal
   */
