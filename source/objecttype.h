...
class ObjectType : public MapItemType, public LoadableItemType {
  ...
  public:
   ...
   //! the group ID allows units to specify whole groups of objects to be buildable / removable
   //! without specifying each individual object.
   int groupID;
   ...
   //! some objects are graphically linked with others on neighbouring fields. \sa no_autonet
   //! linkableTerrain
   vector<IntRange> linkableObjects;

   //! objects can be graphically linked to terrain, as if the terrain was an object of the same
   //! type. \sa no_autonet linkableObjects
   ...
   struct FieldModification {
      ...
      //! the movemalus_plus is added to the current movemalus of the field to form the new
      //! movemalus. Negative values are ok.
      TerrainType::MoveMalus movemalus_plus;

      //! The movemalus_abs replaces the current movemalus of the field by a new one. Values of 0
      //! and -1 won't affect the movemalus of the field, and values ranging from 1 to 9 must not be
      //! used.
      ...
      //! the terrain properties of the field will be AND-masked with this field and then OR-masked
      //! with terrain_or to form the new terrain properties
   ...
   //! The attackbonus_abs replaces the current attackbonus of the field by a new one. A value of -1
   //! won't affect the attackbonus of the field
   int attackbonus_abs;

   //! this is added to the current defensebonus of the field to form the new defensebonus.
   int defensebonus_plus;
   //! The defensebonus_abs replaces the current defensebonus of the field by a new one. A value of
   //! -1 won't affect the attackbonus of the field
   int defensebonus_abs;

   //! this is added to the current basicjamming of the field to form the new jamming.
   int basicjamming_plus;
   //! basicjamming_abs replaces the current basicjamming of the field by a new one. A value < 0
   //! won't affect the jamming of the field
   ...
   //! the level of height the object is on. This is not the simple system of 8 levels used for
   //! units and building, but one with 255 levels to allow a fine grained specification of the
   //! order in which different objects are painted
   int imageHeight;

   //! the 'real' height of the object, which evaluated for construction and attack. The destinction
   //! between imageHeight and physicalHeight was made because of compatibility reasons
   int physicalHeight;

   //! The resources required to construct the object with a unit; Note that units usually don't
   //! have any energy available
   Resources buildcost;

   //! The resources required to remove the object with a unit; Note that units usually don't have
   //! any energy available
   ...
   //! if a building is constructed on the field containing the object, will the object be removed
   //! or will it stay beneath the building.
   ...
   enum NetBehaviour {
      NetToBuildings = 1,
      NetToBuildingEntry = 2,
      NetToSelf = 4,
      NetToBorder = 8,
      SpecialForest = 0x10,
      AutoBorder = 0x20,
      KeepOrientation = 0x40
   };
   ...
   //! some objects require special displaying methods, for example shading the terrain they are
   //! build on
   int displayMethod;

   //! returns the level of height of this object in the normal 8 level scheme of asc (deep
   //! submerged, submerged, ... )
   ...
   /** the time after which any growth stops. This parameter is passed to all child-objects spawned
      through growth and should therefor be bigger than lifetime. Use -1 for unlimited */
   int growthDuration;
