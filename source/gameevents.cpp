...
EventTrigger::State TurnPassed::getState(int player) {
   if (gamemap->time.turn() > turn ||
       (gamemap->time.turn() == turn && gamemap->time.move() >= move))
...
void AllEnemyUnitsDestroyed::arm() {
   ContainerBase::anyContainerDestroyed.connect(
      sigc::hide(sigc::mem_fun(*this, &AllEnemyUnitsDestroyed::triggered)));
   ContainerBase::anyContainerConquered.connect(
      sigc::hide(sigc::mem_fun(*this, &AllEnemyUnitsDestroyed::triggered)));
}
...
void AllEnemyBuildingsDestroyed::arm() {
   ContainerBase::anyContainerDestroyed.connect(
      sigc::hide(sigc::mem_fun(*this, &AllEnemyBuildingsDestroyed::triggered)));
   ContainerBase::anyContainerConquered.connect(
      sigc::hide(sigc::mem_fun(*this, &AllEnemyBuildingsDestroyed::triggered)));
}
...
EventTrigger::State ResourceTribute::getState(int player) {
   if (payingPlayer < 0 ||
       gamemap->tribute.avail[payingPlayer][player] + gamemap->tribute.paid[player][payingPlayer] <
          demand)
      return unfulfilled;
   else
      return finally_fulfilled;
...
void ChangeGameParameter::setup() {
   ...
   if ((nr >= 0) && (nr < gameparameternum)) {
      if (gameParameterSettings[nr].changeableByEvent) {
         ...
         parameterValue = getid("Parameter Val", org, gameParameterSettings[nr].minValue,
                                gameParameterSettings[nr].maxValue);
         parameterNum = nr;
      } else
...
class ChangePlayerState : public EventAction {
  ...
  public:
   ChangePlayerState()
      : EventAction(EventAction_ChangePlayerState), player(0), state(Player::off){};
   ...
   void execute(MapDisplayInterface* md) { gamemap->getPlayer(player).stat = state; }
...
void SetViewSharing ::execute(MapDisplayInterface* md) {
   ...
   else if (gamemap->player[providingPlayer].diplomacy.getState(viewingPlayer) >= PEACE_SV)
...
void Reinforcements ::execute(MapDisplayInterface* md) {
   ...
   while (cnt < objectNum) {
      ...
      if (type == ReinfVehicle) {
         try {
            Vehicle* veh =
               Vehicle::newFromStream(gamemap, stream, gamemap->idManager.getNewNetworkID());
...
bool registerTrigger(EventTrigger_ID id) {
   return eventTriggerFactory::Instance().registerClass(
      id, ObjectCreator<EventTrigger, TriggerType>, TriggerNameProvider<TriggerType>());
}

template <typename ActionType>
bool registerAction(EventAction_ID id) {
   return eventActionFactory::Instance().registerClass(id, ObjectCreator<EventAction, ActionType>,
                                                       ActionNameProvider<ActionType>());
}
