...
class MapItemTypeWidget : public SelectionWidget {
  ...
  public:
   ...
   MapItemTypeWidget(PG_Widget* parent, const PG_Point& pos, const MapItemType* mapItemType)
      : SelectionWidget(
           parent, PG_Rect(pos.x, pos.y, ItemTypeSelector<ItemType>::type::Width(),
                           ItemTypeSelector<ItemType>::type::Height() + MapComponent::fontHeight)),
        item(mapItemType){
...
/**  The BaseMapItemTypeWidgetFactory iterates through all items of an ItemRepository and spawns a
   MapItemTypeWidget for it. It is will belong to a SelectionWidget , which will control the
   MapItemTypeWidget generation.

     This class does nothing if a widget is selected. This behaviour must be added by derived
   classes.
*/
...
//! Adds selection logic to a BaseMapItemTypeWidgetFactory. Selected items are registered as being
//! the active "brush" for editing the map
template <class MapItemWidget>
class MapItemTypeWidgetFactory : public BaseMapItemTypeWidgetFactory<MapItemWidget> {
  ...
  public:
   MapItemTypeWidgetFactory(const ItemRepository<ItemType>& itemRepository)
      : BaseMapItemTypeWidgetFactory<MapItemWidget>(itemRepository){};
   ...
   void itemSelected(const SelectionWidget* widget, bool mouse) {
      ...
      if (mapItemWidget->getItem()) {
         typename ItemTypeSelector<typename MapItemWidget::ItemType>::type item(
            mapItemWidget->getItem());
         selection.setSelection(item);
      }
   }
};

//! Adds selection logic to a BaseMapItemTypeWidgetFactory. The ID is of the selected item is
//! obtained and written to the variable which was passed on construction
template <class MapItemWidget>
...
/** opens a window in which the user can select an item of the given type ( template parameter
   ItemType ). \param id The value of this variable will set the item that is selected when opening
   the window. The selected item will also be written back here. \param itemRepository the
   repository of items from which the user can choose one
*/
template <class ItemType>
bool selectItemID(int& id, const ItemRepository<ItemType>& itemRepository) {
   ItemSelectorWindow isw(
      NULL, PG_Rect(300, 50, 280, PG_Application::GetScreenHeight() - 100), "select item",
      new MapItemTypeWidgetFactory_IDSelection<MapItemTypeWidget<ItemType>>(itemRepository, id));
   isw.Show();
   isw.RunModal();
   return true;
}
