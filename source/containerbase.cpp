...
ContainerBase ::ContainerBase(const ContainerBaseType* bt, GameMap* map, int player)
   : gamemap(map), cargoParent(NULL), baseType(bt) {
   view = bt->view;
   damage = 0;
   color = player * 8;
   ...
   for (int i = 0; i < map->getVehicleTypeNum(); ++i) {
      ...
      if (vt)
         for (int j = 0; j < bt->vehiclesInternallyProduceable.size(); ++j)
            if (vt->id >= bt->vehiclesInternallyProduceable[j].from &&
                vt->id <= bt->vehiclesInternallyProduceable[j].to)
...
void ContainerBase::paintField(const Surface& img, Surface& dest, SPoint pos, int dir, bool shaded,
                               int shadowDist) const {
   pair<const Surface*, int> dirpair = make_pair(&img, directionangle[dir]);

   int height = getHeight();
   if (height <= chgetaucht) {
      if (shaded) {
         megaBlitter<ColorTransform_UnitGray, ColorMerger_AlphaMixer,
         ...
         if (img.GetPixelFormat().BytesPerPixel() == 1) {
            MegaBlitter<1, gamemapPixelSize, ColorTransform_PlayerCol, ColorMerger_AlphaMixer,
            ...
            */
            ...
            MegaBlitter<4, gamemapPixelSize, ColorTransform_PlayerTrueCol, ColorMerger_AlphaMixer,
                        SourcePixelSelector_CacheRotation>
               blitter;
      ...
      if (height >= chfahrend && shadowDist) {
         ...
         megaBlitter<ColorTransform_None, ColorMerger_AlphaShadow,
                     SourcePixelSelector_CacheRotation, TargetPixelSelector_All>(
            img, dest, SPoint(pos.x + shadowDist, pos.y + shadowDist), nullParam, nullParam,
            dirpair, nullParam);
      }

      if (shaded) {
         megaBlitter<ColorTransform_UnitGray, ColorMerger_AlphaOverwrite,
                     SourcePixelSelector_CacheRotation, TargetPixelSelector_All>(
            img, dest, pos, nullParam, nullParam, dirpair, nullParam);
      } else {
         if (img.GetPixelFormat().BytesPerPixel() == 1) {
            MegaBlitter<1, gamemapPixelSize, ColorTransform_PlayerCol, ColorMerger_AlphaOverwrite,
                        SourcePixelSelector_CacheRotation>
               blitter;
            blitter.setPlayer(getOwner());
            blitter.setAngle(img, directionangle[dir]);
            ...
            MegaBlitter<4, gamemapPixelSize, ColorTransform_PlayerTrueCol, ColorMerger_AlphaMerge,
                        SourcePixelSelector_CacheRotation>
               blitter;
            /// blitter.setPlayer( getOwner() );
            blitter.setColor(gamemap->player[getOwner()].getColor());
         ...
         if ((*i)->networkid == nwid) {
            ...
            *i = NULL;
            ...
            if (cargo.size() > baseType->maxLoadableUnits) {
               // we only compact the cargo when we have more slots than allowed (may happen when
               // conquering fully occupied buildings). because else we would confuse the user if
               // the position of units in the cargo dialog changes
               compactCargo();
            }
...
bool ContainerBase ::doesVehicleFit(const Vehicle* vehicle) const {
   bool isConquering = isBuilding() && getMap()->getPlayer(this).diplomacy.isHostile(vehicle) &&
                       vehicle->color != color;
   if (baseType->vehicleFit(vehicle->typ))  // checks size and type
      if (vehiclesLoaded() < baseType->maxLoadableUnits || isConquering)
         if (canCarryWeight(vehicle->weight(), vehicle) || findUnit(vehicle->networkid) ||
             isConquering)  // if the unit is already  loaded, the container already bears its
                            // weight
            return true;
...
bool ContainerBase ::vehicleLoadable(const Vehicle* vehicle, int uheight,
                                     const bool* attacked) const {
   if (vehicle->attacked)
      return false;
   ...
   if (doesVehicleFit(vehicle))
      for (ContainerBaseType::EntranceSystems::const_iterator i = baseType->entranceSystems.begin();
           i != baseType->entranceSystems.end(); i++)
         if ((i->height_abs & uheight) || (i->height_abs == 0))
            if (i->mode & ContainerBaseType::TransportationIO::In)
               if (i->height_rel == -100 ||
                   i->height_rel ==
                      getheightdelta(getPosition().getNumericalHeight(), getFirstBit(uheight)))
                  if ((i->container_height & getPosition().getBitmappedHeight()) ||
                      (i->container_height == 0))
                     if (vehicle->typ->hasAnyFunction(i->requiresUnitFeature) ||
                         i->requiresUnitFeature.none())
                        if (i->vehicleCategoriesLoadable & (1 << vehicle->typ->movemalustyp)) {
                           ...
                           if (isBuilding()) {
                              if (!hasAttacked) {
                                 if (getOwner() == 8)
                                    return true;
                                 if (gamemap->getPlayer(this).diplomacy.isHostile(
                                        vehicle->getOwner()))
                                    if (damage >= minimumBuildingDamageForConquering ||
                                        vehicle->typ->hasFunction(
                                           ContainerBaseType::ConquerBuildings))
                                       return true;
                              }
                           }
                        }
...
const ContainerBaseType::TransportationIO*
ContainerBase::vehicleUnloadSystem(const VehicleType* vehicleType, int height) {
         ...
         if (i->mode & ContainerBaseType::TransportationIO::Out)
            if ((i->container_height & getPosition().getBitmappedHeight()) ||
               ...
               if (i->vehicleCategoriesLoadable & (1 << vehicleType->movemalustyp))
                  if (vehicleType->hasAnyFunction(i->requiresUnitFeature) ||
                      i->requiresUnitFeature.none()) {
                     if (i->height_abs != 0 && i->height_rel != -100) {
                        if (height & (i->height_abs &
                                      (1 << (getPosition().getNumericalHeight() + i->height_rel))))
                           return &(*i);
                     } else if (i->height_rel != -100) {
                        if (height & (1 << (getPosition().getNumericalHeight() + i->height_rel)))
                           return &(*i);
                     } else if (height & i->height_abs)
                        return &(*i);
                  }
   return NULL;
}
               ...
               if (i->vehicleCategoriesLoadable & (1 << vehicle->typ->movemalustyp)) {
                  if (i->dockingHeight_abs != 0 && i->dockingHeight_rel != -100)
                     height |= i->dockingHeight_abs &
                               (1 << (getPosition().getNumericalHeight() + i->dockingHeight_rel));
                  else if (i->dockingHeight_rel != -100)
...
const ContainerBaseType::TransportationIO*
ContainerBase ::getDockingSystem(const Vehicle* vehicle, int receipientHeight) const {
   if (vehicle == this)
      return NULL;
   ...
   if (productionCache.empty() && !internalUnitProduction.empty()) {
      for (int height = 0; height < 8; ++height)
         if ((1 << height) & baseType->height)
            for (Production::const_iterator i = internalUnitProduction.begin();
                 i != internalUnitProduction.end(); ++i)
               if (baseType->hasFunction(ContainerBaseType::ProduceNonLeavableUnits) ||
                   vehicleUnloadable(*i, height))
                  if (find(productionCache.begin(), productionCache.end(), *i) ==
                      productionCache.end())
...
void ContainerBase ::deleteProductionLine(const VehicleType* type) {
   internalUnitProduction.erase(
      remove(internalUnitProduction.begin(), internalUnitProduction.end(), type),
      internalUnitProduction.end());
...
void ContainerBase ::addProductionLine(const VehicleType* type) {
   if (find(internalUnitProduction.begin(), internalUnitProduction.end(), type) ==
       internalUnitProduction.end())
...
bool ContainerBase ::hasProductionLine(const VehicleType* type) {
   return find(internalUnitProduction.begin(), internalUnitProduction.end(), type) !=
          internalUnitProduction.end();
}
...
Resources ContainerBase ::netResourcePlus() const {
   ...
   for (int resourcetype = 0; resourcetype < resourceTypeNum; resourcetype++) {
      ...
      res.resource(resourcetype) +=
         grp.getresource(getPosition().x, getPosition().y, resourcetype, color / 8, 1);
   }
   return res;
}
...
void ContainerBase ::setInternalResourcePlus(const Resources& res) {
   for (int r = 0; r < Resources::count; ++r)
      plus.resource(r) =
         min(res.resource(r), min(maxplus.resource(r), baseType->maxplus.resource(r)));
}
...
Resources ContainerBase ::getResourcePlus() {
   ...
   //  printf ("building %s %d / %d : plus %d %d %d \n", typ->name.c_str(), getPosition().x,
   //  getPosition().y, r.energy, r.material, r.fuel );
...
ContainerBase ::Work* ContainerBase ::spawnWorkClasses(bool justQuery) {
   if (getMap()->_resourcemode != 1) {
      for (WorkerClassList::iterator i = workClassFactoriesASC->begin();
           i != workClassFactoriesASC->end(); ++i)
         if ((*i)->available(this))
            return (*i)->produce(this, justQuery);
   } else {
      for (WorkerClassList::iterator i = workClassFactoriesBI->begin();
           i != workClassFactoriesBI->end(); ++i)
         if ((*i)->available(this))
            return (*i)->produce(this, justQuery);
   }
   return NULL;
