...
void ttributepayments ::init(void) {
   ...
   for (i = 0; i < pos; i++) {
      bar(x1 + 30, y1 + starty + 30 + i * 30, x1 + 50, y1 + starty + 50 + i * 30,
          20 + players[i] * 8);
   ...
   rahmen3("you still have outstanding", x1 + 160, y1 + wind2y, x1 + wind2x - 10, y1 + ysize - 60,
           1);
   ...
   for (i = 0; i < 3; i++) {
      ...
      rectangle(x1 + wind2x, y1 + wind1y + 15 + i * 40, x1 + xsize - 30, y1 + wind1y + 32 + i * 40,
                black);
      rectangle(x1 + wind1x, y1 + wind2y + 15 + i * 40, x1 + wind2x - 15, y1 + wind2y + 32 + i * 40,
                black);
      rectangle(x1 + wind2x, y1 + wind2y + 15 + i * 40, x1 + xsize - 30, y1 + wind2y + 32 + i * 40,
                black);
   } /* endfor */
...
void ttributepayments ::paintvalues(void) {
   ...
   for (int i = 0; i < 3; i++) {
      ...
      showtext2(strrr(trib.paid[actmap->actplayer][players[player]].resource(i)), x1 + wind2x + 5,
                y1 + wind2y + 16 + i * 40);
      showtext2(strrr(trib.paid[players[player]][actmap->actplayer].resource(i)), x1 + wind2x + 5,
                y1 + wind1y + 16 + i * 40);

      activefontsettings.length = wind2x - wind1x - 35;
      showtext2(strrr(trib.avail[players[player]][actmap->actplayer].resource(i)), x1 + wind1x + 5,
                y1 + wind2y + 16 + i * 40);
   } /* endfor */
}
...
void ttributepayments ::run(void) {
   ...
   do {
      ...
      if (mouseparams.taste & 1)
         for (int i = 0; i < 8; i++) {
            if ((mouseparams.x >= x1 + 25) && (mouseparams.y >= y1 + starty + 25 + i * 30) &&
                (mouseparams.x <= x1 + 145) && (mouseparams.y <= y1 + starty + 55 + i * 30))
               if (players[i] != -1)
                  player = i;
...
void tresearchinfo::init(void) {
   ...
   if (actmap->player[actmap->actplayer].research.activetechnology) {
      bar(x1 + 20, y1 + 70,
          x1 + 20 +
             (xsize - 40) * actmap->player[actmap->actplayer].research.progress /
      ...
      showtext2(actmap->player[actmap->actplayer].research.activetechnology->name, textxpos,
                y1 + 110);
      ...
      if (rppt)
         showtext2(strrr(actmap->player[actmap->actplayer].research.currentTechAvailableIn()),
                   textxpos, y1 + 290);
   }
...
void sendGameParameterAsMail(GameMap* actmap) {
   ...
   if (actmap->getgameparameter(cgp_superVisorCanSaveMap)) {
      s = "#color4#Warning ! This game has been started with the option enabled that allows the "
          "Supervisor to save the map. This will allow him to spy on everything. \n"
          "If you did not initiate this game yourself, don't play with this option unless you were "
          "specifically informed why this option had to be enabled. \n"
          "#color0# \n" +
          s;
   }
...
void showGameParameters(void) {
   ViewFormattedText vft("Game Parameters", writeGameParametersToString(actmap),
                         PG_Rect(-1, -1, 500, 550));
   vft.Show();
   vft.RunModal();
}
...
void tgiveunitawaydlg ::paintplayer(int i) {
   ...
   if (i == markedplayer) {
      bar(x1 + 20, y1 + starty + xs + i * 40, x1 + xsize - 20, y1 + starty + 60 + i * 40,
          20 + receivers[i]->getPosition() * 8);
   } else {
   ...
   if (receivers[i]->getPosition() < 8)
      showtext2(receivers[i]->getName().c_str(), x1 + 60,
                y1 + starty + xs + 17 + i * 40 - activefontsettings.font->height / 2);
   else
      showtext2("neutral", x1 + 60,
                y1 + starty + xs + 17 + i * 40 - activefontsettings.font->height / 2);
}
   ...
   do {
      ...
      if (mouseparams.taste == 1)
         for (int i = 0; i < receivers.size(); i++)
            if (mouseinrect(x1 + 20, y1 + starty + xs + i * 40, x1 + xsize - 20,
                            y1 + starty + 60 + i * 40))
               markedplayer = i;
...
void displaywindspeed(void) {
   ...
   displaymessage2("wind speed is %d which equals %s fields / turn",
                   actmap->weatherSystem->getCurrentWindSpeed(),
                   strrrd8d(actmap->weatherSystem->getCurrentWindSpeed() * maxwindspeed / 256));
#else
   displaymessage2("wind speed is %d which equals %s fields / turn", actmap->weather.windSpeed,
                   strrrd8d(actmap->weather.windDirection * maxwindspeed / 256));
#endif
}
