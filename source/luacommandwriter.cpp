...
void LuaCommandWriter ::printCommand(const ASCString& command) {
   if (command.find('\n') != ASCString::npos) {
      ...
      outputLine("if r:successful()==false then asc.displayActionError(r, \"command " +
                 ASCString::toString(++commandCounter) + "\") end \n");
   }
};
...
LuaCommandFileWriter ::LuaCommandFileWriter(const ASCString& filename)
   : stream(filename, tnstream::writing) {
   writeHeader();
}
