...
void duplicateIDError(const ASCString& itemtype, int id, const ASCString& file1,
                      const ASCString& name1, const ASCString& file2, const ASCString& name2) {
   fatalError(
      "Conflicting IDs !\n"
      "These two " +
      itemtype + "s use both the ID of " + strrr(id) + "\n" + " " + name1 + " from file " + file1 +
...
void ItemRepository<T>::RegisterID::operator()(int id) {
   ...
   if (i != repository.hash.end() && i->second)
      duplicateIDError(repository.typeName, id, object->location, object->name, i->second->location,
                       i->second->name);
...
void ItemRepositoryLoader<T>::readTextFiles(PropertyReadingContainer& prc,
                                            const ASCString& fileName, const ASCString& location) {
   T* t = new T;
   t->runTextIO(prc);
...
void ItemRepositoryLoader<T>::write(tnstream& stream) {
   ...
   for (typename ItemRepository<T>::ItemContainerType::iterator i =
           ItemRepository<T>::container.begin();
        i != ItemRepository<T>::container.end(); ++i) {
      (*i)->write(stream);
...
class TechAdapterLoader : public TextFileDataLoader {
   ...
   void read(tnstream& stream) { readPointerContainer(techAdapterContainer, stream); };

   void write(tnstream& stream) { writePointerContainer(techAdapterContainer, stream); };
   ASCString getTypeName() { return "techadapter"; };
};
...
FileCache::FileCache() : stream(NULL) {
...
bool FileCache::checkForModification() {
   ...
   for (vector<tfindfile::FileInfo>::iterator i = actualFileInfo.begin(); i != actualFileInfo.end();
        ++i) {
      bool found = false;
      for (vector<tfindfile::FileInfo>::iterator j = cacheFileInfo.begin();
           j != cacheFileInfo.end(); ++j)
         if (i->name == j->name) {
            found = true;
            if (i->size != j->size)
               return false;
...
void loadAllData(bool useCache) {
   ...
   if (cache.isCurrent() && useCache) {
      try {
         ...
         fatalError(
            "the cache seems to have been generated with a newer version of ASC than this "
            "one.\nPlease upgrade to that version, or delete " +
            ASCString(cacheFileName) + " and try again");
      }
...
void loadalltextfiles() {
   ...
   displayLogMessage(
      10, " Printing all .ASCTXT files after inheritance and aliases have been resolved\n");
   for (TextFileRepository::iterator i = textFileRepository.begin(); i != textFileRepository.end();
        i++) {
      i->second.buildIDs();
...
ItemFiltrationSystem::ItemFilter::ItemFilter(const ASCString& _name,
                                             const IntRangeArray& unitsetIDs, bool _active) {
   name = "UnitSet: " + _name;
   units = unitsetIDs;
   active = _active;
}
...
bool ItemFiltrationSystem::ItemFilter::isContained(ItemFiltrationSystem::Category cat, int id) {
   switch (cat) {
      case Building:
         return isContained(buildings, id);
      case Vehicle:
         return isContained(units, id);
      case Object:
         return isContained(objects, id);
      case Terrain:
         return isContained(terrain, id);
      case Technology:
         return isContained(technologies, id);
   };
   return false;
}
...
void ItemFiltrationSystem::DataLoader::readTextFiles(PropertyReadingContainer& prc,
