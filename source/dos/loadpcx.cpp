...
char loadpcxxy(char* name, char setpal, word x, word y) {
   ...
   if (header.manufacturer != 10 || header.bitsperpixel != 8 ||
   ...
   if (agmp->byteperpix > 1) {
      ...
      scanlineret[1] =
         linelength * agmp->byteperpix - agmp->greenfieldposition / 8 + agmp->redfieldposition / 8;
      scanlineret[2] =
         linelength * agmp->byteperpix - agmp->bluefieldposition / 8 + agmp->greenfieldposition / 8;
      scanlineret[3] = agmp->scanlinelength - linelength * agmp->byteperpix +
                       agmp->redfieldposition / 8 - agmp->bluefieldposition / 8;
