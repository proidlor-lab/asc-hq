...
const char* ttdef[5] = {
   "2 audio-channels without pre-emphasis", "2 audio-channels with pre-emphasis",
   "4 audio-channels without pre-emphasis", "4 audio-channels with pre-emphasis", "Data Track"};
const char* cerror[17] = {"No Error",
...
tcdrom::~tcdrom(void) {
   for (int i = 0; i < cdinfo.lasttrack; i++)
      if (cdinfo.track[i] != NULL)
         free(cdinfo.track[i]);
}
...
char tcdrom::checkbusy(void) {
   ...
   if ((ioctl->requestheader.status & 512) > 0)
      return true;
   else
      return false;
}
...
void tcdrom::getcdinfo(pascal_byte* l, pascal_byte* h, pascal_byte* min, pascal_byte* sec,
                       pascal_byte* frame)
...
void tcdrom::gettracklength(pascal_byte tracknr, pascal_byte* min, pascal_byte* sec,
                            pascal_byte* frame)

{
   ...
   if (tracknr < h - 1)
      gettrackinfo(tracknr + 1, &m1, &s1, &f1, &t);
   else {
      m1 = m2;
      s1 = s2;
      f1 = f2;
...
void tcdrom::getactivetimes(pascal_byte* min, pascal_byte* sec, pascal_byte* frame,
                            pascal_byte* amin, pascal_byte* asec, pascal_byte* aframe) {
   tqinfo* qi;
...
void tcdrom::gettrackinfo(pascal_byte tracknr, pascal_byte* min, pascal_byte* sec,
                          pascal_byte* frame, pascal_byte* type)

{
   ...
   *frame = ti->startpoint & 0x000000FF;
   ti->controlinfo &= 0xF0;
   if ((ti->controlinfo & 0x80) > 0)
      *type = ((ti->controlinfo & 0x10) > 0) ? 3 : 2;
   else
      *type = ((ti->controlinfo & 0x10) > 0) ? 1 : 0;
   if ((ti->controlinfo & 0x40) > 0)
      *type = 4;
}
...
void tcdrom::getcdromdrives(void) {
   ...
   for (int i = 0; i < numberofdrives; i++)
      driveletter[i + 1] = drives[i];
}

void tcdrom::changecdromdrive(char nr) {
   if ((nr < 0) || (nr > numberofdrives - 1))
      return;
