...
int mouse_in_off_area(void) {
   if (mouseparams.off.x1 == -1 || mouseparams.off.y1 == -1)
      ...
      return (mouseparams.x1 + mouseparams.xsize >= mouseparams.off.x1 &&
              ...
              mouseparams.x1 <= mouseparams.off.x2 && mouseparams.y1 <= mouseparams.off.y2);
}
...
void putmousepointer(void) {
   /*
   getimage ( mouseparams.x1, mouseparams.y1, mouseparams.x1 + mouseparams.xsize, mouseparams.y1 +
   mouseparams.ysize, mouseparams.background ); putimage ( mouseparams.x1, mouseparams.y1,
   mouseparams.pictpointer ); mouseparams.altx = mouseparams.x1; mouseparams.alty = mouseparams.y1;
   */
}
...
{ /* calculate size of code to lock */ } /* (cbc_end - click_handler) */
...
void getinvisiblemouserectanglestk(void) {
   ...
   // Does anybody know a more elegant way that does work with Watcom C 11 ? Watcom C 10.6 did not
   // care ...
