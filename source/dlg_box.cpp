...
void tdialogbox::repaintdisplay(void) { /*
                                          ...
                                          hgmp->linearaddress = (PointerSizedInt)
                                          virtualscreenbuf.buf; hgmp->windowstatus = 100;
                                          ...
                                          *agmp = *hgmp;
                                          ...
                                          putspritetexture ( 0, 0, hgmp->resolutionx-1,
                                          hgmp->resolutiony-1, virtualscreenbuf.buf );
         ...
         if ((pb->id + 1 == id)) {
            if (taste == ct_pup) {
               if (*pw >= pb->max)
                  ...
                  *pw = 0;
            } else if (taste == ct_pos1) {
               *pw = 0;
            } else if (*pw > 0) {
               if (pb->newpressed == 0)  // Tastatur
                  (*pw)--;
               else if (pb->newpressed == 1) {
                  (*pw)--;
                  pb->lasttick = ticker;
               } else if (pb->newpressed == 2) {
         ...
         if ((pb->id + 2 == id)) {
            if (taste == ct_pdown) {
               if (*pw + (pb->max * 2) - 1 <= *pw2)
                  ...
                  *pw = *pw2 - pb->max;
            } else if (taste == ct_ende) {
               *pw = *pw2 - pb->max;
            } else if (*pw + pb->max < *pw2) {
               if (pb->newpressed == 0)  // Tastatur
                  (*pw)++;
               else if (pb->newpressed == 1) {
                  (*pw)++;
...
void tdlgengine::addbutton(const char* ltxt, tmouserect rect1, int lart, int lstyle, int lid,
                           bool enabled) {
...
void tdlgengine::addbutton(const char* ltxt, int lx1, int ly1, int lx2, int ly2, int lart,
                           int lstyle, int lid, bool enabled) {
   pbutton pb;
   char ch;
...
void tdlgengine::addscrollbar(tmouserect rec, int* numberofitems, int itemsvisible, int* actitem,
                              int lid, int keys) {
...
void tdlgengine::addDropDown(int x1, int y1, int x2, int y2, int ID, const char** entries,
                             int entrynum, int* pos) {
   pbutton pb = firstbutton;
   if (ID <= 0)
...
void tdlgengine::addscrollbar(int lx1, int ly1, int lx2, int ly2, int* numberofitems,
                              int itemsvisible, int* actitem, int lid, int keys) {
   pbutton pb;
...
void tdialogbox::rebuildtaborder(void) {
   ...
   if (tabcount > 1)
      for (i = 1; i <= tabcount - 1; i++) {
         if ((taborder[i].y1 > taborder[i + 1].y1) ||
             ((taborder[i].y1 == taborder[i + 1].y1) && (taborder[i].x1 > taborder[i + 1].x1))) {
            b = taborder[i];
...
void tdlgengine::addeingabe(int lid, void* data, int min, int max) {
   pbutton pb;
...
void tdialogbox::enablebutton(int id) {
   ...
   if (pb->art == 0) {
      ...
      if (pb->text)
         if (pb->text[0]) {
            ...
            showtext3(pb->text, x1 + pb->x1,
                      y1 + (pb->y1 + pb->y2) / 2 - activefontsettings.font->height / 2);
         }
   }
   ...
   if (pb->art == 3) {
      ...
      showtext3(pb->text, x1 + pb->x1 + (pb->y2 - pb->y1) + 5,
                y1 + (pb->y1 + pb->y2 - activefontsettings.font->height) / 2);
   ...
   if (pb->art == 5) {  // Scrollbar
      ...
      // if (pb->max <= *pw2) {
      int l = pb->y2 - pb->y1 - 2;
      rahmen(false, x1 + pb->x1 + 1, y1 + pb->y1 + 1 + l * *pw / *pw2, x1 + pb->x2 - 1,
             y1 + pb->y1 + 1 + l * (*pw + pb->max) / *pw2);
      // }
      // else
   ...
   if (pb->art == 6) {
      ...
      showtext3(pb->entries[pos], x1 + pb->x1 + (pb->y2 - pb->y1) + 5,
                y1 + (pb->y1 + pb->y2 - activefontsettings.font->height) / 2);
...
void tdialogbox::disablebutton(int id) {
   ...
   if (pb->art == 0) {
      if (pb->text != NULL)
         if (pb->text[0] != 0) {
            activefontsettings.justify = centertext;
            showtext2(s, x1 + pb->x1,
                      y1 + (pb->y1 + pb->y2) / 2 - activefontsettings.font->height / 2);
         }
      if ((pb->style == 1) || (pb->style == 2)) {
         rectangle(x1 + pb->x1, y1 + pb->y1, x1 + pb->x2, y1 + pb->y2, disablecolor);
         rectangle(x1 + pb->x1 - 1, y1 + pb->y1 - 1, x1 + pb->x2 + 1, y1 + pb->y2 + 1,
                   disablecolor);
      }
   }
   if ((pb->art == 1) || (pb->art == 2)) {
   ...
   if (pb->art == 3) {
      rectangle(x1 + pb->x1, y1 + pb->y1, x1 + pb->x1 + (pb->y2 - pb->y1), y1 + pb->y2,
                disablecolor);
      showtext2(s, x1 + pb->x1 + (pb->y2 - pb->y1) + 5,
                y1 + (pb->y1 + pb->y2 - activefontsettings.font->height) / 2);
      pbl = (char*) pb->data;
      if (*pbl)
         cl = disablecolor;
   ...
   if (pb->art == 5) {
      ...
      if (pb->max <= *pw2) {
         l = pb->y2 - pb->y1 - 2;
         rectangle(x1 + pb->x1 + 1, y1 + pb->y1 + 1 + l * *pw / *pw2, x1 + pb->x2 - 1,
                   y1 + pb->y1 + 1 + l * (*pw + pb->max) / *pw2, disablecolor);
      } else
         rectangle(x1 + pb->x1 + 1, y1 + pb->y1 + 1, x1 + pb->x2 - 1, y1 + pb->y2 - 1,
                   disablecolor);
   }
   ...
   if (mouse == false) {
      ...
      if (pb->art == 0) {
         ...
         if (pb->style == 2) {
            ...
            buttonpressed(pb->id);
            pb->newpressed = 2;
            while ((mouseparams.x >= x1 + pb->x1) && (mouseparams.x <= x1 + pb->x2) &&
                   (mouseparams.y >= y1 + pb->y1) && (mouseparams.y <= y1 + pb->y2) &&
                   (mouseparams.taste & 1)) {
      ...
      if (pb->art == 5) {  // Scrollbar
         ...
         if (mouseparams.y >= yb1 + 1 + l * *pw / *pw2 &&
             mouseparams.y <= yb1 + 1 + l * (*pw + pb->max) / *pw2) {
            int mss = getmousestatus();
            if (mss == 2)
               mousevisible(false);
      ...
      if (pb->art == 6) {
         ...
         do {
            ...
            first = false;
                  ...
                  (mouseparams.y >= min(starty, y1 + pb->y1)) &&
                  (mouseparams.y <= starty + height) && (mouseparams.taste & 1));
         mousevisible(false);
      ...
      if (pb->art > 10) {
         buttonpressed(pb->id);
         while (mouseinrect(x1 + pb->x1, y1 + pb->y1, x1 + pb->x2, y1 + pb->y2) &&
                (mouseparams.taste & 1))
            releasetimeslice();
      }
...
void tdialogbox::showtabmark(int b) {
   if (b != 0)
      xorrectangle(x1 + taborder[b].x1 - 2, y1 + taborder[b].y1 - 2, x1 + taborder[b].x2 + 2,
                   y1 + taborder[b].y2 + 2, 15);
}
...
void tdialogbox::run(void) {
   ...
   taste = ct_invvalue;
   if (getmousestatus() == 2) {
      if ((mouseparams.x > x1 + 5) && (mouseparams.y > y1 + 3) &&
          (mouseparams.x < x1 + xsize - 5) && (mouseparams.y < y1 + 20) &&
          (mouseparams.taste == 1) && (dlg_mode & 3) == 3) {
         oldx = x1;
         oldy = y1;
         xp2 = oldx;
         yp2 = oldy;
         ...
         while (mouseparams.taste == 1) {
            if ((mouseparams.x != xp) || (mouseparams.y != yp)) {
               ...
               if (y1 < 0)
                  y1 = 0;
               if (x1 + xsize >= agmp->resolutionx)
                  x1 = agmp->resolutionx - xsize - 1;
               if (y1 + ysize >= agmp->resolutiony)
                  y1 = agmp->resolutiony - ysize - 1;
               if ((x1 != xp2) || (y1 != yp2)) {
                  mousevisible(false);
      ...
      if (mouseparams.taste == 1) {
         ...
         while (pb != NULL) {
            if (pb->status)
               if (pb->active) {
                  if ((mouseparams.x >= x1 + pb->x1) && (mouseparams.x <= x1 + pb->x2) &&
                      (mouseparams.y >= y1 + pb->y1) && (mouseparams.y <= y1 + pb->y2)) {
                     execbutton(pb, true);
                     break;
                  }
...
Uint8 tdialogbox::checkvalue(int id, void* p) {
   return true;
}
...
void tdialogbox::rahmen3(const char* txt, int x1, int y1, int x2, int y2, int style) {
...
void tdialogbox::stredit(char* s, int x1, int y1, int wdth, int max) {
   char *ss, *ss2, *ss3;
   char einfuegen;
   int position;
   int i;
   ...
   do {
      ...
      cc != cto_enter && cc != cto_esc &&
      !(!mouseinrect(x1, y1, x1 + wdth, y1 + activefontsettings.height) && mouseparams.taste > 0));
...
void tdialogbox::lne(int x1, int y1, char* s, int position, int einfuegen) {
   int i, j, k;
...
void tdialogbox::intedit(int* st, int x1, int y1, int wdth, int min, int max) {
   char *ss, *ss2, *ss3;
   char einfuegen;
   int position;
   int j;
      ...
      do {
         ...
         if (cc != cto_invvalue) {
            ...
            lne(x1, y1, ss, position, einfuegen);
               ...
               !(!mouseinrect(x1, y1, x1 + wdth, y1 + activefontsettings.height) &&
                 mouseparams.taste > 0));
      lne(x1, y1, ss, position, einfuegen);
      if (cc != cto_esc) {
         j = strtol(ss, &ss3, 10);
...
void tviewtext::nextline(int einzug, const char* txtptr) {
   if (tvt_dispactive == 0 && txtptr) {
      if ((tvt_startpoint == NULL && (tvt_yp > 500)) ||
          (tvt_startpoint && (tvt_yp > (tvt_startpoint->ypos + 500)))) {
      ...
      if (tvt_yp >= tvt_starty) {
         ...
         if (tvt_yp + activefontsettings.height > tvt_starty) {
            ...
            *agmp = tvt_firstlinebufparm;
            ...
            npop(*agmp);
            putimageprt(tvt_x1 + tvt_xp, tvt_y1, tvt_x1 + tvt_xp + twdth,
                        tvt_y1 + activefontsettings.height + (tvt_yp - tvt_starty),
                        tvt_firstlinebuf, tvt_xp, tvt_starty - tvt_yp);
         }
   }
...
void tviewtext::setparams(int xx1, int yy1, int xx2, int yy2, const char* ttxt, char clr,
                          char bkgr) {
   tvt_x1 = xx1;
   tvt_y1 = yy1;
   tvt_x2 = xx2;
...
void tviewtextwithscrolling::checkscrolling(void) {
   ...
   int tick = ticker;
   while ((skeypress(ct_down) || skeypress(ct_2k)) &&
          (tvt_starty + textsizey < textsizeycomplete)) {
   ...
   tick = ticker;
   while ((skeypress(ct_pdown) || skeypress(ct_3k)) &&
          (tvt_starty + textsizey < textsizeycomplete)) {
...
ASCString readtextmessage(int id) {
   ...
   displayLogMessage(7, ASCString("Retrieving message ") + strrr(id) +
                           "; looking for message file " + tmpstr + "\n");
...
void thelpsystem::init(int id, const char* titlet) {
   ...
   if (textsizeycomplete >= textsizey) {
      ...
      addscrollbar(xsize - 30, starty, xsize - 15, ysize - 40, &textsizeycomplete, textsizey,
                   &tvt_starty, 1, 0);
      setscrollspeed(1, 1);
...
class ViewTextQuery : public ASC_PG_Dialog {
  public:
   ViewTextQuery(int id, const ASCString& title, const ASCString& button1, const ASCString& button2)
      : ASC_PG_Dialog(NULL, PG_Rect(-1, -1, 450, 500), title) {
      PG_Rect r;
      if (button2.length()) {
         r = PG_Rect(10, Height() - 40, Width() / 2 - 15, 30);
         PG_Button* b = new PG_Button(
            this, PG_Rect(Width() / 2 + 5, Height() - 40, Width() / 2 - 15, 30), button2);
...
void tstringselect::buttonpressed(int id) {
   tdialogbox::buttonpressed(id);
   if (id == 1) {
      if (firstvisibleline > redline)
         redline = firstvisibleline;
      if (firstvisibleline + lnshown - 1 < redline)
         redline = firstvisibleline + lnshown - 1;
      viewtext();
   }
}
...
void tstringselect::run(void) {
   ...
   if (numberoflines > 0) {
      msel = 0;
      if (getmousestatus() == 2) {
         if (((ms = mouseparams.taste) == 0) && (dk == 1))
            dk = 2;
      ...
      switch (taste) {
         ...
         case ct_down: {
            ...
            if (redline < numberoflines - 1)
               ...
               view = false;
            if ((redline > firstvisibleline + lnshown - 1) &&
                (firstvisibleline + lnshown - 1 <= numberoflines)) {
               firstvisibleline++;
               showbutton(1);
            }
            if (view)
...
void tstringselect::scrollbar_on(void) {
   scrollbarvisible = true;
   addscrollbar(ex + 10, sy - 10, ex + 30, ey + 10, &numberoflines, lnshown, &firstvisibleline, 1,
                0);
}
...
class ChooseString : public tstringselect {
  ...
  public:
   ChooseString(const ASCString& _title, const vector<ASCString>& _strings,
                const vector<ASCString>& _buttons, int defaultEntry);
   void setup();
   virtual void buttonpressed(int id);
   void run(void);
...
void ChooseString ::setup() {
   ...
   for (int i = 0; i < buttons.size(); ++i)
      addbutton(buttons[i].c_str(), 25 + i * width, ysize - 45, 15 + (i + 1) * width, ysize - 20, 0,
                1, 20 + i, true);
}
...
pair<int, int> chooseString(const ASCString& title, const vector<ASCString>& entries,
                            const vector<ASCString>& buttons, int defaultEntry) {
   if (legacyEventSystemActive()) {
