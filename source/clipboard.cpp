...
void ClipBoardBase::setProperties(const ContainerBase* unit) {
   ...
   properties["materialindex"] =
      ASCString::toString(StatisticsCalculator::resource(unit, true).material / 1000);
...
void ClipBoardBase::place(const MapCoordinate& pos) {
   ...
   if (type == ClipVehicle) {
      ...
      if (!fieldAccessible(fld, veh, -2, NULL, true) &&
         ...
         for (int y = 0; y < 6; y++)
            if (bld->typ->fieldExists(BuildingType::LocalCoordinate(x, y))) {
               MapField* field = actmap->getField(
                  bld->typ->getFieldCoordinate(pos, BuildingType::LocalCoordinate(x, y)));
               if (!field) {
                  delete bld;
...
void ClipBoardBase::writeProperties(PropertyContainer& pc) const {
   for (map<ASCString, ASCString>::const_iterator i = properties.begin(); i != properties.end();
        ++i) {
