...
class GameMap {
   ...
   **/
   ...
   Player& getPlayer(PlayerID p) { return player[p.getID()]; }

   const Player& getPlayer(PlayerID p) const { return player[p.getID()]; }

   Player& getCurrentPlayer() { return player[actplayer]; }

   const Player& getCurrentPlayer() const { return player[actplayer]; }
  ...
  public:
   ...
   enum State {
      ...
      Destruction /*!< only to be used by units and buildings. To speed up map destruction, the view
                     won't be recalculated. No signals will be send when units & buildings are
                     destroyed, either  */
   } state;

   /** The tribute can not only be used to demand resources from enemies but also to transfer
    resources to allies. tribute.avail[S][D].energy is the ammount of energy that player D may
    (still) extract from the net of player S tribute.paid[D][S].energy is the amount of energy that
    player D has already extracted from player S's net S is source player, D is destination player
    **/
   class ResourceTribute {
     public:
      ...
      //! for the messages that are send each turn it is necessary to record how much of the
      //! transfer happend during the last turn
   ...
   //! the list of messages that were written this turn and are waiting to be processed at the end
   //! of the turn
   MessagePntrContainer unsentmessage;
   bool __loadunsentmessage;

   //! these are the messages themselfs. A MessagePntrContainer only stores pointers to message body
   //! which are archived here
   MessageContainer messages;
   bool __loadmessages;

   //! each message has an identification number (which is incremented with each message) for
   //! referencing it in files. The id of the last message is stored here
   int messageid;
  ...
  public:
   ...
   //! in BI resource mode ( see #_resourcemode , #isResourceGlobal ) , this is where the globally
   //! available resources are stored. Note that not all resources are globally available.
