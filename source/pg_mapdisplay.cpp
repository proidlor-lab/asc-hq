...
class PG_MapDisplay : public MapDisplayInterface {
  ...
  public:
   ...
   int displayMovingUnit(const MapCoordinate3D& start, const MapCoordinate3D& dest,
                         ...
                         SoundStartCallback soundStart, int duration);
   ...
   void removeActionCursor(void) { mapDisplayWidget->cursor.invisible = 1; };
...
int PG_MapDisplay ::displayMovingUnit(const MapCoordinate3D& start, const MapCoordinate3D& dest,
                                      Vehicle* vehicle, int fieldnum, int totalmove,
                                      SoundStartCallback soundStart, int duration) {
   ...
   else if (newstart.getNumericalHeight() == -1)
   ...
   if ((view1 >= visible_now || view2 >= visible_now) ||
       (vehicle->getOwner() == actmap->getPlayerView()) || shareView ||
       actmap->getPlayerView() == -1)
      if (((vehicle->height >= chschwimmend) && (vehicle->height <= chhochfliegend)) ||
          ((view1 == visible_all) || (view2 == visible_all)) ||
          (actmap->actplayer == actmap->getPlayerView()) || shareView ||
          actmap->getPlayerView() == -1) {
         soundStart(1);
