/*! \file history.cpp
    \brief Originally planed to provide history functions for ASC, but was never finished. Long
   derelicted.
*/
...
void countunits(void) {
      ...
      ar->player[i] = NULL;
   {
      ...
      ar->nr = turn;
      for (i = 0; i <= 8; i++)
         if (with1->player[i].existent == true) {
            void* with2 = &with1->player[i];
            ...
            if (firstbuilding != NULL) {
               ...
               do {
                  if (ab != NULL) {
                     ...
                     j = 0;
                     do {
                        if (with2->research.developedtechnologies[j] != NULL)
                           inc(j);
   ...
   if (asr != NULL) {
      ...
      displaymessage("History not started !", "", 0);
}
...
#define kg 15
   ...
   showflag = 1;
   for (i = 0; i <= 8; i++)
      if (actmap->player[i].existent == true)
         zplayer[i] = true;
      else
         zplayer[i] = false;
...
#define linemy 5 /* Y-Linien-L„nge */
   ...
   if (flag == 0) {
      ...
      while (b < round(maxpix.x / zoomx)) {
         ...
         nline(pixs.x + round(b * zoomx), pixs.y + linemy, pixs.x + round(b * zoomx),
               pixs.y - linemy, statc);
         if (round(xstep / zoomx) == 0)
            inc(b, 1);
         else
            inc(b, round(xstep / zoomx));
      }
      {
         void* with1 = &activefontsettings;
         justify = righttext;
      ...
      while (b < round(maxpix.y / zoomy)) {
         ...
         nline(pixs.x - linemx, pixs.y - round(b * zoomy), pixs.x + linemx,
               pixs.y - round(b * zoomy), statc);
         if (round(ystep / zoomy) == 0)
            inc(b, 1);
         else
            inc(b, round(ystep / zoomy));
      }
      for (sp = 0; sp <= 8; sp++)
         lastp[sp].x = pixs.x;
      for (sp = 0; sp <= 8; sp++)
      ...
      while (ar != NULL) {
         for (sp = 8; sp >= 0; sp--)
            if ((zplayer[sp]) && (ar->player[sp] != NULL)) {
               ...
               if (w != -1) {
                  setcolor(20 + sp * 8);
                  line(lastp[sp].x, lastp[sp].y, pixs.x + round(ar->nr * zoomx),
                       pixs.y - round(w * zoomy));
                  lastp[sp].x = pixs.x + round(ar->nr * zoomx);
                  lastp[sp].y = pixs.y - round(w * zoomy);
               }
            }
      ...
      for (sp = 0; sp <= 8; sp++)
         if (zplayer[sp])
            showplayerstat(sp, false);
      nline(pixs.x - 5, pixs.y, pixs.x + maxpix.x + 5, pixs.y, statc);
      nline(pixs.x, pixs.y - maxpix.y - 5, pixs.x, pixs.y + 5, statc);
   } else
      showplayerstat(id, false);
}
...
long* tshowhistorygetdata(struct* p, int* player) {
   ...
   switch (showflag) {
      case 1:
         getdata = p->player[player]->vehicle;
         break;

      case 2:
         getdata = p->player[player]->buildings;
         break;

      case 3:
         getdata = p->player[player]->energy;
         break;

      case 4:
         getdata = p->player[player]->plus.energy;
         break;

      case 5:
         getdata = p->player[player]->material;
         break;

      case 6:
         getdata = p->player[player]->plus.material;
         break;

      case 7:
         getdata = p->player[player]->sprit;
         break;

      case 8:
         getdata = p->player[player]->plus.fuel;
         break;

      case 9:
         getdata = p->player[player]->research;
         break;

      case 10:
         getdata = p->player[player]->devtechs;
         break;
   }
}
   ...
   while (p != NULL) {
      for (player = 0; player <= 8; player++)
         if (p->player[player] != NULL) {
            switch (showflag) {
               case 1:
                  getdat = p->player[player]->vehicle;
                  break;

               case 2:
                  getdat = p->player[player]->buildings;
                  break;

               case 3:
                  getdat = p->player[player]->energy;
                  break;

               case 4:
                  getdat = p->player[player]->plus.energy;
                  break;

               case 5:
                  getdat = p->player[player]->material;
                  break;

               case 6:
                  getdat = p->player[player]->plus.material;
                  break;

               case 7:
                  getdat = p->player[player]->sprit;
                  break;

               case 8:
                  getdat = p->player[player]->plus.fuel;
                  break;

               case 9:
                  getdat = p->player[player]->research;
                  break;

               case 10:
                  getdat = p->player[player]->devtechs;
                  break;
            }
            if (getdat > mx)
               mx = getdat;
