...
bool TechDependency::available(const Research& research) const {
   return available(CheckTechAvailabilityFunctor(&research, &Research::techResearched)) ==
          Available;
}

ResearchAvailabilityStatus
TechDependency::available(CheckTechAvailabilityFunctor checkTechAvailability) const {
   for (RequiredTechnologies::const_iterator j = blockingTechnologies.begin();
        j != blockingTechnologies.end(); ++j)
   ...
   if (requiredTechnologies.size()) {
      if (requireAllListedTechnologies) {
         for (RequiredTechnologies::const_iterator j = requiredTechnologies.begin();
              j != requiredTechnologies.end(); ++j)
            for (int k = j->from; k <= j->to; ++k)
               if (!checkTechAvailability(k))
...
bool TechDependency::eventually_available_single(const Research& res,
                                                 list<const Technology*>* dependencies,
                                                 list<int>& stack, int id) {
...
bool TechDependency::eventually_available(const Research& res,
                                          list<const Technology*>* dependencies) const {
   list<int> stack;
   return eventually_available(res, dependencies, stack);
}

int TechDependency::findInheritanceLevel(int id, vector<int>& stack,
                                         const ASCString& sourceTechName) const {
   for (RequiredTechnologies::const_iterator j = requiredTechnologies.begin();
        j != requiredTechnologies.end(); ++j)
      for (int k = j->from; k <= j->to; ++k)
         if (k > 0) {
...
void TechDependency::writeTreeOutput(const ASCString& sourceTechName, tnstream& stream,
                                     bool reduce) const {
   map<int, int> ihl;
   vector<int> stack;
   for (RequiredTechnologies::const_iterator j = requiredTechnologies.begin();
...
void TechDependency::writeInvertTreeOutput(const Technology* tech, tnstream& stream,
                                           vector<int>& history,
                                           vector<pair<int, int>>& blockedPrintList,
                                           const vector<IntRange>* onlyWithBaseTechs) const {
   if (onlyWithBaseTechs && !onlyWithBaseTechs->empty()) {
      vector<int> inheritanceStack;
      bool found = false;
      for (vector<IntRange>::const_iterator i = onlyWithBaseTechs->begin();
           i != onlyWithBaseTechs->end(); ++i)
   ...
   for (RequiredTechnologies::const_iterator i = blockingTechnologies.begin();
        i != blockingTechnologies.end(); ++i) {
      for (int l = i->from; l <= i->to; ++l) {
         const Technology* block = technologyRepository.getObject_byID(l);
         if (block && find(blockedPrintList.begin(), blockedPrintList.end(),
                           make_pair(tech->id, l)) == blockedPrintList.end()) {
            ASCString b =
               "\"" + generateTechName(block) + "\" -> \"" + generateTechName(tech) +
               "\" [color=red label=\"blocks\" fontcolor=red fontsize=10]\n";  // arrowhead=inv
...
void TechDependency::writeInvertTreeOutput(const ASCString techName, tnstream& stream,
                                           vector<int>& history,
                                           vector<pair<int, int>>& blockedPrintList,
                                           const vector<IntRange>* onlyWithBaseTechs) const {
   if (onlyWithBaseTechs && !onlyWithBaseTechs->empty()) {
...
ASCString TechDependency::showDebug(const Research& research) const {
   ...
   s += "#indent=20,20#";
   for (RequiredTechnologies::const_iterator r = requiredTechnologies.begin();
        r != requiredTechnologies.end(); ++r) {
   ...
   if (!blockingTechnologies.empty()) {
      ...
      s += "#indent=20,20#";
      for (RequiredTechnologies::const_iterator r = blockingTechnologies.begin();
           r != blockingTechnologies.end(); ++r) {
         for (int i = r->from; i <= r->to; ++i) {
            Technology* t = technologyRepository.getObject_byID(i);
            if (t) {
               s += "  " + t->name;
...
bool TechAdapterDependency::available(const Research& research) const {
   if (requiredTechAdapter.size()) {
      if (requireAllListedTechAdapter) {
         for (RequiredTechAdapter::const_iterator j = requiredTechAdapter.begin();
              j != requiredTechAdapter.end(); ++j)
...
void TechAdapterDependency::write(tnstream& stream) const {
   ...
   for (RequiredTechAdapter::const_iterator i = requiredTechAdapter.begin();
        i != requiredTechAdapter.end(); ++i)
...
void TechAdapterDependency::runTextIO(PropertyContainer& pc, const ASCString& defaultTechAdapter) {
   if (pc.find("TechAdapterRequired") || !pc.isReading()) {
      ...
      if (pc.isReading())
         for (RequiredTechAdapter::iterator j = requiredTechAdapter.begin();
              j != requiredTechAdapter.end(); ++j)
            j->toLower();

   } else if (defaultTechAdapter.length() > 0)
      requiredTechAdapter.push_back(defaultTechAdapter);

   pc.addBool("RequireAllListedTechAdapter", requireAllListedTechAdapter, true);
}

void TechAdapterDependency::writeInvertTreeOutput(const ASCString& tech, tnstream& stream,
                                                  const vector<IntRange>* onlyWithBaseTechs) const {
   vector<int> history;
   vector<pair<int, int>> blockedPrintList;
   for (RequiredTechAdapter::const_iterator r = requiredTechAdapter.begin();
        r != requiredTechAdapter.end(); ++r)
      for (TechAdapterContainer::iterator i = techAdapterContainer.begin();
           i != techAdapterContainer.end(); ++i)
         if (*r == (*i)->getName())
            (*i)->techDependency.writeInvertTreeOutput(tech, stream, history, blockedPrintList,
                                                       onlyWithBaseTechs);
}
...
bool Technology::eventually_available(const Research& res, list<const Technology*>* dependencies,
                                      list<int>& stack) const {
   if (res.techResearched(id))
      return true;
   ...
   if (version >= 0) {
      ...
      if (version <= -3) {
         if (merge) {
            ...
            predefinedTechAdapter.insert(predefinedTechAdapter.begin(),
                                         predefinedTechAdapter_t.begin(),
                                         predefinedTechAdapter_t.end());
         } else {
...
bool Research::techResearched(int id) const {
   for (vector<int>::const_iterator i = developedTechnologies.begin();
        i != developedTechnologies.end(); ++i)
      if (*i == id)
         return true;
   return false;
}
...
bool Research::techAdapterAvail(const ASCString& ta) const {
   if (find(predefinedTechAdapter.begin(), predefinedTechAdapter.end(), ta) !=
       predefinedTechAdapter.end())
      return true;
...
vector<ASCString> Research::evalTechAdapter() {
   ...
   for (TechAdapterContainer::iterator i = techAdapterContainer.begin();
      ...
      if ((*i)->available(*this)) {
         ...
         if (find(triggeredTechAdapter.begin(), triggeredTechAdapter.end(), techAdap) ==
             triggeredTechAdapter.end()) {
...
ASCString Research::listTriggeredTechAdapter() const {
   ASCString s;
   for (TriggeredTechAdapter::const_iterator i = triggeredTechAdapter.begin();
        i != triggeredTechAdapter.end(); ++i) {
      s += "#fontsize=14# ";
      s += *i + "\n";
      s += "#fontsize=10#";
      int counter = 0;
      for (TechAdapterContainer::iterator j = techAdapterContainer.begin();
           j != techAdapterContainer.end(); ++j) {
         if ((*j)->getName() == *i) {
            if (counter)
               s += " ----\n";
            ++counter;
...
bool Research::isBlocked(const Technology* tech) const {
   for (int i = 0; i < developedTechnologies.size(); ++i) {
      ...
      if (t && !t->techDependency.available(*this)) {
         ...
         for (Technology::BlockingOtherTechnologies::iterator j =
                 t->blockingOtherTechnologies.begin();
              j != t->blockingOtherTechnologies.end(); ++j)
...
int Research ::getResearchPerTurn() const {
   int rppt = 0;
   for (Player::BuildingList::const_iterator i = map->player[player].buildingList.begin();
        i != map->player[player].buildingList.end(); i++)
      rppt += (*i)->researchpoints;

   for (Player::VehicleList::const_iterator i = map->player[player].vehicleList.begin();
        i != map->player[player].vehicleList.end(); i++)
      rppt += (*i)->researchpoints;
...
Resources returnResourcenUseForResearch(const ContainerBase* bld, int research) {
   ...
   if ((bld->baseType->nominalresearchpoints == 0) ||
       !bld->baseType->hasFunction(ContainerBaseType::Research))
      return res;

   int num = 0;
   /*
   for ( Player::BuildingList::iterator i =
   bld->getMap()->player[bld->getOwner()].buildingList.begin(); i !=
   bld->getMap()->player[bld->getOwner()].buildingList.end(); ++i ) if ( (*i)->typ->special &
   cgresearchb) if ( (*i)->researchpoints > 0 )
            ++num;
   */
   num = 1;
   ...
   for (int r = 0; r < 3; ++r)
      if (bld->baseType->maxplus.resource(r) < 0) {
         if (research > bld->baseType->nominalresearchpoints) {
            float a = -bld->baseType->maxplus.resource(r) /
                      pow(double(bld->baseType->nominalresearchpoints), 2);
            res.resource(r) = int(pow(pow(double(research), 2) * a, 0.98 + double(num) / 50));
         } else
            res.resource(r) = -bld->baseType->maxplus.resource(r) * research /
                              bld->baseType->nominalresearchpoints;
      }
