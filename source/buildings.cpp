...
Building ::Building(GameMap* actmap, const MapCoordinate& _entryPosition, const BuildingType* type,
                    int player, bool setupImages, bool chainToField)
...
bool Building::canRepair(const ContainerBase* item) const {
   return typ->hasFunction(ContainerBaseType::InternalUnitRepair) ||
          typ->hasFunction(ContainerBaseType::ExternalRepair) || (item == this);
}
...
void Building ::convert(int player, bool recursive) {
   ...
   Player::BuildingList::iterator i = find(gamemap->player[oldcol].buildingList.begin(),
                                           gamemap->player[oldcol].buildingList.end(), this);
...
void Building::paintSingleField(Surface& s, SPoint imgpos,
                                BuildingType::LocalCoordinate pos) const {
...
Surface Building::getImage() const {
   ...
   Surface s =
      Surface::createSurface(maxx - minx + 1, maxy - miny + 1, 32, Surface::transparent << 24);
...
int Building ::putResource(int amount, int resourcetype, bool queryonly, int scope, int player) {
   return 0;
};
int Building ::getResource(int amount, int resourcetype, bool queryonly, int scope, int player) {
   return actstorage.resource(resourcetype);
};
int Building ::getAvailableResource(int amount, int resourcetype, int scope) const {
   return actstorage.resource(resourcetype);
};
#endif
...
Building ::~Building() {
   if (gamemap) {
      ...
      Player::BuildingList::iterator i =
         find(gamemap->player[c].buildingList.begin(), gamemap->player[c].buildingList.end(), this);
...
Building* Building::newFromStream(GameMap* gamemap, tnstream& stream, bool chainToField) {
   ...
   Building* bld =
      new Building(gamemap, MapCoordinate(xpos, ypos), typ, color / 8, false, chainToField);
...
void Building ::readData(tnstream& stream, int version) {
   ...
   if ((researchpoints < typ->maxresearchpoints &&
        !typ->hasFunction(ContainerBaseType::Research)) ||
...
void doresearch(GameMap* actmap, int player) {
   ...
   for (Player::BuildingList::iterator bi = actmap->player[player].buildingList.begin();
        bi != actmap->player[player].buildingList.end(); bi++) {
      Building* bld = *bi;
   ...
   for (VRE::iterator i = vre.begin(); i != vre.end(); ++i) {
      ...
      if (got < r)
         fatalError(
            "controls : doresearch : inconsistency in getting energy or material for building");
