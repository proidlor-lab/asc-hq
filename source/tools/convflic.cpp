...
byte nflicview::getflicpal(void) {
   ...
   if ((fliheader.magic != 0xAF11) && (fliheader.magic != 0xAF12))
      return 1;
   if ((fliheader.height > 400) || (fliheader.width > 640))
      return 1;
            ...
            switch (chunktype) {
               ...
               case 7: {  // FLC-DELTA
                  ...
                  for (int d = 0; d < p1; d++) {
                     ...
                     c += 2;
                     // fprintf(flc_test,"Check : C %2.0d X %2.0d Y %2.0d Ch %5.0i Cnt
                     // %5.0d\n",d,x,y,skipcnt,c);
                     if (skipcnt < 0) {
                        y += -skipcnt;
                        d--;
                        ...
                        for (p2 = 0; p2 < skipcnt; p2++) {
                           ...
                           if (szcnt > 0) {
                              ...
                              // Ausgabe von (szcnt ) mal Farb-Werten die im Speicher folgen von
                              // position x ab
                  ...
                  do {
                     ...
                     do {
                        ...
                        if (szcnt >= 0) {
                           ...
                           szcnt = -szcnt;
                           for (pc = x; pc < x + szcnt; pc++)
                              used[(chunkpart[c + pc - x] % 256)] = 1;
                           x += szcnt;
                           c += szcnt;
                        }
...
void showused(void) {
   ...
   for (char a = 0; a <= 15; a++)
      for (char b = 0; b <= 15; b++) {
         if (used[a * 16 + b] == 1)
            bar(x + a * 40, b * 20 + 5, a * 40 + 2 * x, b * 20 + 15, red);
      }
   wait();
}
