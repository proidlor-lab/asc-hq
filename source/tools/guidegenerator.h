...
class GuideGenerator {
  public:
    ...
    *@param setID The set of which the guide is created, if set to 0 guides for all sets will be
    *created
    *@param mainCSSPath  The full path to the used css-file for the pages
    *@param createImg Determines if images shall be created.
    */

   GuideGenerator(ASCString filePath, ASCString menuCSSPath, int setID, ASCString mainCSSPath,
                  ASCString techIDs, bool createImg, ASCString relMenuPath, bool upload = false,
                  int imageSize = 0);
    ...
    */
   bool generatesUpload() const { return createUpload; }
   /*
    *@brief Gets the filePath of the output
    */
   const ASCString getFilePath() const { return filePath; }

   const vector<IntRange>& getTechTreeIDs() const { return techTreeIDs; }
...
class BuildingGuideGen : public GuideGenerator {
  public:
    ...
    *created
    *@param createImg Determines if images shall be created.
    *@param buildingsUnique Determines if the pages for a building shall be created only once or for
    *each of its occurence in the set (different directions, different weather)
    */
   BuildingGuideGen(ASCString filePath, ASCString menuCSSPath, int setID, ASCString mainCSSPath,
                    ASCString techIDs, bool createImg, ASCString relMenuPath,
                    bool buildingsUnique = false, bool upload = false, int imageSize = 0);
   /*
    *@brief Destructor
    */
  ...
  private:
    ...
    *       Necessary, because the file names are created from the ids which are only unique in
    *their domain (e.g. buildingTypes)
    */
...
class UnitGuideGen : public GuideGenerator {
  public:
    ...
    */
   UnitGuideGen(ASCString filePath, ASCString menuCSSPath, int setID, ASCString mainCSSPath,
                ASCString techIDs, bool createImg, ASCString relMenuPath, bool upload = false,
                int imageSize = 0);
   /**
    *@brief Destructor
...
class ImageConverter {
  ...
  private:
    ...
    */
   void convert(const ASCString& fileName, Surface& s, ASCString filePath, int xsize = fieldsizex,
                int ysize = fieldsizey);
};
#endif
