...
CategoryMember::CategoryMember(ASCString label, ASCString css, ASCString file, ASCString t)
   : AbstractEntry(label, css), target(t) {
...
void Category::removeEntry(AbstractEntry* entry) {
   std::list<AbstractEntry*>::iterator result =
      std::find(subEntries.begin(), subEntries.end(), entry);
...
Category::Category(const Category& cat) : AbstractEntry(cat) {
   ...
   for (std::list<AbstractEntry*>::const_iterator i = cat.subEntries.begin();
        i != cat.subEntries.end(); i++) {
      subEntries.push_back((*i)->clone());
   }
}
...
ASCString Category::toString() const {
   ...
   for (std::list<AbstractEntry*>::const_iterator i = subEntries.begin(); i != subEntries.end();
        i++) {
      result += (*i)->toString();
   }
