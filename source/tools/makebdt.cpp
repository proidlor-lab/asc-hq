...
// const char* direcs[8] = { "oben","rechts oben","rechts","rechts unten","unten","links
      ...
      for (i = 0; i < cwettertypennum; i++)
         if (weather & (1 << i)) {
                  ...
                  do {
                     if (pics & (1 << j)) {
                        ...
                        if (src != 1) {
                           ...
                           initgraphics(640, 480, 8);
                           getbi3pict(&bdt->weather[i]->bi_picture[j],
                                      &bdt->weather[i]->picture[j]);
                           num++;
                           settxt50mode();
                        }
                     }
            ...
            if (creat_edit == 0) {
               ...
               bdt->weather[i]->movemalus =
                  (char*) realloc(bdt->weather[i]->movemalus, bdt->weather[i]->movemaluscount);
            };
      ...
      /*
         ...
         if ( nb_edit ) {
            ...
            for ( i = 0; i < 8; i++ ) {
               ...
               bar ( 20 + i * 80, 100, 60 + i * 80, 140, bdt->weather[ weather ]->quickview->dir[ i
         ].p1 );

               for ( int y = 0; y < 3; y ++ ) {
                  if ( y < 1 ) {
                     int x = 1;
                     bar ( 20 + i * 80 + x * 13, 200 + y * 13, 20 + i * 80 + ( x + 1 ) * 13, 200 + (
         y + 1 ) * 13, bdt->weather[ weather ]->quickview->dir[ i ].p3[x][y] ); } else { for (int x
         = 1 - ( 2 - y ) ; x <= 1 + ( 2 - y ) ; x++ ) bar ( 20 + i * 80 + x * 13, 200 + y * 13, 20 +
         i * 80 + ( x + 1 ) * 13, 200 + ( y + 1 ) * 13, bdt->weather[ weather ]->quickview->dir[ i
         ].p3[x][y] );
                  }
               }
               ...
               for (  y = 0; y < 5; y ++ ) {
                  if ( y < 3 ) {
                     for (int x = 2 - y ; x <= 2 + y  ; x++ )
                       bar ( 20 + i * 80 + x * 8, 300 + y * 8, 20 + i * 80 + ( x + 1 ) * 8, 300 + (
         y + 1 ) * 8, bdt->weather[ weather ]->quickview->dir[ i ].p5[x][y] ); } else { for (int x =
         2 - ( 4 - y ) ; x <= 2 + ( 4 - y ) ; x++ ) bar ( 20 + i * 80 + x * 8, 300 + y * 8, 20 + i *
         80 + ( x + 1 ) * 8, 300 + ( y + 1 ) * 8, bdt->weather[ weather ]->quickview->dir[ i
         ].p5[x][y] );
                  }
               }
            }
