      ...
      if (mouseparams.taste == 1)
         for (i = 0; i < 4; i++) {
            for (j = 0; j < 6; j++) {
               ...
               if (mouseparams.x >= xp && mouseparams.x < xp + fieldxsize && mouseparams.y >= yp &&
                   mouseparams.y < yp + fieldysize) {
...
void loadpicture(char* filename, int step) {
      ...
      for (x = 0; x <= 3; x++)
         if (bilder[x][y]) {
            ...
            getimage(100 + fielddistx * x + (y & 1) * fielddisthalfx, 100 + fielddisty * y,
                     99 + fielddistx * x + (y & 1) * fielddisthalfx + fieldsizex,
                     99 + fielddisty * y + fieldsizey, bilder2[x][y]);
            mount(p1, p2, size - 4);
         } else
            bilder2[x][y] = NULL;
   ...
   for (y = 0; y <= 5; y++)
      for (x = 0; x <= 3; x++)
         if (bilder2[x][y]) {
            putspriteimage(250 + fielddistx * x + (y & 1) * fielddisthalfx, 250 + fielddisty * y,
                           bilder2[x][y]);
   ...
   for (int y = 0; y <= 5; y++)
      for (int x = 0; x <= 3; x++)
         if (bld->w_picture[weather][0][x][y])
            putspriteimage(100 + fielddistx * x + fielddisthalfx * (y & 1), 100 + fielddisty * (y),
                           bld->w_picture[weather][0][x][y]);
}
               ...
               if (battleisle) {
                  ...
                  for (i = 0; i < pictnum; i++) {
                     ...
                     do {
                        ...
                        bar(0, 0, 639, 479, 0);
                        getbi3pict_double(&bld->bi_picture[weather][i][col % 8][col / 8],
                                          &bld->w_picture[weather][i][col % 8][col / 8]);
                        bar(0, 0, 639, 479, 0);
                        showbld();
