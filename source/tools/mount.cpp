...
int rlemain(const char* argv1, const char* argv2) {
   ...
   int bytes_read, count, eof, first_time, i;
...
int bzmain(const char* argv1, const char* argv2) {
...
void testcompress(const char* name, int size) {
   if (strcmp(name, "temp.mzl") == 0 || strcmp(name, "temp.rle") == 0 ||
...
void processFiles(const char* files, const char* orgWorkingDir) {
   ...
   if (dirp != NULL) {
      for (;;) {
         ...
         if (patimat(filename, direntp->d_name, true) && strcmp(direntp->d_name, ".") != 0 &&
...
int main(int argc, char* argv[]) {
   ...
   if (verbose) {
      ...
      // uncompsize, filesize ( argv[2] ) - compsize );
