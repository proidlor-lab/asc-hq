...
int main(int argc, char* argv[]) {
   ...
   try {
                     ...
                     if (bdt->weather[i]->pict)
                        if (bdt->weather[i]->bi_pict >= 0) {
                           ...
                           if ((i == 0) || (bdt->weather[0]->bi_pict < 0)) {
                              sprintf(bipict[n].entry[t].text, "%s (%d)", bdt->filename.c_str(),
                                      bdt->id);
                              bipict[n].entry[t].color = 1;
                           } else {
                              sprintf(bipict[n].entry[t].text, "-> %d ; w=%d",
                                      bdt->weather[0]->bi_pict, i);
                              bipict[n].entry[t].color = 1;
                           }
                           fprintf(fp, "%d ", n);
                        }
                  ...
                  if (obj->weather.test(w))
                     for (int i = 0; i < obj->weatherPicture[w].images.size(); i++)
                        if (obj->weatherPicture[w].bi3pic[i] >= 0) {
                           ...
                           sprintf(bipict[n].entry[t].text, "%s (%d)", obj->filename.c_str(),
                                   obj->id);
                        ...
                        for (int l = 0; l < 6; l++)
                           if (bld->w_picture[i][j][k][l])
                              if (bld->bi_picture[i][j][k][l] >= 0) {
                                 ...
                                 sprintf(bipict[n].entry[t].text, "%s (%d)", bld->filename.c_str(),
                                         bld->id);
                                 sprintf(bipict[n].entry[t + 1].text, "    W=%d #%d X=%d Y=%d", i,
                                         j, k, l);
         ...
         if (v) {
            ...
            if (usage)
               for (int j = 0; j < bipict[i].textnum; j++) {
                  ...
                  showtext2(bipict[i].entry[j].text,
                            x0 + (i % colnum) * xd + (scale ? fieldsizex : fieldsizex / 2) + 5,
      ...
      writepcx(outputfilename, 0, 0, agmp->resolutionx - 1,
               yp + yd + (activefontsettings.font->height + 2) * (lastlinenum + 1), pal);
      printf("%s written\n", outputfilename);
