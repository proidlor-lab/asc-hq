...
struct tovehicletype { /*  vehicleart: z.B. Schwere Fu·truppe  */
   ...
   char* infotext;     /* optional, kann sehr ausfÅhrlich sein. Empfehlenswert Åber eine Textdatei
                          einzulesen */
   ...
   int terrainkill;    /* falls das aktuelle field nicht befahrbar ist, und bei field->typ->art eine
                          dieser Bits gesetzt ist, verschwindet die vehicle */
   ...
   char loadcapability; /*  BM     CHoehenStufen   die zu ladende vehicle mu· sich auf einer dieser
                           Hîhenstufen befinden */
   char loadcapabilityreq; /*  eine vehicle, die geladen werden soll, mu· auf eine diese Hîhenstufen
                              kommen kînnen */
   char loadcapabilitynot; /*  eine vehicle, die auf eine dieser Hîhenstufen kann, darf NICHT
                              geladen werden. Beispiel: Flugzeuge in Transportflieger */
   Word id;
   int tank;
   ...
   char movemalustyp; /*  wenn ein Bodentyp mehrere Movemali fÅr unterschiedliche vehiclearten, wird
                         dieser genommen.  <= cmovemalitypes */
   tthreatvar generalthreatvalue; /*  Wird von ArtInt benîtigt, au·erhalb keine Bedeutung  */
   tthreatvar threatvalue[8];     /*  dito                                                 */
   char classnum; /* Anzahl der Klassen, max 8, min 0 ;  Der EINZIGE Unterschied zwischen 0 und 1
                     ist der NAME ! */
   char* classnames[8];       /* Name der einzelnen Klassen */
   tclassbound classbound[8]; /* untergrenze (minimum), die zum erreichen dieser Klasse notwendig
                                 ist, classbound[0] gilt fÅr vehicletype allgemein*/
   char maxwindspeedonwater;
   // char         digrange;        // Radius, um den nach bodenschÑtzen gesucht wird. 0 bedeutet,
   // es mu· manuell ein field ausgewÑhlt werden int          dummy[16];
};
