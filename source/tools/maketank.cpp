...
const char* bildnr[8] = {" 0   ( unit facing up ) ",    " 1   ( unit facing up and right ) ",
                         " 2   ( unit facing right ) ", " 3   ( unit facing right and down ) ",
                         " 4   ( unit facing down ) ",  " 5   ( unit facing down and left ) ",
                         " 6   ( unit facing left ) ",  " 7   ( unit facing left and up ) "};
...
main(int argc, char* argv[]) {
   ...
   try {
...

      {
         ...
         printf(
            "\n    energy tank  ( ONLY for generators !!!!! Every other unit must have a energy "
            "tank of 0)  : ");
         ...
         if (muned == 0) {
            ...
            while (i != ft->weapons->count) {
               ...
               printf("\n    min distance : \n");
               if (creat_edit == 0)
                  ft->weapons->weapon[i].mindistance = 8;
               ...
               if (change) {
                  printf(
                     ...
                     "height differences: shooting 'down', positive is shooting 'up'\n");
                  for (int k = 0; k < 13; k++) {
                     printf(" \n height difference = %d \n", k - 6);
               ...
               bitselect(ft->weapons->weapon[i].targets_not_hittable, cmovemalitypes,
                         cmovemalitypenum);
         ...
         if (ft->functions & (cfautodigger | cfmanualdigger)) {
            printf(
               "\n    radius to check for resources : \n The radius is specified in 'number of "
         ...
         if (ft->weapons->count > 0)
            radius = 16200 / ft->weapons->weapon[0].maxdistance /
                     (ft->weapons->weapon[0].maxstrength + 1);
         else
            radius = 1;
      ...
      if (ft->classnum || start == 0) {
         ...
         for (i = start; i <= end; i++) {
            ...
            for (int j = 0; j < waffenanzahl; j++) {
               ...
               if (num > 0) {
                  printf(
                     "\n      needed weapon improvement for %s \n     ( base is 1024; 2048 would "
                     "be doubled strength requiered )\n",
                     cwaffentypen[j]);
