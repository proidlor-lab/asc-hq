      ...
      for (int i = cl.next_param(); i < argc; i++) {
         for (int j = 0; j < vehicletypenum; j++)
            if (patimat(argv[i], getvehicletype_forpos(j)->filename.c_str())) {
               ...
               tn_file_buf_stream stream(extractFileName_withoutSuffix(vt->filename) +
                                            "_generated.asctxt",
                                         tnstream::writing);
         ...
         for (int j = 0; j < objecttypenum; j++)
            if (patimat(argv[i], getobjecttype_forpos(j)->filename.c_str())) {
               ...
               tn_file_buf_stream stream(extractFileName_withoutSuffix(ot->filename) +
                                            "_generated.asctxt",
                                         tnstream::writing);
         ...
         for (int j = 0; j < terraintypenum; j++)
            if (patimat(argv[i], getterraintype_forpos(j)->filename.c_str())) {
               ...
               tn_file_buf_stream stream(extractFileName_withoutSuffix(tt->filename) +
                                            "_generated.asctxt",
                                         tnstream::writing);
         ...
         for (int j = 0; j < buildingtypenum; j++)
            if (patimat(argv[i], getbuildingtype_forpos(j)->filename.c_str())) {
               ...
               tn_file_buf_stream stream(extractFileName_withoutSuffix(bt->filename) +
                                            "_generated.asctxt",
                                         tnstream::writing);
