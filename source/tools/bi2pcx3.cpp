...
int main(int argc, char* argv[]) {
   ...
   for (int i = 1; i < argc; i++) {
      if (argv[i][0] == '/' || argv[i][0] == '-') {
         if (strcmpi(&argv[i][1], "WIDE") == 0)
            wide = 1;
         else if (strcmpi(&argv[i][1], "INDEX") == 0)
            index = 1;
         else if (strcmpi(&argv[i][1], "verbosity") == 0)
            verbosity = 1;
         else if ((strcmpi(&argv[i][1], "?") == 0) || (strcmpi(&argv[i][1], "h") == 0)) {
            printf(
               " Parameters: \n"
               "     /h          this page\n"
               "     /index      output index\n"
   ...
   try {
                     ...
                     if (bdt->weather[i]->picture[j])
                        if (bdt->weather[i]->bi_picture[j] >= 0) {
                           ...
                           if ((i == 0) || (bdt->weather[0]->bi_picture[0] < 0)) {
                              ...
                              sprintf(bipict[n].entry[t].text, "-> %d ; w=%d",
                                      bdt->weather[0]->bi_picture[0], i);
                              bipict[n].entry[t].color = 1;
                           }
                        }
                     ...
                     for (int l = 0; l < 6; l++)
                        if (bld->w_picture[i][j][k][l])
                           if (bld->bi_picture[i][j][k][l] >= 0) {
                              ...
                              sprintf(bipict[n].entry[t + 1].text, "    W=%d #%d X=%d Y=%d", i, j,
                                      k, l);
      ...
      if (wide)
         line(agmp->resolutionx / 2 - 3, 0, agmp->resolutionx / 2 - 3, agmp->resolutiony - 1,
              black);
      ...
      for (i = 0; i < bi3graphnum; i++) {
         ...
         if (v) {
            ...
            for (int j = 0; j < bipict[i].textnum; j++) {
               ...
               showtext2(bipict[i].entry[j].text, x0 + (i % colnum) * xd + fieldsizex / 2 + 5,
                         yp + j * (activefontsettings.font->height + 2));
      ...
      writepcx(outputfilename, 0, 0, agmp->resolutionx - 1,
               yp + fieldsizey / 2 + (activefontsettings.font->height + 2) * (lastlinenum + 1),
               pal);
      printf("%s written\n", outputfilename);
