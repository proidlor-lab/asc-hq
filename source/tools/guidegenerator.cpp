...
void ImageConverter::convert(const ASCString& fileName, Surface& s, ASCString filePath, int xsize,
                             int ysize) {
...
GuideGenerator::GuideGenerator(ASCString fp, ASCString menuCss, int id, ASCString maCSSFile,
                               ASCString techIDs, bool imgCreate, ASCString relativeMenuPath,
                               bool upload, int imageSize)
   : filePath(fp),
     menuCSSFile(menuCss),
     mainCSSFile(maCSSFile),
     createImg(imgCreate),
     setID(id),
     imageWidth(imageSize),
     createUpload(upload),
     techTreeIDs(String2IntRangeVector(techIDs)),
     relMenuPath(relativeMenuPath) {
...
BuildingGuideGen::BuildingGuideGen(ASCString fp, ASCString css, int id, ASCString maCSSFile,
                                   ASCString techIDs, bool imgCreate, ASCString relativeMenuPath,
                                   bool bUnique, bool upload, int imageSize)
   : GuideGenerator(fp, css, id, maCSSFile, techIDs, imgCreate, relativeMenuPath, upload),
     buildingsUnique(bUnique) {}
   ...
   try {
      for (std::vector<SingleUnitSet*>::iterator i = unitSets.begin(); i != unitSets.end(); i++) {
         ...
         if ((s->ID == setID) || (setID <= 0)) {
            ASCString fileName =
               InfoPageUtil::getTmpPath() + "buildingset_id(" + strrr(s->ID) + ")" + GROUPS;
            ...
            for (int building = 0; building < buildingTypeRepository.getNum(); building++) {
               ...
               if (s->isMember(bt->id, SingleUnitSet::building) &&
                   (processedBuildingIds.find(bt->id) != processedBuildingIds.end())) {
                  ASCString fileLink =
                     relMenuPath + strrr(bt->id) + APPENDIX + ASCString(MAINLINKSUFFIX) + HTML;
                  CategoryMember* dataEntry =
                     new CategoryMember(bt->name.toUpper(), menuCSSFile, fileLink, TARGET);
                  ...
                  CategoryMember* idEntry =
                     new CategoryMember(ASCString(strrr(bt->id)) + "(" + bt->name + ")",
                                        menuCSSFile, fileLink, TARGET);
...
void BuildingGuideGen::processSubjects() {
   ...
   for (int building = 0; building < buildingTypeRepository.getNum(); building++) {
      BuildingType* bt = buildingTypeRepository.getObject_byPos(building);
      if (setID > 0) {
         for (std::vector<SingleUnitSet*>::iterator i = unitSets.begin(); i != unitSets.end();
              i++) {
            SingleUnitSet* s = (*i);
            if ((s->ID == setID) && (s->isMember(bt->id, SingleUnitSet::building))) {
               if (buildingNames.end() == buildingNames.find(strrr(s->ID) + bt->name) ||
                   (!buildingsUnique)) {
                  processBuilding(*bt);
...
// UnitGuideGen****************************************************************************************************
UnitGuideGen::UnitGuideGen(ASCString fp, ASCString meCSSFile, int id, ASCString maCSSFile,
                           ASCString techIDs, bool imgCreate, ASCString relativeMenuPath,
                           bool upload, int imageSize)
   : GuideGenerator(fp, meCSSFile, id, maCSSFile, techIDs, imgCreate, relativeMenuPath, upload,
                    imageSize) {}
...
void UnitGuideGen::generateCategories() const {
   try {
      for (std::vector<SingleUnitSet*>::iterator i = unitSets.begin(); i != unitSets.end(); i++) {
         ...
         if ((s->ID == setID) || (setID <= 0)) {
            ASCString fileName =
               InfoPageUtil::getTmpPath() + "unitset_id(" + strrr(s->ID) + ")" + GROUPS;
            Category set(s->name, menuCSSFile);
            ...
            for (int unit = 0; unit < vehicleTypeRepository.getNum(); unit++) {
               ...
               if (s->isMember(vt->id, SingleUnitSet::unit)) {
                  ASCString fileLink =
                     relMenuPath + strrr(vt->id) + APPENDIX + ASCString(MAINLINKSUFFIX) + HTML;

                  CategoryMember* dataEntry =
                     new CategoryMember(vt->name.toUpper(), menuCSSFile, fileLink, TARGET);
                  ...
                  CategoryMember* idEntry =
                     new CategoryMember(ASCString(strrr(vt->id)) + "(" + vt->name + ")",
                                        menuCSSFile, fileLink, TARGET);
                  idCat->addEntry(idEntry);
               }
            }
