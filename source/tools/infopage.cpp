...
InfoPage::InfoPage(const ContainerBaseType& baseType, ASCString fn, ASCString fp, ASCString in,
                   ASCString title, GuideGenerator* gen)
   : fileName(fn),
     filePath(fp),
     index(in),
     pageTitle(title),
     cbt(baseType),
     generator(gen),
     fullFileName(filePath + fileName + index + HTML) {
...
ASCString InfoPage::getHeightImgString(int height) {
   ...
   for (int j = 0; j < 8; j++)
      if (height & (1 << j)) {
         ASCString s2;
         s2 = constructImageLink(STDGFXPATH + ASCString("hoehe") + strrr(j) + ASCString(".gif"),
                                 choehenstufen[j]);
         s += s2;
      }
   return s;
...
void InfoPage::createHTMLHeader(ASCString cssPath) {
   *buildingInfStream << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN\" "
                         "\"http://www.w3.org/TR/html4/strict.dtd\">"
                      << endl;
   *buildingInfStream << "<html>" << endl;
   *buildingInfStream << "<HEAD>" << endl;
   *buildingInfStream << "<meta http-equiv=\"content-type\" content=\"text/html; "
                         "charset=ISO-8859-1\"><TITLE>ASC-Document</TITLE>"
                      << endl;
   *buildingInfStream << "<LINK REL=\"stylesheet\" TYPE=\"text/css\"";
   (cssPath == "") ? *buildingInfStream << "HREF=\"" << CSSPATH << "\""
                   : *buildingInfStream << "HREF=\"" << cssPath << "\"";
...
void InfoPage::startTable(int border, TableWidth interpretWAs, int width, TableWidth interpretHAs,
                          int height, TableFrame frame, ASCString cssClass, const IntVec* cols) {
...
void InfoPage::addTREntry(ASCString title, const ASCString& data) {
   *buildingInfStream << "<tr class=\"wg\">" << "<th class=\"wg\">" << title << "</th>"
                      << "<td class=\"wg\">" << data << "</td>" << "</tr>" << endl;
}
...
void InfoPage::addTREntry(const ASCString& text) {
   *buildingInfStream << "<tr class=\"wg\">" << "<td class=\"wg\">" << text << "</td>" << "</tr>"
                      << endl;
}
...
ASCString InfoPage::constructImageLink(ASCString ref, ASCString altText, ASCString cssClass,
                                       int width) {
...
BuildingInfoPage::BuildingInfoPage(const BuildingType& buildingType, ASCString filePath,
                                   ASCString index, ASCString title, BuildingGuideGen* generator)
   : InfoPage(buildingType, BuildingGuideGen::constructFileName(buildingType), filePath, index,
              title, generator),
     bt(buildingType) {}
...
BuildingMainPage::BuildingMainPage(const BuildingType& bt, ASCString filePath,
                                   BuildingGuideGen* gen)
   : BuildingInfoPage(bt, filePath, MAINLINKSUFFIX, TITLE, gen) {}
...
//*******************************************************************************************
BuildingTerrainPage::BuildingTerrainPage(const BuildingType& bt, ASCString fp,
                                         BuildingGuideGen* gen)
   ...
   for (int i = 0; i < terrainPropertyNum; i++) {
      ...
      if ((bt.terrainaccess.terrain.test(i)) || (bt.terrainaccess.terrainnot.test(i)) ||
         ...
         if (bt.terrainaccess.terrainnot.test(i)) {
            addTREntry(terrain,
                       "</td> <td class=\"wg\">" +
                          constructImageLink(STDGFXPATH + ASCString("hakenrot.gif"), "YES") +
                          "<td class=\"wg\">");
         }
         if (bt.terrainaccess.terrainkill.test(i)) {
            addTREntry(terrain,
                       "</td> <td class=\"wg\"></td> <td class=\"wg\">" +
                          constructImageLink(STDGFXPATH + ASCString("hakenrot.gif"), "YES"));
         }
      }
   }
...
//*********************************************************************************************
BuildingCargoPage::BuildingCargoPage(const BuildingType& bt, ASCString filePath,
                                     BuildingGuideGen* generator)
         ...
         for (ContainerBaseType::EntranceSystems::const_iterator i = cbt.entranceSystems.begin();
            ...
            for (int j = 0; j < ContainerBaseType::functionNum; ++j)
               if (i->requiresUnitFeature.test(j)) {
                  funcs = addTREntryln(funcs, ContainerBaseType::getFunctionName(
                                                 ContainerBaseType::ContainerFunctions(j)));
                  none = false;
               }
...
//*********************************************************************************************
BuildingResourcePage::BuildingResourcePage(const BuildingType& bt, ASCString fp,
                                           BuildingGuideGen* gen)
...
//*********************************************************************************************
UnitInfoPage::UnitInfoPage(const VehicleType& vType, ASCString filePath, ASCString index,
                           ASCString title, UnitGuideGen* generator)
   : InfoPage(vType, UnitGuideGen::constructFileName(vType), filePath, index, title, generator),
     vt(vType) {}
...
//******************************************************************************************************
UnitMainPage::UnitMainPage(const VehicleType& vtype, ASCString filePath, UnitGuideGen* generator)
   : UnitInfoPage(vtype, filePath, MAINLINKSUFFIX, TITLE, generator) {}
...
void UnitMainPage::addCapabilities() {
   ...
   for (int j = 0; j < ContainerBaseType::functionNum; ++j)
      if (vt.hasFunction(ContainerBaseType::ContainerFunctions(j)))
         cap = addTREntryln(
            cap, ContainerBaseType::getFunctionName(ContainerBaseType::ContainerFunctions(j)));
...
UnitTerrainPage::UnitTerrainPage(const VehicleType& vt, ASCString filePath, UnitGuideGen* generator)
   : UnitInfoPage(vt, filePath, MOVEMENTLINKSUFFIX, TITLE, generator) {}
...
void UnitTerrainPage::buildContent() {
   ...
   addTREntry("Range", vt.fuelConsumption
                          ? strrr(vt.getStorageCapacity(0).fuel / vt.fuelConsumption)
                          : "None");
   endTable();
   addHeadline("Speed", 4);
   ...
   *buildingInfStream << "<tr>";
   ...
   for (int i = 0; i < choehenstufennum; i++) {
      addTDEntry(
         constructImageLink(STDGFXPATH + ASCString("hoehe") + strrr(i) + ".gif", choehenstufen[i]));
   }
   *buildingInfStream << "</tr>";
   *buildingInfStream << "<tr>";
   addTitle("max. Speed");
   ...
   *buildingInfStream << "</tr>" << endl;
   for (int i = 0; i < terrainPropertyNum; i++) {
      ...
      if (vt.terrainaccess.terrain.test(i) && vt.terrainaccess.terrainreq.test(i)) {
         addTREntry(terrain,
                    constructImageLink(STDGFXPATH + ASCString("haken.gif"), "YES") +
                       "</td><td class=\"wg\">" +
                       constructImageLink(STDGFXPATH + ASCString("exkl.gif"), "YES") +
         ...
         if (vt.terrainaccess.terrainreq.test(i)) {
            addTREntry(terrain, "</td><td class=\"wg\">" +
                                   constructImageLink(STDGFXPATH + ASCString("exkl.gif"), "YES") +
                                   "<td class=\"wg\"></td><td class=\"wg\">");
         }
      }
      if (vt.terrainaccess.terrainnot.test(i)) {
         addTREntry(terrain, "</td> <td class=\"wg\"> </td><td class=\"wg\">" +
                                constructImageLink(STDGFXPATH + ASCString("hakenrot.gif"), "YES") +
                                "<td class=\"wg\">");
      }
      if (vt.terrainaccess.terrainkill.test(i)) {
         addTREntry(terrain,
                    "</td> <td class=\"wg\"></td> <td class=\"wg\"></td> <td class=\"wg\">" +
                       constructImageLink(STDGFXPATH + ASCString("hakenrot.gif"), "YES"));
      }
   }
   endTable();
            ...
            *buildingInfStream << "<tr>";
            for (int h = 7; h >= 0; h--) {
               if (vt.heightChangeMethod[w].startHeight & (1 << h) & vt.height) {
                  entry = addTREntryln(
                     entry, constructImageLink(STDGFXPATH + ASCString("hoehe") + strrr(h) + ".gif",
                                               choehenstufen[w]));
               }
            }
            addTDEntry(entry);
...
//******************************************************************************************************
UnitWeaponPage::UnitWeaponPage(const VehicleType& vt, ASCString filePath, UnitGuideGen* generator)
   : UnitInfoPage(vt, filePath, WEAPONLINKSUFFIX, TITLE, generator) {}
      ...
      for (int w = 0; w < vt.weapons.count; w++) {
         ...
         for (int h = 0; h < 8; h++)
            if (vt.weapons.weapon[w].targ & (1 << h)) {
               ASCString s;
               s = addTREntryln(
                  s, (constructImageLink((STDGFXPATH + ASCString("hoehe") + strrr(h) + ".gif"),
                                         choehenstufen[h])));
               hstring += s;
            } /*else {
                                                                                                                                            ASCString s;
...
//******************************************************************************************************
UnitConstructionPage::UnitConstructionPage(const VehicleType& vt, ASCString filePath,
                                           UnitGuideGen* generator)
   : UnitInfoPage(vt, filePath, CONSTRUCTIONLINKSUFFIX, TITLE, generator) {}
   ...
   if ((vt.objectsBuildable.size() == 0) && (vt.objectsRemovable.size() == 0) &&
      ...
      for (unsigned int i = 0; i < vt.objectsBuildable.size(); i++) {
         for (int b = 0; b < objectTypeRepository.getNum(); b++) {
            ...
            if (obj->id >= vt.objectsBuildable[i].from && obj->id <= vt.objectsBuildable[i].to) {
               names =
                  addTREntryln(names, obj->name.c_str() + ASCString("(") + strrr(obj->id) + ")");
            }
         }
      }
      if (!names.empty()) {
      ...
      for (unsigned int i = 0; i < vt.objectsRemovable.size(); i++) {
         for (int b = 0; b < objectTypeRepository.getNum(); b++) {
            ObjectType* obj = objectTypeRepository.getObject_byPos(b);
            if (obj->id >= vt.objectsRemovable[i].from && obj->id <= vt.objectsRemovable[i].to) {
               ASCString s;
               names =
                  addTREntryln(names, obj->name.c_str() + ASCString("(") + strrr(obj->id) + ")");
            }
      ...
      names = "";
      for (vector<IntRange>::const_iterator i = vt.objectGroupsRemovable.begin();
           i != vt.objectGroupsRemovable.end(); ++i) {
         if (i->from != i->to) {
      ...
      names = "";
      for (vector<IntRange>::const_iterator i = vt.objectGroupsBuildable.begin();
           i != vt.objectGroupsBuildable.end(); ++i) {
         if (i->from != i->to) {
            for (int k = i->from; k < i->to; k++) {
               names = addTREntryln(names, strrr(k));
            }
      ...
      for (unsigned int i = 0; i < vt.buildingsBuildable.size(); i++) {
         for (int b = 0; b < buildingTypeRepository.getNum(); b++) {
            ...
            if (bld->id >= vt.buildingsBuildable[i].from &&
               ...
               names = addTREntryln(
                  names,
                  constructLink(bld->name.c_str() + ASCString("(") + strrr(bld->id) + ")",
                                RELATIVEBUILDINGSPATH + BuildingGuideGen::constructFileName(*bld) +
                                   MAINLINKSUFFIX + HTML));
            }
         }
      }
      ...
      for (unsigned int i = 0; i < vt.vehiclesBuildable.size(); i++) {
         for (int b = 0; b < vehicleTypeRepository.getNum(); b++) {
            ...
            if (veh->id >= vt.vehiclesBuildable[i].from && veh->id <= vt.vehiclesBuildable[i].to) {
               names = addTREntryln(
                  names, constructLink(veh->name.c_str() + ASCString("(") + strrr(veh->id) + ")",
                                       "./" + UnitGuideGen::constructFileName(*veh) +
                                          MAINLINKSUFFIX + HTML));
            }
         }
      }
...
//******************************************************************************************************
UnitResearchPage::UnitResearchPage(const VehicleType& vt, ASCString filePath,
                                   UnitGuideGen* generator)
   : UnitInfoPage(vt, filePath, RESEARCHLINKSUFFIX, TITLE, generator) {}
...
void UnitResearchPage::buildContent() {
   ...
   *buildingInfStream << "<tr>" << endl;
   ...
   ASCString sysCommand =
      "dot " + dotFile + "  -Tpng -Gcharset=latin1 -o" + tempTechFileName + ".png";
   ...
   addLink(constructImageLink(RELATIVEIMGPATH + techedFileName + ".png", "TECH-TREE", "techtree"),
           techedFileName + HTML, "", "External");
   endTD();
...
//******************************************************************************************************
BuildingResearchPage::BuildingResearchPage(const BuildingType& bt, ASCString filePath,
                                           BuildingGuideGen* generator)
   : BuildingInfoPage(bt, filePath, RESEARCHLINKSUFFIX, TITLE, generator) {}
...
//***************************************************************************************************************
TechTreePage::TechTreePage(const ContainerBaseType& cbt, ASCString fn, ASCString filePath,
                           GuideGenerator* generator, ASCString tPPath)
