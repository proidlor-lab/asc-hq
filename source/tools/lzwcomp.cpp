...
template <class T>
void tnlzwstream<T>::LZWOut(CodeType code) {
...
template <class T>
IndexType tnlzwstream<T>::LZWFind(CodeType currcode, int in) {
   IndexType ndx;
   ...
   for (;;) {
      if (wdictionary[ndx].code == UNUSED_CODE)
         break;
      if (wdictionary[ndx].parent == currcode && wdictionary[ndx].c == in)
         break;
      ndx += step;
...
template <class T>
void tnlzwstream<T>::initreading(void) {
...
template <class T>
void tnlzwstream<T>::initwriting(void) {
   if (mode == none) {
...
template <class T>
tnlzwstream<T>::tnlzwstream(void) {
   mode = none;
   initread = 0;
   initwrite = 0;
   DecodeBufferSize = 0;
...
template <class T>
void tnlzwstream<T>::writedata(const void* buf, int size) {
   if (!initwrite)
      initwriting();
      ...
      while (pos < size) {
         ...
         if (wdictionary[idx].code == UNUSED_CODE) {
            ...
            if (LZWCurrBytesIn >= 10000) {
               unsigned ratio;
               ratio = (LZWCurrBytesOut * 100) / LZWCurrBytesIn;

               LZWCurrBytesIn = 0;
               LZWCurrBytesOut = 0;
               if (ratio > LZWBestRatio) {
                  if (ratio > 50 && (ratio > 90 || ratio > LZWLastRatio + 10)) {
...
template <class T>
void tnlzwstream<T>::LZWIn(void) {
   T::readdata(&incode, sizeof(CodeType));
}

/* the active decompression routine */
template <class T>
unsigned tnlzwstream<T>::LZWLoadBuffer(unsigned count, CodeType code) {
   if (code >= freecode)
...
template <class T>
void tnlzwstream<T>::readdata(void* buf, int size) {
   if (!initread)
      initreading();
