...
main(int argc, char* argv[]) {
   ...
   try {
      ...
      sprintf(buf, " %12.12s %5.5s %6.6s %6.6s %3.3s %7.7s ", "description", "armor", "p_energy",
              "p_material", "id", "fuel");
      ...
      sprintf(buf, "%4.4s %4.4s %2.2s %3.3s %3.3s %3.3s %2.2s %4.4s %4.4s\n\n", "dist",
              "f_consumpt", "ascent", "view", "jamming", "weight", "wait", "load", "maxunitweight");
      strcat(header[1], buf);
      ...
      while (!cn.empty()) {
         ...
         sprintf(buf, " %12.12s %5d %6d %6d %3d %7d ", ft->description, ft->armor,
                 ft->productionCost.energy, ft->productionCost.material, ft->id, ft->tank);
         strcat(text[num], buf);
         ...
         sprintf(buf, "%4d %4d %2d %3d %3d %3d %2d %4d %4d\n", dist, ft->fuelConsumption,
                 ft->steigung, ft->view, ft->jamming, ft->weight, ft->wait, ft->loadcapacity,
                 ft->maxunitweight);
         strcat(text[num], buf);
