...
class SourcePixelSelector_Plain {
  ...
  protected:
   ...
   int getSourcePixelSkip() { return 0; };
  ...
  public:
   ...
   TargetPixelType convert(SourcePixelType sp) {
      ...
      if (srcHasColorKey && (sp & srcColorMask) == srcColorKey)
         ...
         return truecolor2pal_table[((sp >> rshift) & 0x3f) + (((sp >> gshift) & 0x3f) << 6) +
                                    (((sp >> bshift) & 0x3f) << 12)];
#else
      return destColorKey;
#endif
  ...
  public:
   ColorConverter(const Surface& sourceSurface, Surface& targetSurface) {
      ...
      if (targetSurface.GetPixelFormat().Amask())
         ...
         ashift = getFirstBit(~(targetSurface.GetPixelFormat().Rmask() |
                                targetSurface.GetPixelFormat().Gmask() |
                                targetSurface.GetPixelFormat().Bmask()));
   };
   ...
   TargetPixelType convert(SourcePixelType sp) {
      if (sp == 0xff) {
         ...
         return TargetPixelType(palette[sp].r << rshift) +
                ...
                TargetPixelType(palette[sp].b << bshift) + a;
      }
   };
};
...
class MegaBlitter : public SourceColorTransform<BytesPerSourcePixel>,
  ...
  public:
   MegaBlitter(){};
   MegaBlitter(const SourceColorTransform<BytesPerSourcePixel>& scm,
               const ColorMerger<BytesPerTargetPixel>& cm,
               const SourcePixelSelector<BytesPerSourcePixel>& sps, const TargetPixelSelector& tps)
   ...
   void initSource(const Surface& src) { SourcePixelSelector<BytesPerSourcePixel>::init(src); }
   ...
   void blit(const Surface& src, Surface& dst, SPoint dstPos) {
      ...
      ColorMerger<BytesPerTargetPixel>::init(BytesPerSourcePixel == BytesPerTargetPixel ? src
                                                                                        : dst);
      ...
      for (int y = 0; y < h; ++y) {
         for (int x = 0; x < w; ++x) {
            ...
            if (s == 0) {
               ColorMerger<BytesPerTargetPixel>::assign(
                  colorConverter.convert(SourceColorTransform<BytesPerSourcePixel>::transform(
                     SourcePixelSelector<BytesPerSourcePixel>::nextPixel())),
                  pix);
               ++pix;
            } else {
               if (s > 0) {
...
template <template <int> class SourceColorTransform, template <int> class ColorMerger,
          template <int> class SourcePixelSelector, class TargetPixelSelector,
          typename SourceColorTransformParameter, typename ColorMergerParameter,
          typename SourcePixelSelectorParameter, typename TargetPixelSelectorParameter>
void megaBlitter(const Surface& src, Surface& dst, const SPoint& pos,
   ...
   switch (src.GetPixelFormat().BytesPerPixel()) {
      case 1: {
         switch (dst.GetPixelFormat().BytesPerPixel()) {
            case 1: {
               MegaBlitter<1, 1, SourceColorTransform, ColorMerger, SourcePixelSelector,
                           TargetPixelSelector>
                  blitter((SourceColorTransform<1>) (scmp), (ColorMerger<1>) (cmp),
                          (SourcePixelSelector<1>) (spsp), TargetPixelSelector(tpsp));
               blitter.blit(src, dst, pos);
            } break;
            case 3:
            case 4: {
               MegaBlitter<1, 4, SourceColorTransform, ColorMerger, SourcePixelSelector,
                           TargetPixelSelector>
                  blitter((SourceColorTransform<1>) (scmp), (ColorMerger<4>) (cmp),
                          (SourcePixelSelector<1>) (spsp), TargetPixelSelector(tpsp));
               blitter.blit(src, dst, pos);
            } break;
         };
      ...
      case 4: {
         switch (dst.GetPixelFormat().BytesPerPixel()) {
            case 1: {
               MegaBlitter<4, 1, SourceColorTransform, ColorMerger, SourcePixelSelector,
                           TargetPixelSelector>
                  blitter((SourceColorTransform<4>) (scmp), (ColorMerger<1>) (cmp),
                          (SourcePixelSelector<4>) (spsp), TargetPixelSelector(tpsp));
               blitter.blit(src, dst, pos);
            } break;
            case 3:
            case 4: {
               MegaBlitter<4, 4, SourceColorTransform, ColorMerger, SourcePixelSelector,
                           TargetPixelSelector>
                  blitter((SourceColorTransform<4>) (scmp), (ColorMerger<4>) (cmp),
                          (SourcePixelSelector<4>) (spsp), TargetPixelSelector(tpsp));
               blitter.blit(src, dst, pos);
            } break;
         };
...
class ColorTransform_XLAT {
  ...
  public:
   void setTranslationTable(const Uint8* translationTable) { table = translationTable; };
...
class ColorMerger_ColoredOverwrite {
  ...
  public:
   void init(const Surface& dst) const { transformedColor = dst.GetPixelFormat().MapRGB(col); };
  ...
  protected:
   ...
   void init(const Surface& srf) {
      if (srf.flags() & SDL_SRCCOLORKEY) {
         ...
         if (pixelsize > 1)
            mask = srf.GetPixelFormat().Rmask() | srf.GetPixelFormat().Gmask() |
                   srf.GetPixelFormat().Bmask();
      } else if (pixelsize == 1)
         colorKey = 0xff;
      else
         colorKey = 0xfefefe00;
   ...
   bool isOpaque(PixelType src) { return (src & mask) != colorKey; }
};
  ...
  protected:
   ...
   void init(const Surface& srf) {
      if (srf.flags() & SDL_SRCCOLORKEY) {
         hasColorKey = true;
         colorKey = srf.GetPixelFormat().colorkey();
         ckmask = srf.GetPixelFormat().Rmask() | srf.GetPixelFormat().Gmask() |
                  srf.GetPixelFormat().Bmask();
      } else {
         if (srf.GetPixelFormat().Amask()) {
            ...
            amask = ~(srf.GetPixelFormat().Rmask() | srf.GetPixelFormat().Gmask() |
                      srf.GetPixelFormat().Bmask());
   ...
   void assign(PixelType src, PixelType* dest) {
      if (hasColorKey) {
         ...
         if (alpha == PixelType(Surface::opaque))
            *dest = src;
         else if (alpha != PixelType(Surface::transparent)) {
            // copied from SDL
...
class ColorMerger_AlphaShadow<4> : public ColorMerger_AlphaHandler<4> {
  ...
  protected:
   void init(const Surface& srf) { ColorMerger_AlphaHandler<4>::init(srf); };
...
class ColorMerger_Alpha_XLAT_TableShifter : public ColorMerger_AlphaHandler<pixelsize> {
  ...
  public:
   void setNeutralTranslationTable(const Uint8* translationTable) { table = translationTable; };
...
class ColorMerger_Set {
  ...
  public:
   void assign(PixelType src, PixelType* dest) const { *dest = col; };
...
class SourcePixelSelector_Rotation : public SourcePixelSelector {
  ...
  protected:
   ...
   PixelType nextPixel() { return getPixel(x++, y); };
  ...
  public:
   void setAngle(int degrees) { this->degrees = degrees; };

   SourcePixelSelector_Rotation(NullParamType npt = nullParam)
      : degrees(0), x(0), y(0), w(0), h(0) {}

   SourcePixelSelector_Rotation(int degreesToRotate)
      : degrees(degreesToRotate), x(0), y(0), w(0), h(0) {}
};
...
class SourcePixelSelector_DirectRotation : public SourcePixelSelector_Rotation<pixelsize> {
  public:
   ...
   SourcePixelSelector_DirectRotation(int degreesToRotate)
      : SourcePixelSelector_Rotation<pixelsize>(degreesToRotate) {}
};
         ...
         if (cache.find(degrees) == cache.end()) {
            ...
            for (int y = 0; y < surface->h(); ++y)
               for (int x = 0; x < surface->w(); ++x) {
                  SPoint pnt =
                     getPixelRotationLocation(SPoint(x, y), surface->w(), surface->h(), degrees);
...
class SourcePixelSelector_Zoom : public SourcePixelSelector {
  ...
  protected:
   ...
   PixelType getPixel(int x, int y) {
      return SourcePixelSelector::getPixel(int(float(x) / zoomFactorX),
                                           int(float(y) / zoomFactorY));
   };
  ...
  public:
   ...
   float getZoomX() { return zoomFactorX; };
   float getZoomY() { return zoomFactorY; };
   ...
   SourcePixelSelector_Zoom(NullParamType npt = nullParam)
      : zoomFactorX(1), zoomFactorY(1), x(0), y(0){};
};
...
class SourcePixelSelector_CacheZoom : private ZoomCache, public SourcePixelSelector {
  ...
  public:
   ...
   SourcePixelSelector_CacheZoom(NullParamType npt = nullParam)
      : surface(NULL), zoomFactor(1), xp(NULL), yp(NULL), offsetx(0), offsety(0) {
...
class SourcePixelSelector_Flip : public SourcePixelSelector {
  ...
  public:
   ...
   SourcePixelSelector_Flip(NullParamType npt = nullParam)
      : hflip(false), vflip(false), x(0), y(0), w(0), h(0){};
...
// template<int pixelsize, class SourcePixelSelector = SourcePixelSelector_Plain<pixelsize> > //
// MSVC7 fails to understand that...
template <int pixelsize>
...
class SourcePixelSelector_DirectRectangle {
  ...
  protected:
   SourcePixelSelector_DirectRectangle()
      : y(0), x1(0), y1(0), w(0), h(0), pointer(NULL), surface(NULL){};
...
class TargetPixelSelector_Rect {
  ...
  public:
   ...
   void setClippingRect(SDL_Surface* srv) { setTargetRect(srv->clip_rect); };
...
/*
 ...
 {
    switch ( dst.GetPixelFormat().BytesPerPixel() ) {
       case 1: megaBlit<1,SourceColorTransform,ColorMerger,SourcePixelSelector>( src, dst, dstPos );
 break; case 2: megaBlit<2,SourceColorTransform,ColorMerger,SourcePixelSelector>( src, dst, dstPos
 ); break; case 4: megaBlit<4,SourceColorTransform,ColorMerger,SourcePixelSelector>( src, dst,
 dstPos ); break;
    };
 }
