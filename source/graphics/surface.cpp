...
Surface Surface::Duplicate() const {
   ...
   megaBlitter<ColorTransform_None, ColorMerger_PlainOverwrite, SourcePixelSelector_Plain,
               TargetPixelSelector_All>(*this, new_surface, SPoint(0, 0), nullParam, nullParam,
                                        nullParam, nullParam);
...
void Surface::convert() {
   ...
   if (default32bit && GetPixelFormat().BytesPerPixel() == 4) {
      if (default32bit->Rmask() != GetPixelFormat().Rmask() ||
          default32bit->Gmask() != GetPixelFormat().Gmask() ||
          default32bit->Bmask() != GetPixelFormat().Bmask()) {
         SDL_Surface* tmp;
...
void Surface::read(tnstream& stream) {
   ...
   if (hd.id == 16973) {
      ...
      if (hd.id == 16974) {
         ...
         if (bytesPerPixel == 1) {
            SDL_Surface* s =
               SDL_CreateRGBSurface(SDL_SWSURFACE, hd.x, hd.y, 8, 0xff, 0xff, 0xff, 0xff);
            ...
            SDL_Surface* s =
               SDL_CreateRGBSurface(SDL_SWSURFACE, hd.x, hd.y, 32, Rmask, Gmask, Bmask, Amask);
         ...
         SDL_Surface* s =
            SDL_CreateRGBSurface(SDL_SWSURFACE, hd.id + 1, hd.size + 1, 8, 0, 0, 0, 0);
         ...
         /*
                  ...
                  SDL_SWSURFACE
                  SDL_Surface* s = SDL_CreateRGBSurfaceFrom(pntr, hd.id+1, hd.size+1, 8, hd.id+1, 0,
         0, 0, 0 );
...
Surface Surface::createSurface(int width, int height, int depth, SDLmm::Color color) {
   ...
   if (depth == 32 && default32bit) {
      ...
      surf = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, depth, 0xff, 0xff00, 0xff0000,
                                  0xff000000);
...
/*
void SDL_StretchSurface(SDL_Surface* src_surface, int xs1, int ys1, int xs2, int ys2, SDL_Surface*
dst_surface, int xd1, int yd1, int xd2, int yd2, Uint32* lutVOI)
{
}
*/
...
void Surface::strech(int width, int height) {
   if (width != w() || height != h()) {
      ...
      if (GetPixelFormat().BytesPerPixel() == 1)
         ...
         s = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, 0xff, 0xff00, 0xff0000,
                                  0xff000000);
...
void Surface::detectColorKey(bool RLE) {
   ...
   if (GetPixelFormat().BitsPerPixel() > 8) {
      SetColorKey(flags, GetPixel(0, 0) & (GetPixelFormat().Rmask() | GetPixelFormat().Gmask() |
                                           GetPixelFormat().Bmask()));
   } else
...
bool Surface::isTransparent(SDLmm::Color col) const {
   if (flags() & SDL_SRCCOLORKEY)
      return (col & (GetPixelFormat().Rmask() | GetPixelFormat().Gmask() |
                     GetPixelFormat().Bmask())) == GetPixelFormat().colorkey();
   else {
      if (GetPixelFormat().BitsPerPixel() == 8)
         return false;
      else {
...
void applyFieldMask(Surface& s, int x, int y, bool detecColorKey) {
   if (s.GetPixelFormat().BitsPerPixel() == 8) {
      // we don't want any transformations from one palette to another; we just assume that all
      // 8-Bit images use the same colorspace
