...
class ColorTransform_PlayerCol<1> {
  ...
  public:
   ColorTransform_PlayerCol(const PlayerColor& player) { setPlayer(player.getNum()); };

   ColorTransform_PlayerCol(NullParamType npt) : shift(0){};

   void setPlayer(int player) { shift = player * 8; };
};
...
class ColorTransform_PlayerTrueCol {
  ...
  protected:
   ...
   PixelType transform(PixelType col) {
      ...
      if (g == 0 && b == 0) {
         ...
         return ((refr + (255 - refr) * g / 255) << 16) + ((refg + (255 - refg) * g / 255) << 8) +
                (refb + (255 - refb) * g / 255) + (col & 0xff000000);
      } else
         return col;
   };
  ...
  public:
   ...
   ColorTransform_PlayerTrueCol(NullParamType npt)
      : lateConversion(false), refColor(0), refr(0), refg(0), refb(0){};
...
class ColorTransform_PlayerTrueColHSV {
  ...
  public:
   ...
   void setPlayer(int playerNum) { player = playerNum; }
...
class ColorTransform_PlayerCol<4> : public ColorTransform_PlayerTrueCol<4> {
  public:
   ColorTransform_PlayerCol(const PlayerColor& player)
      : ColorTransform_PlayerTrueCol<4>(player.getColor()){};
};
