...
void rectangle(Surface& surface, const SPoint& pos, int w, int h, const ColorMergerUL& ul,
...
template <int BytePerPixel, class ColorMergerUL, class ColorMergerLR>
void rectangle(Surface& surface, const SPoint& pos, int w, int h, const ColorMergerUL& ul,
               const ColorMergerLR& lr, const SDLmm::SRect& clip) {
   SurfaceLock lock(surface);
   typedef typename PixelSize2Type<BytePerPixel>::PixelType TargetPixelType;
...
void paintFilledRectangle(Surface& surface, const SPoint& pos, int w, int h,
...
template <int pixelsize, template <int> class ColorMerger>
class PutPixel : public ColorMerger<pixelsize> {
   typedef typename PixelSize2Type<pixelsize>::PixelType PixelType;
   Surface& surf;

  public:
   PutPixel(Surface& surface) : surf(surface) { ColorMerger<pixelsize>::init(surface); };
