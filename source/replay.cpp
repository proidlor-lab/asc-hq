...
class ReplayRecorderWatcherGlobal {
  ...
  public:
   ...
   void set(ReplayRecorder* rec) { recorder = rec; }
...
class ReplayRecorder : public sigc::trackable {
  ...
  public:
   ...
   void start(const ASCString& filename, bool append, int framerate, int ascFramerateLimit,
              int quality) {
      ...
      if (!rec || !append || newFilename != rec->getFilename()) {
         delete rec;
         rec = new VideoRecorder(newFilename, PG_Application::GetScreen(), framerate,
                                 ascFramerateLimit, quality);
      }
   ...
   bool isRunning() { return connection.connected() && rec; }

   bool isOpen() { return rec != NULL; }

   ASCString getLastFilename() { return lastFilename; }
...
class ReplayRecord : public GuiFunction {
  public:
   ...
   void execute(const MapCoordinate& pos, ContainerBase* subject, int num) {
      ...
      replayRecorder->start(rrd.getFilename(), rrd.getAppend(), rrd.getFramerate(),
                            rrd.getASCFramerateLimit(), rrd.getQuality());
  ...
  public:
   bool available(const MapCoordinate& pos, ContainerBase* subject, int num) {
      if (replayRecorder && replayRecorder->isOpen() &&
          (runreplay.status == 1 || runreplay.status == 10 || runreplay.status == 11))
         return true;
...
#ifndef ASC_DEBUG
      catch (...) {  // this will catch NullPointer-Exceptions and stuff like that, which we want to
                     // pass to the debugger  in debug mode
#else
   ...
   if (player.stat == Player::human || player.stat == Player::supervisor)
      if (CGameOptions::Instance()->debugReplay && player.getParentMap()->replayinfo)
         if (choice_dlg("run replay of your turn ?", "~y~es", "~n~o") == 1) {
            // cursor.gotoxy( actmap->cursorpos.position[oldplayer].cx,
            // actmap->cursorpos.position[oldplayer].cy );
...
void checkforreplay() {
   ...
   if (actmap->replayinfo && rpnum &&
       (actmap->player[actmap->actplayer].stat == Player::human ||
        actmap->player[actmap->actplayer].stat == Player::supervisor))
      if (choice_dlg("run replay of last turn ?", "~y~es", "~n~o") == 1) {
         MainScreenWidget::StandardActionLocker locker(mainScreenWidget,
                                                       MainScreenWidget::LockOptions::Menu);
...
void initReplayLogging(Player& player) {
   ...
   if (gamemap->replayinfo && player.stat != Player::off) {
      ...
      gamemap->replayinfo->actmemstream =
         new MemoryStream(gamemap->replayinfo->guidata[player.getPosition()], tnstream::writing);
   }
}

LockReplayRecording::LockReplayRecording(GameMap::ReplayInfo& _ri) : ri(_ri) {
...
class LogActionIntoReplayInfo {
  ...
  public:
   ...
   void saveCommand(const Command& cmd) {
      if (gamemap->replayinfo && gamemap->replayinfo->actmemstream &&
          !gamemap->replayinfo->stopRecordingActions) {
...
void trunreplay ::wait(MapCoordinate pos1, MapCoordinate pos2, int t) {
   if (fieldvisiblenow(actmap->getField(pos1), actmap->getPlayerView()) ||
       fieldvisiblenow(actmap->getField(pos2), actmap->getPlayerView()))
      wait();
}
      ...
      switch (nextaction) {
         case rpl_move: {
            ...
            if (eht) {
               ...
               if (res.successful())
                  ...
                  error("severe replay inconsistency:\nerror for move1 command at " +
                        MapCoordinate(x1, y1).toString());
            }

            if (!eht)
               error("severe replay inconsistency:\nno vehicle for move1 command at " +
                     MapCoordinate(x1, y1).toString());
         } break;
         case rpl_move5:
         case rpl_move4:
            ...
            if (eht) {
               ...
               if (destDamage >= 0) {
                  ...
                  if (destDamage != realDamage)
                     error(MapCoordinate(x1, y1),
                           "severe replay inconsistency:\ndamage after movement differs: "
                           "recorded=%d, actual=%d",
                           destDamage, realDamage);
               }
            }

            if (!eht)
               error("severe replay inconsistency:\nno vehicle for move2 command at " +
                     MapCoordinate(x1, y1).toString());
         } break;
         case rpl_attack: {
            stream->readInt();  // size
            ...
            int attackvisible = fieldvisiblenow(fld, actmap->getPlayerView()) ||
                                fieldvisiblenow(targ, actmap->getPlayerView());
               ...
               if (fieldvisiblenow(targ, fld->vehicle->getOwner())) {
                  if (targ->vehicle) {
                     ...
                     if (battle.av.damage < ad2 || battle.dv.damage > dd2)
                        error(MapCoordinate(x2, y2),
                              "severe replay inconsistency:\nresult of attack differ !\nexpected "
                              "target damage: %d ; recorded target damage: %d\nexpected attacker "
                              "damage: %d ; recorded attacker damage: %d",
                  ...
                  error(MapCoordinate(x2, y2),
                        "severe replay inconsistency:\nthe attacking unit can't view the target "
                        "field!");
         ...
         case rpl_buildobj2: {
            ...
            if (obj && fld) {
               ...
               bool objectAffectsVisibility = obj->basicjamming_plus || obj->viewbonus_plus ||
                                              obj->viewbonus_abs != -1 ||
                                              obj->basicjamming_abs != -1;
               ...
               if (unit > 0) {
                  ...
                  if (veh) {
                     if (veh->getMovement() < movecost)
                        error(MapCoordinate(x, y),
                              "not enough movement to construct/remove object !");
                     ...
                     for (int r = 0; r < 3; r++)
                        if (res2.resource(r) < cost.resource(r) && cost.resource(r) > 0)
                           error(
                              "Resource mismatch: not enough resources to construct/remove object "
                              "!\nPosition: " +
                              MapCoordinate(x, y).toString());
            ...
            if (fld && tnk && !fld->vehicle) {
               ...
               if (constx >= 0 && consty >= 0) {
                  ...
                  if (constructorField->vehicle) {
                     Resources r(
                        0, tnk->productionCost.material,
                        tnk->productionCost
                           .energy);  //  = constructorField->vehicle->getProductionCost(tnk );
                     Resources rr = constructorField->getContainer()->getResource(r, 0);
                     if (rr < r) {
                        displayActionCursor(x, y);
                        error(MapCoordinate(x, y),
                              "severe replay inconsistency: \nNot enough resources to produce unit "
                              "%s !\nRequired: %d/%d/%d ; Available: %d/%d/%d",
                              v->typ->description.c_str(), r.energy, r.material, r.fuel, rr.energy,
                              rr.material, rr.fuel);
                     }
                  } else
                     error(MapCoordinate(x, y),
                           "severe replay inconsistency: could not find constructor !");
               }
         ...
         case rpl_putbuilding: {
            ...
            if (bld && fld && networkid && actmap->getUnit(networkid)) {
               displayActionCursor(x, y);
               std::unique_ptr<ConstructBuildingCommand> cbc(
                  new ConstructBuildingCommand(actmap->getUnit(networkid)));
               ...
               if (res.successful())
                  ...
                  error(MapCoordinate(x, y),
                        "severe replay inconsistency, could not construct building; Code=" +
                           ASCString::toString(res.getCode()));
               ...
               if (fld->building) {
                  ...
                  if (r < cost) {
                     ...
                     error(MapCoordinate(x, y),
                           ...
                           "!\nRequired: %d/%d/%d ; Available: %d/%d/%d",
                           eht->typ->description.c_str(), cost.energy, cost.material, cost.fuel,
                           r.energy, r.material, r.fuel);
                  }
         ...
         case rpl_removeunit: {
            ...
            if ((!fld->vehicle || fld->vehicle->networkid != nwid) && fld->building) {
               ...
               if (res.successful())
                  ...
                  displayActionError(res);
            } else if (!fld->getContainer() ||
                       !fld->getContainer()->removeUnitFromCargo(nwid, true))
         ...
         case rpl_repairUnit: {
            ...
            if (eht && dest) {
               ...
               if (eht->getTank().fuel != fuelremain || eht->getTank().material != matremain)
                  error(
                     "severe replay inconsistency:\nthe resources of unit not matching for repair "
                     "operation!");
            } else
         ...
         case rpl_produceAmmo: {
            ...
            if (bld) {
               bld->getResource(Resources(ammoProductionCost[weaptype][0] * n,
                                          ammoProductionCost[weaptype][1] * n,
                                          ammoProductionCost[weaptype][2] * n),
                                false);
         ...
         case rpl_buildProdLine: {
            ...
            if (bld && veh) {
               if (veh->techDependency.available(actmap->player[bld->getOwner()].research)) {
                  std::unique_ptr<BuildProductionLineCommand> bplc(
                     new BuildProductionLineCommand(bld));
                  ...
                  error(
                     "severe replay inconsistency:\ntechnology for building production line not "
                     "available!");
         ...
         case rpl_removeProdLine: {
            ...
            VehicleType* veh = actmap->getvehicletype_byid(vehicleid);
            if (bld && veh) {
               std::unique_ptr<RemoveProductionLineCommand> rplc(
                  new RemoveProductionLineCommand(bld));
               rplc->setRemoval(veh);
         ...
         case rpl_setResourceProcessingAmount: {
            ...
            if (bld) {
               ...
               error(MapCoordinate(x, y),
                     "Building not found on for rpl_setResourceProcessingAmount ");
         } break;
         case rpl_netcontrol: {
            stream->readInt();
            int x = stream->readInt();
   ...
   do {
      ...
      if (nextaction == rpl_finished || status != 2) {
         if (CGameOptions::Instance()->replayMovieMode && status != 1 &&
             !(replayRecorder && replayRecorder->isRunning()))
            status = 100;
         else {
            if (nextaction == rpl_finished && !resourcesCompared) {
               ...
               if (!CGameOptions::Instance()->replayMovieMode) {
                  if (comparisonMap) {
                     if (compareMapResources(comparisonMap, actmap, player,
                                             &resourceComparisonResult)) {
                        ViewFormattedText vft("warning", resourceComparisonResult,
                                              PG_Rect(-1, -1, 500, 550));
                        vft.Show();
                        vft.RunModal();
                     }
