...
TextRenderer ::TextRenderer(PG_Widget* parent, const PG_Rect& r)
   : PG_ScrollWidget(parent, r, "ScrollWidget"), lastWidget(NULL) {
   SetTransparency(255);
   SetLineSize(scrollsize);
};

TextRenderer ::TextRenderer(PG_Widget* parent, const PG_Rect& r, const std::string& text,
                            const std::string& style)
   : PG_ScrollWidget(parent, r, style), lastWidget(NULL) {
   SetTransparency(255);
   SetText(text);
};
...
ASCString TextRenderer ::substr(const ASCString& text, ASCString::const_iterator begin,
...
TextRenderer::Widgets TextRenderer ::eval_command(const ASCString& token) {
   ...
   if (boost::regex_match(token, what, legacycolor)) {
      ...
      if (index > 0)
         textAttributes.textcolor =
            (pal[index][0] << 18) + (pal[index][1] << 10) + (pal[index][2] << 2);
      else
         textAttributes.textcolor = GetFontColor();
      return widgets;
   }
   ...
   for (deallocating_vector<TextRenderer::TagRenderer*>::iterator i =
           RenderingAddons::Instance().begin();
        i != RenderingAddons::Instance().end(); ++i) {
      Widgets w;
...
bool TextRenderer ::eventKeyDown(const SDL_KeyboardEvent* key) {
   ...
   if ((key->keysym.sym == SDLK_UP && keyStates[SDLK_UP]) ||
       (key->keysym.sym == SDLK_KP8 && keyStates[SDLK_KP8])) {
      ScrollTo(GetScrollPosX(), GetScrollPosY() - scrollsize);
      return true;
   }
   if ((key->keysym.sym == SDLK_DOWN && keyStates[SDLK_DOWN]) ||
       (key->keysym.sym == SDLK_KP2 && keyStates[SDLK_KP2])) {
      ScrollTo(GetScrollPosX(), GetScrollPosY() + scrollsize);
      return true;
   }
...
ViewFormattedText ::ViewFormattedText(const ASCString& title, const ASCString& text,
                                      const PG_Rect& pos)
   : ASC_PG_Dialog(NULL, pos, title) {
